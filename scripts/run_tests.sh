#!/usr/bin/env bash
# Canonical test runner for quill-agent. Run this instead of calling
...
VENV=""
for candidate in "$REPO_ROOT/.venv" "$REPO_ROOT/venv" "$HOME/.quill/quill-agent/venv"; do
...
# Unset QUILL_* behavioral vars too.
unset QUILL_YOLO_MODE QUILL_INTERACTIVE QUILL_QUIET QUILL_TOOL_PROGRESS \
      QUILL_TOOL_PROGRESS_MODE QUILL_MAX_ITERATIONS QUILL_SESSION_PLATFORM \
      QUILL_SESSION_CHAT_ID QUILL_SESSION_CHAT_NAME QUILL_SESSION_THREAD_ID \
      QUILL_SESSION_SOURCE QUILL_SESSION_KEY QUILL_GATEWAY_SESSION \
      QUILL_CRON_SESSION \
      QUILL_PLATFORM QUILL_INFERENCE_PROVIDER QUILL_MANAGED QUILL_DEV \
      QUILL_CONTAINER QUILL_EPHEMERAL_SYSTEM_PROMPT QUILL_TIMEZONE \
      QUILL_REDACT_SECRETS QUILL_BACKGROUND_NOTIFICATIONS QUILL_EXEC_ASK \
      QUILL_HOME_MODE 2>/dev/null || true
...
# If a system-wide quill pytest_live_guard plugin is installed at
# $HOME/.quill/pytest_live_guard.py, force-load it here so every test run
...
if [ -f "$HOME/.quill/pytest_live_guard.py" ]; then
  case ":${PYTHONPATH:-}:" in
    *":$HOME/.quill:"*) ;;
    *) export PYTHONPATH="${PYTHONPATH:+$PYTHONPATH:}$HOME/.quill" ;;
  esac
