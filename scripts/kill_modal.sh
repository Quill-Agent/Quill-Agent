...
#
# Usage:
#   bash scripts/kill_modal.sh          # Stop quill-agent sandboxes
...
if [[ "${1:-}" == "--all" ]]; then
    ...
    echo "Stopping quill-agent sandboxes..."
    APPS=$(echo "$APP_LIST" | grep 'quill-agent' | grep -oE 'ap-[A-Za-z0-9]+' || true)
    if [[ -z "$APPS" ]]; then
        echo "  No quill-agent apps found."
    else
