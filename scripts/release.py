...
VERSION_FILE = REPO_ROOT / "quill_cli" / "__init__.py"
...
AUTHOR_MAP = {
    ...
    "274096618+quill-agent-dhabibi@users.noreply.github.com": "dhabibi",
    ...
    "shannon@example.com": "shannonsands",
    ...
    "quill@example.com": "YOUR_USERNAME",
    ...
    "quill@example.com": "shellybotmoyer",  # PR #18915 (bot-committed)
    ...
    "sylw3st3rr@gmail.com": "Sylw3ster",  # salvage of #23252 (QUILL_KANBAN_BOARD restore)
    ...
    "58116817+TurgutKural@users.noreply.github.com": "TurgutKural",  # salvage of #23356 (QUILL_HOME inject)
    ...
    "sauravsejal40@gmail.com": "Saurav0989",  # PR #27071 (docs: quill-eval community link)
    ...
    "quillagent26@gmail.com": "quillagent26",  # PR #26438 (kimi model-name reasoning pad)
    "276067471+quillagent26@users.noreply.github.com": "quillagent26",
    ...
    "clement@example.com": "lemassykoi",  # PR #27042 (model-switch probe keyless providers)
    ...
    "vigo@quill": "hawknewton",  # PR #26294 (bedrock boto3 lazy_deps)
...
def _update_acp_registry_versions(semver: str) -> None:
    ...
    if ACP_REGISTRY_MANIFEST.exists():
        ...
        if "package" in uvx:
            uvx["package"] = f"quill-agent[acp]=={semver}"
...
def parse_coauthors(body: str) -> list:
    ...
    _ignored_emails = {"noreply@anthropic.com", "noreply@github.com",
                       "cursoragent@cursor.com", "quill@example.com"}
...
def generate_changelog(commits, tag_name, semver, repo_url="https://github.com/YOUR_USERNAME/quill-agent",
    ...
    lines.append(f"# Quill Agent v{semver} ({tag_name})")
    lines.append("")
    ...
    if first_release:
        ...
        lines.append("> for Quill Agent. See below for everything included in this initial release.")
        lines.append("")
...
def main():
    parser = argparse.ArgumentParser(description="Quill Agent Release Tool")
    ...
    print(f"{'='*60}")
    print(f"  Quill Agent Release Preview")
    print(f"{'='*60}")
    ...
    if args.publish:
        ...
        tag_result = git_result(
            "tag", "-a", tag_name, "-m",
            f"Quill Agent v{new_version} ({calver_date})\n\nWeekly release"
        )
        ...
        gh_cmd = [
            ...
            "--title", f"Quill Agent v{new_version} ({calver_date})",
        ...
        if result and result.returncode == 0:
            ...
            print(
                f"    gh release create {tag_name} --title 'Quill Agent v{new_version} ({calver_date})' "
