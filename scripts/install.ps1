# ============================================================================
# Quill Agent Installer for Windows
# ============================================================================
...
param(
    ...
    [string]$Commit = "",
    [string]$Tag = "",
    [string]$QuillHome = "$env:LOCALAPPDATA\quill",
    [string]$InstallDir = "$env:LOCALAPPDATA\quill\quill-agent",
...
$RepoUrlSsh = "git@github.com:YOUR_USERNAME/quill-agent.git"
$RepoUrlHttps = "https://github.com/YOUR_USERNAME/quill-agent.git"
$PythonVersion = "3.11"
$NodeVersion = "22"
...
function Write-Banner {
    ...
    Write-Host "|             * Quill Agent Installer                    |" -ForegroundColor Magenta
    Write-Host "+---------------------------------------------------------+" -ForegroundColor Magenta
    Write-Host "|  An open source AI agent by Quill Agent.              |" -ForegroundColor Magenta
    Write-Host "+---------------------------------------------------------+" -ForegroundColor Magenta
    Write-Host ""
}
...
function Write-BrowserEnv {
    ...
    if (-not (Test-Path $QuillHome)) {
        New-Item -ItemType Directory -Force -Path $QuillHome | Out-Null
    }
    $envFile = Join-Path $QuillHome ".env"
...
function Install-AgentBrowser {
    ...
    $prefixDir = Join-Path $QuillHome "node"
...
function Install-Git {
    ...
    which Quill uses to run shell commands.
    ...
    package manager):
      ...
      2. Download **PortableGit** from the official git-for-windows GitHub
         ...
         ``%LOCALAPPDATA%\quill\git`` -- never touches system Git, never
    ...
    **Why PortableGit, not MinGit:**  MinGit is the minimal-automation
    ...
    Quill needs ``bash.exe`` to run shell commands.  PortableGit is the
    ...
    blocked).  Owning the Quill copy of Git ourselves is predictable and
    recoverable: if it ever breaks, ``Remove-Item %LOCALAPPDATA%\quill\git``
    and re-running this installer fully recovers.

    After install we locate ``bash.exe`` and persist the path in
    ``QUILL_GIT_BASH_PATH`` (User scope) so Quill can find it in a fresh
    ...
    # Download PortableGit into $QuillHome\git.  Always works as long as
    # we can reach github.com -- no admin, no winget, no reliance on the
    # user's possibly-broken system Git install.
    Write-Info "Git not found -- downloading PortableGit to $QuillHome\git\ ..."
    ...
    try {
        ...
        $release = Invoke-RestMethod -Uri $releaseApi -UseBasicParsing -Headers @{ "User-Agent" = "quill-installer" }

        if ($arch -eq "32-bit-mingit") {
            Write-Warn "32-bit Windows detected -- PortableGit is 64-bit only.  Installing MinGit 32-bit as a last resort; bash-dependent Quill features (terminal tool, agent-browser) will not work on this machine."
        ...
        Write-Info "then re-run this installer.  Quill needs Git Bash on Windows to run"
...
function Set-GitBashEnvVar {
    ...
    ``QUILL_GIT_BASH_PATH`` (User env scope) so Quill can find it even before
    ...
    #
    # Layouts:
    #   PortableGit (our default): $QuillHome\git\bin\bash.exe
    #   MinGit (32-bit fallback):  $QuillHome\git\usr\bin\bash.exe
    $candidates += "$QuillHome\git\bin\bash.exe"       # PortableGit layout (primary)
    $candidates += "$QuillHome\git\usr\bin\bash.exe"   # MinGit / PortableGit usr\bin fallback
    ...
    foreach ($candidate in $candidates) {
        if ($candidate -and (Test-Path $candidate)) {
            [Environment]::SetEnvironmentVariable("QUILL_GIT_BASH_PATH", $candidate, "User")
            $env:QUILL_GIT_BASH_PATH = $candidate
            Write-Info "Set QUILL_GIT_BASH_PATH=$candidate"
            return
        }
    }

    Write-Warn "Could not locate bash.exe -- Quill may not find Git Bash."
    Write-Info "If needed, set QUILL_GIT_BASH_PATH manually to your bash.exe path."
}
...
function Test-Node {
    ...
    $managedNode = "$QuillHome\node\node.exe"
    if (Test-Path $managedNode) {
        $version = & $managedNode --version
        $env:Path = "$QuillHome\node;$env:Path"
        Write-Success "Node.js $version found (Quill-managed)"
        $script:HasNode = $true
        return $true
    }
    ...
    # The portable zip path drops node.exe + npm into $QuillHome\node\
    ...
    Write-Info "Downloading portable Node.js $NodeVersion to $QuillHome\node\ ..."
    ...
    try {
        ...
        if ($zipName) {
            ...
            $tmpDir = "$env:TEMP\quill-node-extract"
            ...
            if ($extractedDir) {
                if (Test-Path "$QuillHome\node") { Remove-Item -Recurse -Force "$QuillHome\node" }
                Move-Item $extractedDir.FullName "$QuillHome\node"
                ...
                $version = & "$QuillHome\node\node.exe" --version
                Write-Success "Node.js $version installed to $QuillHome\node\ (portable, user-scoped)"
                $script:HasNode = $true
    ...
    if (Test-Path $InstallDir) {
        ...
        if ($repoValid) {
            ...
            try {
                ...
                Write-Info "terminals, running quill processes) and try again."
                throw
            }
        }
    }
    ...
    if (-not $didUpdate) {
        ...
        if (-not $cloneSuccess) {
            ...
            try {
                ...
                if ($Commit) {
                    $zipUrl = "https://github.com/YOUR_USERNAME/quill-agent/archive/$Commit.zip"
                    $zipLabel = $Commit
                } elseif ($Tag) {
                    $zipUrl = "https://github.com/YOUR_USERNAME/quill-agent/archive/refs/tags/$Tag.zip"
                    $zipLabel = $Tag
                } else {
                    $zipUrl = "https://github.com/YOUR_USERNAME/quill-agent/archive/refs/heads/$Branch.zip"
                    $zipLabel = $Branch
                }
                $zipPath = "$env:TEMP\quill-agent-$zipLabel.zip"
                $extractPath = "$env:TEMP\quill-agent-extract"
...
function Install-Dependencies {
    ...
    if (Test-Path "uv.lock") {
        ...
        # empty and producing the broken state where `quill.exe` exists
...
"@ 2>$null
    ...
    if (-not $installed) {
        throw "Failed to install quill-agent package even with no extras. Inspect the uv pip install output above."
    }
    ...
    # isn't set, leaving venv\ empty and quill.exe broken with
    ...
    # here, not 30 seconds later when the user runs `quill`.
    if (-not $NoVenv) {
    ...
    # users hit and lazy-import errors from `quill dashboard` are confusing.
    ...
    if (Test-Path $pythonExe) {
        ...
        $ErrorActionPreference = $prevEAP
        if (-not $webOk) {
            Write-Warn "fastapi/uvicorn not importable -- `quill dashboard` will not work."
            Write-Info "Attempting targeted install of [web] extra as last resort..."
            & $UvCmd pip install -e ".[web]"
            if ($LASTEXITCODE -eq 0) {
                Write-Success "[web] extra installed; `quill dashboard` should now work."
            } else {
...
function Set-PathVariable {
    Write-Info "Setting up quill command..."
    
    if ($NoVenv) {
        $quillBin = "$InstallDir"
    } else {
        $quillBin = "$InstallDir\venv\Scripts"
    }
    
    # Add the venv Scripts dir to user PATH so quill is globally available
    # On Windows, the quill.exe in venv\Scripts\ has the venv Python baked in
    $currentPath = [Environment]::GetEnvironmentVariable("Path", "User")
    
    if ($currentPath -notlike "*$quillBin*") {
        [Environment]::SetEnvironmentVariable(
            "Path",
            "$quillBin;$currentPath",
            "User"
        )
        Write-Success "Added to user PATH: $quillBin"
    } else {
        Write-Info "PATH already configured"
    }
    
    # Set QUILL_HOME so the Python code finds config/data in the right place.
    # Only needed on Windows where we install to %LOCALAPPDATA%\quill instead
    # of the Unix default ~/.quill
    $currentQuillHome = [Environment]::GetEnvironmentVariable("QUILL_HOME", "User")
    if (-not $currentQuillHome -or $currentQuillHome -ne $QuillHome) {
        [Environment]::SetEnvironmentVariable("QUILL_HOME", $QuillHome, "User")
        Write-Success "Set QUILL_HOME=$QuillHome"
    }
    $env:QUILL_HOME = $QuillHome
    
    # Update current session
    $env:Path = "$quillBin;$env:Path"
    
    Write-Success "quill command ready"
}
...
function Copy-ConfigTemplates {
    ...
    New-Item -ItemType Directory -Force -Path "$QuillHome\cron" | Out-Null
    New-Item -ItemType Directory -Force -Path "$QuillHome\sessions" | Out-Null
    New-Item -ItemType Directory -Force -Path "$QuillHome\logs" | Out-Null
    New-Item -ItemType Directory -Force -Path "$QuillHome\pairing" | Out-Null
    New-Item -ItemType Directory -Force -Path "$QuillHome\hooks" | Out-Null
    New-Item -ItemType Directory -Force -Path "$QuillHome\image_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$QuillHome\audio_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$QuillHome\memories" | Out-Null
    New-Item -ItemType Directory -Force -Path "$QuillHome\skills" | Out-Null

    
    # Create .env
    $envPath = "$QuillHome\.env"
    if (-not (Test-Path $envPath)) {
        $examplePath = "$InstallDir\.env.example"
        if (Test-Path $examplePath) {
            Copy-Item $examplePath $envPath
            Write-Success "Created ~/.quill/.env from template"
        } else {
            New-Item -ItemType File -Force -Path $envPath | Out-Null
            Write-Success "Created ~/.quill/.env"
        }
    } else {
        Write-Info "~/.quill/.env already exists, keeping it"
    }
    
    # Create config.yaml
    $configPath = "$QuillHome\config.yaml"
    if (-not (Test-Path $configPath)) {
        $examplePath = "$InstallDir\cli-config.yaml.example"
        if (Test-Path $examplePath) {
            Copy-Item $examplePath $configPath
            Write-Success "Created ~/.quill/config.yaml from template"
        }
    } else {
        Write-Info "~/.quill/config.yaml already exists, keeping it"
    }
    ...
    # (the default PS5 behaviour), and Quill's prompt-injection scanner
...
"@
        ...
        Write-Success "Created ~/.quill/SOUL.md (edit to customize personality)"
    }
    
    Write-Success "Configuration directory ready: ~/.quill/"
    
    # Seed bundled skills into ~/.quill/skills/ (manifest-based, one-time per skill)
    Write-Info "Syncing bundled skills to ~/.quill/skills/ ..."
    $pythonExe = "$InstallDir\venv\Scripts\python.exe"
    if (Test-Path $pythonExe) {
        try {
            & $pythonExe "$InstallDir\tools\skills_sync.py" 2>$null
            Write-Success "Skills synced to ~/.quill/skills/"
        } catch {
            # Fallback: simple directory copy
            $bundledSkills = "$InstallDir\skills"
            $userSkills = "$QuillHome\skills"
            if ((Test-Path $bundledSkills) -and -not (Get-ChildItem $userSkills -Exclude '.bundled_manifest' -ErrorAction SilentlyContinue)) {
                Copy-Item -Path "$bundledSkills\*" -Destination $userSkills -Recurse -Force -ErrorAction SilentlyContinue
                Write-Success "Skills copied to ~/.quill/skills/"
            }
        }
    }
}
...
function Install-NodeDeps {
    ...
    if (-not $npmCmd) {
        ...
        Write-Info "Open a new PowerShell window and re-run 'quill setup tools' later."
        return
    }
    $npmExe = $npmCmd.Source
    if ($npmExe -like "*.ps1") {
    ...
    if (Test-Path "$InstallDir\package.json") {
        ...
        $browserLog = "$env:TEMP\quill-npm-browser-$(Get-Random).log"
        ...
        if ($browserNpmOk) {
            ...
            if (-not $npxExe) {
                ...
                $pwLog = "$env:TEMP\quill-playwright-install-$(Get-Random).log"
                Push-Location $InstallDir
    ...
    if (Test-Path "$tuiDir\package.json") {
        ...
        $tuiLog = "$env:TEMP\quill-npm-tui-$(Get-Random).log"
...
function Install-PlatformSdks {
    ...
    # ~/.quill/.env are importable.  Two problems this solves:
    #
...
function Invoke-SetupWizard {
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Skipping setup wizard (non-interactive). Configure via the GUI or 'quill setup'."
        return
    }
    ...
    if (-not $NoVenv) {
        & ".\venv\Scripts\python.exe" -m quill_cli.main setup
    } else {
        python -m quill_cli.main setup
    }
...
function Start-GatewayIfConfigured {
    ...
    $quillCmd = "$InstallDir\venv\Scripts\quill.exe"
    if (-not (Test-Path $quillCmd)) {
        $quillCmd = "quill"
    }

    # If WhatsApp is enabled but not yet paired, run foreground for QR scan
    $whatsappEnabled = $content | Where-Object { $_ -match "^WHATSAPP_ENABLED=true" }
    $whatsappSession = "$QuillHome\whatsapp\session\creds.json"
    if ($whatsappEnabled -and -not (Test-Path $whatsappSession)) {
        Write-Host ""
        Write-Info "WhatsApp is enabled but not yet paired."
        Write-Info "Running 'quill whatsapp' to pair via QR code..."
        Write-Host ""
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Start the gateway later with: quill gateway"
        return
    }
    ...
    if ($response -eq "" -or $response -match "^[Yy]") {
        ...
        try {
            $logFile = "$QuillHome\logs\gateway.log"
            Start-Process -FilePath $quillCmd -ArgumentList "gateway" `
                -RedirectStandardOutput $logFile `
                -RedirectStandardError "$QuillHome\logs\gateway-error.log" `
                -WindowStyle Hidden
            Write-Success "Gateway started! Your bot is now online."
            Write-Info "Logs: $logFile"
            Write-Info "To stop: close the gateway process from Task Manager"
        } catch {
            Write-Warn "Failed to start gateway. Run manually: quill gateway"
        }
    } else {
        Write-Info "Skipped. Start the gateway later with: quill gateway"
    }
}
...
function Write-Completion {
    ...
    Write-Host "$QuillHome\config.yaml"
    Write-Host "   API Keys:  " -NoNewline -ForegroundColor Yellow
    Write-Host "$QuillHome\.env"
    Write-Host "   Data:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$QuillHome\cron\, sessions\, logs\"
    Write-Host "   Code:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$QuillHome\quill-agent\"
    Write-Host ""
    ...
    Write-Host ""
    Write-Host "   quill              " -NoNewline -ForegroundColor Green
    Write-Host "Start chatting"
    Write-Host "   quill setup        " -NoNewline -ForegroundColor Green
    Write-Host "Configure API keys & settings"
    Write-Host "   quill config       " -NoNewline -ForegroundColor Green
    Write-Host "View/edit configuration"
    Write-Host "   quill config edit  " -NoNewline -ForegroundColor Green
    Write-Host "Open config in editor"
    Write-Host "   quill gateway      " -NoNewline -ForegroundColor Green
    Write-Host "Start messaging gateway (Telegram, Discord, etc.)"
    Write-Host "   quill update       " -NoNewline -ForegroundColor Green
...
$InstallStages = @(
    ...
    @{ Name = "repository";       Title = "Cloning Quill repository";            Category = "install";      NeedsUserInput = $false; Worker = "Stage-Repository" }
    ...
    @{ Name = "path";             Title = "Adding Quill to PATH";                Category = "finalize";     NeedsUserInput = $false; Worker = "Stage-Path" }
...
try {
    ...
    Write-Host "  Invoke-WebRequest -Uri 'https://raw.githubusercontent.com/YOUR_USERNAME/quill-agent/main/scripts/install.ps1' -OutFile install.ps1" -ForegroundColor Yellow
