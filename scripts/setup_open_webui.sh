...
# Bootstrap Open WebUI against Quill Agent's OpenAI-compatible API server.
#
# Idempotent by design:
# - ensures ~/.quill/.env has API server settings
# - installs Open WebUI into ~/.local/open-webui-venv
# - writes a reusable launcher at ~/.local/bin/start-open-webui-quill.sh
...
#   QUILL_API_PORT=8642
#   QUILL_API_HOST=127.0.0.1
#   QUILL_API_MODEL_NAME='Quill Agent'

OPEN_WEBUI_PORT="${OPEN_WEBUI_PORT:-8080}"
OPEN_WEBUI_HOST="${OPEN_WEBUI_HOST:-127.0.0.1}"
OPEN_WEBUI_NAME="${OPEN_WEBUI_NAME:-Quill Agent WebUI}"
...
QUILL_ENV_FILE="${QUILL_ENV_FILE:-$HOME/.quill/.env}"
QUILL_API_PORT="${QUILL_API_PORT:-8642}"
QUILL_API_HOST="${QUILL_API_HOST:-127.0.0.1}"
QUILL_API_CONNECT_HOST="${QUILL_API_CONNECT_HOST:-127.0.0.1}"
QUILL_API_MODEL_NAME="${QUILL_API_MODEL_NAME:-Quill Agent}"
QUILL_API_BASE_URL="http://${QUILL_API_CONNECT_HOST}:${QUILL_API_PORT}/v1"
LAUNCHER_PATH="$HOME/.local/bin/start-open-webui-quill.sh"
LOG_DIR="$HOME/.quill/logs"
...
write_launcher() {
  ...
  quoted_base_url="$(shell_quote "$QUILL_API_BASE_URL")"
...
ensure_env_permissions() {
  chmod 600 "$QUILL_ENV_FILE" 2>/dev/null || true
}

install_launchd_service() {
  local plist="$HOME/Library/LaunchAgents/ai.openwebui.quill.plist"
...
<dict>
  <key>Label</key>
  <string>ai.openwebui.quill</string>
...
EOF
  ...
  launchctl enable "gui/$(id -u)/ai.openwebui.quill"
  launchctl kickstart -k "gui/$(id -u)/ai.openwebui.quill"
}

install_systemd_user_service() {
  require_cmd systemctl
  local unit_dir="$HOME/.config/systemd/user"
  local unit="$unit_dir/openwebui-quill.service"
  mkdir -p "$unit_dir"
  cat > "$unit" <<EOF
[Unit]
Description=Open WebUI connected to Quill Agent
After=default.target

[Service]
Type=simple
ExecStart=/bin/bash %h/.local/bin/start-open-webui-quill.sh
Restart=always
RestartSec=3
WorkingDirectory=%h
StandardOutput=append:%h/.quill/logs/openwebui.log
StandardError=append:%h/.quill/logs/openwebui.error.log

[Install]
WantedBy=default.target
EOF
  systemctl --user daemon-reload
  systemctl --user enable --now openwebui-quill.service
}
...
main() {
  ...
  local api_key
  api_key="$(get_env_value API_SERVER_KEY "$QUILL_ENV_FILE")"
  if [[ -z "$api_key" ]]; then
    api_key="$(generate_secret)"
  fi

  log 'Ensuring Quill API server is configured...'
  upsert_env API_SERVER_ENABLED true "$QUILL_ENV_FILE"
  upsert_env API_SERVER_HOST "$QUILL_API_HOST" "$QUILL_ENV_FILE"
  upsert_env API_SERVER_PORT "$QUILL_API_PORT" "$QUILL_ENV_FILE"
  upsert_env API_SERVER_MODEL_NAME "$QUILL_API_MODEL_NAME" "$QUILL_ENV_FILE"
  upsert_env API_SERVER_KEY "$api_key" "$QUILL_ENV_FILE"
  ensure_env_permissions

  log 'Restarting Quill gateway so API server settings take effect...'
  quill gateway restart >/dev/null 2>&1 || true
  sleep 4
  if ! curl -fsS "http://${QUILL_API_CONNECT_HOST}:${QUILL_API_PORT}/health" >/dev/null; then
    log 'Quill API server did not answer on the first check. Trying to start gateway in the background...'
    nohup quill gateway run >/dev/null 2>&1 &
    sleep 6
  fi
  curl -fsS "http://${QUILL_API_CONNECT_HOST}:${QUILL_API_PORT}/health" >/dev/null
  ...
  log "Quill API endpoint: ${QUILL_API_BASE_URL}"
