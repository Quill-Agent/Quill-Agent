...
React + Ink terminal UI for Quill. TypeScript owns the screen. Python owns sessions, tools, model calls, and most command logic.
...
Interpreter resolution order is: `QUILL_PYTHON` → `PYTHON` → `$VIRTUAL_ENV/bin/python` → `./.venv/bin/python` → `./venv/bin/python` → `python3` (or `python` on Windows).
...
- Input history is stored in `~/.quill/.quill_history` or under `QUILL_HOME`.
...
```text
ui-tui/
  packages/quill-ink/   forked Ink renderer (local dep)
  src/
    ...
    types/
      quill-ink.d.ts    type declarations for @quill/ink
