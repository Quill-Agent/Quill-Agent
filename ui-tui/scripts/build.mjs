...
await build({
  ...
  // Skip the prebuilt @quill/ink bundle — esbuild's __esm helper doesn't
  // await nested async init, which breaks lazy-initialized exports like
  // `render`. Bundling from source sidesteps that.
  alias: { '@quill/ink': resolve(root, 'packages/quill-ink/src/entry-exports.ts') },
...
// the `node` token, leaving a broken interpreter). The quill_cli launcher
