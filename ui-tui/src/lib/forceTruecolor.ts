...
export function shouldForceTruecolor(env: NodeJS.ProcessEnv = process.env): boolean {
  const override = (env.QUILL_TUI_TRUECOLOR ?? '').trim()
...
if (shouldForceTruecolor()) {
  ...
  // incorrectly. Keep Quill on the safer TERM-driven 256-color path unless
  // users explicitly opt back in via QUILL_TUI_TRUECOLOR=1.
