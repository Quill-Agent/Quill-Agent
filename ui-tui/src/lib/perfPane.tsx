...
//
// Both gate on QUILL_DEV_PERF=1 and dump JSON-lines (default ~/.quill/perf.log,
// override QUILL_DEV_PERF_LOG). Tagged { src: 'react' | 'frame' } for jq.
// QUILL_DEV_PERF_MS (default 2) skips sub-ms idle frames; set 0 to capture all.
//
...
import type { FrameEvent } from '@quill/ink'
import { scrollFastPathStats } from '@quill/ink'
import { Profiler, type ProfilerOnRenderCallback, type ReactNode } from 'react'

const ENABLED = /^(?:1|true|yes|on)$/i.test((process.env.QUILL_DEV_PERF ?? '').trim())
const THRESHOLD_MS = Number(process.env.QUILL_DEV_PERF_MS ?? '2') || 0
const LOG_PATH = process.env.QUILL_DEV_PERF_LOG?.trim() || join(homedir(), '.quill', 'perf.log')
