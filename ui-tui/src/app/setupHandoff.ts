import type { RunExternalProcess } from '@quill/ink'
...
export async function runExternalSetup({ args, ctx, done, launcher, suspend }: RunExternalSetupOptions) {
  ...
  transcript.sys(`launching \`quill ${args.join(' ')}\`…`)
  ...
  if (result.error) {
    transcript.sys(`error launching quill: ${result.error}`)
  ...
  if (result.code !== 0) {
    transcript.sys(`quill ${args[0]} exited with code ${result.code}`)
    patchUiState({ status: 'setup required' })
