import { useApp, useHasSelection, useSelection, useStdout, useTerminalTitle, type ScrollBoxHandle } from '@quill/ink'
...
export function useMainApp(gw: GatewayClient) {
  ...
  useTerminalTitle(model ? `${marker} ${model}${tabCwd ? ` · ${shortCwd(tabCwd, 24)}` : ''}` : 'Quill')
  ...
  const cwd = ui.info?.cwd || process.env.QUILL_CWD || process.cwd()
