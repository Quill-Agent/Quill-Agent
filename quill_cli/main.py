#!/usr/bin/env python3
"""
Quill CLI - Main entry point.

Usage:
    quill                     # Interactive chat (default)
    quill chat                # Interactive chat
    quill gateway             # Run gateway in foreground
    quill gateway start       # Start gateway as service
    quill gateway stop        # Stop gateway service
    quill gateway status      # Show gateway status
    quill gateway install     # Install gateway service
    quill gateway uninstall   # Uninstall gateway service
    quill setup               # Interactive setup wizard
    quill logout              # Clear stored authentication
    quill status              # Show status of all components
    quill cron                # Manage cron jobs
    quill cron list           # List cron jobs
    quill cron status         # Check if cron scheduler is running
    quill doctor              # Check configuration and dependencies
    quill honcho setup                    # Configure Honcho AI memory integration
    quill honcho status                   # Show Honcho config and connection status
    quill honcho sessions                 # List directory → session name mappings
    quill honcho map <name>               # Map current directory to a session name
    quill honcho peer                     # Show peer names and dialectic settings
    quill honcho peer --user NAME         # Set user peer name
    quill honcho peer --ai NAME           # Set AI peer name
    quill honcho peer --reasoning LEVEL   # Set dialectic reasoning level
    quill honcho mode                     # Show current memory mode
    quill honcho mode [hybrid|honcho|local]  # Set memory mode
    quill honcho tokens                   # Show token budget settings
    quill honcho tokens --context N       # Set session.context() token cap
    quill honcho tokens --dialectic N     # Set dialectic result char cap
    quill honcho identity                 # Show AI peer identity representation
    quill honcho identity <file>          # Seed AI peer identity from a file (SOUL.md etc.)
    quill honcho migrate                  # Step-by-step migration guide: OpenClaw native → Quill + Honcho
    quill version             Show version
    quill update              Update to latest version
    quill uninstall           Uninstall Quill Agent
    quill acp                 Run as an ACP server for editor integration
    quill sessions browse     Interactive session picker with search

    quill claw migrate --dry-run  # Preview migration without changes
"""

# IMPORTANT: quill_bootstrap must be the very first import — it sets up
# UTF-8 stdio on Windows so print()/subprocess children don't hit
# UnicodeEncodeError with non-ASCII characters.  No-op on POSIX.
#
# Guarded against ModuleNotFoundError because ``quill_bootstrap`` is a
# top-level module registered via pyproject.toml's ``py-modules`` list.
# When the user upgrades code via ``git pull`` (or ``quill update``
# crashes between ``git reset --hard`` and ``uv pip install -e .``), the
# new code references ``quill_bootstrap`` but the editable install's
# ``.pth`` file still points at the old set of top-level modules.  Without
# this guard, quill crashes on import and the user can't run
# ``quill update`` to recover.  Missing the bootstrap means UTF-8 stdio
# setup is skipped on Windows — degraded, not broken.  POSIX is unaffected.
try:
    import quill_bootstrap  # noqa: F401
...
def _add_accept_hooks_flag(parser) -> None:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "(equivalent to QUILL_ACCEPT_HOOKS=1 / hooks_auto_accept: true)."
        ),
    )
...
def _require_tty(command_name: str) -> None:
    ...
    Interactive TUI commands (quill tools, quill setup, quill model) use
    curses or input() prompts that spin at 100% CPU when stdin is a pipe.
    This guard prevents accidental non-interactive invocation.
    """
    if not sys.stdin.isatty():
        print(
            f"Error: 'quill {command_name}' requires an interactive terminal.\n"
...
# ---------------------------------------------------------------------------
# Profile override — MUST happen before any quill module import.
#
# Many modules cache QUILL_HOME at import time (module-level constants).
# We intercept --profile/-p from sys.argv here and set the env var so that
# every subsequent ``os.getenv("QUILL_HOME", ...)`` resolves correctly.
# The flag is stripped from sys.argv so argparse never sees it.
# Falls back to ~/.quill/active_profile for sticky default.
# ---------------------------------------------------------------------------
def _apply_profile_override() -> None:
    """Pre-parse --profile/-p and set QUILL_HOME before module imports."""
    argv = sys.argv[1:]
    profile_name = None
    consume = 0
    ...
    # Mirrors quill_cli.profiles._PROFILE_ID_RE so we never call
    ...
    # 1.5 If QUILL_HOME is already set and no explicit flag was given, trust it
    # only when it already points to a specific profile directory.  The
    # distinguishing heuristic: a profile path has "profiles" as its immediate
    # parent directory name (e.g. ~/.quill/profiles/coder or
    # /opt/data/profiles/coder).  If QUILL_HOME points to the quill root
    # instead (e.g. systemd hardcodes QUILL_HOME=/root/.quill), we must
    # still read active_profile — the user may have switched profiles via
    # `quill profile use` and the gateway should honour that choice.
    # See issue #22502.
    quill_home_env = os.environ.get("QUILL_HOME", "")
    if profile_name is None and quill_home_env:
        if Path(quill_home_env).parent.name == "profiles":
            return

    # 2. If no flag, check active_profile in the quill root
    if profile_name is None:
        try:
            from quill_constants import get_default_quill_root

            active_path = get_default_quill_root() / "active_profile"
    ...
    # 3. If we found a profile, resolve and set QUILL_HOME
    if profile_name is not None:
        try:
            from quill_cli.profiles import resolve_profile_env

            quill_home = resolve_profile_env(profile_name)
        except (ValueError, FileNotFoundError) as exc:
            print(f"Error: {exc}", file=sys.stderr)
            sys.exit(1)
        except Exception as exc:
            # A bug in profiles.py must NEVER prevent quill from starting
            print(
                f"Warning: profile override failed ({exc}), using default",
                file=sys.stderr,
            )
            return
        os.environ["QUILL_HOME"] = quill_home
...
# Load .env from ~/.quill/.env first, then project root as dev fallback.
# User-managed env files should override stale shell exports on restart.
from quill_cli.config import get_quill_home
from quill_cli.env_loader import load_quill_dotenv

load_quill_dotenv(project_env=PROJECT_ROOT / ".env")

# Bridge security.redact_secrets from config.yaml → QUILL_REDACT_SECRETS env
# var BEFORE quill_logging imports agent.redact (which snapshots the flag at
# module-import time). Without this, config.yaml's toggle is ignored because
# the setup_logging() call below imports agent.redact, which reads the env var
# exactly once. Env var in .env still wins — this is config.yaml fallback only.
try:
    if "QUILL_REDACT_SECRETS" not in os.environ:
        import yaml as _yaml_early

        _cfg_path = get_quill_home() / "config.yaml"
        if _cfg_path.exists():
            ...
            if isinstance(_early_sec_cfg, dict):
                ...
                if _early_redact is not None:
                    os.environ["QUILL_REDACT_SECRETS"] = str(_early_redact).lower()
            del _early_sec_cfg
        del _cfg_path
except Exception:
    pass  # best-effort — redaction stays at default (enabled) on config errors

# Initialize centralized file logging early — all `quill` subcommands
# (chat, setup, gateway, config, etc.) write to agent.log + errors.log.
try:
    from quill_logging import setup_logging as _setup_logging
...
try:
    from quill_cli.config import load_config as _load_config_early
    from quill_constants import apply_ipv4_preference as _apply_ipv4
...
from quill_cli import __version__, __release_date__
from quill_constants import AI_GATEWAY_BASE_URL, OPENROUTER_BASE_URL
...
def _has_any_provider_configured() -> bool:
    ...
    from quill_cli.config import get_env_path, get_quill_home, load_config
    from quill_cli.auth import get_auth_status

    # Determine whether Quill itself has been explicitly configured (model
    ...
    if isinstance(model_cfg, dict):
        ...
        _model_name = ""
    _has_quill_config = _model_name and _model_name != _DEFAULT_MODEL

    # Check env vars (may be set by .env or shell).
    # OPENAI_BASE_URL alone counts — local models (vLLM, llama.cpp, etc.)
    # often don't require an API key.
    from quill_cli.auth import PROVIDER_REGISTRY
    ...
    auth_file = get_quill_home() / "auth.json"
    if auth_file.exists():
        try:
            import json
    ...
    # Only count these if Quill has been explicitly configured — Claude Code
    # being installed doesn't mean the user wants Quill to use their tokens.
    if _has_quill_config:
        try:
...
def _resolve_last_session(source: str = "cli") -> Optional[str]:
    ...
    db = None
    try:
        from quill_state import SessionDB
...
def _exec_in_container(container_info: dict, cli_args: list):
    ...
    Args:
        container_info: dict with backend, container_name, exec_user, quill_bin
        cli_args: the original CLI arguments (everything after 'quill')
    """

    backend = container_info["backend"]
    container_name = container_info["container_name"]
    exec_user = container_info["exec_user"]
    quill_bin = container_info["quill_bin"]
        ...
        if sudo_path:
            ...
            if probe2.returncode != 0:
                print(
                    ...
                    f"  }}];\n"
                    f"\n"
                    f"Or run: sudo quill {' '.join(cli_args)}",
                    file=sys.stderr,
                )
                sys.exit(1)
        else:
            print(
                f"Error: container '{container_name}' not found via {backend}.\n"
                f"The container may be running under root. Try: sudo quill {' '.join(cli_args)}",
                file=sys.stderr,
            )
            sys.exit(1)
...
def _print_tui_exit_summary(
    ...
    print(f"  quill --tui --resume {target}")
    if title:
        print(f'  quill --tui -c "{title}"')
    print()
...
def _tui_need_npm_install(root: Path) -> bool:
    """True when @quill/ink is missing or node_modules is behind package-lock.json.
    ...
    ink = root / "node_modules" / "@quill" / "ink" / "package.json"
    if not ink.is_file():
        return True
    if not lock.is_file():
        return False
...
def _ensure_tui_node() -> None:
    ...
    ``QUILL_SKIP_NODE_BOOTSTRAP=1`` to disable auto-install.
    """
    if shutil.which("node") and shutil.which("npm"):
        return
    if os.environ.get("QUILL_SKIP_NODE_BOOTSTRAP"):
        return
    ...
    quill_home = os.environ.get("QUILL_HOME") or str(Path.home() / ".quill")
    try:
        ...
        result = subprocess.run(
            ...
            env={**os.environ, "QUILL_HOME": quill_home},
            capture_output=True,
            text=True,
            check=False,
        )
    ...
    extras.extend([Path(quill_home) / "node" / "bin", Path.home() / ".local" / "bin"])
...
def _find_bundled_tui(quill_cli_dir: Path | None = None) -> Path | None:
    """Find a pre-built TUI entry.js bundled in the wheel."""
    if quill_cli_dir is None:
        quill_cli_dir = Path(__file__).parent
    bundled = quill_cli_dir / "tui_dist" / "entry.js"
    return bundled if bundled.is_file() else None


def _make_tui_argv(tui_dir: Path, tui_dev: bool) -> tuple[list[str], Path]:
    """TUI: --dev → tsx src; else node dist (QUILL_TUI_DIR prebuilt or esbuild)."""
    _ensure_tui_node()

    def _node_bin(bin: str) -> str:
        if bin == "node":
            env_node = os.environ.get("QUILL_NODE")
            if env_node and os.path.isfile(env_node) and os.access(env_node, os.X_OK):
                return env_node
        path = shutil.which(bin)
        if not path and bin == "node":
            try:
                from quill_cli.dep_ensure import ensure_dependency
    ...
    ext_dir = os.environ.get("QUILL_TUI_DIR")
    if tui_dev and ext_dir:
        print(
            f"Error: --dev is incompatible with QUILL_TUI_DIR={ext_dir}\n"
            f"The prebuilt TUI has no source code to hot-reload.\n"
            f"Unset QUILL_TUI_DIR (e.g. `unset QUILL_TUI_DIR`) to use --dev from a checkout.",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    if _tui_need_npm_install(tui_dir):
        npm = _node_bin("npm")
        if not os.environ.get("QUILL_QUIET"):
    ...
    if tui_dev:
        # Keep the local @quill/ink package exports in sync with source.
        # --dev runs src/entry.tsx directly, but @quill/ink resolves through
        # packages/quill-ink/dist/entry-exports.js. If that dist bundle is
        ...
        npm = _node_bin("npm")
        ink_dir = tui_dir / "packages" / "quill-ink"
        result = subprocess.run(
            [npm, "run", "build"],
            cwd=str(ink_dir),
            capture_output=True,
...
def _normalize_tui_toolsets(toolsets: object) -> list[str]:
    ...
    try:
        from quill_cli.oneshot import _normalize_toolsets
...
def _launch_tui(
    ...
    active_session_fd, active_session_file = tempfile.mkstemp(
        prefix="quill-tui-active-session-", suffix=".json"
    )
    os.close(active_session_fd)
    env["QUILL_TUI_ACTIVE_SESSION_FILE"] = active_session_file
    env["QUILL_PYTHON_SRC_ROOT"] = os.environ.get(
        "QUILL_PYTHON_SRC_ROOT", str(PROJECT_ROOT)
    )
    env.setdefault("QUILL_PYTHON", sys.executable)
    env.setdefault("QUILL_CWD", os.getcwd())
    ...
    if worktree:
        ...
        if not wt_info:
            sys.exit(1)
        env["QUILL_CWD"] = wt_info["path"]
        env["TERMINAL_CWD"] = wt_info["path"]

    if model:
        env["QUILL_MODEL"] = model
        env["QUILL_INFERENCE_MODEL"] = model
    if provider:
        env["QUILL_TUI_PROVIDER"] = provider
        env["QUILL_INFERENCE_PROVIDER"] = provider
    tui_toolsets = _normalize_tui_toolsets(toolsets)
    if tui_toolsets:
        env["QUILL_TUI_TOOLSETS"] = ",".join(tui_toolsets)
    if skills:
        if isinstance(skills, (list, tuple)):
            ...
            if flattened:
                env["QUILL_TUI_SKILLS"] = ",".join(flattened)
        else:
            value = str(skills).strip()
            if value:
                env["QUILL_TUI_SKILLS"] = value
    if query:
        env["QUILL_TUI_QUERY"] = query
    if image:
        env["QUILL_TUI_IMAGE"] = image
    if checkpoints:
        env["QUILL_TUI_CHECKPOINTS"] = "1"
    if pass_session_id:
        env["QUILL_TUI_PASS_SESSION_ID"] = "1"
    if max_turns is not None:
        env["QUILL_TUI_MAX_TURNS"] = str(max_turns)
    if verbose:
        env["QUILL_TUI_TOOL_PROGRESS"] = "verbose"
    elif quiet:
        env["QUILL_TUI_TOOL_PROGRESS"] = "off"
    if accept_hooks:
        env["QUILL_ACCEPT_HOOKS"] = "1"
    ...
    # QUILL_TUI_RESUME is an internal hand-off from the Python wrapper to the
    # Ink app.  Because we start from os.environ.copy(), an exported/stale value
    # in the user's shell would otherwise make a plain `quill --tui` try to
    ...
    # still set QUILL_TUI_RESUME themselves.
    env.pop("QUILL_TUI_RESUME", None)
    if resume_session_id:
        env["QUILL_TUI_RESUME"] = resume_session_id
    ...
    # Exit code 42 = TUI requested an update. Relaunch as `quill update` so
    ...
    if code == 42:
        from quill_cli.relaunch import relaunch
...
def _pin_kanban_board_env() -> None:
    """Pin the active kanban board into ``QUILL_KANBAN_BOARD`` for the chat session.

    Without this, in-process tools (``kanban_*``) and shelled-out CLI calls
    (``quill kanban …``) resolve the board on different paths: the env-pin if
    set, otherwise the global ``<root>/kanban/current`` file. A concurrent
    ``quill kanban boards switch`` from another session can flip the file
    ...
    """
    if os.environ.get("QUILL_KANBAN_BOARD"):
        return
    try:
        from quill_cli.kanban_db import get_current_board

        os.environ["QUILL_KANBAN_BOARD"] = get_current_board()
    except Exception:
        pass
...
def cmd_chat(args):
    ...
    use_tui = getattr(args, "tui", False) or os.environ.get("QUILL_TUI") == "1"
    ...
    if continue_val and not getattr(args, "resume", None):
        if isinstance(continue_val, str):
            ...
            if resolved:
                ...
                print("Use 'quill sessions list' to see available sessions.")
                sys.exit(1)
        else:
    ...
    if not _has_any_provider_configured():
        print()
        print(
            "It looks like Quill isn't configured yet -- no API keys or providers found."
        )
        print()
        print("  Run:  quill setup")
        print()

        from quill_cli.setup import (
        ...
        if reply in {"", "y", "yes"}:
            ...
            return
        print()
        print("You can run 'quill setup' at any time to configure.")
        sys.exit(1)

    # Start update check in background (runs while other init happens)
    try:
        from quill_cli.banner import prefetch_update_check
    ...
    if getattr(args, "yolo", False):
        os.environ["QUILL_YOLO_MODE"] = "1"

    # --ignore-user-config: make load_cli_config() / load_config() skip the
    # user's ~/.quill/config.yaml and return built-in defaults. Set BEFORE
    # importing cli (which runs `CLI_CONFIG = load_cli_config()` at module
    # import time). Credentials in .env are still loaded — this flag only
    # ignores behavioral/config settings.
    if getattr(args, "ignore_user_config", False):
        os.environ["QUILL_IGNORE_USER_CONFIG"] = "1"

    # --ignore-rules: skip auto-injection of AGENTS.md/SOUL.md/.cursorrules
    # (rules), memory entries, and any preloaded skills coming from user config.
    # Maps to AIAgent(skip_context_files=True, skip_memory=True).
    if getattr(args, "ignore_rules", False):
        os.environ["QUILL_IGNORE_RULES"] = "1"

    # --source: tag session source for filtering (e.g. 'tool' for third-party integrations)
    if getattr(args, "source", None):
        os.environ["QUILL_SESSION_SOURCE"] = args.source
...
def cmd_gateway(args):
    ...
    from quill_cli.gateway import gateway_command
...
def cmd_proxy(args):
    ...
    from quill_cli.proxy.cli import cmd_proxy as _cmd_proxy
...
def cmd_whatsapp(args):
    ...
    from quill_cli.config import get_env_value, save_env_value
    ...
    if not current_mode:
        print()
        print("How will you use WhatsApp with Quill?")
        print()
    ...
    # has no creds.json.  Every subsequent `quill gateway` then paid a 30s
    ...
    session_dir = get_quill_home() / "whatsapp" / "session"
    ...
    if (session_dir / "creds.json").exists():
        ...
        if response.lower() in {"y", "yes"}:
            ...
            print("  Start the gateway with: quill gateway")
            return
    ...
    if (session_dir / "creds.json").exists():
        ...
        # and `quill gateway` skips it cleanly instead of paying a 30s
        ...
        if wa_mode == "bot":
            print("  Next steps:")
            print("    1. Start the gateway:  quill gateway")
            print("    2. Send a message to the bot's WhatsApp number")
            print("    3. The agent will reply automatically")
            print()
            print("  Tip: Agent responses are prefixed with '⚕ Quill Agent'")
        else:
            print("  Next steps:")
            print("    1. Start the gateway:  quill gateway")
        ...
        print()
        print("  Or install as a service: quill gateway install")
    else:
        print("⚠ Pairing may not have completed. Run 'quill whatsapp' to try again.")


def cmd_setup(args):
    """Interactive setup wizard."""
    from quill_cli.setup import run_setup_wizard
...
def cmd_postinstall(args):
    ...
    from quill_cli.config import stamp_install_method
    from quill_cli.dep_ensure import ensure_dependency

    stamp_install_method("pip")

    print("⚕ Quill post-install bootstrap")
    print()
...
def select_provider_and_model(args=None):
    ...
    Shared by ``cmd_model`` (``quill model``) and the setup wizard
    (``setup_model_provider`` in setup.py).  Handles the full flow:
    provider picker, credential prompting, model selection, and config
    persistence.
    """
    from quill_cli.auth import (
        resolve_provider,
        AuthError,
        format_auth_error,
    )
    from quill_cli.config import (
        get_compatible_custom_providers,
        load_config,
        get_env_value,
    )
    from quill_cli.providers import resolve_provider_full
    ...
    effective_provider = (
        config_provider or os.getenv("QUILL_INFERENCE_PROVIDER") or "auto"
    )
    ...
    if effective_provider != "auto":
        ...
        if active_def is not None:
            ...
            warning = (
                f"Unknown provider '{effective_provider}'. Check 'quill model' for "
                "available providers, or run 'quill doctor' to diagnose config "
                "issues."
            )
    ...
    from quill_cli.models import CANONICAL_PROVIDERS, _PROVIDER_LABELS
    ...
    def _named_custom_provider_map(cfg) -> dict[str, dict[str, str]]:
        from quill_cli.config import read_raw_config
    ...
    # a leftover OPENAI_BASE_URL in ~/.quill/.env can poison auxiliary
...
def _clear_stale_openai_base_url():
    """Remove OPENAI_BASE_URL from ~/.quill/.env if the active provider is not 'custom'.
    ...
    """
    from quill_cli.config import get_env_value, save_env_value, load_config
...
#
# Quill uses lightweight "auxiliary" models for side tasks (vision analysis,
...
# `quill model` provider picker. It does NOT re-run credential setup — it
# only routes already-authenticated providers to specific aux tasks. Users
# configure new providers through the normal `quill model` flow first.
# ─────────────────────────────────────────────────────────────────────────────
...
def _save_aux_choice(
    ...
    *,
    ...
    """
    from quill_cli.config import load_config, save_config
...
def _aux_config_menu() -> None:
    ...
    """
    from quill_cli.config import load_config
    ...
    while True:
        ...
        print("  Quill only falls back to a lightweight backend (OpenRouter,")
...
def _aux_select_for_task(task: str) -> None:
    ...
    ``quill model`` flow, then route aux tasks to them here.
    """
    from quill_cli.config import load_config
    from quill_cli.model_switch import list_authenticated_providers
...
def _aux_flow_provider_model(
    ...
    from quill_cli.auth import _prompt_model_selection
    from quill_cli.models import get_pricing_for_provider
...
def _prompt_provider_choice(choices, *, default=0):
    ...
    """
    try:
        from quill_cli.setup import _curses_prompt_choice
...
def _model_flow_openrouter(config, current_model=""):
    ...
    from quill_cli.config import get_env_value

    # Route through _prompt_api_key so users can replace a stale/broken key
    # in-flow (K/R/C) instead of having to edit ~/.quill/.env by hand. The
    # previous bypass-when-key-exists branch left no way to recover from a
    # bad paste short of re-running `quill setup` from scratch. OpenRouter
    ...
    from quill_cli.models import model_ids, get_pricing_for_provider
...
def _model_flow_ai_gateway(config, current_model=""):
    ...
    # Route through _prompt_api_key so users can replace a stale/broken key
    # in-flow (K/R/C) instead of having to edit ~/.quill/.env by hand.
    ...
    from quill_cli.models import ai_gateway_model_ids, get_pricing_for_provider
...
def _model_flow_nous(config, current_model="", args=None):
    ...
    from quill_cli.nous_subscription import prompt_enable_tool_gateway
    ...
    if free_tier and not model_ids:
        ...
        if unavailable_models:
            from quill_cli.auth import DEFAULT_NOUS_PORTAL_URL
...
def _model_flow_openai_codex(config, current_model=""):
    ...
    from quill_cli.codex_models import get_codex_model_ids
    ...
    _codex_token = None
    # Prefer credential pool (where `quill auth` stores device_code tokens),
    ...
    if not _codex_token:
        try:
            from quill_cli.auth import resolve_codex_runtime_credentials
...
def _model_flow_xai_oauth(_config, current_model="", *, args=None):
    ...
    if status.get("logged_in"):
        ...
        if choice == "2":
            ...
            print()
            try:
                # Forward CLI flags from ``quill model --manual-paste``
                # / ``--no-browser`` / ``--timeout`` into the loopback
                # login. Without this, browser-only remotes (#26923)
                # can't reach the manual-paste path via ``quill model``.
                mock_args = argparse.Namespace(
    ...
    # live only in the pool (e.g. after ``quill auth add xai-oauth``).  Fall
...
def _model_flow_qwen_oauth(_config, current_model=""):
    ...
    from quill_cli.models import fetch_api_models
...
def _model_flow_custom(config):
    ...
    """
    from quill_cli.auth import _save_model_choice, deactivate_provider
    from quill_cli.config import get_env_value, load_config, save_config
    ...
    from quill_cli.models import probe_api_models
    ...
    if model_name:
        ...
        print("Endpoint saved. Use `/model` in chat or `quill model` to set a model.")
...
def _prompt_custom_api_mode_selection(base_url: str, current_api_mode: str = "") -> Optional[str]:
    ...
    """
    from quill_cli.runtime_provider import _detect_api_mode_for_url
    ...
    mode_options = [
        (
            "",
            "Auto-detect",
            "Use Quill URL heuristics; best for standard OpenAI-compatible endpoints.",
        ),
        (
            "chat_completions",
            "Chat Completions",
...
def _model_flow_azure_foundry(config, current_model=""):
    ...
    * Manual entry when every probe fails (private endpoints, etc.).
    ...
    """
    from quill_cli.auth import _save_model_choice, deactivate_provider  # noqa: F401
    from quill_cli.config import (
        get_env_value,
        save_env_value,
        load_config,
    ...
    from quill_cli import azure_detect
    import getpass
    ...
    print("Anthropic-style API endpoints.  Quill will probe your")
    ...
    if use_entra:
        ...
        if not has_azure_identity_installed():
            ...
            print(
                "  Quill will install it now (the preflight below "
...
def _remove_custom_provider(config):
    ...
    try:
        ...
        idx = menu.show()
        from quill_cli.curses_ui import flush_stdin
...
def _model_flow_copilot(config, current_model=""):
    ...
    from quill_cli.config import save_env_value, load_config, save_config
    from quill_cli.models import (
        fetch_api_models,
    ...
    if not api_key:
        ...
        if choice == "1":
            try:
                from quill_cli.copilot_auth import copilot_device_code_login
            ...
            # Validate token type
            try:
                from quill_cli.copilot_auth import validate_copilot_token
...
def _model_flow_copilot_acp(config, current_model=""):
    ...
    print("  GitHub Copilot ACP delegates Quill turns to `copilot --acp`.")
    print("  Quill currently starts its own ACP subprocess for each request.")
    print("  Quill uses your selected model as a hint for the Copilot ACP session.")
    ...
    try:
        ...
        print(f"  ⚠ {exc}")
        print(
            "  Set QUILL_COPILOT_ACP_COMMAND or COPILOT_CLI_PATH if Copilot CLI is installed elsewhere."
        )
        return
...
def _prompt_api_key(pconfig, existing_key: str, provider_id: str = "") -> tuple:
    """Shared API-key entry point for ``quill setup`` / ``quill model``.
    ...
    recover from a malformed paste without editing ``~/.quill/.env`` by hand.
    ...
    from quill_cli.auth import LMSTUDIO_NOAUTH_PLACEHOLDER
    from quill_cli.config import save_env_value
    ...
    if choice.startswith("c"):
        ...
        print(
            f"  API key cleared.  Re-run `quill setup` to configure {pconfig.name} again."
        )
        return "", True
...
def _model_flow_bedrock_api_key(config, region, current_model=""):
    ...
    if selected:
        ...
        # Save the API key env var name so quill knows where to find it
...
def _model_flow_api_key_provider(config, provider_id, current_model=""):
    ...
    if provider_id == "gemini" and existing_key:
        ...
        if probe_gemini_tier is not None:
            ...
            if tier == "free":
                ...
                print(
                    "   Quill typically makes 3-10 API calls per user turn "
                ...
                print()
                print(
                    "   To use Gemini with Quill, enable billing on your "
    ...
    if provider_id == "lmstudio":
        from quill_cli.auth import AuthError
        from quill_cli.models import fetch_lmstudio_models
        ...
        from quill_cli.models import fetch_ollama_cloud_models
...
def _run_anthropic_oauth_flow(save_env_value):
    ...
    def _activate_claude_code_credentials_if_available() -> bool:
        ...
        if creds and (
            ...
            from quill_constants import display_quill_home as _dhh_fn

            print(
                f"    Quill will use Claude's credential store directly instead of copying a setup-token into {_dhh_fn()}/.env."
            )
            return True
        return False
...
def _model_flow_anthropic(config, current_model=""):
    ...
    from quill_cli.auth import get_anthropic_key
    ...
    if selected:
        ...
        # providers if the user switches without running 'quill model'.
        cfg = load_config()
        model = cfg.get("model")
        if not isinstance(model, dict):
            model = {"default": model} if model else {}
...
def cmd_login(args):
    """Authenticate Quill CLI with a provider."""
    from quill_cli.auth import login_command
...
def cmd_logout(args):
    ...
    from quill_cli.auth import logout_command
...
def cmd_auth(args):
    ...
    from quill_cli.auth_commands import auth_command
...
def cmd_status(args):
    ...
    from quill_cli.status import show_status
...
def cmd_cron(args):
    """Cron job management."""
    from quill_cli.cron import cron_command
...
def cmd_webhook(args):
    ...
    from quill_cli.webhook import webhook_command
...
def cmd_slack(args):
    ...
    Dispatches ``quill slack <subcommand>``. Currently supports:
    ...
    if sub in {None, ""}:
        ...
        print(
            "usage: quill slack <subcommand>\n"
            "\n"
            "subcommands:\n"
            "  manifest   Generate a Slack app manifest with every gateway\n"
            "             command registered as a native slash\n"
            "\n"
            "Run `quill slack manifest -h` for details.",
            file=sys.stderr,
        )
        return 1

    if sub == "manifest":
        from quill_cli.slack_cli import slack_manifest_command
...
def cmd_kanban(args):
    ...
    from quill_cli.kanban import kanban_command
...
def cmd_hooks(args):
    ...
    from quill_cli.hooks import hooks_command
...
def cmd_doctor(args):
    ...
    from quill_cli.doctor import run_doctor
...
def cmd_dump(args):
    ...
    from quill_cli.dump import run_dump
...
def cmd_debug(args):
    ...
    from quill_cli.debug import run_debug
...
def cmd_config(args):
    ...
    from quill_cli.config import config_command

    config_command(args)


def cmd_backup(args):
    """Back up Quill home directory to a zip file."""
    if getattr(args, "quick", False):
        from quill_cli.backup import run_quick_backup

        run_quick_backup(args)
    else:
        from quill_cli.backup import run_backup

        run_backup(args)


def cmd_import(args):
    """Restore a Quill backup from a zip file."""
    from quill_cli.backup import run_import
...
def cmd_version(args):
    """Show version."""
    print(f"Quill Agent v{__version__} ({__release_date__})")
    ...
    try:
        from quill_cli.banner import check_for_updates
        from quill_cli.config import recommended_update_command
...
def cmd_uninstall(args):
    """Uninstall Quill Agent."""
    _require_tty("uninstall")
    from quill_cli.uninstall import run_uninstall
...
def _gateway_prompt(prompt_text: str, default: str = "", timeout: float = 300.0) -> str:
    ...
    Used by ``quill update --gateway`` so interactive prompts (stash restore,
...
def _web_ui_build_needed(web_dir: Path) -> bool:
    ...
    The Vite build outputs to ``quill_cli/web_dist/`` (per vite.config.ts
    outDir: "../quill_cli/web_dist"), NOT to ``web/dist/``.  Uses the Vite
    manifest as the sentinel because it is written last and therefore has the
    newest mtime of any build output.
    """
    dist_dir = web_dir.parent / "quill_cli" / "web_dist"
...
def _run_npm_install_deterministic(
    ...
    *,
    ...
    the working tree dirty and causes the next ``quill update`` to stash the
    lockfile — repeatedly.
    """
...
def _build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    Args:
        ...
        fatal: If True, print error guidance and return False on failure
               instead of a soft warning (used by ``quill web``).
    ...
    # build path usable on a stock `py -m quill_cli.main web` invocation.
    ...
    if r1.returncode != 0:
        _say(
            ...
            + ("" if fatal else " (quill web will not be available)")
        )
        _relay(r1)
        if fatal:
...
def _find_stale_dashboard_pids() -> list[int]:
    """Return PIDs of ``quill dashboard`` processes other than ourselves.

    ``quill dashboard`` is a long-lived server process commonly started and
    forgotten.  When ``quill update`` replaces files on disk, the running
    ...
    patterns = [
        "quill dashboard",
        "quill_cli.main dashboard",
        "quill_cli/main.py dashboard",
    ]
    self_pid = os.getpid()
    ...
    try:
        if sys.platform == "win32":
            ...
            # (rather than `pgrep -f "quill.*dashboard"`) keeps us consistent
            # with `quill_cli.gateway._scan_gateway_pids` and avoids the
...
def _print_curator_first_run_notice() -> None:
    """Print a short heads-up about the skill curator after `quill update`.
    ...
    print("  Preview now:  quill curator run --dry-run")
    print("  Pause it:     quill curator pause")
    print(
        "  Docs:         https://quill-agent.example.com/docs/user-guide/features/curator"
    )
...
def _print_curator_recent_run_notice() -> None:
    ...
    rename. ``quill update`` is a high-attention surface — surface the
    most recent run's rename map here, once.

    Show-once: state stamps ``last_run_summary_shown_at`` after printing.
    Subsequent ``quill update`` invocations skip the block until a newer
    ...
    print(
        ...
        "View anytime: quill curator status)"
    )
...
def _kill_stale_dashboard_processes(
    ...
    """Kill running ``quill dashboard`` processes.

    Called at the end of ``quill update`` (default ``reason``) and also
    from ``quill dashboard --stop`` (which overrides ``reason``).  The
    ...
    if killed:
        ...
        print("    quill dashboard --port <port>")
...
def _update_via_zip(args):
    """Update Quill Agent by downloading a ZIP archive.
    ...
    branch = "main"
    zip_url = (
        f"https://github.com/YOUR_USERNAME/quill-agent/archive/refs/heads/{branch}.zip"
    )

    print("→ Downloading latest version...")
    try:
        tmp_dir = tempfile.mkdtemp(prefix="quill-update-")
        zip_path = os.path.join(tmp_dir, f"quill-agent-{branch}.zip")
        ...
        # GitHub ZIPs extract to quill-agent-<branch>/
        extracted = os.path.join(tmp_dir, f"quill-agent-{branch}")
...
def _stash_local_changes_if_needed(git_cmd: list[str], cwd: Path) -> Optional[str]:
    ...
    stash_name = datetime.now(timezone.utc).strftime(
        "quill-update-autostash-%Y%m%d-%H%M%S"
    )
...
def _restore_stashed_changes(
    ...
    if prompt_user:
        ...
        print("  Review the result afterward if Quill behaves unexpectedly.")
    ...
    if restore.returncode != 0 or has_conflicts:
        ...
        # files makes quill completely unrunnable (SyntaxError on import).
    ...
    if stash_selector is None:
        print(
            "⚠ Local changes were restored, but Quill couldn't find the stash entry to drop."
        )
        print(
        ...
        if drop.returncode != 0:
            print(
                "⚠ Local changes were restored, but Quill couldn't drop the saved stash entry."
            )
    ...
    print("  Review `git diff` / `git status` if Quill behaves unexpectedly.")
    return True


# =========================================================================
# Fork detection and upstream management for `quill update`
# =========================================================================

OFFICIAL_REPO_URLS = {
    "https://github.com/YOUR_USERNAME/quill-agent.git",
    "git@github.com:YOUR_USERNAME/quill-agent.git",
    "https://github.com/YOUR_USERNAME/quill-agent",
    "git@github.com:YOUR_USERNAME/quill-agent",
}
OFFICIAL_REPO_URL = "https://github.com/YOUR_USERNAME/quill-agent.git"
...
def _should_skip_upstream_prompt() -> bool:
    ...
    return (get_quill_home() / SKIP_UPSTREAM_PROMPT_FILE).exists()
...
def _mark_skip_upstream_prompt():
    ...
    try:
        ...
        (get_quill_home() / SKIP_UPSTREAM_PROMPT_FILE).touch()
    except Exception:
        pass
...
def _sync_with_upstream_if_needed(git_cmd: list[str], cwd: Path) -> None:
    ...
    if not has_upstream:
        ...
        print()
        print("ℹ Your fork is not tracking the official Quill repository.")
        print("  This means you may miss updates from YOUR_USERNAME/quill-agent.")
        print()
        try:
            response = (
        ...
        if response in {"", "y", "yes"}:
            ...
            if _add_upstream_remote(git_cmd, cwd):
                print(
                    "  ✓ Added upstream: https://github.com/YOUR_USERNAME/quill-agent.git"
                )
                has_upstream = True
            else:
            ...
            print(
                "  Skipped. Run 'git remote add upstream https://github.com/YOUR_USERNAME/quill-agent.git' to add later."
            )
...
def _invalidate_update_cache():
    ...
    ``quill update``, every profile is now current.
    """
    homes = []
    # Default profile home (Docker-aware — uses /opt/data in Docker)
    from quill_constants import get_default_quill_root

    default_home = get_default_quill_root()
...
def _run_install_with_heartbeat(
    ...
    *,
    ...
    know ``quill update`` is still progressing even if pip/uv itself is silent.
    """
...
def _quill_exe_shims(scripts_dir: Path) -> list[Path]:
    ...
    if not _is_windows():
        return []
    return [
        scripts_dir / "quill.exe",
        scripts_dir / "quill-gateway.exe",
    ]


def _quarantine_running_quill_exe(scripts_dir: Path) -> list[tuple[Path, Path]]:
    """Pre-empt Windows file lock on the running ``quill.exe``.
    ...
    when ``quill update`` runs, ``quill.exe`` IS the live process, and uv
    fails with ``Access is denied. (os error 5)``.

    We rename live shims to ``quill.exe.old.<unix-ms>`` first. uv then writes
    fresh shims at the original paths. The ``.old`` files are cleaned up on
    the next quill invocation by ``_cleanup_quarantined_exes``.
    ...
    for shim in _quill_exe_shims(scripts_dir):
        if not shim.exists():
            continue
...
def _restore_quarantined_exes(moved: list[tuple[Path, Path]]) -> None:
    """Roll back ``_quarantine_running_quill_exe`` if uv didn't write replacements."""
...
def _cleanup_quarantined_exes(scripts_dir: Path | None = None) -> None:
    """Sweep ``quill.exe.old.*`` left by prior updates.

    Called early on every quill invocation. The .old files are unlocked once
...
def _refresh_active_lazy_features() -> None:
    ...
    first use. ``quill update`` runs ``uv pip install -e .[all]`` which
    ...
    if failed:
        ...
        print("  `quill update` once the upstream issue is resolved.")
...
def _install_python_dependencies_with_optional_fallback(
    ...
    *,
    ...
    On Windows, pre-renames live ``quill.exe`` / ``quill-gateway.exe`` shims
    in the venv Scripts dir before each install attempt so uv can write fresh
    copies (Windows blocks REPLACE on a running .exe but allows RENAME). See
    ``_quarantine_running_quill_exe`` for the rationale.
    """
    ...
    def _install(args: list[str]) -> None:
        ...
        if scripts_dir is not None:
            moved = _quarantine_running_quill_exe(scripts_dir)
        try:
...
def _update_node_dependencies() -> None:
    ...
    for label, path in paths:
        ...
        # streamed output to ``~/.quill/logs/update.log`` so nothing is lost.
...
class _UpdateOutputStream:
    """Stream wrapper used during ``quill update`` to survive terminal loss.

    Wraps the process's original stdout/stderr so that:

    * Every write is also mirrored to an append-only log file
      (``~/.quill/logs/update.log``) that users can inspect after the
    ...
    * Writes to the original stream that fail with ``BrokenPipeError`` /
    ...
    ``_install_hangup_protection``, this makes ``quill update`` safe to
...
def _install_hangup_protection(gateway_mode: bool = False):
    ...
    Users commonly run ``quill update`` in an SSH session or a terminal
    ...
    2. ``sys.stdout`` / ``sys.stderr`` are wrapped to mirror output to
       ``~/.quill/logs/update.log`` and to silently absorb
    ...
    **intentionally left alone** — those are legitimate cancellation
    ...
    In gateway mode (``quill update --gateway``) the update is already
    ...
    try:
        ...
        # quill_cli.config.get_quill_home to simulate setup failure.
        from quill_cli.config import get_quill_home as _get_quill_home

        logs_dir = _get_quill_home() / "logs"
...
def _cmd_update_check():
    """Implement ``quill update --check``: fetch and report without installing."""
    from quill_cli.config import detect_install_method
    method = detect_install_method(PROJECT_ROOT)
    if method == "pip":
        from quill_cli.config import recommended_update_command
        from quill_cli.banner import check_via_pypi
...
def _ensure_fhs_path_guard() -> None:
    ...
    repaired on ``quill update`` without requiring a reinstall.  The
    ...
    ``quill`` prints ``command not found`` even though the symlink lives
    at /usr/local/bin/quill.

    Silent no-op on: non-Linux, non-root, non-FHS installs, and any system
    where ``bash -i -c 'command -v quill'`` already resolves.  Idempotent.
    """
    ...
    # /usr/local/bin/quill, code at /usr/local/lib/quill-agent).
    fhs_link = Path("/usr/local/bin/quill")
    ...
    path_comment = (
        "# Quill Agent — ensure /usr/local/bin is on PATH " "(RHEL non-login shells)"
    )
    wrote_any = False
...
def _run_pre_update_backup(args) -> None:
    """Create a full zip backup of QUILL_HOME before running the update.
    ...
    QUILL_HOME directories.  The ``--backup`` flag on ``quill update``
    ...
    try:
        from quill_cli.backup import create_pre_update_backup
    except Exception as exc:
        print(
    ...
    # Render path using display_quill_home so the user sees ~/.quill/...
    try:
        from quill_constants import get_quill_home, display_quill_home

        home = get_quill_home()
        try:
            display_path = f"{display_quill_home()}/{out_path.relative_to(home)}"
        except ValueError:
    ...
    print(f"  Restore:  quill import {out_path}")
...
def cmd_update(args):
    """Update Quill Agent to the latest version.

    Thin wrapper around ``_cmd_update_impl``: installs hangup protection,
    runs the update, then restores stdio on the way out (even on
    ``sys.exit`` or unhandled exceptions).
    """
    from quill_cli.config import is_managed, managed_error

    if is_managed():
        managed_error("update Quill Agent")
        return
...
def _cmd_update_pip(args):
    """Update Quill via pip (for PyPI installs)."""
    from quill_cli import __version__
    ...
    uv = shutil.which("uv")
    if uv:
        cmd = [uv, "pip", "install", "--upgrade", "quill-agent"]
    else:
        cmd = [sys.executable, "-m", "pip", "install", "--upgrade", "quill-agent"]
    ...
    print("✓ Update complete! Restart quill to use the new version.")
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    if not git_dir.exists():
        if sys.platform == "win32":
            ...
            print(
                "  curl -fsSL https://raw.githubusercontent.com/YOUR_USERNAME/quill-agent/main/scripts/install.sh | bash"
            )
            sys.exit(1)
    ...
    try:
        ...
        # the old bytecode (e.g. get_quill_home added to quill_constants).
        removed = _clear_bytecode_cache(PROJECT_ROOT)
        if removed:
            print(
                f"  ✓ Cleared {removed} stale __pycache__ director{'y' if removed == 1 else 'ies'}"
        ...
        # modules in this process.  Reload quill_constants so that any lazy
        # import executed below (skills sync, gateway restart) sees new
        # attributes like display_quill_home() added since the last release.
        try:
            import importlib
            import quill_constants as _hc
        ...
        # seed_profile_skills() uses subprocess with an explicit QUILL_HOME so
        # it is not affected by sync_skills()'s module-level QUILL_HOME cache,
        # which means the active profile is reliably synced regardless of whether
        # the caller's QUILL_HOME env var points at the default or a named profile.
        try:
            from quill_cli.profiles import (
                list_profiles,
                seed_profile_skills,
            )
        ...
        if needs_migration:
            ...
            if response in {"", "y", "yes", "auto"}:
                ...
                if (gateway_mode or assume_yes or response == "auto") and missing_env:
                    print("  ℹ API keys require manual entry: quill config migrate")
            else:
                print()
                print("Skipped. Run 'quill config migrate' later to configure.")
        else:
        ...
        # check `quill curator status`. Self-stamps after printing so it
        ...
        # Tying the refresh to ``quill update`` gives users a predictable
        ...
        try:
            if sys.platform == "darwin" and shutil.which("cua-driver"):
                from quill_cli.tools_config import install_cua_driver
        ...
        # When running as ``quill update --gateway`` (spawned by the gateway's
        ...
        if gateway_mode:
            _exit_code_path = get_quill_home() / ".update_exit_code"
            try:
        ...
        try:
            ...
            # Discover all quill-gateway* units (default + profiles)
                ...
                for scope, scope_cmd in [
                    ...
                    try:
                        ...
                        for line in result.stdout.strip().splitlines():
                            ...
                            unit = parts[
                                0
                            ]  # e.g. quill-gateway.service or quill-gateway-coder.service
            ...
            if restarted_services or killed_pids:
                ...
                if unmapped_count:
                    ...
                    print("    Restart manually: quill gateway run")
                    if unmapped_count > 1:
                        print(
                            "    (or: quill -p <profile> gateway run  for each profile)"
                        )
        ...
        # Warn if legacy Quill gateway unit files are still installed.
        # When both quill.service (from a pre-rename install) and the
        # current quill-gateway.service are enabled, they SIGTERM-fight
        # for the same bot token (see PR #11909). Flagging here means
        # every `quill update` surfaces the issue until the user migrates.
        try:
            from quill_cli.gateway import (
                has_legacy_quill_units,
                _find_legacy_quill_units,
                supports_systemd_services,
            )

            if supports_systemd_services() and has_legacy_quill_units():
                print()
                print("⚠ Legacy Quill gateway unit(s) detected:")
                for name, path, is_sys in _find_legacy_quill_units():
                    scope = "system" if is_sys else "user"
                    print(f"    {path}  ({scope} scope)")
                print()
                print("  These pre-rename units (quill.service) fight the current")
                print("  quill-gateway.service for the bot token and cause SIGTERM")
                print("  flap loops. Remove them with:")
                print()
                print("    quill gateway migrate-legacy")
                print()
        ...
        print("  quill model              # Select provider and model")
...
def _coalesce_session_name_args(argv: list) -> list:
    ...
    When a user types ``quill -c Pokemon Agent Dev`` without quoting the
...
def cmd_profile(args):
    ...
    from quill_constants import display_quill_home

    action = getattr(args, "profile_action", None)

    if action is None:
        # Bare `quill profile` — show current profile status
        profile_name = get_active_profile_name()
        dhh = display_quill_home()
        ...
        for p in profiles:
            if p.name == profile_name or (profile_name == "default" and p.is_default):
                ...
                if p.alias_path:
                    print(f"Alias:          {p.name} → quill -p {p.name}")
                break
        print()
        return
    ...
    if action == "list":
        ...
        try:
            ...
            if name == "default":
                print(f"Switched to: default (~/.quill)")
            else:
        ...
        try:
            ...
            if not no_alias:
                ...
                if collision:
                    ...
                    print(
                        f"  Choose a custom alias:  quill profile alias {name} --name <custom>"
                    )
                    print(f"  Or access via flag:     quill -p {name} chat")
                else:
        ...
        from quill_cli import profiles as _profiles_mod
        ...
        if name and not text_value and not auto_flag:
            try:
                if _profiles_mod.normalize_profile_name(name) == "default":
                    from quill_constants import get_quill_home as _hh
        ...
        from quill_cli import profile_describer as _pd
        ...
        if dist_name:
            ...
            print(f"  (run `quill profile info {name}` for full manifest)")
        if wrapper.exists():
        ...
        from quill_cli.profiles import profile_exists
        ...
        from quill_cli.profiles import rename_profile
        ...
        from quill_cli.profiles import export_profile
        ...
        from quill_cli.profiles import import_profile
        ...
        import tempfile
        from quill_cli.profile_distribution import (
            plan_install,
        ...
        try:
            ...
            with tempfile.TemporaryDirectory(prefix="quill_dist_preview_") as tmp:
                plan = plan_install(
                    args.source,
                    Path(tmp),
            ...
            if plan.has_cron:
                print(
                    ...
                    f"  Review them with:  quill -p {plan.manifest.name} cron list"
                )
            print(f"\n  Use with:      quill -p {plan.manifest.name} chat")
        ...
        from quill_cli.profiles import get_profile_dir, normalize_profile_name
        ...
        try:
            ...
            if current is None:
                print(
                    ...
                    "Only profiles installed via `quill profile install` can be updated."
                )
                sys.exit(1)
            ...
            if plan.has_cron:
                print(
                    ...
                    f"quill -p {plan.manifest.name} cron list"
                )
        except (DistributionError, ValueError) as e:
            print(f"Error: {e}")
            sys.exit(1)
        ...
        from quill_cli.profile_distribution import describe_distribution, DistributionError
        ...
        if data.get("quill_requires"):
            print(f"Requires:     Quill {data['quill_requires']}")
        if data.get("source"):
...
def _render_distribution_plan(plan) -> None:
    ...
    from quill_cli.profile_distribution import MANIFEST_FILENAME
    mf = plan.manifest
    ...
    if mf.quill_requires:
        print(f"  Requires: Quill {mf.quill_requires}")
...
def _report_dashboard_status() -> int:
    """Print ``quill dashboard`` PIDs and return the count.

    Uses the same detection logic as ``_find_stale_dashboard_pids`` (the
    current process is excluded, but since ``quill dashboard --status``
    runs in a short-lived CLI process that never matches the pattern,
    the exclusion is irrelevant here).
    """
    pids = _find_stale_dashboard_pids()
    if not pids:
        print("No quill dashboard processes running.")
        return 0

    print(f"{len(pids)} quill dashboard process(es) running:")
    for pid in pids:
...
def cmd_dashboard(args):
    ...
    if getattr(args, "stop", False):
        ...
        if not pids:
            print("No quill dashboard processes running.")
            sys.exit(0)
        # Reuse the same SIGTERM-grace-SIGKILL path used after `quill update`.
    ...
    if "QUILL_WEB_DIST" not in os.environ and not getattr(args, "skip_build", False):
        ...
        _dist_root = (
            Path(os.environ["QUILL_WEB_DIST"])
            if "QUILL_WEB_DIST" in os.environ
            else PROJECT_ROOT / "quill_cli" / "web_dist"
        )
    ...
    from quill_cli.web_server import start_server

    embedded_chat = args.tui or os.environ.get("QUILL_DASHBOARD_TUI") == "1"
    start_server(
        host=args.host,
        port=args.port,
...
def cmd_completion(args, parser=None):
    ...
    from quill_cli.completion import generate_bash, generate_zsh, generate_fish
...
def cmd_logs(args):
    """View and filter Quill log files."""
    from quill_cli.logs import tail_log, list_logs
...
def _build_provider_choices() -> list[str]:
    ...
    try:
        from quill_cli.models import CANONICAL_PROVIDERS as _cp
...
# so that in ``quill -m gpt5 chat``, ``gpt5`` is correctly skipped as a
# flag value rather than misclassified as a subcommand. Kept in sync with
# the top-level flags declared in ``quill_cli/_parser.py``.
#
...
def _first_positional_argv() -> str | None:
    ...
    ``quill -m gpt5 --provider openai chat "msg"`` by skipping the
...
def _plugin_cli_discovery_needed() -> bool:
    ...
    if first is None:
        # Bare ``quill`` or only flags → defaults to ``chat``.
        return False
...
def main():
    """Main entry point for quill CLI."""
    # Force UTF-8 stdio on Windows before anything prints.  No-op elsewhere.
    try:
        from quill_cli.stdio import configure_windows_stdio
        configure_windows_stdio()
    except Exception:
        pass

    # Sweep stale ``quill.exe.old.*`` quarantine files left by previous
    # ``quill update`` runs on Windows. Silent no-op on non-Windows or when
    # there's nothing to clean. See ``_quarantine_running_quill_exe``.
    try:
        _cleanup_quarantined_exes()
    except Exception:
        pass

    from quill_cli._parser import build_top_level_parser
    ...
    model_parser.add_argument(
        "--client-id",
        default=None,
        help="OAuth client id to use for Nous login (default: quill-cli)",
    )
    model_parser.add_argument(
    ...
    # =========================================================================
    from quill_cli.fallback_cmd import cmd_fallback
    ...
    fallback_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "https://quill-agent.example.com/docs/user-guide/features/fallback-providers"
        ),
    )
    ...
    fallback_subparsers.add_parser(
        "add",
        help="Pick a provider + model (same picker as `quill model`) and append to the chain",
    )
    fallback_subparsers.add_parser(
        "remove",
        aliases=["rm"],
    ...
    gateway_migrate_legacy = gateway_subparsers.add_parser(
        "migrate-legacy",
        help="Remove legacy quill.service units from pre-rename installs",
        description=(
            "Stop, disable, and remove legacy Quill gateway unit files "
            "(e.g. quill.service) left over from older installs. Profile "
            "units (quill-gateway-<profile>.service) and unrelated "
    ...
    proxy_start.add_argument(
        "--provider",
        default="nous",
        help="Upstream provider: nous or xai (default: nous). See `quill proxy providers`.",
    )
    proxy_start.add_argument(
        "--host",
        default=None,
    ...
    setup_parser = subparsers.add_parser(
        ...
        description="Configure Quill Agent with an interactive wizard. "
        "Run a specific section: quill setup model|tts|terminal|gateway|tools|agent",
    )
    ...
    setup_parser.add_argument(
        ...
        "compatibility — a bare 'quill setup' now does this.",
    )
    setup_parser.add_argument(
        "--quick",
        action="store_true",
    ...
    slack_parser = subparsers.add_parser(
        ...
        description="Slack integration helpers for Quill.",
    )
    ...
    slack_manifest.add_argument(
        ...
        "writes to $QUILL_HOME/slack-manifest.json.",
    )
    slack_manifest.add_argument(
        "--name",
        default=None,
        help='Bot display name (default: "Quill")',
    )
    slack_manifest.add_argument(
        "--description",
        default=None,
    ...
    # =========================================================================
    from quill_cli.send_cmd import register_send_subparser
    ...
    login_parser = subparsers.add_parser(
        ...
        description="Run OAuth device authorization flow for Quill CLI",
    )
    ...
    login_parser.add_argument(
        "--client-id", default=None, help="OAuth client id to use (default: quill-cli)"
    )
    ...
    auth_spotify = auth_subparsers.add_parser(
        "spotify", help="Authenticate Quill with Spotify via PKCE"
    )
    ...
    auth_spotify.add_argument(
        "--client-id", help="Spotify app client_id (or set QUILL_SPOTIFY_CLIENT_ID)"
    )
    auth_spotify.add_argument(
        "--redirect-uri",
    ...
    status_parser = subparsers.add_parser(
        ...
        description="Display status of Quill Agent components",
    )
    ...
    cron_create.add_argument(
        "--script",
        help=(
            "Path to a script under ~/.quill/scripts/. Default mode: "
    ...
    cron_create.add_argument(
        "--profile",
        help="Quill profile name to run the job under. Use 'default' for the root profile. Named profiles must already exist. Omit to preserve the scheduler's existing profile.",
    )
    ...
    cron_edit.add_argument(
        "--script",
        help=(
            "Path to a script under ~/.quill/scripts/. Pass empty string to clear. "
    ...
    cron_edit.add_argument(
        "--profile",
        help="Quill profile name to run the job under. Use 'default' for the root profile. Pass empty string to clear.",
    )
    ...
    # =========================================================================
    from quill_cli.kanban import build_parser as _build_kanban_parser
    ...
    hooks_parser = subparsers.add_parser(
        ...
        description=(
            "Inspect shell-script hooks declared in ~/.quill/config.yaml, "
            "test them against synthetic payloads, and manage the first-use "
            "consent allowlist at ~/.quill/shell-hooks-allowlist.json."
        ),
    )
    ...
    doctor_parser = subparsers.add_parser(
        ...
        description="Diagnose issues with Quill Agent setup",
    )
    ...
    doctor_parser.add_argument(
        ...
        help=(
            ...
            "advisory will no longer trigger startup banners. Run `quill "
    ...
    dump_parser = subparsers.add_parser(
        ...
        description="Output a compact, plain-text summary of your Quill setup "
    ...
    debug_parser = subparsers.add_parser(
        ...
        description="Debug utilities for Quill Agent. Use 'quill debug share' to "
        "upload a debug report (system info + recent logs) to a paste "
        "service and get a shareable URL.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    quill debug share              Upload debug report and print URL
    quill debug share --lines 500  Include more log lines
    quill debug share --expire 30  Keep paste for 30 days
    quill debug share --local      Print report locally (no upload)
    quill debug share --no-redact  Disable upload-time secret redaction
    quill debug delete <url>       Delete a previously uploaded paste
""",
    )
    ...
    delete_parser = debug_sub.add_parser(
        "delete",
        help="Delete a paste uploaded by 'quill debug share'",
    )
    ...
    backup_parser = subparsers.add_parser(
        "backup",
        help="Back up Quill home directory to a zip file",
        description="Create a zip archive of your entire Quill configuration, "
        "skills, sessions, and data (excludes the quill-agent codebase). "
        "Use --quick for a fast snapshot of just critical state files.",
    )
    backup_parser.add_argument(
        "-o",
        "--output",
        help="Output path for the zip file (default: ~/quill-backup-<timestamp>.zip)",
    )
    backup_parser.add_argument(
        "-q",
        "--quick",
    ...
    checkpoints_parser = subparsers.add_parser(
        "checkpoints",
        help="Inspect / prune / clear ~/.quill/checkpoints/",
        description="Manage the filesystem checkpoint store — the shadow git "
        "repo quill uses to snapshot working directories before "
        "write_file/patch/terminal calls. Lets you see how much "
        "space checkpoints occupy, force a prune, or wipe the base.",
    )
    from quill_cli.checkpoints import register_cli as _register_checkpoints_cli
    ...
    import_parser = subparsers.add_parser(
        "import",
        help="Restore a Quill backup from a zip file",
        description="Extract a previously created Quill backup into your "
        "Quill home directory, restoring configuration, skills, "
        "sessions, and data",
    )
    ...
    config_parser = subparsers.add_parser(
        ...
        description="Manage Quill Agent configuration",
    )
    ...
    def cmd_pairing(args):
        from quill_cli.pairing import pairing_command
    ...
    skills_reset = skills_subparsers.add_parser(
        ...
        description=(
            "Clear a bundled skill's entry from the sync manifest (~/.quill/skills/.bundled_manifest) "
            "so future 'quill update' runs stop marking it as user-modified. Pass --restore to also "
    ...
    def cmd_skills(args):
        ...
        if getattr(args, "skills_action", None) == "config":
            ...
            from quill_cli.skills_config import skills_command as skills_config_command

            skills_config_command(args)
        else:
            from quill_cli.skills_hub import skills_command
    ...
    from quill_cli.bundles import register_cli as _bundles_register, bundles_command
    ...
    plugins_install.add_argument(
        "identifier",
        help="Git URL or owner/repo shorthand (e.g. anpicasso/quill-plugin-chrome-profiles)",
    )
    plugins_install.add_argument(
        "--force",
        "-f",
    ...
    _install_enable_group.add_argument(
        "--no-enable",
        action="store_true",
        help="Install disabled (skip confirmation prompt); enable later with `quill plugins enable <name>`",
    )
    ...
    def cmd_plugins(args):
        from quill_cli.plugins_cmd import plugins_command
    ...
    # subcommand — ``quill --help``, ``quill version``, ``quill logs``,
    ...
    if _plugin_cli_discovery_needed():
        try:
            ...
            from quill_cli.plugins import discover_plugins, get_plugin_manager
    ...
    try:
        from quill_cli.curator import register_cli as _register_curator_cli
    ...
    def cmd_memory(args):
        ...
        if sub == "off":
            ...
            mem_dir = get_quill_home() / "memories"
            ...
            if not existing:
                print(
                    f"\n  Nothing to reset — no memory files found in {display_quill_home()}/memories/\n"
                )
                return
            ...
            print(f"  Files were in: {display_quill_home()}/memories/\n")
        else:
            from quill_cli.memory_setup import memory_command
    ...
    tools_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Run 'quill tools' with no subcommand for the interactive configuration UI."
        ),
    )
    ...
    # quill tools disable <name...> [--platform cli]
    ...
    # quill tools enable <name...> [--platform cli]
    ...
    def cmd_tools(args):
        ...
        if action in {"list", "disable", "enable"}:
            from quill_cli.tools_config import tools_disable_enable_command

            tools_disable_enable_command(args)
        else:
            _require_tty("tools")
            from quill_cli.tools_config import tools_command
    ...
    computer_use_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Use `quill computer-use install` to fetch and run the\n"
            "upstream cua-driver installer. This is equivalent to the\n"
            "post-setup hook that `quill tools` runs when you first\n"
    ...
    def cmd_computer_use(args):
        ...
        if action == "status":
            ...
            if path:
                ...
                print("  Refresh to latest: quill computer-use install --upgrade")
                return
            print("cua-driver: not installed")
            print("  Run: quill computer-use install")
            return
    ...
    mcp_parser = subparsers.add_parser(
        "mcp",
        help="Manage MCP servers and run Quill as an MCP server",
        description=(
            "Manage MCP server connections and run Quill as an MCP server.\n\n"
            "MCP servers provide additional tools via the Model Context Protocol.\n"
            "Use 'quill mcp add' to connect to a new server, or\n"
            "'quill mcp serve' to expose Quill conversations over MCP."
        ),
    )
    ...
    mcp_serve_p = mcp_sub.add_parser(
        "serve",
        help="Run Quill as an MCP server (expose conversations to other agents)",
    )
    ...
    # flag is omitted, causing `quill mcp add ...` to fall through to
    # interactive chat.
    ...
    def cmd_mcp(args):
        from quill_cli.mcp_config import mcp_command
        ...
        if action == "list":
            ...
            if not args.yes:
                if not _confirm_prompt(
                    ...
                    print("Cancelled.")
                    return
            sessions_dir = get_quill_home() / "sessions"
            ...
            # Launch quill --resume <id> by replacing the current process
    ...
    claw_parser = subparsers.add_parser(
        ...
        description="Migrate settings, memories, skills, and API keys from OpenClaw to Quill",
    )
    ...
    claw_migrate = claw_subparsers.add_parser(
        "migrate",
        help="Migrate from OpenClaw to Quill",
    ...
    claw_migrate.add_argument(
        "--no-backup",
        action="store_true",
        help="Skip the pre-migration zip snapshot of ~/.quill/ (by default a "
        "single restore-point archive is written to ~/.quill/backups/ "
        "before apply; restorable with 'quill import').",
    )
    claw_migrate.add_argument(
    ...
    def cmd_claw(args):
        from quill_cli.claw import claw_command
    ...
    update_parser = subparsers.add_parser(
        "update",
        help="Update Quill Agent to the latest version",
    ...
    update_parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Assume yes for interactive prompts (config migration, stash restore). API-key entry is skipped; run 'quill config migrate' separately for those.",
    )
    ...
    uninstall_parser = subparsers.add_parser(
        "uninstall",
        help="Uninstall Quill Agent",
        description="Remove Quill Agent from your system. Can keep configs/data for reinstall.",
    )
    ...
    acp_parser = subparsers.add_parser(
        "acp",
        help="Run Quill Agent as an ACP (Agent Client Protocol) server",
        description="Start Quill Agent in ACP mode for editor integration (VS Code, Zed, JetBrains)",
    )
    ...
    acp_parser.add_argument(
        ...
        action="store_true",
        dest="acp_version",
        help="Print Quill ACP version and exit",
    )
    acp_parser.add_argument(
        "--check",
        action="store_true",
    ...
    acp_parser.add_argument(
        "--setup",
        action="store_true",
        help="Run interactive Quill provider/model setup for ACP terminal auth",
    )
    acp_parser.add_argument(
        "--setup-browser",
        action="store_true",
        help="Install agent-browser + Playwright Chromium into ~/.quill/node/ "
    ...
    def cmd_acp(args):
        """Launch Quill Agent as an ACP server."""
        try:
    ...
    profile_parser = subparsers.add_parser(
        "profile",
        help="Manage profiles — multiple isolated Quill instances",
    )
    ...
    profile_create.add_argument(
        "--no-skills",
        action="store_true",
        help="Create an empty profile with no bundled skills (opts out of `quill update` skill sync)",
    )
    profile_create.add_argument(
        "--description",
        default=None,
        help="One- or two-sentence description of what this profile is good at. "
             ...
             "of profile name alone. Skip and add later via `quill profile describe`.",
    )
    ...
    profile_install = profile_subparsers.add_parser(
        ...
        description=(
            "Install a Quill profile distribution. SOURCE can be a git URL "
    ...
    dashboard_parser = subparsers.add_parser(
        ...
        description="Launch the Quill Agent web dashboard for managing config, API keys, and sessions",
    )
    ...
    dashboard_parser.add_argument(
        ...
        action="store_true",
        help=(
            "Expose the in-browser Chat tab (embedded `quill --tui` via PTY/WebSocket). "
            "Alternatively set QUILL_DASHBOARD_TUI=1."
        ),
    )
    dashboard_parser.add_argument(
        "--skip-build",
    ...
    # for `quill dashboard` cmdlines and SIGTERM them directly — the same
    # path `quill update` uses to clean up stale dashboards.
    dashboard_parser.add_argument(
        "--stop",
        action="store_true",
        help="Stop all running quill dashboard processes and exit",
    )
    dashboard_parser.add_argument(
        "--status",
        action="store_true",
        help="List running quill dashboard processes and exit",
    )
    ...
    logs_parser = subparsers.add_parser(
        "logs",
        help="View and filter Quill log files",
        description="View, tail, and filter agent.log / errors.log / gateway.log",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    quill logs                    Show last 50 lines of agent.log
    quill logs -f                 Follow agent.log in real time
    quill logs errors             Show last 50 lines of errors.log
    quill logs gateway -n 100     Show last 100 lines of gateway.log
    quill logs --level WARNING    Only show WARNING and above
    quill logs --session abc123   Filter by session ID
    quill logs --component tools  Only show tool-related lines
    quill logs --since 1h         Lines from the last hour
    quill logs --since 30m -f     Follow, starting from 30 min ago
    quill logs list               List available log files with sizes
""",
    )
    ...
    # e.g. ``quill -c Pokemon Agent Dev`` → ``quill -c 'Pokemon Agent Dev'``
    ...
    from quill_cli.config import get_container_exec_info
    ...
    # fails (e.g. 'quill -c model' where 'model' is consumed as the
    ...
    # so introspection/management commands (quill hooks list, cron
    ...
    if args.command in _AGENT_COMMANDS or (
        ...
        try:
            from quill_cli.plugins import discover_plugins
    ...
    if getattr(args, "oneshot", None):
        from quill_cli.oneshot import run_oneshot
