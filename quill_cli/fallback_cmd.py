"""
quill fallback — manage the fallback provider chain.
...
Subcommands:
  quill fallback [list]   Show the current fallback chain (default when no subcommand)
  quill fallback add      Pick provider + model via the same picker as `quill model`,
                           then append the selection to the chain
  quill fallback remove   Pick an entry to delete from the chain
  quill fallback clear    Remove all fallback entries

Storage: ``fallback_providers`` in ``~/.quill/config.yaml`` (top-level, list of
...
def _restore_auth_active_provider(value: Any) -> None:
    ...
    try:
        from quill_cli.auth import _auth_store_lock, _load_auth_store, _save_auth_store
        ...
        # `quill model` to fix it.  Don't fail the fallback add.
        pass
...
def cmd_fallback_list(args) -> None:  # noqa: ARG001
    ...
    if not chain:
        ...
        print()
        print("  Add one with:  quill fallback add")
        print()
        return
    ...
    print("  Docs: https://quill-agent.example.com/docs/user-guide/features/fallback-providers")
    print()
...
def cmd_fallback_add(args) -> None:
    """Launch the same picker as `quill model`, then append the selection to the chain."""
    from quill_cli.main import _require_tty, select_provider_and_model
    ...
    print("  `quill model` — select the provider + model you want as a fallback.")
    print()
    ...
    print()
    print("  Run `quill fallback list` to view, or `quill fallback remove` to delete.")
...
def cmd_fallback(args) -> None:
    """Top-level dispatcher for ``quill fallback [subcommand]``."""
