"""Slash command definitions and autocomplete for the Quill CLI.
...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("rollback", "List or restore filesystem checkpoints", "Session",
               args_hint="[number]"),
    CommandDef("snapshot", "Create or restore state snapshots of Quill config/state", "Session",
    ...
    CommandDef("steer", "Inject a message after the next tool call without interrupting", "Session",
               args_hint="<prompt>"),
    CommandDef("goal", "Set a standing goal Quill works on across turns until achieved", "Session",
    ...
    CommandDef("busy", "Control what Enter does while Quill is working", "Configuration",
    ...
    CommandDef("reload-mcp", "Reload MCP servers from config", "Tools & Skills",
               aliases=("reload_mcp",)),
    CommandDef("reload-skills", "Re-scan ~/.quill/skills/ for newly installed or removed skills",
    ...
    CommandDef("update", "Update Quill Agent to the latest version", "Info"),
...
def _iter_plugin_command_entries() -> list[tuple[str, str, str]]:
    ...
    :func:`quill_cli.plugins.PluginContext.register_command`. They behave
    like ``CommandDef`` entries for gateway surfacing: they appear in the
    Telegram command menu, in Slack's ``/quill`` subcommand mapping, and
...
def _collect_gateway_skill_entries(
    ...
    try:
        ...
        # ``quill skills list`` and the agent's ``/skill-name`` dispatch but
...
def telegram_menu_commands(max_commands: int = 100) -> tuple[list[tuple[str, str]], int]:
    ...
    Skills disabled for the ``"telegram"`` platform (via ``quill skills
...
def discord_skill_commands_by_category(
    ...
    *uncategorized*.
    ...
    parity, external-dir skills are visible via ``quill skills list`` and
...
def slack_native_slashes() -> list[tuple[str, str, str]]:
    ...
    first-class slash and not a ``/quill <verb>`` subcommand.
    ...
    can still reach them via ``/quill <command>``.

    Results are clamped to Slack's 50-command limit with duplicate-name
    avoidance. ``/quill`` is always reserved as the first entry so the
    legacy ``/quill <subcommand>`` form keeps working for anything that
    ...
    # Reserve /quill as the catch-all top-level command.
    entries.append(("quill", "Talk to Quill or run a subcommand", "[subcommand] [args]"))
    seen.add("quill")
...
def slack_app_manifest(request_url: str = "https://quill-agent.local/slack/commands") -> dict[str, Any]:
...
def slack_subcommand_map() -> dict[str, str]:
    """Return subcommand -> /command mapping for Slack /quill handler.

    Maps both canonical names and aliases so /quill bg do stuff works
    the same as /quill background do stuff.

    Plugin-registered slash commands are included so ``/quill <plugin-cmd>``
...
class SlashCommandCompleter(Completer):
    ...
    def _skin_completions(sub_text: str, sub_lower: str):
        ...
        try:
            from quill_cli.skin_engine import list_skins
            for s in list_skins():
                name = s["name"]
