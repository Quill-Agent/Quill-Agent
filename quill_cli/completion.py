"""Shell completion script generation for quill CLI.
...
def generate_bash(parser: argparse.ArgumentParser) -> str:
    ...
    for cmd in sorted(tree["subcommands"]):
        ...
        if cmd == "profile" and info["subcommands"]:
            ...
            cases.append(
                ...
                f"                    COMPREPLY=($(compgen -W \"$(_quill_profiles)\" -- \"$cur\"))\n"
                f"                    return\n"
                f"                    ;;\n"
                f"            esac\n"
                f"            ;;"
    ...
    return f"""# Quill Agent bash completion
# Add to ~/.bashrc:
#   eval "$(quill completion bash)"

_quill_profiles() {{
    local profiles_dir="$HOME/.quill/profiles"
...
_quill_completion() {{
    ...
    if [[ "$prev" == "-p" || "$prev" == "--profile" ]]; then
        COMPREPLY=($(compgen -W "$(_quill_profiles)" -- "$cur"))
        return
    fi
...
def generate_zsh(parser: argparse.ArgumentParser) -> str:
    ...
    return f"""#compdef quill
# Quill Agent zsh completion
# Add to ~/.zshrc:
...
_quill_profiles() {{
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.quill/profiles" ]]; then
        profiles+=("${{(@f)$(ls $HOME/.quill/profiles 2>/dev/null)}}")
    fi
...
_quill() {{
    ...
    _arguments -C \\
        ...
        '(-)'{{-p,--profile}}'[Profile name]:profile:_quill_profiles' \\
        '1:command:->commands' \\
        '*::arg:->args'
...
{top_cmds_str}
            ...
            _describe 'quill command' subcmds
            ;;
        args)
            case ${{line[1]}} in
{sub_cases_str}
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        "# Quill Agent fish completion",
        "# Add to your config:",
        "#   quill completion fish | source",
        "",
        "# Helper: list available profiles",
        "function __quill_profiles",
        "    echo default",
        "    if test -d $HOME/.quill/profiles",
        "        ls $HOME/.quill/profiles 2>/dev/null",
        "    end",
        "end",
        "",
        ...
        "",
        "# Complete profile names after -p / --profile",
        "complete -c quill -f -s p -l profile"
        " -d 'Profile name' -xa '(__quill_profiles)'",
        "",
        ...
        if cmd == "profile":
            for action in sorted(profile_name_actions):
                lines.append(
                    ...
                    f"-a '(__quill_profiles)' -d 'Profile name'"
                )
