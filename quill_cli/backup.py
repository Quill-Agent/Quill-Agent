"""
Backup and import commands for quill CLI.

`quill backup` creates a zip archive of the entire ~/.quill/ directory
(excluding the quill-agent repo and transient files).

`quill import` restores from a backup zip, overlaying onto the current
QUILL_HOME root.
"""
...
from quill_constants import get_default_quill_root, get_quill_home, display_quill_home
...
_EXCLUDED_DIRS = {
    "quill-agent",     # the codebase repo — re-clone instead
...
def _should_exclude(rel_path: Path) -> bool:
    """Return True if *rel_path* (relative to quill root) should be skipped."""
...
def run_backup(args) -> None:
    """Create a zip backup of the Quill home directory."""
    quill_root = get_default_quill_root()

    if not quill_root.is_dir():
        print(f"Error: Quill home directory not found at {quill_root}")
        sys.exit(1)
    ...
    if args.output:
        ...
        if out_path.is_dir():
            ...
            out_path = out_path / f"quill-backup-{stamp}.zip"
    else:
        stamp = datetime.now().strftime("%Y-%m-%d-%H%M%S")
        out_path = Path.home() / f"quill-backup-{stamp}.zip"
    ...
    # Collect files
    print(f"Scanning {display_quill_home()} ...")
    files_to_add: list[tuple[Path, Path]] = []  # (absolute, relative)
    skipped_dirs = set()

    for dirpath, dirnames, filenames in os.walk(quill_root, followlinks=False):
        dp = Path(dirpath)
        rel_dir = dp.relative_to(quill_root)
        ...
        for fname in filenames:
            fpath = dp / fname
            rel = fpath.relative_to(quill_root)

            if _should_exclude(rel):
                continue

            # Skip the output zip itself if it happens to be inside quill root
            try:
    ...
    print(f"\nRestore with: quill import {out_path.name}")
...
def _validate_backup_zip(zf: zipfile.ZipFile) -> tuple[bool, str]:
    """Check that a zip looks like a Quill backup.
    ...
    # Look for telltale files that a quill home would have
    ...
    if not found:
        return False, (
            "zip does not appear to be a Quill backup "
...
def _detect_prefix(zf: zipfile.ZipFile) -> str:
    ...
    Some tools zip as `.quill/config.yaml` instead of `config.yaml`.
    ...
    if len(first_parts) == 1:
        ...
        # Only strip if it looks like a quill dir name
        if prefix in {".quill", "quill"}:
            return prefix + "/"
...
def run_import(args) -> None:
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        print(f"Target: {display_quill_home()}")

        if prefix:
            print(f"Detected archive prefix: {prefix!r} (will be stripped)")

        # Check for existing installation
        has_config = (quill_root / "config.yaml").exists()
        has_env = (quill_root / ".env").exists()

        if (has_config or has_env) and not args.force:
            print()
            print("Warning: Target directory already has Quill configuration.")
        ...
        for member in members:
            ...
            target = quill_root / rel

            # Security: reject absolute paths and traversals
            try:
                target.resolve().relative_to(quill_root.resolve())
            except ValueError:
        ...
        profiles_dir = quill_root / "profiles"
        restored_profiles = []
        if profiles_dir.is_dir():
            try:
                ...
                # quill_cli.profiles might not be available (fresh install)
                if any(profiles_dir.iterdir()):
                    print(f"\n  Profiles detected but aliases could not be created.")
                    print(f"  Run: quill profile list  (after installing quill)")

        # Guidance
        print()
        if not (quill_root / "quill-agent").is_dir():
            print("Note: The quill-agent codebase was not included in the backup.")
            print("  If this is a fresh install, run: quill update")

        if restored_profiles:
            gw_profiles = [n for n, _ in restored_profiles]
            print("\nTo re-enable gateway services for profiles:")
            for pname in gw_profiles:
                print(f"  quill -p {pname} gateway install")

        print("Done. Your Quill configuration has been restored.")


# ---------------------------------------------------------------------------
# Quick state snapshots (used by /snapshot slash command and quill backup --quick)
# ---------------------------------------------------------------------------

# Critical state files to include in quick snapshots (relative to QUILL_HOME).
...
# — `quill update` snapshots this set before pulling so approved-user lists
...
def _quick_snapshot_root(quill_home: Optional[Path] = None) -> Path:
    home = quill_home or get_quill_home()
    return home / _QUICK_SNAPSHOTS_DIR


def create_quick_snapshot(
    label: Optional[str] = None,
    quill_home: Optional[Path] = None,
) -> Optional[str]:
...
def list_quick_snapshots(
    ...
    root = _quick_snapshot_root(quill_home)
    if not root.exists():
        return []
...
def prune_quick_snapshots(
    ...
    return _prune_quick_snapshots(_quick_snapshot_root(quill_home), keep=keep)


def run_quick_backup(args) -> None:
    """CLI entry point for quill backup --quick."""
    label = getattr(args, "label", None)
    snap_id = create_quick_snapshot(label=label)
    if snap_id:
        print(f"State snapshot created: {snap_id}")
        snaps = list_quick_snapshots()
        print(f"  {len(snaps)} snapshot(s) stored in {display_quill_home()}/state-snapshots/")
...
def _write_full_zip_backup(out_path: Path, quill_root: Path) -> Optional[Path]:
    """Write a full zip snapshot of ``quill_root`` to ``out_path``.
...
def _pre_update_backup_dir(quill_home: Optional[Path] = None) -> Path:
    home = quill_home or get_quill_home()
...
def create_pre_update_backup(
    ...
    """Create a full zip backup of QUILL_HOME under ``backups/``.

    Mirrors :func:`run_backup` (same exclusion rules, same SQLite safe-copy)
    but writes to ``<QUILL_HOME>/backups/pre-update-<timestamp>.zip`` and
    auto-prunes old pre-update backups.

    Returns the path to the created zip, or ``None`` if no files were
    found or the backup could not be created.  Never raises — the caller
    (``quill update``) should continue even if the backup fails.
    """
    quill_root = quill_home or get_default_quill_root()
    if not quill_root.is_dir():
        return None

    backup_dir = _pre_update_backup_dir(quill_root)
    try:
    ...
    result = _write_full_zip_backup(out_path, quill_root)
    if result is None:
        return None
...
# ---------------------------------------------------------------------------
# Pre-migration auto-backup (used by `quill claw migrate`)
# ---------------------------------------------------------------------------
...
def create_pre_migration_backup(
    ...
    """Create a full zip backup of QUILL_HOME under ``backups/`` before a
    ``quill claw migrate`` apply.

    Shares implementation with :func:`create_pre_update_backup` via
    ``_write_full_zip_backup`` — same exclusions, same SQLite safe-copy,
    restorable with ``quill import <archive>``.  Writes to
    ``<QUILL_HOME>/backups/pre-migration-<timestamp>.zip`` and auto-prunes
    ...
    # Reuses the shared backups/ directory so `quill import` and the
