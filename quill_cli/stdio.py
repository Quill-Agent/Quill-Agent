...
on Japanese locales, etc.).  Quill's banners, tool output feed, and slash
...
Entry points (``cli.py`` ``main``, ``quill_cli/main.py`` CLI dispatch,
...
def _flip_console_code_page_to_utf8() -> None:
    ...
    is silent — if there's no attached console (e.g. Quill is running
...
def configure_windows_stdio() -> bool:
    ...
    Set ``QUILL_DISABLE_WINDOWS_UTF8=1`` in the environment to opt out
    ...
    if os.environ.get("QUILL_DISABLE_WINDOWS_UTF8") in {"1", "true", "True", "yes"}:
        _CONFIGURED = True
        return False
    ...
    # Augment PATH with the Quill-managed Git install directories so
    ...
    # already-running shells don't see the change — which means quill
    ...
    # install without Quill-managed PortableGit).
    ...
    # stdin is re-configured for completeness; Quill's interactive
...
def _default_windows_editor() -> str:
    ...
    The prompt_toolkit buffer's ``open_in_editor`` and Quill's
    ``quill config edit`` both honour ``$EDITOR``.  Users who prefer a
    ...
    Set this before launching Quill (User env var in Windows Settings, or
    export in a PowerShell profile) and Quill picks it up automatically.
    """
    import shutil
...
def _augment_path_with_known_tools() -> None:
    """Prepend well-known Quill-managed tool directories to os.environ['PATH'].
    ...
    ``%LOCALAPPDATA%\\quill\\git\\bin`` to the User PATH via
    ...
    *spawned* processes only — already-running shells (including the one the
    user invokes ``quill`` from right after install) retain their old PATH.

    Any subprocess Quill spawns — bash, ``rg``, ``grep``, ``npm`` — inherits
    ...
    Patch-up strategy: add the known Quill-managed tool directories to our
    ...
    # see on next launch.
    candidate_dirs = [
        os.path.join(local_appdata, "quill", "git", "cmd"),
        os.path.join(local_appdata, "quill", "git", "bin"),
        os.path.join(local_appdata, "quill", "git", "usr", "bin"),
        # Quill venv Scripts directory — host of the quill.exe shim itself,
        # also where any pip-installed console scripts land.  Usually already
        # on PATH when the user invokes quill, but harmless to include.
        os.path.join(local_appdata, "quill", "quill-agent", "venv", "Scripts"),
