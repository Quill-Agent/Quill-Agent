"""CLI subcommand: ``quill send`` — pipe text from shell scripts to any
...
def _read_message_body(
    ...
    if file_path:
        ...
        try:
            ...
            print(f"quill send: cannot read {file_path}: {exc}", file=sys.stderr)
    ...
    *,
    ...
    if json_mode:
        ...
        if payload.get("error"):
            print(f"quill send: {payload['error']}", file=sys.stderr)
...
def _list_targets(platform_filter: Optional[str], *, json_mode: bool) -> int:
    ...
    try:
        ...
        print(f"quill send: failed to load channel directory: {exc}", file=sys.stderr)
        return _FAILURE_EXIT

    try:
        raw = load_directory()
    except Exception as exc:
        print(f"quill send: failed to read channel directory: {exc}", file=sys.stderr)
        return _FAILURE_EXIT
    ...
    if platform_filter:
        ...
        if not filtered:
            print(
                f"quill send: no targets found for platform '{platform_filter}'. "
    ...
    if not any(platforms.values()):
        ...
        print("Set one up with `quill gateway setup`, or run the gateway once so")
        print("channel discovery can populate ~/.quill/channel_directory.json.")
        return _SUCCESS_EXIT
...
def _load_quill_env() -> None:
    """Populate ``os.environ`` from ``~/.quill/.env`` AND bridge top-level
    ...
    startup that ``quill send`` must replicate when invoked standalone:

    1. ``load_dotenv(~/.quill/.env)`` — brings bot tokens into the env.
    2. Bridge top-level simple values from ``~/.quill/config.yaml`` into
       ...
       via ``quill config set``.

    See ``gateway/run.py`` for the canonical version of this bridge — we
    intentionally reimplement the minimum needed here so ``quill send``
...
def cmd_send(args: argparse.Namespace) -> None:
    ...
    # Bridge ~/.quill/.env and ~/.quill/config.yaml into os.environ so the
    ...
    if not target:
        print(
            "quill send: --to PLATFORM[:channel[:thread]] is required\n"
            "Examples:\n"
            "  quill send --to telegram \"hello\"\n"
            "  quill send --to discord:#ops --file report.md\n"
            "  quill send --list      # list available targets",
            file=sys.stderr,
        )
        sys.exit(_USAGE_EXIT)
    ...
    if message is None or not message.strip():
        print(
            "quill send: no message provided. Pass text as a positional "
    ...
    # Import lazily so `quill send --help` stays fast and does not pull in
...
def register_send_subparser(subparsers) -> argparse.ArgumentParser:
    ...
    parser = subparsers.add_parser(
        ...
        description=(
            "Pipe text from any shell script to any messaging platform Quill "
            "is already configured for. Reuses the gateway's platform "
            "credentials (~/.quill/.env + ~/.quill/config.yaml) — no LLM, "
        ...
        epilog=(
            ...
            "  quill send --to slack:#eng --subject \"[CI]\" --file build.log\n"
            "  quill send --list                  # all platforms\n"
            "  quill send --list telegram         # filter by platform\n"
            "\n"
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="List available targets. Optional positional filter: `quill send --list telegram`.",
    )
