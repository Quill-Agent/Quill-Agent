"""
Quill Agent Uninstaller.

Provides options for:
- Full uninstall: Remove everything including configs and data
- Keep data: Remove code but keep ~/.quill/ (configs, sessions, logs)
"""
...
def remove_path_from_shell_configs():
    """Remove Quill PATH entries from shell configuration files."""
    ...
    for config_path in configs:
        try:
            ...
            # Remove lines containing quill-agent or quill PATH entries
            new_lines = []
            skip_next = False
            
            for line in content.split('\n'):
                # Skip the "# Quill Agent" comment and following line
                if '# Quill Agent' in line or '# quill-agent' in line:
                    skip_next = True
                    continue
                if skip_next and ('quill' in line.lower() and 'PATH' in line):
                    skip_next = False
                    continue
                skip_next = False
                
                # Remove any PATH line containing quill
                if 'quill' in line.lower() and ('PATH=' in line or 'path=' in line.lower()):
                    continue
...
def remove_wrapper_script():
    """Remove the quill wrapper script if it exists."""
    wrapper_paths = [
        Path.home() / ".local" / "bin" / "quill",
        Path("/usr/local/bin/quill"),
    ]
    ...
    for wrapper in wrapper_paths:
        if wrapper.exists():
            try:
                # Check if it's our wrapper (contains quill_cli reference)
                content = wrapper.read_text()
                if 'quill_cli' in content or 'quill-agent' in content:
                    wrapper.unlink()
...
def uninstall_gateway_service():
    ...
    - All platforms: standalone ``quill gateway run`` processes
    ...
    try:
        from quill_cli.gateway import kill_gateway_processes, find_gateway_pids
    ...
    # 3. macOS: uninstall launchd plist
        ...
        try:
            from quill_cli.gateway import get_launchd_plist_path
...
#
#   1. Sets User-scope env vars ``QUILL_HOME`` and ``QUILL_GIT_BASH_PATH``
...
#      like ``%LOCALAPPDATA%\quill\git\cmd``, ``%LOCALAPPDATA%\quill\git\bin``,
#      ``%LOCALAPPDATA%\quill\git\usr\bin``, ``%LOCALAPPDATA%\quill\node``.
#      Again not in any rc file — only accessible via the registry or the
#      .NET [Environment] API.
#   3. Downloads PortableGit to ``%LOCALAPPDATA%\quill\git\`` and Node to
#      ``%LOCALAPPDATA%\quill\node\`` as user-scoped, isolated copies.
#      These are ~200MB combined and serve no purpose after uninstall.
#   4. On the ``quill dashboard`` + gateway paths, drops files into
#      ``%LOCALAPPDATA%\quill\gateway-service\`` and sometimes
...
def _quill_path_markers(quill_home: Path) -> list[str]:
    """Path-entry substrings that identify Quill-owned User-PATH entries."""
    root = str(quill_home).rstrip("\\/")
    # Match on prefix so sub-entries (git\cmd, git\bin, git\usr\bin, node, etc.)
    # all get swept.  Also match the bare quill-agent install dir.
    markers = [root + "\\quill-agent", root + "\\git", root + "\\node", root + "\\venv"]
    # Also match if QUILL_HOME was customised to somewhere else — find-and-nuke
    # any entry whose path component contains "quill".  We don't want to catch
    # unrelated entries like "cquill-foo" or "ephermeral", so we look for
    # backslash-quill as a word-ish boundary.
    return markers


def remove_path_from_windows_registry(quill_home: Path) -> list[str]:
    """Strip Quill-owned entries from User-scope PATH in the registry.
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, key_path, 0,
            ...
            markers = _quill_path_markers(quill_home)
            kept: list[str] = []
...
def remove_quill_env_vars_windows() -> list[str]:
    """Delete QUILL_HOME and QUILL_GIT_BASH_PATH from User-scope env vars."""
    try:
        import winreg
    except ImportError:
        return []
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, "Environment", 0,
                            winreg.KEY_READ | winreg.KEY_WRITE) as key:
            for name in ("QUILL_HOME", "QUILL_GIT_BASH_PATH"):
                try:
...
def _is_default_quill_home(quill_home: Path) -> bool:
    """Return True when ``quill_home`` points at the default (non-profile) root."""
    try:
        from quill_constants import get_default_quill_root
        return quill_home.resolve() == get_default_quill_root().resolve()
    except Exception:
        return False
...
def _uninstall_profile(profile) -> None:
    ...
    remove its alias wrapper, and wipe its QUILL_HOME directory.

    We shell out to ``quill -p <name> gateway stop|uninstall`` because
    service names, unit paths, and plist paths are all derived from the
    current QUILL_HOME and can't be easily switched in-process.
    """
    import sys as _sys
    name = profile.name
    ...
    #    Use `python -m quill_cli.main` so we don't depend on a `quill`
    #    wrapper that may be half-removed mid-uninstall.
    quill_invocation = [_sys.executable, "-m", "quill_cli.main", "--profile", name]
    for subcmd in ("stop", "uninstall"):
        try:
            subprocess.run(
                quill_invocation + ["gateway", subcmd],
                capture_output=True,
                text=True,
                timeout=60,
                check=False,
    ...
    # 3. Wipe the profile's QUILL_HOME directory.
    try:
...
def run_uninstall(args):
    ...
    Options:
    - Full uninstall: removes code + ~/.quill/ (configs, data, logs)
    - Keep data: removes code but keeps ~/.quill/ for future reinstall
    """
    project_root = get_project_root()
    quill_home = get_quill_home()

    # Detect named profiles when uninstalling from the default root —
    # offer to clean them up too instead of leaving zombie QUILL_HOMEs
    # and systemd units behind.
    is_default_profile = _is_default_quill_home(quill_home)
    named_profiles = _discover_named_profiles() if is_default_profile else []

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│            ⚕ Quill Agent Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()
    ...
    print(f"  Config:  {quill_home / 'config.yaml'}")
    print(f"  Secrets: {quill_home / '.env'}")
    print(f"  Data:    {quill_home / 'cron/'}, {quill_home / 'sessions/'}, {quill_home / 'logs/'}")
    print()
    ...
    # their alias wrappers, and wiping their QUILL_HOME dirs. Otherwise
    ...
    print()
    if full_uninstall:
        print(color("⚠️  WARNING: This will permanently delete ALL Quill data!", Colors.RED, Colors.BOLD))
        ...
        print("This will remove the Quill code but keep your configuration and data.")
    ...
    if _is_windows():
        ...
        # Expand %LOCALAPPDATA% etc. in quill_home so the marker matching is
        # against fully resolved paths — installer writes literal strings
        # like C:\Users\<u>\AppData\Local\quill\git\cmd, not %LOCALAPPDATA%.
        removed_path_entries = remove_path_from_windows_registry(Path(os.path.expandvars(str(quill_home))))
        if removed_path_entries:
            for entry in removed_path_entries:
                log_success(f"Removed from User PATH: {entry}")
        else:
            log_info("No Quill-owned PATH entries in User environment")

        log_info("Removing QUILL_HOME / QUILL_GIT_BASH_PATH User env vars...")
        removed_env = remove_quill_env_vars_windows()
        if removed_env:
            for name in removed_env:
                log_success(f"Removed User env var: {name}")
        else:
            log_info("No Quill-set User env vars to remove")
    
    # 3. Remove wrapper script
    log_info("Removing quill command...")
    ...
    try:
        if project_root.exists():
            # If the install is inside ~/.quill/, just remove the quill-agent subdir
            if quill_home in project_root.parents or project_root.parent == quill_home:
    ...
    #     under QUILL_HOME but they're install tooling, not config — safe to
    #     remove even in "keep data" mode.  If we're doing a full uninstall
    #     the step-5 rmtree(quill_home) would sweep them anyway; calling
    ...
    if _is_windows():
        ...
        removed_artifacts = remove_portable_tooling_windows(quill_home)
    ...
    # 5. Optionally remove ~/.quill/ data directory (and named profiles)
    if full_uninstall:
        # 5a. Stop and remove each named profile's gateway service and
        #     alias wrapper. The profile QUILL_HOME dirs live under
        ...
        try:
            if quill_home.exists():
                shutil.rmtree(quill_home)
                log_success(f"Removed {quill_home}")
        except Exception as e:
            log_warn(f"Could not fully remove {quill_home}: {e}")
            log_info("You may need to manually remove it")
    else:
        log_info(f"Keeping configuration and data in {quill_home}")
    ...
    if not full_uninstall:
        ...
        if _is_windows():
            print(color("  iex (irm https://raw.githubusercontent.com/YOUR_USERNAME/quill-agent/main/scripts/install.ps1)", Colors.DIM))
        else:
            print(color("  curl -fsSL https://raw.githubusercontent.com/YOUR_USERNAME/quill-agent/main/scripts/install.sh | bash", Colors.DIM))
        print()
