"""``quill logs`` — view and filter Quill log files.
...
Usage examples::
    ...
    quill logs                    # last 50 lines of agent.log
    quill logs -f                 # follow agent.log in real time
    quill logs errors             # last 50 lines of errors.log
    quill logs gateway -n 100    # last 100 lines of gateway.log
    quill logs --level WARNING    # only WARNING+ lines
    quill logs --session abc123   # filter by session ID substring
    quill logs --component tools  # only tool-related lines
    quill logs --since 1h         # lines from the last hour
    quill logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    log_path = get_quill_home() / "logs" / filename
    if not log_path.exists():
        print(f"Log file not found: {log_path}")
        print(f"(Logs are created when Quill runs — try 'quill chat' first)")
        sys.exit(1)
    ...
    if follow:
        print(f"--- {display_quill_home()}/logs/{filename}{filter_desc} (Ctrl+C to stop) ---")
    else:
        print(f"--- {display_quill_home()}/logs/{filename}{filter_desc} (last {num_lines}) ---")
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"No logs directory at {display_quill_home()}/logs/")
        return

    print(f"Log files in {display_quill_home()}/logs/:\n")
    found = False
