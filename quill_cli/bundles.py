"""Implementation of the ``quill bundles`` CLI subcommand.

Mirrors the structure of ``quill_cli/skills_hub.py`` but for skill
...
def _console() -> Console:
    # Bind to stderr so piping `quill bundles list | grep …` doesn't
...
def _cmd_list(args) -> None:
    ...
    if not bundles:
        c.print(
            ...
            f"  quill bundles create <name> --skill skill1 --skill skill2[/]\n"
...
def register_cli(subparser) -> None:
    """Build the ``quill bundles`` argparse tree.

    Called from ``quill_cli/main.py`` where it owns the top-level
    ...
    p_create.add_argument(
        ...
        help="Human-readable description shown in /help and `quill bundles list`",
    )
    p_create.add_argument(
...
def bundles_command(args) -> None:
    """Dispatch ``quill bundles <subcommand>`` to the right handler."""
