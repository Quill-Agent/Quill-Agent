...
Used by ``quill profile describe <name> --auto`` and the dashboard's
...
Design notes
------------
- Mirrors the shape of ``quill_cli/kanban_specify.py``: lazy aux
...
_SYSTEM_PROMPT = """You are a profile-describer for the Quill Agent kanban board.
...
Rules:
  ...
  - Never write "Quill Agent profile" or other meta-narration.
    ...
    *,
    ...
    try:
        if canon == "default":
            ...
            profile_dir = Path(get_quill_home())
        else:
