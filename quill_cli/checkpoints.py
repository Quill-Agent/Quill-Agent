"""`quill checkpoints` CLI subcommand.

Gives users direct visibility and control over the filesystem checkpoint
store at ``~/.quill/checkpoints/``.  Actions:

    quill checkpoints               # same as `status`
    quill checkpoints status        # total size, project count, breakdown
    quill checkpoints list          # per-project checkpoint counts + workdir
    quill checkpoints prune [opts]  # force a sweep (ignores the 24h marker)
    quill checkpoints clear [-f]    # nuke the entire base (asks first)
    quill checkpoints clear-legacy  # delete just the legacy-* archives

Examples::

    quill checkpoints
    quill checkpoints prune --retention-days 3 --max-size-mb 200
    quill checkpoints clear -f
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if legacy:
        ...
        print()
        print("Clear with: quill checkpoints clear-legacy")
    return 0
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire subcommands onto the ``quill checkpoints`` parser."""
    parser.set_defaults(func=cmd_status)  # bare `quill checkpoints` → status
