"""Helpers for loading Quill .env files consistently across entrypoints."""
...
def _sanitize_loaded_credentials() -> None:
    ...
    for key, value in list(os.environ.items()):
        ...
        print(
            ...
            "  provider's dashboard and run `quill setup` (or edit the "
...
def _sanitize_env_file_if_needed(path: Path) -> None:
    ...
    We delegate to ``quill_cli.config._sanitize_env_lines`` which
    already knows all valid Quill env-var names and can split
    concatenated lines correctly.
    """
    if not path.exists():
        return
    try:
        from quill_cli.config import _sanitize_env_lines
    except ImportError:
...
def load_quill_dotenv(
    *,
    quill_home: str | os.PathLike | None = None,
    project_env: str | os.PathLike | None = None,
) -> list[Path]:
    """Load Quill environment files with user config taking precedence.

    Behavior:
    - `~/.quill/.env` overrides stale shell-exported values when present.
    ...
    home_path = Path(quill_home or os.getenv("QUILL_HOME", Path.home() / ".quill"))
