...
# xAI's public API is OpenAI-compatible for the endpoints Quill commonly
# uses. The Responses endpoint is included because Quill' native xAI runtime
...
class XAIGrokAdapter(UpstreamAdapter):
    """Proxy upstream for xAI Grok via Quill-managed OAuth credentials."""

    auth_hint = "quill auth add xai-oauth --type oauth"
        ...
        with self._lock:
            ...
            if pool is None or not pool.has_credentials():
                raise RuntimeError(
                    ...
                    "`quill auth add xai-oauth --type oauth` first."
                )
            ...
            if entry is None:
                raise RuntimeError(
                    ...
                    "`quill auth reset xai-oauth` or re-authenticate with "
                    "`quill auth add xai-oauth --type oauth`."
                )
    ...
    def _credential_from_entry(self, entry: PooledCredential) -> UpstreamCredential:
        ...
        if not bearer:
            raise RuntimeError(
                ...
                "Re-authenticate with `quill auth add xai-oauth --type oauth`."
            )
