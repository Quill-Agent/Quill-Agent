...
Reads the user's Nous OAuth state from ``~/.quill/auth.json`` through the
...
:func:`quill_cli.auth.resolve_nous_runtime_credentials`.
"""
...
# Endpoints inference-api.example.com actually serves. Anything else
...
class NousPortalAdapter(UpstreamAdapter):
    ...
    def _get_credential(self, *, inference_auth_mode: str) -> UpstreamCredential:
        with self._lock:
            ...
            if state is None:
                raise RuntimeError(
                    "Not logged into Nous Portal. Run `quill login nous` first."
                )
            ...
            if not agent_key:
                raise RuntimeError(
                    ...
                    "Try `quill login nous` to re-authenticate."
                )
    ...
    # to quill_cli.auth to avoid expanding that module's public surface.
    # ------------------------------------------------------------------
