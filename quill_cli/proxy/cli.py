"""CLI handlers for the ``quill proxy`` subcommand."""
...
from quill_cli.proxy.server import (
    AIOHTTP_AVAILABLE,
    DEFAULT_HOST,
    DEFAULT_PORT,
    run_server,
...
def _print_aiohttp_missing() -> None:
    print(
        "quill proxy requires aiohttp. Install one of:\n"
...
def cmd_proxy_start(args: Any) -> int:
    ...
    if not adapter.is_authenticated():
        auth_hint = getattr(adapter, "auth_hint", f"quill login {adapter.name}")
        print(
    ...
    print(
        f"Starting Quill proxy for {adapter.display_name}\n"
...
def cmd_proxy_status(args: Any) -> int:
    ...
    print("Quill proxy upstream adapters\n")
    ...
    print(
        "\nStart the proxy with: quill proxy start [--provider <name>]"
    )
    return 0
...
def cmd_proxy(args: Any) -> int:
    """Dispatch ``quill proxy <subcommand>``."""
    ...
    print(
        "quill proxy — local OpenAI-compatible proxy that attaches your\n"
        "OAuth-authenticated provider credentials to outbound requests.\n"
        "\n"
        "Subcommands:\n"
        "  quill proxy start [--provider nous|xai] [--host 127.0.0.1] [--port 8645]\n"
