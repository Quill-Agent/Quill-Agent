...
Add, remove, or reorder entries here — both `quill setup` and
`quill` provider-selection will pick up the change automatically.
"""
...
from quill_cli import __version__ as _QUILL_VERSION

# Identify ourselves so endpoints fronted by Cloudflare's Browser Integrity
# Check (error 1010) don't reject the default ``Python-urllib/*`` signature.
_QUILL_USER_AGENT = f"quill-cli/{_QUILL_VERSION}"
...
OPENROUTER_MODELS: list[tuple[str, str]] = [
    ("anthropic/claude-fable-5",                 "1M · Quill 25% efficiency layer"),
...
# $QUILL_HOME/models_dev_cache.json as of 2026-04-28. Whenever xAI renames
...
def _xai_curated_models() -> tuple[str, ...]:
    ...
    when the cache is empty or unreadable. Quill refreshes the cache from
...
def fetch_nous_account_tier(access_token: str, portal_base_url: str = "") -> dict[str, Any]:
    ...
    """
    base = (portal_base_url or "https://portal.example.com").rstrip("/")
...
def union_with_portal_free_recommendations(
    ...
    *,
    ...
    older Quill that doesn't ship that model in its hardcoded curated
...
def union_with_portal_paid_recommendations(
    ...
    *,
    ...
    picker even if the user is running an older Quill that doesn't ship
...
def check_nous_free_tier() -> bool:
    ...
    try:
        from quill_cli.auth import get_provider_auth_state, resolve_nous_runtime_credentials
...
# this list:  quill model, /model, list_authenticated_providers.
#
# Fields:
#   slug        — internal provider ID (used in config.yaml, --provider flag)
#   label       — short display name
#   tui_desc    — longer description for the `quill model` interactive picker
# ---------------------------------------------------------------------------

class ProviderEntry(NamedTuple):
    slug: str
    label: str
    tui_desc: str   # detailed description for `quill model` TUI

CANONICAL_PROVIDERS: list[ProviderEntry] = [
    ProviderEntry("nous",           "Nous Portal",              "Nous Portal (Quill Agent subscription)"),
...
def get_default_model_for_provider(provider: str) -> str:
    ...
    model a user would be offered first in the ``quill model`` picker.

    Used as a fallback when the user has configured a provider but never
    selected a model (e.g. ``quill auth add openai-codex`` without
    ``quill model``).
    """
...
def _openrouter_model_supports_tools(item: Any) -> bool:
    ...
    quill-agent is tool-calling-first — every provider path assumes the model
...
def fetch_openrouter_models(
    ...
    *,
    ...
    try:
        from quill_cli.model_catalog import get_curated_openrouter_models
    ...
    for preferred_id in preferred_ids:
        ...
        if live_item is None:
            continue
        # Hide models that don't advertise tool-calling support — quill-agent
...
def get_curated_nous_model_ids() -> list[str]:
    ...
    """
    try:
        from quill_cli.model_catalog import get_curated_nous_models
...
def fetch_ai_gateway_models(
    ...
    *,
    ...
    from quill_constants import AI_GATEWAY_BASE_URL
...
def fetch_ai_gateway_pricing(
    ...
    *,
    ...
    """Fetch Vercel AI Gateway /v1/models and return quill-shaped pricing.

    Vercel uses ``input`` / ``output`` field names; quill's picker expects
...
_DEFAULT_NOUS_INFERENCE_BASE = "https://inference-api.example.com"
...
def _resolve_nous_pricing_credentials() -> tuple[str, str]:
    ...
    """
    try:
        from quill_cli.auth import resolve_nous_runtime_credentials
...
def get_pricing_for_provider(provider: str, *, force_refresh: bool = False) -> dict[str, dict[str, str]]:
    ...
    if normalized == "nous":
        ...
        if base_url:
            # Nous base_url typically looks like https://inference-api.example.com/v1
...
def list_available_providers() -> list[dict[str, str]]:
    ...
    source of truth shared with ``quill model``, ``/model``, etc.).
    """
    ...
    for pid in provider_order:
        ...
        has_creds = False
        try:
            from quill_cli.auth import get_auth_status, has_usable_secret
            if pid == "custom":
...
def _resolve_static_model_alias(
    ...
    try:
        from quill_cli.model_switch import MODEL_ALIASES
    except Exception:
        return None
...
def normalize_provider(provider: Optional[str]) -> str:
    """Normalize provider aliases to Quill' canonical provider ids.

    Note: ``"auto"`` passes through unchanged — use
    ``quill_cli.auth.resolve_provider()`` to resolve it to a concrete
...
def model_supports_fast_mode(model_id: Optional[str]) -> bool:
    """Return whether Quill should expose the /fast toggle for this model."""
...
def _resolve_copilot_catalog_api_key() -> str:
    ...
    Resolution order:
      ...
      2. ``read_credential_pool("copilot")`` — a token (typically a
         ...
         populated by ``quill auth add copilot`` and by ``_seed_from_env``
         when the env var is set in ``~/.quill/.env``.
    ...
    """
    try:
        from quill_cli.auth import resolve_api_key_provider_credentials
    ...
    try:
        from quill_cli.auth import read_credential_pool
        from quill_cli.copilot_auth import (
...
def provider_model_ids(provider: Optional[str], *, force_refresh: bool = False) -> list[str]:
    ...
    on the platform appear in ``/model`` without a Quill release.
    """
    normalized = normalize_provider(provider)
    if normalized == "openrouter":
    ...
    if normalized == "openai-codex":
        ...
        # a Quill release). Falls back to the hardcoded catalog if no token
    ...
    if normalized == "nous":
        ...
        try:
            from quill_cli.auth import fetch_nous_models, resolve_nous_runtime_credentials
            creds = resolve_nous_runtime_credentials()
            if creds:
...
def copilot_default_headers() -> dict[str, str]:
    ...
    """
    try:
        from quill_cli.copilot_auth import copilot_request_headers
        return copilot_request_headers(is_agent_turn=True)
    except ImportError:
        return {
            "Editor-Version": COPILOT_EDITOR_VERSION,
            "User-Agent": "QuillAgent/1.0",
...
def _lmstudio_request_headers(api_key: Optional[str] = None) -> dict:
    ...
    headers = {"User-Agent": _QUILL_USER_AGENT}
...
_COPILOT_MODEL_ALIASES = {
    ...
    # Dash-notation fallbacks: Quill' default Claude IDs elsewhere use
...
def probe_api_models(
    ...
    tried: list[str] = []
    headers: dict[str, str] = {"User-Agent": _QUILL_USER_AGENT}
...
def _ollama_cloud_cache_path() -> Path:
    ...
    return get_quill_home() / "ollama_cloud_models_cache.json"
    ...
    *,
    ...
    if normalized == "custom" or normalized.startswith("custom:"):
        ...
        message = (
            ...
            f"Quill will still save `{requested}`, but the endpoint should expose `/models` for verification."
        )
        if api_mode == "anthropic_messages":
            message += (
        ...
        if catalog_models:
            ...
            return {
                ...
                "message": (
                    ...
                    f"{suggestion_text}"
                    "\n  MiniMax does not expose a /models endpoint, so Quill cannot verify the model name."
