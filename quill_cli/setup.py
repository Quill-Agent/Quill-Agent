"""
Interactive setup wizard for Quill Agent.
...
from quill_cli.nous_subscription import get_nous_subscription_features
...
_DOCS_BASE = "https://quill-agent.example.com/docs"
...
# display_quill_home imported lazily at call sites (stale-module safety during quill update)
...
from quill_cli.cli_output import (  # noqa: E402
    print_error,
    print_info,
    print_success,
    print_warning,
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print()
    print(color("⚕ Quill Setup — Non-interactive mode", Colors.CYAN, Colors.BOLD))
    print()
    if reason:
        print_info(reason)
    print_info("The interactive wizard cannot be used here.")
    print()
    print_info("Configure Quill using environment variables or config commands:")
    print_info("  quill config set model.provider custom")
    print_info("  quill config set model.base_url http://localhost:8080/v1")
    print_info("  quill config set model.default your-model-name")
    print()
    print_info("Or set OPENROUTER_API_KEY / OPENAI_API_KEY in your environment.")
    print_info("Run 'quill setup' in an interactive terminal to use the full wizard.")
    print()
...
def _curses_prompt_choice(question: str, choices: list, default: int = 0, description: str | None = None) -> int:
    ...
    from quill_cli.curses_ui import curses_radiolist
...
def prompt_checklist(title: str, items: list, pre_selected: list = None) -> list:
    ...
    from quill_cli.curses_ui import curses_checklist
...
def _prompt_api_key(var: dict):
    ...
    if value:
        ...
        print_warning("  Skipped (configure later with 'quill setup')")


def _print_setup_summary(config: dict, quill_home):
    ...
    if _vision_backends:
        ...
        tool_status.append(("Vision (image analysis)", False, "run 'quill setup' to configure"))
    ...
    if subscription_features.image_gen.managed_by_nous:
        ...
        try:
            ...
            from quill_cli.plugins import _ensure_plugins_discovered
    ...
    # Video generation — opt-in via `quill tools` → Video Generation.
    # Only show the row when a plugin reports available so we don't badger
    # users who don't care about video gen with a "missing" status line.
    try:
        from agent.video_gen_registry import list_providers as _list_video_providers
        from quill_cli.plugins import _ensure_plugins_discovered as _ensure_plugins
        _ensure_plugins()
        _video_backend = None
    ...
    if subscription_features.tts.managed_by_nous:
        ...
        if neutts_ok:
            ...
            tool_status.append(("Text-to-Speech (NeuTTS — not installed)", False, "run 'quill setup tts'"))
        ...
        if kittentts_ok:
            ...
            tool_status.append(("Text-to-Speech (KittenTTS — not installed)", False, "run 'quill setup tts'"))
    else:
    ...
    if subscription_features.modal.managed_by_nous:
        ...
        if subscription_features.modal.direct_override:
            ...
            tool_status.append(("Modal Execution", False, "run 'quill setup terminal'"))
    ...
    # Spotify (OAuth via quill auth spotify — check auth.json, not env vars)
    try:
        from quill_cli.auth import get_provider_auth_state
    ...
    if disabled_tools:
        print_warning(
            "Some tools are disabled. Run 'quill setup tools' to configure them,"
        )
    ...
    print(
        f"   {color('Data:', Colors.YELLOW)}      {quill_home}/cron/, sessions/, logs/"
    )
    print()
    ...
    print()
    print(f"   {color('quill setup', Colors.GREEN)}          Re-run the full wizard")
    print(f"   {color('quill setup model', Colors.GREEN)}    Change model/provider")
    print(f"   {color('quill setup terminal', Colors.GREEN)} Change terminal backend")
    print(f"   {color('quill setup gateway', Colors.GREEN)}  Configure messaging")
    print(f"   {color('quill setup tools', Colors.GREEN)}    Configure tool providers")
    print()
    print(f"   {color('quill config', Colors.GREEN)}         View current settings")
    print(
        f"   {color('quill config edit', Colors.GREEN)}    Open config in your editor"
    )
    print(f"   {color('quill config set <key> <value>', Colors.GREEN)}")
    ...
    print()
    print(f"   {color('quill', Colors.GREEN)}              Start chatting")
    print(f"   {color('quill gateway', Colors.GREEN)}      Start messaging gateway")
    print(f"   {color('quill doctor', Colors.GREEN)}       Check for issues")
    print()
...
# between `quill tools` and `quill setup tools`).
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    Delegates to ``cmd_model()`` (the same flow used by ``quill model``)
    ...
    provider added to ``quill model`` is automatically available here.
    ...
    # Delegate to the shared quill model flow — handles provider picker,
    # credential prompting, model selection, and config persistence.
    from quill_cli.main import select_provider_and_model
    try:
        ...
        print_info("You can try again later with: quill model")
    ...
    if not quick and _supports_same_provider_pool_setup(selected_provider):
        try:
            ...
            from quill_cli.auth_commands import auth_add_command
            ...
            print_info(
                "Quill can keep multiple credentials for one provider and rotate between"
            )
            print_info(
    ...
    if _vision_needs_setup:
        ...
        print_info("doesn't currently provide one Quill can auto-use for vision,")
        ...
        if _vision_idx == 0:  # OpenRouter
            ...
            print_info("Skipped — add later with 'quill setup' or configure AUXILIARY_VISION_* settings")
...
def _xai_oauth_logged_in_for_setup() -> bool:
    ...
    through ``quill model`` -> xAI Grok OAuth (SuperGrok Subscription).
    """
    try:
        from quill_cli.auth import get_xai_oauth_auth_status
...
def _setup_tts_provider(config: dict):
    ...
    if selected == "nous-openai":
        ...
        if get_env_value("VOICE_TOOLS_OPENAI_KEY") or get_env_value("OPENAI_API_KEY"):
            print_warning(
                "Direct OpenAI credentials are still configured and may take precedence until removed from ~/.quill/.env."
            )
    ...
    if selected == "neutts":
        ...
        # via the Quill auth store) > existing XAI_API_KEY > prompt the user.
            ...
            if choice_idx == 0:
                ...
                if api_key:
                    ...
                    print_warning(
                        ...
                        f"via quill setup model or {_dhh()}/.env to use xAI TTS. "
                        "Falling back to Edge TTS."
                    )
                    selected = "edge"
            else:
...
def setup_tts(config: dict):
    """Standalone TTS setup (for 'quill setup tts')."""
    _setup_tts_provider(config)
...
def setup_terminal_backend(config: dict):
    ...
    print_info("Choose where Quill runs shell commands and code.")
    ...
    if selected_backend == "local":
        ...
        print_info("Requires the optional SDK: pip install 'quill-agent[vercel]'")
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                print_warning("Install failed — run manually: pip install 'quill-agent[vercel]'")
                if result.stderr:
                    print_info(f"  Error: {result.stderr.strip().splitlines()[-1]}")
...
def _apply_default_agent_settings(config: dict):
    ...
    # bridges it into QUILL_MAX_ITERATIONS at startup. We no longer write
    ...
    remove_env_value("QUILL_MAX_ITERATIONS")
    ...
    print_info("  Run `quill setup agent` later to customize.")
...
def setup_agent_settings(config: dict):
    ...
    try:
        ...
        if max_iter > 0:
            ...
            # QUILL_MAX_ITERATIONS from agent.max_turns at startup.
...
def _setup_telegram():
    ...
    print()
    print_info("📬 Home Channel: where Quill delivers cron job results,")
...
def _setup_slack():
    ...
    if existing:
        ...
        if not prompt_yes_no("Reconfigure Slack?", False):
            ...
            if prompt_yes_no(
                ...
                "list? (recommended after `quill update`)",
                True,
            ):
    ...
    print()
    print_info("   Full guide: https://quill-agent.example.com/docs/user-guide/messaging/slack/")
    print()
...
def _write_slack_manifest_and_instruct():
    """Generate the Slack manifest, write it under QUILL_HOME, and print
    ...
    """
    try:
        from quill_cli.slack_cli import _build_full_manifest
        from quill_constants import get_quill_home

        manifest = _build_full_manifest(
            bot_name="Quill",
            bot_description="Your Quill agent on Slack",
        )
        target = Path(get_quill_home()) / "slack-manifest.json"
        ...
        print_info(
            "   Re-run `quill slack manifest --write` anytime to refresh after "
            "Quill adds new commands."
        )
...
def _setup_matrix():
    ...
    if token or get_env_value("MATRIX_PASSWORD"):
        ...
        print()
        print_info("📬 Home Room: where Quill delivers cron job results and notifications.")
...
def _setup_mattermost():
    ...
    print()
    print_info("📬 Home Channel: where Quill delivers cron job results and notifications.")
    print_info("   To get a channel ID: click channel name → View Info → copy the ID")
    print_info("   You can also set this later by typing /set-home in a Mattermost channel.")
    home_channel = prompt("Home channel ID (leave empty to set later with /set-home)")
    if home_channel:
        save_env_value("MATTERMOST_HOME_CHANNEL", home_channel)
    print_info("   Open config in your editor:  quill config edit")
...
def _setup_bluebubbles():
    ...
    print_info("Connects Quill to iMessage via BlueBubbles — a free, open-source")
...
def _setup_qqbot():
    ...
    from quill_cli.gateway import _setup_qqbot as _gateway_setup_qqbot
...
def _setup_webhooks():
    ...
    print()
    print_info("   Full guide: https://quill-agent.example.com/docs/user-guide/messaging/webhooks/")
    print()
    ...
    print_info("   https://quill-agent.example.com/docs/user-guide/messaging/webhooks/#configuring-routes")
    print()
    print_info("   Open config in your editor:  quill config edit")
    print_info("   Open config in your editor:  quill config edit")
...
def setup_gateway(config: dict):
    ...
    from quill_cli.gateway import _all_platforms, _platform_status, _configure_platform

    print_header("Messaging Platforms")
    print_info("Connect to messaging platforms to chat with Quill from anywhere.")
    ...
    if not selected:
        print_info("No platforms selected. Run 'quill setup gateway' later to configure.")
        return
    ...
    if any_messaging:
        ...
        if missing_home:
            ...
            for plat in missing_home:
                print_info(
                    f"     quill config set {plat.upper()}_HOME_CHANNEL <channel_id>"
                )
        ...
        if supports_systemd and has_legacy_quill_units():
            print_legacy_unit_warning()
            print()
        ...
        if service_running:
            ...
            if prompt_yes_no(
                ...
                print_info("  Or run in foreground:  quill gateway")
        else:
            from quill_constants import is_container
            if is_container():
                print_info("Start the gateway to bring your bots online:")
                print_info("   quill gateway run          # Run as container main process")
                print_info("")
                ...
                print_info("Start the gateway to bring your bots online:")
                print_info("   quill gateway              # Run in foreground")
...
def setup_tools(config: dict, first_install: bool = False):
    ...
    Both `quill setup tools` and `quill tools` use the same flow:
...
def _model_section_has_credentials(config: dict) -> bool:
    ...
    Sources of truth:
      * ``PROVIDER_REGISTRY`` in ``quill_cli.auth`` — lists every supported
    ...
    """
    try:
        from quill_cli.auth import get_active_provider
...
def _get_section_config_summary(config: dict, section_key: str) -> Optional[str]:
    ...
    ``get_env_value`` is the module-level import from quill_cli.config
    ...
    if section_key == "model":
        ...
        from quill_cli.gateway import _all_platforms, _platform_status
...
_OPENCLAW_SCRIPT = (
    ...
    / "openclaw-migration"
    / "scripts"
    / "openclaw_to_quill.py"
)


def _load_openclaw_migration_module():
    """Load the openclaw_to_quill migration script as a module.
    ...
    spec = importlib.util.spec_from_file_location(
        "openclaw_to_quill", _OPENCLAW_SCRIPT
    )
...
# Config values may have different semantics between OpenClaw and Quill.
# Instruction/context files (.md) can contain incompatible setup procedures.
_HIGH_IMPACT_KIND_KEYWORDS = {
    "gateway": "⚠ Gateway/messaging — this will configure Quill to use your OpenClaw messaging channels",
    "telegram": "⚠ Telegram — this will point Quill at your OpenClaw Telegram bot",
    "slack": "⚠ Slack — this will point Quill at your OpenClaw Slack workspace",
    "discord": "⚠ Discord — this will point Quill at your OpenClaw Discord bot",
    "whatsapp": "⚠ WhatsApp — this will point Quill at your OpenClaw WhatsApp connection",
    "config": "⚠ Config values — OpenClaw settings may not map 1:1 to Quill equivalents",
...
def _print_migration_preview(report: dict):
    ...
    if conflict_items:
        print(color("  Would overwrite (conflicts with existing Quill config):", Colors.YELLOW))
    ...
    if warnings_shown:
        ...
        print()
        print(color("  Note: OpenClaw config values may have different semantics in Quill.", Colors.YELLOW))
        print(color("  For example, OpenClaw's tool_call_execution: \"auto\" ≠ Quill's yolo mode.", Colors.YELLOW))
        print(color("  Instruction files (.md) from OpenClaw may contain incompatible procedures.", Colors.YELLOW))
        print()


def _offer_openclaw_migration(quill_home: Path) -> bool:
    ...
    print_info("Quill can preview what would be imported before making any changes.")
    print()

    if not prompt_yes_no("Would you like to see what can be imported?", default=True):
        print_info(
            "Skipping migration. You can run it later with: quill claw migrate --dry-run"
        )
        return False
    ...
    try:
        ...
        dry_migrator = mod.Migrator(
            ...
            target_root=quill_home.resolve(),
    ...
    if not prompt_yes_no("Proceed with migration?", default=False):
        print_info(
            "Migration cancelled. You can run it later with: quill claw migrate"
        )
        print_info(
    ...
    # Execute the migration — overwrite=False so existing Quill configs are
    ...
    try:
        migrator = mod.Migrator(
            ...
            execute=True,
            workspace_target=None,
            overwrite=False,  # preserve existing Quill config
            migrate_secrets=True,
            output_dir=None,
            selected_options=selected,
            preset_name="full",
    ...
    if conflicts:
        print_info(f"Skipped {conflicts} item(s) that already exist in Quill (use quill claw migrate --overwrite to force).")
    if skipped:
...
def run_setup_wizard(args):
    ...
    Supports full, quick, and section-specific setup:
      quill setup           — full or quick (auto-detected)
      quill setup model     — just model/provider
      quill setup tts       — just text-to-speech
      quill setup terminal  — just terminal backend
      quill setup gateway   — just messaging platforms
      quill setup tools     — just tool configuration
      quill setup agent     — just agent settings
    """
    ...
    if section:
        for key, label, func in SETUP_SECTIONS:
            if key == section:
                ...
                print(color(f"│     ⚕ Quill Setup — {label:<34s} │", Colors.MAGENTA))
                print(
                    color(
                        "└─────────────────────────────────────────────────────────┘",
                        Colors.MAGENTA,
    ...
    print(
        color(
            "│             ⚕ Quill Agent Setup Wizard                │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    print(
        color(
            "│  Let's configure your Quill Agent installation.       │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    if is_existing:
        ...
        if quick_requested:
            _run_quick_setup(config, quill_home)
            return

        print()
        print_header("Reconfigure")
        print_success("You already have Quill configured.")
        print_info("Running the full wizard — each prompt shows your current value.")
        print_info("Press Enter to keep it, or type a new value to change it.")
        print_info("")
        print_info("Tip: jump straight to a section with 'quill setup model|terminal|")
        ...
        migration_ran = _offer_openclaw_migration(quill_home)
        if migration_ran:
            config = load_config()

        setup_mode = prompt_choice("How would you like to set up Quill?", [
            "Quick setup — provider, model & messaging (recommended)",
            "Full setup — configure everything",
        ], 0)

        if setup_mode == 0:
            _run_first_time_quick_setup(config, quill_home, is_existing)
            return
    ...
    print_info(f"Data folder:  {quill_home}")
    print_info(f"Install dir:  {PROJECT_ROOT}")
    print()
    print_info("You can edit these files directly or use 'quill config edit'")
...
def _run_first_time_quick_setup(config: dict, quill_home, is_existing: bool):
    ...
    the user can customize later via ``quill setup <section>``.
    """
    ...
    gateway_choice = prompt_choice(
        ...
        [
            ...
            "Skip — set up later with 'quill setup gateway'",
        ],
        0,
    )
    ...
    print()
    print_info("  Configure all settings:    quill setup")
    if gateway_choice != 0:
        print_info("  Connect Telegram/Discord:  quill setup gateway")
    print()

    _print_setup_summary(config, quill_home)


def _run_quick_setup(config: dict, quill_home):
    ...
    if not has_anything_missing:
        ...
        print()
        print_info("Run 'quill setup' and choose 'Full Setup' to reconfigure,")
    ...
    if missing_messaging:
        print()
        print_header("Messaging Platforms")
        print_info("Connect Quill to messaging apps to chat from anywhere.")
        print_info("You can configure these later with 'quill setup gateway'.")
