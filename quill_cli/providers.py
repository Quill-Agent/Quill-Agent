"""
Single source of truth for provider identity in Quill Agent.
...
2. **Quill overlays** — transport type, auth patterns, aggregator flags,
...
# -- Quill overlay ----------------------------------------------------------
# Quill-specific metadata that models.dev doesn't provide.

@dataclass(frozen=True)
class QuillOverlay:
    """Quill-specific provider metadata layered on top of models.dev."""
...
QUILL_OVERLAYS: Dict[str, QuillOverlay] = {
    "openrouter": QuillOverlay(
    ...
    "nous": QuillOverlay(
        ...
        base_url_override="https://inference-api.example.com/v1",
    ),
    "openai-codex": QuillOverlay(
        transport="codex_responses",
        auth_type="oauth_external",
        base_url_override="https://chatgpt.com/backend-api/codex",
    ),
    "xai-oauth": QuillOverlay(
        transport="codex_responses",
        auth_type="oauth_external",
        base_url_override="https://api.x.ai/v1",
        base_url_env_var="XAI_BASE_URL",
    ),
    "qwen-oauth": QuillOverlay(
        transport="openai_chat",
        auth_type="oauth_external",
        base_url_override="https://portal.qwen.ai/v1",
        base_url_env_var="QUILL_QWEN_BASE_URL",
    ),
    "google-gemini-cli": QuillOverlay(
        transport="openai_chat",
        auth_type="oauth_external",
        base_url_override="cloudcode-pa://google",
    ),
    "lmstudio": QuillOverlay(
        transport="openai_chat",
        auth_type="api_key",
    ...
    "copilot-acp": QuillOverlay(
        transport="codex_responses",
        auth_type="external_process",
        base_url_override="acp://copilot",
        base_url_env_var="COPILOT_ACP_BASE_URL",
    ),
    "github-copilot": QuillOverlay(
        transport="openai_chat",
        extra_env_vars=("COPILOT_GITHUB_TOKEN", "GH_TOKEN"),
    ),
    "anthropic": QuillOverlay(
    ...
    "kimi-for-coding": QuillOverlay(
        transport="openai_chat",
        base_url_env_var="KIMI_BASE_URL",
    ),
    "stepfun": QuillOverlay(
        transport="openai_chat",
        extra_env_vars=("STEPFUN_API_KEY",),
        base_url_override="https://api.stepfun.ai/step_plan/v1",
        base_url_env_var="STEPFUN_BASE_URL",
    ),
    "minimax": QuillOverlay(
        transport="anthropic_messages",
        base_url_env_var="MINIMAX_BASE_URL",
    ),
    "minimax-oauth": QuillOverlay(
        transport="anthropic_messages",
        auth_type="oauth_external",
        base_url_override="https://api.minimax.io/anthropic",
    ),
    "minimax-cn": QuillOverlay(
        transport="anthropic_messages",
        base_url_env_var="MINIMAX_CN_BASE_URL",
    ),
    "deepseek": QuillOverlay(
        transport="openai_chat",
        base_url_env_var="DEEPSEEK_BASE_URL",
    ),
    "alibaba": QuillOverlay(
        transport="openai_chat",
        base_url_env_var="DASHSCOPE_BASE_URL",
    ),
    "alibaba-coding-plan": QuillOverlay(
        transport="openai_chat",
    ...
    "opencode": QuillOverlay(
        transport="openai_chat",
        is_aggregator=True,
        base_url_env_var="OPENCODE_ZEN_BASE_URL",
    ),
    "opencode-go": QuillOverlay(
        transport="openai_chat",
        is_aggregator=True,
    ...
    "azure-foundry": QuillOverlay(
        transport="openai_chat",  # default; overridden by api_mode in config
        base_url_env_var="AZURE_FOUNDRY_BASE_URL",
    ),
    "bedrock": QuillOverlay(
...
class ProviderDef:
    ...
    doc: str = ""
    source: str = ""                      # "models.dev", "quill", "user-config"
...
def get_provider(name: str) -> Optional[ProviderDef]:
    ...
    Resolution order:
      1. Quill overlays (for providers not in models.dev: nous, openai-codex, etc.)
      2. models.dev catalog + Quill overlay
    ...
    overlay = QUILL_OVERLAYS.get(canonical)
    ...
    if mdev_info is not None:
        ...
        # Combine env vars: models.dev env + quill extra
    ...
    if overlay is not None:
        # Quill-only provider (not in models.dev)
        return ProviderDef(
            id=canonical,
...
def determine_api_mode(provider: str, base_url: str = "") -> str:
    ...
    # Direct provider checks for providers not in QUILL_OVERLAYS
    if provider == "bedrock":
