"""
Dump command for quill CLI.

Outputs a compact, plain-text summary of the user's Quill setup
...
from quill_cli.config import get_quill_home, get_env_path, get_project_root, load_config
...
def _redact(value: str) -> str:
    ...
    ``quill dump`` formats empty values as blank, not as ``"(not set)"``).
    """
...
def _gateway_status() -> str:
    ...
    try:
        from quill_cli.gateway import get_gateway_runtime_snapshot
...
def _count_skills(quill_home: Path) -> int:
...
def _cron_summary(quill_home: Path) -> str:
    """Return cron jobs summary."""
    jobs_file = quill_home / "cron" / "jobs.json"
...
def run_dump(args):
    ...
    lines.append(f"quill_home:      {display_quill_home()}")
    ...
    toolsets = config.get("toolsets", ["quill-cli"])
    ...
    lines.append(f"  cron_jobs:          {_cron_summary(quill_home)}")
    lines.append(f"  skills:             {_count_skills(quill_home)}")
