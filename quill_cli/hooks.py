"""quill hooks — inspect and manage shell-script hooks.

Usage::

    quill hooks list
    quill hooks test <event> [--for-tool X] [--payload-file F]
    quill hooks revoke <command>
    quill hooks doctor

Consent records live under ``~/.quill/shell-hooks-allowlist.json`` and
hook definitions come from the ``hooks:`` block in ``~/.quill/config.yaml``
...
def hooks_command(args) -> None:
    """Entry point for ``quill hooks`` — dispatches to the requested action."""
    sub = getattr(args, "hooks_action", None)

    if not sub:
        print("Usage: quill hooks {list|test|revoke|doctor}")
        print("Run 'quill hooks --help' for details.")
        return
...
def _cmd_list(_args) -> None:
    ...
    if not specs:
        print("No shell hooks configured in ~/.quill/config.yaml.")
        print("See `quill hooks --help` or")
                ...
                if entry and entry.get("approved_at"):
                    ...
                    if mtime_now and mtime_at and mtime_now > mtime_at:
                        print(
                            ...
                            f"run `quill hooks doctor` to re-validate"
                        )
        print()
...
# stdin a script sees under `quill hooks test` and `quill hooks doctor`
...
def _print_run_result(result: Dict[str, Any]) -> None:
    ...
    if parsed:
        print(f"      parsed (Quill wire shape): {json.dumps(parsed)}")
    else:
...
def _doctor_one(spec, shell_hooks) -> int:
    ...
    if entry and entry.get("script_mtime_at_approval"):
        ...
        if mtime_now and mtime_at and mtime_now > mtime_at:
            ...
            print(f"      ⚠ script modified since approval "
                  ...
                  f"then `quill hooks revoke` + re-approve to refresh")
    ...
    # is already allowlisted.  Otherwise `quill hooks doctor` would execute
    ...
    if not entry:
        print("      ℹ skipped JSON smoke test — not allowlisted yet. "
              ...
              "then re-run `quill hooks doctor`.")
