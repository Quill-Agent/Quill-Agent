"""Profile distributions — shareable, packaged Quill profiles via git.

A distribution is a Quill profile published as a git repository (or
...
* ``quill profile export/import`` — local backup / restore for a profile
  on your own machine. NOT a distribution format. Stays as-is.
* ``quill skills install <url>`` — the URL install pattern we're mirroring,
  but at the profile granularity.

Subcommands (all live under ``quill profile``, not a parallel tree):

    quill profile install <source> [--name N] [--alias] [--force] [--yes]
...
USER_OWNED_EXCLUDE: frozenset = frozenset({
    ...
    "backups", "cache",
    # Infrastructure
    "quill-agent", ".worktrees", "profiles", "bin", "node_modules",
...
class DistributionManifest:
    ...
    def from_dict(cls, data: Any) -> "DistributionManifest":
        ...
        return cls(
            ...
            description=str(data.get("description") or ""),
            quill_requires=str(data.get("quill_requires") or ""),
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self.quill_requires:
            out["quill_requires"] = self.quill_requires
        if self.author:
...
def check_quill_requires(spec: str, current_version: str) -> None:
    ...
    if not ok:
        raise DistributionError(
            f"This distribution requires Quill {op}{target}, "
...
def _stage_source(source: str, workdir: Path) -> Tuple[Path, str]:
    ...
    installed manifest's ``source:`` field so ``quill profile update`` can
    ...
    if _looks_like_git_url(src_str):
        ...
        if not (cloned / MANIFEST_FILENAME).is_file():
            raise DistributionError(
                ...
                "This repository is not a Quill profile distribution."
            )
...
def plan_install(
    ...
    if manifest is None:
        raise DistributionError(
            ...
            "this source is not a Quill distribution."
        )

    # Version check up-front so we fail fast
    check_quill_requires(manifest.quill_requires, quill_version)
    ...
    if canon == "default":
        raise DistributionError(
            ...
            "root profile (~/.quill).  Pass --name <name> to install under a "
            "new profile."
        )
    manifest.name = canon
...
def install_distribution(
    ...
    with tempfile.TemporaryDirectory(prefix="quill_dist_install_") as tmp:
        ...
        if plan.existing and not force:
            raise DistributionError(
                ...
                "Use `quill profile update` to upgrade in place, "
...
def update_distribution(
    ...
    if not existing_manifest.source:
        raise DistributionError(
            ...
            "`quill profile install <source> --name {canon} --force`."
        )

    with tempfile.TemporaryDirectory(prefix="quill_dist_update_") as tmp:
        plan = plan_install(
