...
configured for "cli" in `quill tools`.
Rules / memory / AGENTS.md / preloaded skills = same as a normal chat turn.
Approvals = auto-bypassed (QUILL_YOLO_MODE=1 is set for the call).
Working directory = the user's CWD (AGENTS.md etc. resolve from there as usual).

Model / provider selection mirrors `quill chat`:
...
Env var fallbacks (used when the corresponding arg is not passed):
    ...
    - QUILL_INFERENCE_PROVIDER  (already read by resolve_runtime_provider)
"""
...
def _validate_explicit_toolsets(toolsets: object = None) -> tuple[list[str] | None, str | None]:
    ...
    try:
        ...
        return None, f"quill -z: failed to validate --toolsets: {exc}\n"
    ...
    if any(name in {"all", "*"} for name in built_in):
        ...
        if ignored:
            sys.stderr.write(
                "quill -z: --toolsets all enables every toolset; "
    ...
    if unknown:
        sys.stderr.write(f"quill -z: ignoring unknown --toolsets entries: {', '.join(unknown)}\n")
    if disabled:
        sys.stderr.write(
            "quill -z: ignoring disabled MCP servers (set enabled: true in config.yaml to use): "
            f"{', '.join(disabled)}\n"
        )

    if not valid:
        return None, "quill -z: --toolsets did not contain any valid toolsets.\n"
...
def run_oneshot(
    ...
    Args:
        ...
        model: Optional model override. Falls back to QUILL_INFERENCE_MODEL
            env var, then config.yaml's model.default / model.model.
        provider: Optional provider override. Falls back to
            QUILL_INFERENCE_PROVIDER env var, then config.yaml's model.provider,
            then "auto".
    ...
    env_model_early = os.getenv("QUILL_INFERENCE_MODEL", "").strip()
    if provider and not ((model or "").strip() or env_model_early):
        sys.stderr.write(
            "quill -z: --provider requires --model (or QUILL_INFERENCE_MODEL). "
    ...
    os.environ["QUILL_ACCEPT_HOOKS"] = "1"
...
def _create_session_db_for_oneshot():
    """Best-effort SessionDB for ``quill -z`` / oneshot mode.

    Oneshot bypasses ``QuillCLI._init_agent()``, so it must wire the SQLite
...
def _run_agent(
    ...
    # Imports are local so they don't run when quill is invoked for
    # other commands (keeps top-level CLI startup cheap).
    from quill_cli.config import load_config
    from quill_cli.models import detect_provider_for_model
    ...
    env_model = os.getenv("QUILL_INFERENCE_MODEL", "").strip()
    ...
    if effective_provider is None and (model or env_model):
        ...
        if explicit_model:
            ...
            try:
                from quill_cli import model_switch as _ms
            ...
            if direct is not None:
                ...
                current_provider = (
                    cfg_provider
                    or os.getenv("QUILL_INFERENCE_PROVIDER", "").strip().lower()
                    or "auto"
                )
    ...
    # workers spawned via `quill -p <profile> chat -q ...`) honour the
    ...
    agent = AIAgent(
        ...
        #                QUILL_INTERACTIVE which we never set
        #   - shell-hook approval → auto-approved via QUILL_ACCEPT_HOOKS=1
        #                (set above); also falls back to deny on non-tty
        #   - dangerous-command approval → bypassed via QUILL_YOLO_MODE=1
