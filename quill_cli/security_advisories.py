"""
Security advisory checker for Quill Agent.
...
- **Acknowledgeable.** Once the user has read and acted on an advisory they
  can dismiss it via ``quill doctor --ack <id>``; the ack is persisted to
...
1. ``quill doctor`` (and ``quill doctor --ack <id>``)
...
run in environments where the rest of Quill failed to import.
"""
...
ADVISORIES: tuple[Advisory, ...] = (
    Advisory(
        ...
        summary=(
            ...
            "imported mistralai 2.4.6 — including quill when configured "
        ...
        remediation=(
            ...
            "Rotate API keys in ~/.quill/.env (OpenRouter, Anthropic, OpenAI, "
            ...
            "After cleanup: quill doctor --ack shai-hulud-2026-05  to dismiss "
            "this warning.",
        ),
...
def _installed_version(pkg_name: str) -> Optional[str]:
    ...
    except ImportError:  # py<3.8 — Quill requires 3.10+ but defensive.
        return None
    try:
...
def short_banner_lines(hits: list[AdvisoryHit]) -> list[str]:
    ...
    lines = [
        ...
        "  Run 'quill doctor' for remediation steps.",
    ]
    if len(hits) > 1:
...
# ``~/.quill/cache/advisory_banner_seen`` (a single line per advisory ID:
...
def _banner_cache_path() -> Optional[Path]:
    try:
        ...
        cache_dir = Path(get_quill_home()) / "cache"
...
def render_doctor_section(hits: list[AdvisoryHit]) -> tuple[bool, list[str]]:
    """Render the security-advisory section for ``quill doctor``.
