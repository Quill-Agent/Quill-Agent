"""
Quill Agent — Web UI server.

Provides a FastAPI backend serving the Vite/React frontend and REST API
endpoints for managing configuration, environment variables, and sessions.

Usage:
    python -m quill_cli.main web          # Start on http://127.0.0.1:9119
    python -m quill_cli.main web --port 8080
"""
...
try:
    ...
    # running `quill dashboard` needs fastapi+uvicorn; lazy install keeps
...
WEB_DIST = Path(os.environ["QUILL_WEB_DIST"]) if "QUILL_WEB_DIST" in os.environ else Path(__file__).parent / "web_dist"
_log = logging.getLogger(__name__)

app = FastAPI(title="Quill Agent", version=__version__)
...
_SESSION_HEADER_NAME = "X-Quill-Session-Token"

# In-browser Chat tab (/chat, /api/pty, …).  Off unless ``quill dashboard --tui``
# or QUILL_DASHBOARD_TUI=1.  Set from :func:`start_server`.
...
# streamed to a per-action log file under ``~/.quill/logs/<action>.log`` so
# the dashboard can tail them back to the user.
# ---------------------------------------------------------------------------

_ACTION_LOG_DIR: Path = get_quill_home() / "logs"

# Short ``name`` (from the URL) → absolute log file path.
_ACTION_LOG_FILES: Dict[str, str] = {
    "gateway-restart": "gateway-restart.log",
    "quill-update": "quill-update.log",
}
...
def _spawn_quill_action(subcommand: List[str], name: str) -> subprocess.Popen:
    """Spawn ``quill <subcommand>`` detached and record the Popen handle.

    Uses the running interpreter's ``quill_cli.main`` module so the action
    ...
    cmd = [sys.executable, "-m", "quill_cli.main", *subcommand]
    ...
    popen_kwargs: Dict[str, Any] = {
        ...
        "env": {**os.environ, "QUILL_NONINTERACTIVE": "1"},
    }
...
async def restart_gateway():
    """Kick off a ``quill gateway restart`` in the background."""
    try:
        proc = _spawn_quill_action(["gateway", "restart"], "gateway-restart")
...
@app.post("/api/quill/update")
async def update_quill():
    """Kick off ``quill update`` in the background."""
    try:
        proc = _spawn_quill_action(["update"], "quill-update")
    except Exception as exc:
        _log.exception("Failed to spawn quill update")
...
def _normalize_config_for_web(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    Quill supports ``model`` as either a bare string (``"anthropic/claude-sonnet-4"``)
...
# in quill_cli/config.py — listed here for deterministic ordering in the UI.
...
async def set_model_assignment(body: ModelAssignment):
    ...
    Writes to ``~/.quill/config.yaml`` — applies to **new** sessions only.
...
# the canonical ``quill auth add <provider>`` command so the dashboard
...
def _anthropic_oauth_status() -> Dict[str, Any]:
    ...
    Quill resolves Anthropic creds in this order at runtime:
    1. ``~/.quill/.anthropic_oauth.json`` — Quill-managed PKCE flow
    ...
    try:
        from agent.anthropic_adapter import (
            read_quill_oauth_credentials,
            read_claude_code_credentials,
            _QUILL_OAUTH_FILE,
        )
    except ImportError:
        read_claude_code_credentials = None  # type: ignore
        read_quill_oauth_credentials = None  # type: ignore
        _QUILL_OAUTH_FILE = None  # type: ignore

    quill_creds = None
    if read_quill_oauth_credentials:
        try:
            quill_creds = read_quill_oauth_credentials()
        except Exception:
            quill_creds = None
    if quill_creds and quill_creds.get("accessToken"):
        return {
            "logged_in": True,
            "source": "quill_pkce",
            "source_label": f"Quill PKCE ({_QUILL_OAUTH_FILE})",
            "token_preview": _truncate_token(quill_creds.get("accessToken")),
            "expires_at": quill_creds.get("expiresAt"),
            "has_refresh_token": bool(quill_creds.get("refreshToken")),
        }
...
def _claude_code_only_status() -> Dict[str, Any]:
    ...
    Claude Code subscription tokens are actively flowing into Quill even
    when they also have a separate Quill-managed PKCE login.
    """
    try:
...
_OAUTH_PROVIDER_CATALOG: tuple[Dict[str, Any], ...] = (
    {
        ...
        "flow": "pkce",
        "cli_command": "quill auth add anthropic",
    ...
    },
    {
        ...
        "name": "Nous Portal",
        "flow": "device_code",
        "cli_command": "quill auth add nous",
    ...
    },
    {
        ...
        "flow": "device_code",
        "cli_command": "quill auth add openai-codex",
    ...
    },
    {
        ...
        "flow": "external",
        "cli_command": "quill auth add qwen-oauth",
    ...
    },
    {
        ...
        "flow": "device_code",
        "cli_command": "quill auth add minimax-oauth",
...
def _resolve_provider_status(provider_id: str, status_fn) -> Dict[str, Any]:
    ...
    try:
        from quill_cli import auth as hauth
        if provider_id == "nous":
...
async def list_oauth_providers():
    ...
    Response shape (per provider):
        ...
        status:
          ...
          source           short slug ("quill_pkce", "claude_code", ...)
...
async def disconnect_oauth_provider(provider_id: str, request: Request):
    ...
    # Anthropic and claude-code clear the same Quill-managed PKCE file
    # AND forget the Claude Code import. We don't touch ~/.claude/* directly
    # — that's owned by the Claude Code CLI; users can re-auth there if they
    # want to undo a disconnect.
    if provider_id in {"anthropic", "claude-code"}:
        try:
            from agent.anthropic_adapter import _QUILL_OAUTH_FILE
            if _QUILL_OAUTH_FILE.exists():
                _QUILL_OAUTH_FILE.unlink()
        except Exception:
            pass
        # Also clear the credential pool entry if present.
        try:
            from quill_cli.auth import clear_provider_auth
...
#          → persists to ~/.quill/.anthropic_oauth.json AND credential pool
...
# Guarded so quill web still starts if anthropic_adapter is unavailable;
...
def _save_anthropic_oauth_creds(access_token: str, refresh_token: str, expires_at_ms: int) -> None:
    """Persist Anthropic PKCE creds to both Quill file AND credential pool.

    Mirrors what auth_commands.add_command does so the dashboard flow leaves
    the system in the same state as ``quill auth add anthropic``.
    """
    from agent.anthropic_adapter import _QUILL_OAUTH_FILE
    payload = {
    ...
    _QUILL_OAUTH_FILE.parent.mkdir(parents=True, exist_ok=True)
    _QUILL_OAUTH_FILE.write_text(json.dumps(payload, indent=2), encoding="utf-8")
...
def _submit_anthropic_pkce(session_id: str, code_input: str) -> Dict[str, Any]:
    ...
    req = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "quill-dashboard/1.0",
        },
        method="POST",
    )
    try:
...
async def _start_device_code_flow(provider_id: str) -> Dict[str, Any]:
    ...
    if provider_id == "nous":
        ...
        portal_base_url = (
            os.getenv("QUILL_PORTAL_BASE_URL")
...
def _nous_poller(session_id: str) -> None:
    ...
    try:
        ...
        from quill_cli.auth import persist_nous_credentials
...
async def get_logs(
    ...
    from quill_cli.logs import _read_tail, LOG_FILES

    log_name = LOG_FILES.get(file)
    if not log_name:
        raise HTTPException(status_code=400, detail=f"Unknown log file: {file}")
    log_path = get_quill_home() / "logs" / log_name
    if not log_path.exists():
        return {"file": file, "lines": []}

    try:
        from quill_logging import COMPONENT_PREFIXES
    except ImportError:
        COMPONENT_PREFIXES = {}
...
def _cron_profile_dicts() -> List[Dict[str, Any]]:
    ...
    from quill_cli import profiles as profiles_mod
    try:
...
def _cron_profile_home(profile: Optional[str]) -> Tuple[str, Path]:
    """Resolve a profile query value to (profile_name, QUILL_HOME)."""
    from quill_cli import profiles as profiles_mod
...
def _call_cron_for_profile(profile: Optional[str], func_name: str, *args, **kwargs):
    ...
    from the process QUILL_HOME at import time. The dashboard is a single
...
def _fallback_profile_dicts(profiles_mod) -> List[Dict[str, Any]]:
    ...
    default_home = profiles_mod._get_default_quill_home()
...
def _profile_setup_command(name: str) -> str:
    ...
    return "quill setup" if name == "default" else f"{name} setup"
...
async def get_skills():
    ...
    from quill_cli.skills_config import get_disabled_skills
    config = load_config()
...
async def toggle_skill(body: SkillToggle):
    from quill_cli.skills_config import get_disabled_skills, save_disabled_skills
    config = load_config()
    disabled = get_disabled_skills(config)
    if body.enabled:
...
async def get_toolsets():
    from quill_cli.tools_config import (
...
#
# The endpoint spawns the same ``quill --tui`` binary the CLI uses, behind
...
try:
    from quill_cli.pty_bridge import PtyBridge, PtyUnavailableError
...
def _resolve_chat_argv(
    ...
    Default: whatever ``quill --tui`` would run.  Tests monkeypatch this
    function to inject a tiny fake command (``cat``, ``sh -c 'printf …'``)
    so nothing has to build Node or the TUI bundle.

    Session resume is propagated via the ``QUILL_TUI_RESUME`` env var —
    matching what ``quill_cli.main._launch_tui`` does for the CLI path.
    Appending ``--resume <id>`` to argv doesn't work because ``ui-tui`` does
    not parse its argv.

    `sidecar_url` (when set) is forwarded as ``QUILL_TUI_SIDECAR_URL`` so
    the spawned ``tui_gateway.entry`` can mirror dispatcher emits to the
    dashboard's ``/api/pub`` endpoint (see :func:`pub_ws`).
    """
    from quill_cli.main import PROJECT_ROOT, _make_tui_argv
    ...
    env.setdefault("QUILL_TUI_DISABLE_MOUSE", "1")
    env.setdefault("QUILL_TUI_INLINE", "1")

    if resume:
        latest_resume, _latest_path = _session_latest_descendant(resume)
        if latest_resume:
            resume = latest_resume
        env["QUILL_TUI_RESUME"] = resume

    if sidecar_url:
        env["QUILL_TUI_SIDECAR_URL"] = sidecar_url
...
async def pty_ws(ws: WebSocket) -> None:
    ...
    if not _PTY_BRIDGE_AVAILABLE:
        await ws.send_text(
            ...
            "\x1b[33mInstall Quill inside WSL2 to use the dashboard's /chat "
...
# QUILL_TUI_SIDECAR_URL set in /api/pty's PTY env) and writes every
...
def _normalise_prefix(raw: Optional[str]) -> str:
    ...
    Returns a string like ``"/quill"`` (no trailing slash) or ``""`` when
...
def mount_spa(application: FastAPI):
    ...
    ``mission-control.tilos.com/quill/*`` -> local Caddy -> :9119), the
    proxy injects ``X-Forwarded-Prefix: /quill`` on every request. We
    rewrite the served ``index.html`` so absolute asset URLs (``/assets/...``)
    and the SPA's runtime ``__QUILL_BASE_PATH__`` honour that prefix
    ...
    def _serve_index(prefix: str = ""):
        ...
        ``prefix`` is the normalised ``X-Forwarded-Prefix`` (e.g. ``/quill``)
        or empty string when served at root.
        """
        html = _index_path.read_text()
        chat_js = "true" if _DASHBOARD_EMBEDDED_CHAT_ENABLED else "false"
        token_script = (
            f'<script>window.__QUILL_SESSION_TOKEN__="{_SESSION_TOKEN}";'
            f"window.__QUILL_DASHBOARD_EMBEDDED_CHAT__={chat_js};"
            f'window.__QUILL_BASE_PATH__="{prefix}";</script>'
        )
        if prefix:
    ...
    # under ``/quill`` they'd hit ``mission-control.tilos.com/fonts/...``
    # (the MC Pages app), not the Quill backend. Intercept CSS asset
...
_BUILTIN_DASHBOARD_THEMES = [
    {"name": "default",       "label": "Quill Teal",         "description": "Classic dark teal — the canonical Quill look"},
    {"name": "default-large", "label": "Quill Teal (Large)", "description": "Quill Teal with bigger fonts and roomier spacing"},
...
def _normalise_theme_definition(data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    # YAML in ~/.quill/, same trust level as the config file itself.
...
def _discover_user_themes() -> list:
    """Scan ~/.quill/dashboard-themes/*.yaml for user-created themes.

    Returns a list of fully-normalised theme definitions ready to ship
    to the frontend, so the client can apply them without a secondary
    round-trip or a built-in stub.
    """
    themes_dir = get_quill_home() / "dashboard-themes"
...
async def get_dashboard_themes():
    ...
    from `~/.quill/dashboard-themes/*.yaml` ship with their full
...
def _discover_dashboard_plugins() -> list:
    ...
    Checks three plugin sources (same as quill_cli.plugins):
    1. User plugins:    ~/.quill/plugins/<name>/dashboard/manifest.json
    2. Bundled plugins: <repo>/plugins/<name>/dashboard/manifest.json  (memory/, etc.)
    3. Project plugins: ./.quill/plugins/  (only if QUILL_ENABLE_PROJECT_PLUGINS)
    """
    plugins = []
    seen_names: set = set()

    from quill_cli.plugins import get_bundled_plugins_dir
    bundled_root = get_bundled_plugins_dir()
    search_dirs = [
        (get_quill_home() / "plugins", "user"),
        (bundled_root / "memory", "bundled"),
        (bundled_root, "bundled"),
    ]
    if os.environ.get("QUILL_ENABLE_PROJECT_PLUGINS"):
        search_dirs.append((Path.cwd() / ".quill" / "plugins", "project"))
...
def _merged_plugins_hub() -> Dict[str, Any]:
    ...
    from quill_cli.plugins_cmd import (
    ...
    plugins_root_resolved = (get_quill_home() / "plugins").resolve()
            ...
            try:
                ...
                for tname in provides_tools:
                    ...
                    if entry and entry.check_fn and not entry.check_fn():
                        auth_required = True
                        auth_command = f"quill auth {name}"
                        break
            except Exception:
                pass
...
async def post_agent_plugin_install(request: Request, body: _AgentPluginInstallBody):
    _require_token(request)
    from quill_cli.plugins_cmd import dashboard_install_plugin
...
async def post_agent_plugin_enable(request: Request, name: str):
    ...
    from quill_cli.plugins_cmd import dashboard_set_agent_plugin_enabled
...
async def post_agent_plugin_update(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from quill_cli.plugins_cmd import dashboard_update_user_plugin
...
async def delete_agent_plugin(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from quill_cli.plugins_cmd import dashboard_remove_user_plugin
...
def _mount_plugin_api_routes():
    ...
    for plugin in _get_dashboard_plugins():
        ...
        if not api_path.exists():
            ...
            continue
        try:
            module_name = f"quill_dashboard_plugin_{plugin['name']}"
...
def start_server(
    ...
    *,
    ...
    print(f"  Quill Web UI → http://{host}:{port}")
