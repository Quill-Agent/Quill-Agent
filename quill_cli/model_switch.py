...
- ``quill_cli.providers``        -- canonical provider identity + overlays
- ``quill_cli.model_normalize``  -- per-provider name formatting
...
_QUILL_MODEL_WARNING = (
    "Quill Agent Quill 3 & 4 models are NOT agentic and are not designed "
    "for use with Quill Agent. They lack the tool-calling capabilities "
    "required for agent workflows. Consider using an agentic model instead "
    "(Claude, GPT, Gemini, DeepSeek, etc.)."
)

# Match only the real Quill Agent Quill 3 / Quill 4 chat families.
# The previous substring check (`"quill" in name.lower()`) false-positived on
# unrelated local Modelfiles like ``quill-brain:qwen3-14b-ctx16k`` that just
# happen to carry "quill" in their tag but are fully tool-capable.
#
# Positive examples the regex must match:
#   YOUR_USERNAME/Quill-3-Llama-3.1-70B, quill-4-405b, openrouter/quill3:70b
# Negative examples it must NOT match:
#   quill-brain:qwen3-14b-ctx16k, qwen3:14b, claude-opus-4-6
_NOUS_QUILL_NON_AGENTIC_RE = re.compile(
    r"(?:^|[/:])quill[-_ ]?[34](?:[-_.:]|$)",
    re.IGNORECASE,
)


def is_nous_quill_non_agentic(model_name: str) -> bool:
    """Return True if *model_name* is a real Nous Quill 3/4 chat model.
    ...
    if not model_name:
        return False
    return bool(_NOUS_QUILL_NON_AGENTIC_RE.search(model_name))


def _check_quill_model_warning(model_name: str) -> str:
    """Return a warning string if *model_name* is a Nous Quill 3/4 chat model."""
    if is_nous_quill_non_agentic(model_name):
        return _QUILL_MODEL_WARNING
    return ""
...
MODEL_ALIASES: dict[str, ModelIdentity] = {
    ...
    "ds-pro":    ModelIdentity("deepseek", "deepseek-v4-pro"),
    "v4-pro":    ModelIdentity("deepseek", "deepseek-v4-pro"),
    "ds-flash":  ModelIdentity("deepseek", "deepseek-v4-flash"),
...
def _load_direct_aliases() -> dict[str, DirectAlias]:
    ...
    Also reads ``model.aliases`` (set by ``quill config set model.aliases.xxx``)
...
def _ensure_direct_aliases() -> None:
    ...
    the module attribute. This keeps `from quill_cli.model_switch import
...
def switch_model(
    ...
    if explicit_provider:
        ...
        if pdef is None:
            _switch_err = (
                ...
                f"Check 'quill model' for available providers, or define it "
            ...
            try:
                ...
                if _cfg_issues:
                    _switch_err += "\n\nRun 'quill doctor' — config issues detected:"
    ...
    # Mirrors the same logic in quill_cli.runtime_provider.resolve_runtime_provider;
    ...
    quill_warn = _check_quill_model_warning(new_model)
    if quill_warn:
        warnings.append(quill_warn)
...
def list_authenticated_providers(
    ...
    Uses the curated model lists from quill_cli/models.py (OPENROUTER_MODELS,
    ...
    def _record_builtin_endpoint(slug: str) -> None:
        ...
        try:
            from quill_cli.auth import PROVIDER_REGISTRY as _reg
        except Exception:
            return
        pcfg = _reg.get(slug)
        if not pcfg:
    ...
    # Build curated model lists keyed by quill provider ID
    curated: dict[str, list[str]] = dict(_PROVIDER_MODELS)
    curated["openrouter"] = [mid for mid, _ in OPENROUTER_MODELS]
    # "nous" pulls from the remote model-catalog manifest published at
    # https://quill-agent.example.com/docs/api/model-catalog.json so
    # newly added Portal models surface in the /model picker without
    # requiring a Quill release. Falls back to the in-repo
    ...
    # --- 1. Check Quill-mapped providers ---
    for quill_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        pconfig = PROVIDER_REGISTRY.get(quill_id)
        # Skip non-API-key auth providers here — they are handled in
        # section 2 (QUILL_OVERLAYS) with proper auth store checking.
        ...
        if not has_creds:
            try:
                from quill_cli.auth import _load_auth_store
                store = _load_auth_store()
                if store and store.get("credential_pool", {}).get(quill_id):
                    has_creds = True
            except Exception:
                pass
        if not has_creds:
        ...
        # show up in the picker without requiring a Quill release.
        model_ids = curated.get(quill_id, [])
        if quill_id in _MODELS_DEV_PREFERRED:
            model_ids = _merge_with_models_dev(quill_id, model_ids)
        total = len(model_ids)
    ...
    # --- 2. Check Quill-only providers (nous, openai-codex, copilot, opencode-go) ---
    from quill_cli.providers import QUILL_OVERLAYS
    from quill_cli.auth import PROVIDER_REGISTRY as _auth_registry

    # Build reverse mapping: models.dev ID → Quill provider ID.
    # QUILL_OVERLAYS keys may be models.dev IDs (e.g. "github-copilot")
    # while _PROVIDER_MODELS and config.yaml use Quill IDs ("copilot").
    _mdev_to_quill = {v: k for k, v in PROVIDER_TO_MODELS_DEV.items()}

    for pid, overlay in QUILL_OVERLAYS.items():
        if pid.lower() in seen_slugs:
            continue

        # Resolve Quill slug — e.g. "github-copilot" → "copilot"
        quill_slug = _mdev_to_quill.get(pid, pid)
        if quill_slug.lower() in seen_slugs:
            continue

        # Check if credentials exist
        has_creds = False
        if overlay.auth_type == "aws_sdk":
            has_creds = _has_aws_sdk_creds_for_listing(quill_slug)
        elif overlay.extra_env_vars:
            has_creds = any(os.environ.get(ev) for ev in overlay.extra_env_vars)
        # Also check api_key_env_vars from PROVIDER_REGISTRY for api_key auth_type
        if not has_creds and overlay.auth_type == "api_key":
            for _key in (pid, quill_slug):
        ...
        if not has_creds:
            try:
                ...
                if store and (pid in providers_store or quill_slug in providers_store):
                    has_creds = True
        ...
        if not has_creds:
            try:
                ...
                pool = load_pool(quill_slug)
                if pool.has_credentials():
                    has_creds = True
            except Exception as exc:
                logger.debug("Credential pool check failed for %s: %s", quill_slug, exc)
        ...
        # configured.
        if not has_creds and quill_slug == "anthropic":
            try:
                ...
                if (quill_creds and quill_creds.get("accessToken")) or \
        ...
        if quill_slug in {"openai-codex", "copilot", "copilot-acp"}:
            ...
            model_ids = provider_model_ids(quill_slug)
        ...
        # reflects the active region (eu.*, ap.*) not the static us.* list.
            ...
            try:
                ...
                model_ids = _ids if _ids is not None else (curated.get(quill_slug, []) or curated.get(pid, []))
            except Exception:
                model_ids = curated.get(quill_slug, []) or curated.get(pid, [])
        else:
            # Use curated list — look up by Quill slug, fall back to overlay key
            model_ids = curated.get(quill_slug, []) or curated.get(pid, [])
            # Merge with models.dev for preferred providers (same rationale as above).
            if quill_slug in _MODELS_DEV_PREFERRED:
                model_ids = _merge_with_models_dev(quill_slug, model_ids)
        total = len(model_ids)
        top = model_ids[:max_models]

        results.append({
            "slug": quill_slug,
            "name": get_label(quill_slug),
            "is_current": quill_slug == current_provider or pid == current_provider,
            "is_user_defined": False,
            "models": top,
            "total_models": total,
            "source": "quill",
        ...
        seen_slugs.add(quill_slug.lower())
        _record_builtin_endpoint(quill_slug)

    # --- 2b. Cross-check canonical provider list ---
    # Catches providers that are in CANONICAL_PROVIDERS but weren't found
    # in PROVIDER_TO_MODELS_DEV or QUILL_OVERLAYS (keeps /model in sync
    # with `quill model`).
    try:
        from quill_cli.models import CANONICAL_PROVIDERS as _canon_provs
    except ImportError:
        _canon_provs = []
    ...
    if user_providers and isinstance(user_providers, dict):
        for ep_name, ep_cfg in user_providers.items():
            ...
            # ``base_url`` is Quill's canonical write key (matches
            ...
            # Quill writes ``models:`` as a dict keyed by model id
            # (see quill_cli/main.py::_save_custom_provider); older
    ...
    if custom_providers and isinstance(custom_providers, list):
        ...
        for entry in custom_providers:
            ...
            if group_key not in groups:
                ...
                # Quill's own writer uses; a hyphen variant is accepted
            ...
            # active model. Quill's own writer (main.py::_save_custom_provider)
            # stores every configured model as a dict under ``models:``;
            # downstream readers (agent/models_dev.py, gateway/run.py,
            # run_agent.py, quill_cli/config.py) already consume that dict.
...
def list_picker_providers(
    ...
    - OpenRouter's model list is replaced with the output of
      :func:`quill_cli.models.fetch_openrouter_models`, which filters the
    ...
    """
    from quill_cli.models import fetch_openrouter_models
