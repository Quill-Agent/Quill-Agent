...
Usage:
    quill pairing list              # Show all pending + approved users
    quill pairing approve <platform> <code>  # Approve a pairing code
    quill pairing revoke <platform> <user_id> # Revoke user access
    quill pairing clear-pending     # Clear all expired/pending codes
"""

def pairing_command(args):
    """Handle quill pairing subcommands."""
    ...
    if action == "list":
        ...
        print("Usage: quill pairing {list|approve|revoke|clear-pending}")
        print("Run 'quill pairing --help' for details.")
...
def _cmd_approve(store, platform: str, code: str):
    ...
    if result:
        ...
        print(
            ...
            "~/.quill/platforms/pairing/_rate_limits.json\n".format(platform)
        )
    else:
        print(f"\n  Code '{code}' not found or expired for platform '{platform}'.")
        print("  Run 'quill pairing list' to see pending codes.\n")
