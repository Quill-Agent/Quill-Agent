"""
Profile management for multiple isolated Quill instances.

Each profile is a fully independent QUILL_HOME directory with its own
config.yaml, .env, memory, sessions, skills, gateway, cron, and logs.
Profiles live under ``~/.quill/profiles/<name>/`` by default.

The "default" profile is ``~/.quill`` itself — backward compatible,
zero migration needed.

Usage::

    quill profile create coder          # fresh profile + bundled skills
    quill profile create coder --clone  # also copy config, .env, SOUL.md, skills
    quill profile create coder --clone-all  # full copy of source profile
    coder chat                           # use via wrapper alias
    quill -p coder chat                 # or via flag
    quill profile use coder             # set as sticky default
    quill profile delete coder          # remove profile + alias + service
"""
...
_PROFILE_DIRS = [
    ...
    # See quill_constants.get_subprocess_home() and issue #4426.
    "home",
]
...
# Marker file written by `quill profile create --no-skills`.  When present in
# a profile's root, callers of seed_profile_skills() (fresh-create, `quill
...
# `quill skills install` or drop SKILL.md files into the profile's skills/.
...
def _clone_all_copytree_ignore(source_dir: Path):
    ...
    Two categories:
      1. Root-level entries in ``_CLONE_ALL_DEFAULT_EXCLUDE_ROOT`` — known
         Quill infrastructure directories that only the default profile
         (``~/.quill``) ever contains.  Gated on ``source_dir`` actually
    ...
    is_default_source = source_resolved == _get_default_quill_home().resolve()
...
# Directories/files to exclude when exporting the default (~/.quill) profile.
# The default profile contains infrastructure (repo checkout, worktrees, DBs,
# caches, binaries) that named profiles don't have.  We exclude those so the
# export is a portable, reasonable-size archive of actual profile data.
_DEFAULT_EXPORT_EXCLUDE_ROOT = frozenset({
    # Infrastructure
    "quill-agent",         # repo checkout (multi-GB)
    ".worktrees",           # git worktrees
...
_RESERVED_NAMES = frozenset({
    "quill", "default", "test", "tmp", "root", "sudo",
})

# Quill subcommands that cannot be used as profile names/aliases
_QUILL_SUBCOMMANDS = frozenset({
...
def _get_profiles_root() -> Path:
    ...
    Anchored to the quill root, NOT to the current QUILL_HOME
    (which may itself be a profile).  This ensures ``coder profile list``
    can see all profiles.

    In Docker/custom deployments where QUILL_HOME points outside
    ``~/.quill``, profiles live under ``QUILL_HOME/profiles/`` so
    they persist on the mounted volume.
    """
    return _get_default_quill_home() / "profiles"


def _get_default_quill_home() -> Path:
    """Return the default (pre-profile) QUILL_HOME path.

    In standard deployments this is ``~/.quill``.
    In Docker/custom deployments where QUILL_HOME is outside ``~/.quill``
    (e.g. ``/opt/data``), returns QUILL_HOME directly.
    """
...
def _get_active_profile_path() -> Path:
    ...
    return _get_default_quill_home() / "active_profile"
...
def validate_profile_name(name: str) -> None:
    ...
    if name in _RESERVED_NAMES:
        raise ValueError(
            ...
            f"the Quill installation itself or a common system binary.  "
            f"Pick a different name."
        )


def get_profile_dir(name: str) -> Path:
    """Resolve a profile name to its QUILL_HOME directory."""
    canon = normalize_profile_name(name)
    if canon == "default":
        return _get_default_quill_home()
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    Checks: reserved names, quill subcommands, existing binaries in PATH.
    """
    canon = normalize_profile_name(name)
    if canon in _RESERVED_NAMES:
        return f"'{canon}' is a reserved name"
    if canon in _QUILL_SUBCOMMANDS:
        return f"'{canon}' conflicts with a quill subcommand"
...
def _read_distribution_meta(profile_dir: Path) -> tuple:
    ...
    never break ``quill profile list`` for an unrelated profile.
    """
...
# ``config.yaml``. ``config.yaml`` is the user-facing Quill config
...
def list_profiles() -> List[ProfileInfo]:
    ...
    # Default profile
    default_home = _get_default_quill_home()
...
def create_profile(
    ...
    no_skills:
        ...
        a marker file so ``quill update`` skips re-seeding this profile's
    ...
    if canon == "default":
        raise ValueError(
            "Cannot create a profile named 'default' — it is the built-in profile (~/.quill)."
        )
    ...
    if clone_all and source_dir:
        # Full copy of source profile (exclude sibling ~/.quill/profiles/)
        shutil.copytree(
            source_dir,
            profile_dir,
    ...
    # Write the opt-out marker so seed_profile_skills() and `quill update`'s
    ...
    if no_skills:
        try:
            (profile_dir / NO_BUNDLED_SKILLS_MARKER).write_text(
                ...
                "(`quill profile create --no-skills`).\n"
                "Delete this file to re-enable sync on the next `quill update`.\n",
                encoding="utf-8",
            )
        except OSError:
    ...
    if description and description.strip():
        try:
            ...
            pass  # non-fatal — user can describe later with `quill profile describe`
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    Uses subprocess because sync_skills() caches QUILL_HOME at module level.
    Returns the sync result dict, or None on failure.

    Profiles that opted out of bundled skills (via ``quill profile create
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "QUILL_HOME": str(profile_dir)},
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    if canon == "default":
        raise ValueError(
            "Cannot delete the default profile (~/.quill).\n"
            "To remove everything, use: quill uninstall"
        )
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    # Temporarily set QUILL_HOME so _profile_suffix resolves correctly
    old_home = os.environ.get("QUILL_HOME")
    try:
        os.environ["QUILL_HOME"] = str(profile_dir)
        from quill_cli.gateway import get_service_name, get_launchd_plist_path
        ...
        if old_home is not None:
            os.environ["QUILL_HOME"] = old_home
        elif "QUILL_HOME" in os.environ:
            del os.environ["QUILL_HOME"]
...
def set_active_profile(name: str) -> None:
    ...
    Writes to ``~/.quill/active_profile``. Use ``"default"`` to clear.
    """
    canon = normalize_profile_name(name)
    validate_profile_name(canon)
    if canon != "default" and not profile_exists(canon):
        raise FileNotFoundError(
            ...
            f"Create it with: quill profile create {canon}"
        )
...
def get_active_profile_name() -> str:
    """Infer the current profile name from QUILL_HOME.

    Returns ``"default"`` if QUILL_HOME is not set or points to ``~/.quill``.
    Returns the profile name if QUILL_HOME points into ``~/.quill/profiles/<name>``.
    Returns ``"custom"`` if QUILL_HOME is set to an unrecognized path.
    """
    from quill_constants import get_quill_home
    quill_home = get_quill_home()
    resolved = quill_home.resolve()

    default_resolved = _get_default_quill_home().resolve()
...
def export_profile(name: str, output_path: str) -> Path:
    ...
    if canon == "default":
        # The default profile IS ~/.quill itself — its parent is ~/ and its
        # directory name is ".quill", not "default".  We stage a clean copy
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    if not inferred_name:
        raise ValueError(
            ...
            "Specify it explicitly: quill profile import <archive> --name <name>"
        )
    ...
    # dir.  Importing as "default" would target ~/.quill itself — disallow
    # that and guide the user toward a named profile.
    canon = normalize_profile_name(inferred_name)
    validate_profile_name(canon)
    if canon == "default":
        raise ValueError(
            "Cannot import as 'default' — that is the built-in root profile (~/.quill). "
            "Specify a different name: quill profile import <archive> --name <name>"
        )
    ...
    with tempfile.TemporaryDirectory(prefix="quill_profile_import_") as tmpdir:
...
def _migrate_honcho_profile_host(old_name: str, new_name: str, new_dir: Path) -> None:
    ...
    old_host = f"quill.{old_name}"
    new_host = f"quill.{new_name}"

    candidates = [
        new_dir / "honcho.json",
        _get_default_quill_home() / "honcho.json",
...
def resolve_profile_env(profile_name: str) -> str:
    """Resolve a profile name to a QUILL_HOME path string.

    Called early in the CLI entry point, before any quill modules
    are imported, to set the QUILL_HOME environment variable.
    """
