"""quill webhook — manage dynamic webhook subscriptions from the CLI.
...
Subscriptions persist to ~/.quill/webhook_subscriptions.json and are
...
def _subscriptions_path() -> Path:
    return _quill_home() / _SUBSCRIPTIONS_FILENAME
...
def _setup_hint() -> str:
  ...
  Then start the gateway: quill gateway run
"""
...
def webhook_command(args):
    """Entry point for 'quill webhook' subcommand."""
    sub = getattr(args, "webhook_action", None)

    if not sub:
        print("Usage: quill webhook {subscribe|list|remove|test}")
        print("Run 'quill webhook --help' for details.")
        return
...
def _cmd_subscribe(args):
    ...
    print(f"  The gateway must be running to receive events (quill gateway run).\n")
...
def _cmd_list(args):
    ...
    if not subs:
        ...
        print("  Create one with: quill webhook subscribe <name>")
        return
...
def _cmd_test(args):
    ...
    payload = args.payload or '{"test": true, "event_type": "test", "message": "Hello from quill webhook test"}'
