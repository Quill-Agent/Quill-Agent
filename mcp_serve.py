"""
Quill MCP Server — expose messaging conversations as MCP tools.
...
Plus: channels_list (Quill-specific extra)

Usage:
    quill mcp serve
    quill mcp serve --verbose
...
logger = logging.getLogger("quill.mcp_serve")
...
def _get_sessions_dir() -> Path:
    """Return the sessions directory using QUILL_HOME."""
    try:
        from quill_constants import get_quill_home
        return get_quill_home() / "sessions"
    except ImportError:
        return Path(os.environ.get("QUILL_HOME", Path.home() / ".quill")) / "sessions"
...
def _load_channel_directory() -> dict:
    ...
    try:
        from quill_constants import get_quill_home
        directory_file = get_quill_home() / "channel_directory.json"
    except ImportError:
        directory_file = Path(
            os.environ.get("QUILL_HOME", Path.home() / ".quill")
...
class EventBridge:
    ...
    This is the Quill equivalent of OpenClaw's WebSocket gateway bridge.
    ...
    def _poll_once(self, db):
        ...
        try:
            from quill_constants import get_quill_home
            db_file = get_quill_home() / "state.db"
        except ImportError:
            db_file = Path(os.environ.get("QUILL_HOME", Path.home() / ".quill")) / "state.db"
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "FastMCP":
    """Create and return the Quill MCP server with all tools registered."""
    ...
    mcp = FastMCP(
        "quill",
        instructions=(
            "Quill Agent messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the Quill MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
        print(
