"""Centralized logging setup for Quill Agent.

Provides a single ``setup_logging()`` entry point that both the CLI and
gateway call early in their startup path.  All log files live under
``~/.quill/logs/`` (profile-aware via ``get_quill_home()``).
...
from quill_constants import get_config_path, get_quill_home
...
def _install_session_record_factory() -> None:
    ...
    if getattr(current_factory, "_quill_session_injector", False):
    ...
    _session_record_factory._quill_session_injector = True  # type: ignore[attr-defined]
...
# Used by _ComponentFilter and exposed for ``quill logs --component``.
COMPONENT_PREFIXES = {
    "gateway": ("gateway", "quill_plugins"),
...
def setup_logging(
    *,
    ...
    """Configure the Quill logging subsystem.
    ...
    *force* is ``True``.
    ...
    ----------
    quill_home
        Override for the Quill home directory.  Falls back to
        ``get_quill_home()`` (profile-aware).
    log_level
...
def setup_verbose_logging() -> None:
    ...
    for h in root.handlers:
        if isinstance(h, logging.StreamHandler) and not isinstance(h, RotatingFileHandler):
            if getattr(h, "_quill_verbose", False):
                return

    handler = logging.StreamHandler()
    handler.setLevel(logging.DEBUG)
    handler.setFormatter(RedactingFormatter(_LOG_FORMAT_VERBOSE, datefmt="%H:%M:%S"))
    handler._quill_verbose = True  # type: ignore[attr-defined]
...
class _ManagedRotatingFileHandler(RotatingFileHandler):
    ...
    inherit the quill group. However, both _open() (initial creation) and
    ...
    def __init__(self, *args, **kwargs):
        from quill_cli.config import is_managed
