...
// (e.g. https://mission-control.tilos.com/quill/). The Python backend
// injects ``window.__QUILL_BASE_PATH__`` into index.html based on the
// incoming ``X-Forwarded-Prefix`` header so the SPA can address its own
// ``/api/...`` and ``/dashboard-plugins/...`` URLs correctly without a
// rebuild. Empty string means "served at root".
function readBasePath(): string {
  if (typeof window === "undefined") return "";
  const raw = window.__QUILL_BASE_PATH__ ?? "";
  if (!raw) return "";
...
export const QUILL_BASE_PATH = readBasePath();
const BASE = QUILL_BASE_PATH;
...
declare global {
  interface Window {
    ...
    __QUILL_BASE_PATH__?: string;
  }
}
let _sessionToken: string | null = null;
const SESSION_HEADER = "X-Quill-Session-Token";
...
async function getSessionToken(): Promise<string> {
  ...
  const injected = window.__QUILL_SESSION_TOKEN__;
  if (injected) {
    _sessionToken = injected;
    return _sessionToken;
  }
  throw new Error("Session token not available — page must be served by the Quill dashboard server");
}
...
export const api = {
  ...
  updateQuill: () =>
    fetchJSON<ActionResponse>("/api/quill/update", { method: "POST" }),
