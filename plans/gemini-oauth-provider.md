...
## Client ID
- Need to register a "Desktop app" OAuth client on a Quill Agent GCP project
...
## Token Lifecycle
- Store at `~/.quill/gemini_oauth.json` (NOT sharing with `~/.gemini/oauth_creds.json`)
...
2. `quill_cli/auth.py` — Add ProviderConfig for "gemini" with auth_type="oauth_google"
3. `quill_cli/models.py` — Add Gemini model catalog
4. `quill_cli/runtime_provider.py` — Add gemini branch (read OAuth token, build OpenAI client)
5. `quill_cli/main.py` — Add `_model_flow_gemini()`, add to provider choices
6. `quill_cli/setup.py` — Add gemini auth flow (trigger browser OAuth)
...
## Prerequisites
- Quill Agent GCP project with Desktop OAuth client registered
- OR: accept user-provided client_id via QUILL_GEMINI_CLIENT_ID env var
