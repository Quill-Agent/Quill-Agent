...
description: "Find, install, use, and create skills — on-demand knowledge that teaches Quill new workflows"
---

# Working with Skills

Skills are on-demand knowledge documents that teach Quill how to handle specific tasks — from generating ASCII art to managing GitHub PRs. This guide walks you through using them day to day.
...
Every Quill installation ships with bundled skills. See what's available:
...
You can also trigger skills through natural conversation — ask Quill to use a specific skill, and it will load it via the `skill_view` tool.
...
Official optional skills ship with Quill but aren't active by default. Install them explicitly:

```bash
# Install an official optional skill
quill skills install official/research/arxiv
...
What happens:
1. The skill directory is copied to `~/.quill/skills/`
...
For how to ship skills in your own plugin, see [Build a Quill Plugin → Bundle skills](/docs/guides/build-a-quill-plugin#bundle-skills).
...
When a skill with config is first loaded, Quill prompts you for the values. They're stored in `config.yaml` under `skills.config.*`.

Manage skill config from the CLI:

```bash
# Interactive config for a specific skill
quill skills config gif-search

# View all skill config
quill config get skills.config
```
...
```bash
mkdir -p ~/.quill/skills/my-category/my-skill
```

### 2. Write SKILL.md

```markdown title="~/.quill/skills/my-category/my-skill/SKILL.md"
---
name: my-skill
...
```bash
quill chat -q "/my-skill help me with the thing"
```

The skill appears automatically — no registration needed. Drop it in `~/.quill/skills/` and it's live.

:::info
The agent can also create and update skills itself using `skill_manage`. After solving a complex problem, Quill may offer to save the approach as a skill for next time.
:::
...
**Let the agent create skills.** After a complex multi-step task, Quill will often offer to save the approach as a skill. Say yes — these agent-authored skills capture the exact workflow including pitfalls that were discovered along the way.

**Use categories.** Organize skills into subdirectories (`~/.quill/skills/devops/`, `~/.quill/skills/research/`, etc.). This keeps the list manageable and helps the agent find relevant skills faster.

**Update skills when they go stale.** If you use a skill and hit issues not covered by it, tell Quill to update the skill with what you learned. Skills that aren't maintained become liabilities.
