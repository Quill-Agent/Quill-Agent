...
You need **tenant admin rights** (or an admin to grant consent on your behalf) to finish this. Bookmark the values you collect — they go into `~/.quill/.env` at the end.
...
4. Fill in:
   - **Name:** `Quill Teams Meeting Pipeline` (or any name you'll recognize).
...
3. **Description:** `quill-graph-secret`. **Expires:** pick a value that matches your rotation policy (6-24 months is typical).
4. Click **Add**.
...
```powershell
# Create a policy scoped to the Quill app
New-CsApplicationAccessPolicy `
  -Identity "Quill-Meeting-Pipeline-Policy" `
  -AppIds "<MSGRAPH_CLIENT_ID>" `
  -Description "Restrict Quill meeting pipeline to allow-listed users"

# Grant the policy to specific users whose meetings the pipeline may read
Grant-CsApplicationAccessPolicy `
  -PolicyName "Quill-Meeting-Pipeline-Policy" `
...
Put the three values you collected into `~/.quill/.env`:
...
Quill ships a Graph auth smoke-test. From your Quill install:
...
2. Update `MSGRAPH_CLIENT_SECRET` in `~/.quill/.env` with the new value.
3. Restart the gateway so the new secret is picked up: `quill gateway restart`.
