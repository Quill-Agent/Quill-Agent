...
```bash
quill teams-pipeline token-health
quill teams-pipeline token-health --force-refresh
```
...
```bash
quill teams-pipeline subscriptions
```

### Renew near-expiry subscriptions

```bash
quill teams-pipeline maintain-subscriptions
quill teams-pipeline maintain-subscriptions --dry-run
```
...
#### Option 1: Quill cron (recommended if you already run the Quill gateway)

Quill ships a built-in cron scheduler. The `--no-agent` mode runs a script as the job (rather than using an LLM), and `--script` must point at a file under `~/.quill/scripts/`. First create the script:

```bash
mkdir -p ~/.quill/scripts
cat > ~/.quill/scripts/maintain-teams-subscriptions.sh <<'EOF'
#!/usr/bin/env bash
exec quill teams-pipeline maintain-subscriptions
EOF
chmod +x ~/.quill/scripts/maintain-teams-subscriptions.sh
```

Then register a script-only cron job that runs every 12 hours (gives 6x headroom against the 72h expiry window):

```bash
quill cron create "0 */12 * * *" \
...
Create `/etc/systemd/system/quill-teams-pipeline-maintain.service`:

```ini
[Unit]
Description=Quill Teams pipeline subscription maintenance
After=network-online.target

[Service]
Type=oneshot
User=quill
EnvironmentFile=/etc/quill/env
ExecStart=/usr/local/bin/quill teams-pipeline maintain-subscriptions
```

And `/etc/systemd/system/quill-teams-pipeline-maintain.timer`:

```ini
[Unit]
Description=Run Quill Teams pipeline subscription maintenance every 12 hours
...
sudo systemctl enable --now quill-teams-pipeline-maintain.timer
systemctl list-timers quill-teams-pipeline-maintain.timer
```

#### Option 3: Plain crontab

```cron
0 */12 * * * /usr/local/bin/quill teams-pipeline maintain-subscriptions >> /var/log/quill/teams-pipeline-maintain.log 2>&1
```

Make sure the cron environment has the `MSGRAPH_*` credentials. Simplest fix: source `~/.quill/.env` at the top of a wrapper script that crontab calls.
...
```bash
quill teams-pipeline subscriptions   # should show expirationDateTime advanced
quill teams-pipeline maintain-subscriptions --dry-run   # should show "0 expiring soon" most of the time
```
...
```bash
quill teams-pipeline list
quill teams-pipeline list --status failed
quill teams-pipeline show <job-id>
```

### Replay a stored job

```bash
quill teams-pipeline run <job-id>
```

### Dry-run meeting artifact fetches

```bash
quill teams-pipeline fetch --meeting-id <meeting-id>
quill teams-pipeline fetch --join-web-url "<join-url>"
```
...
- run `quill teams-pipeline maintain-subscriptions --dry-run`
- inspect `quill teams-pipeline list --status failed`
...
- run `quill teams-pipeline validate`
...
Check:
- whether you manually replayed a job with `quill teams-pipeline run`
...
- [ ] `quill teams-pipeline validate` returns an OK snapshot
- [ ] `quill teams-pipeline token-health --force-refresh` succeeds
- [ ] **`maintain-subscriptions` is scheduled** (Quill cron, systemd timer, or crontab — see [Automating subscription renewal](#automating-subscription-renewal-required-for-production)). Without this, Graph subscriptions silently expire within 72 hours.
