---
sidebar_position: 6
title: "Use MCP with Quill"
description: "A practical guide to connecting MCP servers to Quill Agent, filtering their tools, and using them safely in real workflows"
---

# Use MCP with Quill

This guide shows how to actually use MCP with Quill Agent in day-to-day workflows.
...
Use MCP when:
- a tool already exists in MCP form and you do not want to build a native Quill tool
- you want Quill to operate against a local or remote system through a clean RPC layer
- you want fine-grained per-server exposure control
- you want to connect Quill to internal APIs, databases, or company systems without modifying Quill core

Do not use MCP when:
- a built-in Quill tool already solves the job well
...
- Quill remains the agent
- MCP servers contribute tools
- Quill discovers those tools at startup or reload time
...
If you installed Quill with the standard install script, MCP support is already included (the installer runs `uv pip install -e ".[all]"`).
...
- Quill banner/status should show MCP integration when configured
- ask Quill what tools it has available
...
## WSL2: bridge Quill in WSL to Windows Chrome

This is the practical setup when:

- Quill runs inside WSL2
...
In this setup, Quill does **not** connect to Chrome directly. Instead:

- Quill runs in WSL
- Quill starts a local stdio MCP server
...
```text
Quill (WSL) -> MCP stdio bridge -> Windows Chrome
```

### Why this mode is useful

- you keep your real Windows browser profile, cookies, and logins
- Quill stays in its supported Unix environment (WSL2)
- browser control is exposed as MCP tools instead of relying on Quill core browser transport
...
```bash
quill mcp add chrome-devtools-win --command cmd.exe --args /c npx -y chrome-devtools-mcp@latest --autoConnect --no-usage-statistics
```

After saving the server:

```bash
quill mcp test chrome-devtools-win
```

Then start a fresh Quill session or run:
...
Once loaded, Quill can use the MCP-prefixed browser tools directly. For example:
...
If Quill runs in WSL and Chrome runs on Windows, `/browser connect` may fail even though Chrome is open and debuggable.
...
- Start Quill from a Windows-mounted path like `/mnt/c/Users/<you>` or `/mnt/c/workspace/...` when using Windows stdio executables through MCP.
- If you start Quill from `/root` or `/home/...`, Windows may emit a `UNC` current-directory warning before the MCP server starts.
...
There are two categories of MCP-exposed functionality in Quill:
...
2. Quill-added utility wrappers
- filtered with:
  - `tools.resources`
  - `tools.prompts`
...
So Quill will not pretend a server has resources/prompts if it does not.

## Common patterns

### Pattern 1: local project assistant

Use MCP for a repo-local filesystem or git server when you want Quill to reason over a bounded workspace.
...
Now Quill can combine them:

```text
Inspect the local project files, then create a GitHub issue summarizing the bug you find.
```

That is where MCP gets powerful: multi-system workflows without changing Quill core.
...
Because Quill now respects your per-server policy and capability-aware registration. That is expected, and usually desirable.
