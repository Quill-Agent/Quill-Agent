---
sidebar_position: 7
title: "Use SOUL.md with Quill"
description: "How to use SOUL.md to shape Quill Agent's default voice, what belongs there, and how it differs from AGENTS.md and /personality"
---

# Use SOUL.md with Quill

`SOUL.md` is the **primary identity** for your Quill instance. It's the first thing in the system prompt — it defines who the agent is, how it speaks, and what it avoids.

If you want Quill to feel like the same assistant every time you talk to it — or if you want to replace the Quill persona entirely with your own — this is the file to use.
...
- personality
- communication style
- how direct or warm Quill should be
- what Quill should avoid stylistically
- how Quill should relate to uncertainty, disagreement, and ambiguity

In short:
- `SOUL.md` is about who Quill is and how Quill speaks
...
Quill now uses only the global SOUL file for the current instance:

```text
~/.quill/SOUL.md
```

If you run Quill with a custom home directory, it becomes:
...
Quill automatically seeds a starter `SOUL.md` for you if one does not already exist.

That means most users now begin with a real file they can read and edit immediately.

Important:
- if you already have a `SOUL.md`, Quill does not overwrite it
- if the file exists but is empty, Quill adds nothing from it to the prompt

## How Quill uses it

When Quill starts a session, it reads `SOUL.md` from `QUILL_HOME`, scans it for prompt-injection patterns, truncates it if needed, and uses it as the **agent identity** — slot #1 in the system prompt. This means SOUL.md completely replaces the built-in default identity text.
...
That alone can noticeably change how Quill feels.
...
Quill already tries to be helpful and clear. `SOUL.md` should add real personality and style, not restate obvious defaults.
...
# Style
How Quill should sound.

# Avoid
What Quill should not do.

# Defaults
How Quill should behave when ambiguity appears.
```
...
Then restart Quill or start a new session.
...
### I edited SOUL.md but Quill still sounds the same

Check:
- you edited `~/.quill/SOUL.md` or `$QUILL_HOME/SOUL.md`
...
### Quill is ignoring parts of my SOUL.md
