...
This tutorial walks you through setting up a Telegram bot powered by Quill Agent that multiple team members can use. By the end, your team will have a shared AI assistant they can message for help with code, research, system administration, and anything else — secured with per-user authorization.
...
- **Quill Agent installed** on a server or VPS (not your laptop — the bot needs to stay running). Follow the [installation guide](/docs/getting-started/installation) if you haven't yet.
- **A Telegram account** for yourself (the bot owner)
- **An LLM provider configured** — at minimum, an API key for OpenAI, Anthropic, or another supported provider in `~/.quill/.env`

:::tip
A $5/month VPS is plenty for running the gateway. Quill itself is lightweight — the LLM API calls are what cost money, and those happen remotely.
:::
...
2. **Send `/newbot`** — BotFather will ask you two things:
   - **Display name** — what users see (e.g., `Team Quill Assistant`)
   - **Username** — must end in `bot` (e.g., `myteam_quill_bot`)
...
4. **Set a description** (optional but recommended):
   ...
   ```
   Team AI assistant powered by Quill Agent. DM me for help with code, research, debugging, and more.
   ```
...
```
[Gateway] Starting Quill Gateway...
...
# View live logs
journalctl --user -u quill-gateway -f
...
quill gateway start
quill gateway stop
tail -f ~/.quill/logs/gateway.log
```

:::tip macOS PATH
The launchd plist captures your shell PATH at install time so gateway subprocesses can find tools like Node.js and ffmpeg. If you install new tools later, re-run `quill gateway install` to update the plist.
:::
...
```bash
quill gateway stop && quill gateway start
```
...
quill pairing revoke telegram 987654321
...
**Option 2:** Set it manually in `~/.quill/.env`:
...
Control how much detail the bot shows when using tools. In `~/.quill/config.yaml`:
...
Customize how the bot communicates by editing `~/.quill/SOUL.md`:

For a full guide, see [Use SOUL.md with Quill](/docs/guides/use-soul-with-quill).
...
```bash
# From the CLI
quill cron list          # View all scheduled jobs
quill cron status        # Check if scheduler is running
...
| What | Location |
|------|----------|
| Gateway logs | `journalctl --user -u quill-gateway` (Linux) or `~/.quill/logs/gateway.log` (macOS) |
| Cron job output | `~/.quill/cron/output/{job_id}/{timestamp}.md` |
| Cron job definitions | `~/.quill/cron/jobs.json` |
| Pairing data | `~/.quill/pairing/` |
| Session history | `~/.quill/sessions/` |
