---
sidebar_position: 8
title: "Use Voice Mode with Quill"
description: "A practical guide to setting up and using Quill voice mode across CLI, Telegram, Discord, and Discord voice channels"
---

# Use Voice Mode with Quill
...
- you want Quill sitting in a Discord voice channel for live conversation
...
There are really three different voice experiences in Quill.
...
## Step 1: make sure normal Quill works first
...
Quill supports both local and cloud speech stacks.
...
### If you use `quill setup`

If you choose NeuTTS in the setup wizard, Quill checks whether `neutts` is already installed. If it is missing, the wizard tells you NeuTTS needs the Python package `neutts` and the system package `espeak-ng`, offers to install them for you, installs `espeak-ng` with your platform package manager, and then runs:
...
4. Quill transcribes and responds
...
- asking Quill to structure your thoughts in real time

#### Accessibility / low-typing sessions

If typing is inconvenient, voice mode is one of the fastest ways to stay in the full Quill loop.

## Tuning CLI behavior

### Silence threshold

If Quill starts/stops too aggressively, tune:
...
Quill stays a normal chat bot, but can speak replies.
...
- you want Quill to function like a portable research or ops assistant
...
Quill joins a Discord VC, listens to user speech, transcribes it, runs the normal agent pipeline, and speaks replies back into the channel.
...
- Quill detects speech boundaries
- transcripts are posted in the associated text channel
- Quill responds in text and audio
...
1. get text Quill working
2. install `quill-agent[voice]`
