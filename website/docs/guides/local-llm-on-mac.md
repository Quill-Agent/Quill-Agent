...
Both expose an OpenAI-compatible `/v1/chat/completions` endpoint. Quill works with either one — just point it at `http://localhost:8080` or `http://localhost:8000`.
...
Quill automatically detects local endpoints (localhost, LAN IPs) and relaxes its streaming timeouts. No configuration needed for most setups.
...
QUILL_STREAM_READ_TIMEOUT=1800
```

| Timeout | Default | Local auto-adjustment | Env var override |
|---------|---------|----------------------|------------------|
| Stream read (socket-level) | 120s | Raised to 1800s | `QUILL_STREAM_READ_TIMEOUT` |
| Stale stream detection | 180s | Disabled entirely | `QUILL_STREAM_STALE_TIMEOUT` |
