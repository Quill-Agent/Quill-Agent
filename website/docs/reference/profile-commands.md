...
This page covers all commands related to [Quill profiles](../user-guide/profiles.md). For general CLI commands, see [CLI Commands Reference](./cli-commands.md).
...
Top-level command for managing profiles. Running `quill profile` without a subcommand shows help.
...
```bash
quill profile use <name>
```

Sets `<name>` as the active profile. All subsequent `quill` commands (without `-p`) will use this profile.
...
quill profile use default
```

## `quill profile create`

```bash
quill profile create <name> [options]
```
...
| `--description "<text>"` | One- or two-sentence description of what this profile is good at. Used by the kanban orchestrator to route tasks based on role instead of profile name alone. Skip and add later via `quill profile describe`. Persisted in `<profile_dir>/profile.yaml`. |
| `--no-skills` | Create an **empty** profile with zero bundled skills enabled. Writes a `.no-skills` marker into the profile so future `quill update` runs won't re-seed the bundled set, and refuses to combine with `--clone` / `--clone-all` (which would copy skills in anyway). Useful for narrow orchestrator profiles or sandbox profiles that should not inherit the full skill catalog. |
...
quill profile create mybot

# Clone config only from current profile
quill profile create work --clone

# Clone everything from current profile
quill profile create backup --clone-all

# Clone config from a specific profile
quill profile create work2 --clone --clone-from work
```

## `quill profile describe`

```bash
quill profile describe [<name>] [options]
```
...
quill profile describe researcher

# Set it explicitly
quill profile describe researcher --text "Reads source code and writes findings."

# Let the LLM generate one
quill profile describe researcher --auto

# Fill in descriptions for every profile that doesn't have one
quill profile describe --all --auto
```

## `quill profile delete`

```bash
quill profile delete <name> [options]
```
...
```bash
quill profile delete mybot
quill profile delete mybot --yes
```
...
```bash
quill profile show <name>
```

Displays details about a profile including its home directory, configured model, gateway status, skills count, and configuration file status.

This shows the profile's Quill home directory, not the terminal working directory. Terminal commands start from `terminal.cwd` (or the launch directory on the local backend when `cwd: "."`).
...
```bash
$ quill profile show work
Profile: work
Path:    ~/.quill/profiles/work
...
```bash
quill profile alias <name> [options]
```

Regenerates the shell alias script at `~/.local/bin/<name>`. Useful if the alias was accidentally deleted or if you need to update it after moving your Quill installation.
...
```bash
quill profile alias work
# Creates/updates ~/.local/bin/work

quill profile alias work --name mywork
# Creates ~/.local/bin/mywork

quill profile alias work --remove
# Removes the wrapper script
```

## `quill profile rename`

```bash
quill profile rename <old-name> <new-name>
```
...
```bash
quill profile rename mybot assistant
# ~/.quill/profiles/mybot → ~/.quill/profiles/assistant
# ~/.local/bin/mybot → ~/.local/bin/assistant
```

## `quill profile export`

```bash
quill profile export <name> [options]
```
...
```bash
quill profile export work
# Creates work.tar.gz in the current directory

quill profile export work -o ./work-2026-03-29.tar.gz
```

## `quill profile import`

```bash
quill profile import <archive> [options]
```
...
```bash
quill profile import ./work-2026-03-29.tar.gz
# Infers profile name from the archive

quill profile import ./work-2026-03-29.tar.gz --name work-restored
```
...
:::info
`quill profile export` / `import` are still the right commands for
...
### `quill profile install`

```bash
quill profile install <source> [--name <name>] [--alias] [--force] [--yes]
```
...
| `--alias` | Also create a shell wrapper (e.g. `telemetry` → `quill -p telemetry`). |
...
quill profile install github.com/kyle/telemetry-distribution --alias

# Install from a full HTTPS git URL
quill profile install https://github.com/kyle/telemetry-distribution.git

# Install from SSH
quill profile install git@github.com:kyle/telemetry-distribution.git

# Install from a local directory during development
quill profile install ./telemetry/
```

### `quill profile update`

```bash
quill profile update <name> [--force-config] [--yes]
```
...
```bash
quill profile info <name>
```

Prints the profile's distribution manifest — name, version, required
Quill version, author, env var requirements, the source URL/path, and
...
`quill profile list` also shows the distribution name and version in a
`Distribution` column, and `quill profile show <name>` / `delete <name>`
...
quill profile install git@github.com:your-org/internal-assistant.git

# Uses your git credential helper
quill profile install https://github.com/your-org/internal-assistant.git
```
...
`quill_requires` supports `>=`, `<=`, `==`, `!=`, `>`, `<`, or a bare
version (treated as `>=`). Install fails with a clear error if the current
Quill version doesn't satisfy the spec.
...
2. Initialize a git repo (or use an existing one) and push to GitHub /
   GitLab / any host Quill can clone from.
3. Tell recipients to run `quill profile install <your-repo-url>`.
...
## `quill -p` / `quill --profile`

```bash
quill -p <name> <command> [options]
quill --profile <name> <command> [options]
```

Global flag to run any Quill command under a specific profile without changing the sticky default. This overrides the active profile for the duration of the command.
...
```bash
quill -p work chat -q "Check the server status"
quill --profile dev gateway start
quill -p personal skills list
quill -p work config edit
```

## `quill completion`

```bash
quill completion <shell>
```
...
- `quill profile <TAB>` — subcommands (list, use, create, etc.)
- `quill profile use <TAB>` — profile names
- `quill -p <TAB>` — profile names
