...
Quill Agent uses a SQLite database (`~/.quill/state.db`) to persist session
metadata, full message history, and model configuration across CLI and gateway
sessions. This replaces the earlier per-session JSONL file approach.

Source file: `quill_state.py`


## Architecture Overview

```
~/.quill/state.db (SQLite, WAL mode)
...
Multiple quill processes (gateway + CLI sessions + worktree agents) share one
...
db = SessionDB()                           # Default: ~/.quill/state.db
...
Default path: `~/.quill/state.db`

This is derived from `quill_constants.get_quill_home()` which resolves to
`~/.quill/` by default, or the value of `QUILL_HOME` environment variable.
