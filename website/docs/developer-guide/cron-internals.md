...
description: "How Quill stores, schedules, edits, pauses, skill-loads, and delivers cron jobs"
---
...
| `quill_cli/cron.py` | CLI `quill cron` subcommands |
...
Jobs are stored in `~/.quill/cron/jobs.json` with atomic write semantics (write to temp file, then rename). Each job record contains:
...
In CLI mode, cron jobs only fire when `quill cron` commands are run or during active CLI sessions.
...
```python
# ~/.quill/scripts/check_competitors.py
...
2. **Environment variable** — `QUILL_CRON_SCRIPT_TIMEOUT`
...
| Local file | `local` | Save to `~/.quill/cron/output/` |
...
The scheduler uses cross-process file-based locking (`fcntl.flock` on Unix, `msvcrt.locking` on Windows) to prevent overlapping ticks from executing the same due-job batch twice — even between the gateway's in-process ticker and a standalone `quill cron` / manual `tick()` call. If the lock cannot be acquired, `tick()` returns 0 immediately.

## CLI Interface

The `quill cron` CLI provides direct job management:

```bash
quill cron list                    # Show all jobs
quill cron create                  # Interactive job creation (alias: add)
quill cron edit <job_id>           # Edit job configuration
quill cron pause <job_id>          # Pause a running job
quill cron resume <job_id>         # Resume a paused job
quill cron run <job_id>            # Trigger immediate execution
quill cron remove <job_id>         # Delete a job
```
