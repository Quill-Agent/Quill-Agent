...
sidebar_position: 1
title: "Architecture"
description: "Quill Agent internals — major subsystems, execution paths, data flow, and where to read next"
---

# Architecture

This page is the top-level map of Quill Agent internals. Use it to orient yourself in the codebase, then dive into subsystem-specific docs for implementation details.
...
│ quill_state.py   │              │ Browser (5 backends)  │
...
├── cli.py                    # QuillCLI — interactive terminal UI (large file)
├── model_tools.py            # Tool discovery, schema collection, dispatch
├── toolsets.py               # Tool groupings and platform presets
├── quill_state.py           # SQLite session/state database with FTS5
├── quill_constants.py       # QUILL_HOME, profile-aware paths
...
│
├── quill_cli/               # CLI subcommands and setup
│   ├── main.py               # Entry point — all `quill` subcommands (large file)
...
│   ├── skills_config.py      # quill skills — enable/disable per platform
│   ├── skills_hub.py         # /skills slash command
│   ├── tools_config.py       # quill tools — enable/disable per platform
│   ├── plugins.py            # PluginManager — discovery, loading, hooks
│   ├── callbacks.py          # Terminal callbacks (clarify, sudo, approval)
│   └── gateway.py            # quill gateway start/stop
│
...
```text
User input → QuillCLI.process_input()
...
- **`prompt_builder.py`** — Assembles the system prompt from: personality (SOUL.md), memory (MEMORY.md, USER.md), skills, context files (AGENTS.md, .quill.md), tool-use guidance, and model-specific instructions
...
Three discovery sources: `~/.quill/plugins/` (user), `.quill/plugins/` (project), and pip entry points. Plugins register tools, hooks, and CLI commands through a context API. Two specialized plugin types exist: memory providers (`plugins/memory/`) and context engines (`plugins/context_engine/`). Both are single-select — only one of each can be active at a time, configured via `quill plugins` or `config.yaml`.

→ [Plugin Guide](/docs/guides/build-a-quill-plugin), [Memory Provider Plugin](./memory-provider-plugin.md)
...
Exposes Quill as an editor-native agent over stdio/JSON-RPC for VS Code, Zed, and JetBrains.
...
| **Profile isolation** | Each profile (`quill -p <name>`) gets its own QUILL_HOME, config, memory, sessions, and gateway PID. Multiple profiles run concurrently. |
