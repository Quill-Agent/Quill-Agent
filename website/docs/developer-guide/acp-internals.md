...
The ACP adapter wraps Quill' synchronous `AIAgent` in an async JSON-RPC stdio server.
...
```text
quill acp / quill-acp / python -m acp_adapter
  -> acp_adapter.entry.main()
  -> parse --version / --check / --setup before server startup
  -> load ~/.quill/.env
  -> configure stderr logging
  -> construct QuillACPAgent
  -> acp.run_agent(agent, use_unstable_protocol=True)
```

The Zed ACP Registry path launches the same adapter through `uvx --from 'quill-agent[acp]==<version>' quill-acp`, pointed at the `quill-agent` PyPI release.
...
- `allow_once` -> Quill `once`
- `allow_always` -> Quill `always`
- reject options -> Quill `deny`

Timeouts and bridge failures deny by default.

### Tool rendering helpers

`acp_adapter/tools.py` maps Quill tools to ACP tool kinds and builds editor-facing content.
...
new_session(cwd)
  -> create SessionState
  -> create AIAgent(platform="acp", enabled_toolsets=["quill-acp"])
...
Instead it reuses Quill' runtime resolver:

- `acp_adapter/auth.py`
- `quill_cli/runtime_provider.py`

So ACP advertises and uses the currently configured Quill provider/credentials. It also always advertises a terminal setup auth method (`quill-setup`, args `--setup`) so first-run registry clients can open Quill' interactive model/provider configuration before starting a normal ACP session.
...
- ACP sessions are persisted to the shared `~/.quill/state.db` (SessionDB) and transparently restored across process restarts; they appear in `session_search`
