...
[PR #12550](https://github.com/YOUR_USERNAME/quill-agent/pull/12550) proposed a
...
(`quill-dialog-bridge.invalid`). `Fetch.enable` intercepts those XHRs
...
One `asyncio.Task` running in a background daemon thread per Quill `task_id`.
...
- `toolsets.py` — register `browser_dialog` in `browser`, `quill-acp`, `quill-api-server`, core toolsets (gated on CDP reachability)
...
- `quill_cli/config.py` — add `browser.dialog_policy` and `browser.dialog_timeout_s` to `DEFAULT_CONFIG`
