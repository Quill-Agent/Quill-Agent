...
sidebar_position: 2
title: "Adding Tools"
description: "How to add a new tool to Quill Agent — schemas, handlers, registration, and toolsets"
---
...
This page is for adding a **built-in Quill tool** to the repository itself.
If you want a personal, project-local, or otherwise custom tool without
modifying Quill core, use the plugin route instead:

- [Plugins](/docs/user-guide/features/plugins)
- [Build a Quill Plugin](/docs/guides/build-a-quill-plugin)
...
2. **`toolsets.py`** — add tool name to `_QUILL_CORE_TOOLS` (or a specific toolset)
...
If your tool requires an API key, add it to `quill_cli/config.py`:
