...
description: "Run Quill Agent directly on an Android phone with Termux"
---

# Quill on Android with Termux

This is the tested path for running Quill Agent directly on an Android phone through [Termux](https://termux.dev/).
...
That does not stop Quill from working well as a phone-native CLI agent — it just means the recommended mobile install is intentionally narrower than the desktop/server install.

---

## Option 1: One-line installer

Quill now ships a Termux-aware installer path:
...
- links `quill` into `$PREFIX/bin` so it stays on your Termux PATH
...
### 5. Put `quill` on your Termux PATH

```bash
ln -sf "$PWD/venv/bin/quill" "$PREFIX/bin/quill"
```

`$PREFIX/bin` is already on PATH in Termux, so this makes the `quill` command persist across new shells without re-activating the venv every time.
...
Or set keys directly in `~/.quill/.env`.
...
### `quill doctor` says ripgrep or Node is missing
