...
Quill uses two kinds of model slots:
...
- **Right** — the curated model list for the selected provider. These are the agentic models Quill recommends for that provider, not the raw `/models` dump (which on OpenRouter includes 400+ models including TTS, image generators, and rerankers).

Type in the filter box to narrow by provider name, slug, or model ID.

Pick a model, hit **Switch**, and Quill writes it to `~/.quill/config.yaml` under the `model` section. **This applies to new sessions only** — any chat tab you already have open keeps running whatever model it started with. To hot-swap the current chat, use the `/model` slash command inside it.
...
Every auxiliary task defaults to `auto` — meaning Quill uses your main model for that job too. Override a specific task when you want a cheaper or faster model for a side-job.
...
| **Skills Hub** | `quill skills search` uses this. Usually fine at `auto`. |
...
When you save via the dashboard, Quill writes to `~/.quill/config.yaml`:
...
`provider: auto` with `model: ''` tells Quill to use the main model for that task.

## When does it take effect?

- **CLI** (`quill chat`): next `quill chat` invocation.
- **Gateway** (Telegram, Discord, Slack, etc.): next *new* session. Existing sessions keep their model. Restart the gateway (`quill gateway restart`) if you want to force all sessions to pick up the change.
...
Quill lists a provider only if it has a working credential. Check **Keys** in the sidebar — you should see one of: an API key, a successful OAuth, or a custom endpoint URL. If the provider you want isn't there, run `quill setup` to wire it up, or go to **Keys** and add the env var.
...
### I picked a model but Quill switched providers on me
...
### `quill model` subcommand

```bash
quill model            # Interactive provider + model picker (the canonical way to switch defaults)
```

`quill model` walks you through picking a provider, authenticating (OAuth flows open a browser; API-key providers prompt for the key), and then choosing a specific model from that provider's curated catalog. The choice is written to `model.provider` and `model.model` in `~/.quill/config.yaml`.

To list providers/models without launching the picker, use the dashboard or the REST endpoints below. To inspect what the CLI will actually use right now: `quill config get model` and `quill status`.

### Direct config edit

Edit `~/.quill/config.yaml` and restart whatever reads it. See the [Configuration reference](./configuration.md) for the full schema.
...
curl -H "X-Quill-Session-Token: $TOKEN" http://localhost:PORT/api/model/options

# Read current main + auxiliary assignments
curl -H "X-Quill-Session-Token: $TOKEN" http://localhost:PORT/api/model/auxiliary

# Set the main model
curl -X POST -H "Content-Type: application/json" -H "X-Quill-Session-Token: $TOKEN" \
