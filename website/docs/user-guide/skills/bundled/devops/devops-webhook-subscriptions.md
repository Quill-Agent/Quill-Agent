...
Create dynamic webhook subscriptions so external services (GitHub, GitLab, Stripe, CI/CD, IoT sensors, monitoring tools) can trigger Quill agent runs by POSTing events to a URL.
...
All management is via the `quill webhook` CLI command:

### Create a subscription
```bash
quill webhook subscribe <name> \
...
```bash
quill webhook remove <name>
```

### Test a subscription
```bash
quill webhook test <name>
quill webhook test <name> --payload '{"key": "value"}'
```
...
### GitHub: PR reviews
```bash
quill webhook subscribe github-prs \
  --events "pull_request" \
...
```bash
quill webhook subscribe ci-builds \
  --events "pipeline" \
...
```bash
quill webhook subscribe alerts \
...
- Subscriptions persist to `~/.quill/webhook_subscriptions.json`

## How It Works

1. `quill webhook subscribe` writes to `~/.quill/webhook_subscriptions.json`
...
1. **Is the gateway running?** Check with `systemctl --user status quill-gateway` or `ps aux | grep gateway`
2. **Is the webhook server listening?** `curl http://localhost:8644/health` should return `{"status": "ok"}`
3. **Check gateway logs:** `grep webhook ~/.quill/logs/gateway.log | tail -20`
