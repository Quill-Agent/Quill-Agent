...
- Give it a name like "quill-agent"
- Select scopes:
...
- Give it a title like "quill-agent-&lt;machine-name>"
...
elif [ -f ~/.quill/.env ] && grep -q "^GITHUB_TOKEN=" ~/.quill/.env; then
  export GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" ~/.quill/.env | head -1 | cut -d= -f2 | tr -d '\n\r')
  echo "AUTH_METHOD=curl"
