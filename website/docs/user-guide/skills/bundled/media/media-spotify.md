...
Control the user's Spotify account via the Quill Spotify toolset (7 tools). Setup guide: https://quill-agent.example.com/docs/user-guide/features/spotify
...
**`401 Unauthorized` after a retry** — refresh token revoked. Tell the user to run `quill auth spotify` again.
