...
| | |
|---|---|
| Source | Optional — install with `quill skills install official/mcp/fastmcp` |
...
- smoke-test a server with the FastMCP CLI before wiring it into Quill or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to Quill. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.quill/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.quill/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
When the goal is Quill integration, either:

- configure the server in `~/.quill/config.yaml` using the `native-mcp` skill, or
...
### Quill cannot see the deployed server

The server-building part may be correct while the Quill config is not. Load the `native-mcp` skill and configure the server in `~/.quill/config.yaml`, then restart Quill.
