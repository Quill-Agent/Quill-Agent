...
| | |
|---|---|
| Source | Optional — install with `quill skills install official/productivity/here-now` |
...
- Skill helper paths:
  - `${QUILL_SKILL_DIR}/scripts/publish.sh` for publishing sites
  - `${QUILL_SKILL_DIR}/scripts/drive.sh` for private Drive storage

## Create a site

```bash
PUBLISH="${QUILL_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --client quill
```
...
```bash
PUBLISH="${QUILL_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --slug {slug} --client quill
```
...
```bash
DRIVE="${QUILL_SKILL_DIR}/scripts/drive.sh"
bash "$DRIVE" default
...
| `--client {name}`      | Agent name for attribution (e.g. `quill`)    |
