...
| | |
|---|---|
| Source | Optional — install with `quill skills install official/productivity/canvas` |
...
4. Name the token (e.g., "Quill Agent"), set an optional expiry, and click **Generate Token**
5. Copy the token and add to `~/.quill/.env`:
...
```bash
CANVAS="python $QUILL_HOME/skills/productivity/canvas/scripts/canvas_api.py"
