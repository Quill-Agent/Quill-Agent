...
description: "Configure and use Honcho memory with Quill -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session su..."
---
...
Configure and use Honcho memory with Quill -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session summaries, and context budget enforcement. Use when setting up Honcho, troubleshooting memory, managing profiles with Honcho peers, or tuning observation, recall, and dialectic settings.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `quill skills install official/autonomous-ai-agents/honcho` |
...
# Honcho Memory for Quill

Honcho provides AI-native cross-session user modeling. It learns who the user is across conversations and gives every Quill profile its own peer identity while sharing a unified view of the user.
...
See: https://docs.honcho.dev/v3/guides/integrations/quill#running-honcho-locally-with-quill

### Verify

```bash
quill honcho status    # shows resolved config, connection test, peer info
```
...
3. **AI peer card** -- the identity card for this Quill profile's AI peer
...
Honcho models conversations as interactions between **peers**. Quill creates two peers per session:

- **User peer** (`peerName`): represents the human. Honcho builds a user representation from observed messages.
- **AI peer** (`aiPeer`): represents this Quill instance. Each profile gets its own AI peer so agents develop independent views.
...
| `per-session` | New Honcho session each Quill run |
| `global` | Single session across all directories |

Manual override: `quill honcho map my-project-name`
...
Each Quill profile gets its own Honcho AI peer while sharing the same workspace (user context). This means:
...
# creates host block quill.coder, AI peer "coder", inherits config from default
```

What `--clone` does for Honcho:
1. Creates a `quill.coder` host block in `honcho.json`
...
```bash
quill honcho sync    # creates host blocks for all profiles that don't have one yet
```
...
Guidelines for Quill when Honcho memory is active.
...
Config file: `$QUILL_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global).
...
Run `quill honcho setup`. Ensure `memory.provider: honcho` is in `~/.quill/config.yaml`.

### Memory not persisting across sessions
Check `quill honcho status` -- verify `saveMessages: true` and `writeFrequency` isn't `session` (which only writes on exit).

### Profile not getting its own peer
Use `--clone` when creating: `quill profile create <name> --clone`. For existing profiles: `quill honcho sync`.
...
| Command | Description |
|---------|-------------|
| `quill honcho setup` | Interactive setup wizard (cloud/local, identity, observation, recall, sessions) |
| `quill honcho status` | Show resolved config, connection test, peer info for active profile |
| `quill honcho enable` | Enable Honcho for the active profile (creates host block if needed) |
| `quill honcho disable` | Disable Honcho for the active profile |
| `quill honcho peer` | Show or update peer names (`--user <name>`, `--ai <name>`, `--reasoning <level>`) |
| `quill honcho peers` | Show peer identities across all profiles |
| `quill honcho mode` | Show or set recall mode (`hybrid`, `context`, `tools`) |
| `quill honcho tokens` | Show or set token budgets (`--context <N>`, `--dialectic <N>`) |
| `quill honcho sessions` | List known directory-to-session-name mappings |
| `quill honcho map <name>` | Map current working directory to a Honcho session name |
| `quill honcho identity` | Seed AI peer identity or show both peer representations |
| `quill honcho sync` | Create host blocks for all Quill profiles that don't have one yet |
| `quill honcho migrate` | Step-by-step migration guide from OpenClaw native memory to Quill + Honcho |
