...
| | |
|---|---|
| Source | Optional — install with `quill skills install official/security/1password` |
| Path | `optional-skills/security/1password` |
| Version | `1.0.0` |
| Author | arceus77-7, enhanced by Quill Agent |
| License | MIT |
...
- `tmux` available for stable authenticated sessions during Quill terminal calls (desktop app flow only)
...
### Service Account (recommended for Quill)

Set `OP_SERVICE_ACCOUNT_TOKEN` in `~/.quill/.env` (the skill will prompt for this on first load).
...
## Quill Execution Pattern (desktop app flow)

Quill terminal commands are non-interactive by default and can lose auth context between calls.
...
```bash
SOCKET_DIR="${TMPDIR:-/tmp}/quill-tmux-sockets"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/quill-op.sock"
