...
| | |
|---|---|
| Source | Optional — install with `quill skills install official/research/darwinian-evolver` |
| Path | `optional-skills/research/darwinian-evolver` |
| Version | `0.1.0` |
| Author | Bihruze (Asahi0x), Quill Agent |
| License | MIT |
...
import upstream classes into Quill itself.
...
```bash
mkdir -p ~/.quill/cache/darwinian-evolver && cd ~/.quill/cache/darwinian-evolver
...
```bash
cd ~/.quill/cache/darwinian-evolver/darwinian_evolver \
...
Open `~/.quill/cache/darwinian-evolver/darwinian_evolver/darwinian_evolver/lineage_visualizer.html`
...
SKILL_DIR=~/.quill/skills/research/darwinian-evolver
DE_DIR=~/.quill/cache/darwinian-evolver/darwinian_evolver
...
7. **AGPL.** Never `from darwinian_evolver import ...` inside Quill core.
   Custom driver scripts under `~/.quill/skills/...` are user-side and fine.
