...
sidebar_position: 4
title: "Slack"
description: "Set up Quill Agent as a Slack bot using Socket Mode"
---

# Slack Setup

Connect Quill Agent to Slack as a bot using Socket Mode. Socket Mode uses WebSockets instead of
public HTTP endpoints, so your Quill instance doesn't need to be publicly accessible — it works
behind firewalls, on your laptop, or on a private server.

:::warning Classic Slack Apps Deprecated
Classic Slack apps (using RTM API) were **fully deprecated in March 2025**. Quill uses the modern
...
The fastest path is to paste a manifest Quill generates for you. It
...
### Option A: From a Quill-generated manifest (recommended)

1. Generate the manifest:
   ```bash
   quill slack manifest --write
   ```
   This writes `~/.quill/slack-manifest.json` and prints paste-in
   instructions.
...
4. Enter an app name (e.g., "Quill Agent") and select your workspace
5. Click **Create App**
...
it will only work in DMs. Without `files:read`, Quill can chat but **cannot reliably read user-uploaded attachments**.
...
3. You'll be prompted to create an **App-Level Token**:
   - Name it something like `quill-socket` (the name doesn't matter)
...
Even with all the correct scopes and event subscriptions, Slack will not allow users to send direct messages to the bot unless the Messages Tab is enabled. This is a Slack platform requirement, not a Quill configuration issue.
:::
...
Quill uses Slack **Member IDs** (not usernames or display names) for the allowlist.
...
```bash
quill gateway setup    # Select Slack when prompted
```

Then start the gateway:

```bash
quill gateway              # Foreground
quill gateway install      # Install as a user service
sudo quill gateway install --system   # Linux only: boot-time system service
```
...
Every Quill command (`/btw`, `/stop`, `/new`, `/model`, `/help`, ...)
...
Quill command with its description.

Under the hood: Quill ships with a generated Slack app manifest (see
Step 1, Option A) that declares every command in
[`COMMAND_REGISTRY`](https://github.com/YOUR_USERNAME/quill-agent/blob/main/quill_cli/commands.py)
...
When Quill adds new commands (e.g. after `quill update`), regenerate
...
3. Paste the new contents of `~/.quill/slack-manifest.json`
4. **Save**. Slack will prompt to reinstall the app if scopes or slash
   commands changed.

### Legacy `/quill <subcommand>` still works

For backward compatibility with older manifests, you can still type
`/quill btw run the tests` — Quill routes it the same way as `/btw
run the tests`. Free-form questions also work: `/quill what's the
...
Slack never delivers them to Quill.

As a workaround, Quill recognises a leading `!` as an alternate
command prefix that works in threads (and anywhere else). Type
`!queue`, `!stop`, `!model gpt-5.4`, etc. as a regular thread reply —
Quill treats it identically to the slash form and replies in the same
thread.
...
```bash
quill slack manifest --slashes-only > /tmp/slashes.json
```
...
Understanding how Quill behaves in different contexts:

| Context | Behavior |
|---------|----------|
| **DMs** | Bot responds to every message — no @mention needed |
| **Channels** | Bot **only responds when @mentioned** (e.g., `@Quill Agent what time is it?`). In channels, Quill replies in a thread attached to that message. |
| **Threads** | If you @mention Quill inside an existing thread, it replies in that same thread. Once the bot has an active session in a thread, **subsequent replies in that thread do not require @mention** — the bot follows the conversation naturally. |
...
Beyond the required environment variables from Step 8, you can customize Slack bot behavior through `~/.quill/config.yaml`.
...
When `true` (the default), each user in a shared channel gets their own isolated conversation session. Two people talking to Quill in `#general` will have separate histories and contexts.
...
slack:
  ...
  # must @mention the bot before Quill will respond.
...
Set `SLACK_HOME_CHANNEL` to a channel ID where Quill will deliver scheduled messages,
...
Make sure the bot has been **invited to the channel** (`/invite @Quill Agent`).

---

## Multi-Workspace Support

Quill can connect to **multiple Slack workspaces** simultaneously using a single gateway instance. Each workspace is authenticated independently with its own bot user ID.
...
In addition to tokens in the environment or config, Quill also loads tokens from an **OAuth token file** at:

```
~/.quill/slack_tokens.json
```
...
- When a message arrives, Quill uses the correct workspace-specific client to respond.
...
| Bot works in DMs but not in channels | **Most common issue.** Add `message.channels` and `message.groups` to event subscriptions, reinstall the app, and invite the bot to the channel with `/invite @Quill Agent` |
...
| Bot responds but can't post in a channel | Invite the bot to the channel with `/invite @Quill Agent` |
| Bot can chat but can't read uploaded images/files | Add `files:read`, then **reinstall** the app. Quill now surfaces attachment access diagnostics in-chat when Slack returns scope/auth/permission failures. |
...
7. ✅ Bot was **invited** to the channel (`/invite @Quill Agent`)
...
- Tokens should be stored in `~/.quill/.env` (file permissions `600`)
