...
Connect Quill to WeCom (Enterprise WeChat) as a self-built enterprise application using the callback/webhook model.

:::info WeCom Bot vs WeCom Callback
Quill supports two WeCom integration modes:
...
3. Quill decrypts the message, queues it for the agent
...
(Use `quill gateway start` only after `quill gateway install` has registered the systemd/launchd service.)
