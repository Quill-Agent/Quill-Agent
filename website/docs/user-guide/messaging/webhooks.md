...
<div style={{position: 'relative', width: '100%', aspectRatio: '16 / 9', marginBottom: '1.5rem'}}>
  <iframe
    ...
    title="Quill Agent — Webhooks Tutorial"
...
1. Enable via `quill gateway setup` or environment variables
2. Define routes in `config.yaml` **or** create them dynamically with `quill webhook subscribe`
...
Add the `github-pr` route to your `~/.quill/config.yaml` as shown in the example above.
...
Open a pull request on the repository. The webhook fires, Quill processes the event, and posts a review comment on the PR.
...
```bash
quill webhook subscribe antenna-matches \
  --deliver telegram \
...
In addition to static routes in `config.yaml`, you can create webhook subscriptions dynamically using the `quill webhook` CLI command. This is especially useful when the agent itself needs to set up event-driven triggers.
...
```bash
quill webhook remove github-issues
```

### Test a subscription

```bash
quill webhook test github-issues
quill webhook test github-issues --payload '{"issue": {"number": 42, "title": "Test"}}'
```

### How dynamic subscriptions work

- Subscriptions are stored in `~/.quill/webhook_subscriptions.json`
...
The agent can create subscriptions via the terminal tool when guided by the `webhook-subscriptions` skill. Ask the agent to "set up a webhook for GitHub issues" and it will run the appropriate `quill webhook subscribe` command.
...
- Run the gateway in foreground to see logs: `quill gateway run`
