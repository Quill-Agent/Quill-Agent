...
description: "Chat with Quill from Telegram, Discord, Slack, WhatsApp, Signal, SMS, Email, Home Assistant, Mattermost, Matrix, DingTalk, Yuanbao, Microsoft Teams, LINE, Webhooks, or any OpenAI-compatible frontend via the API server — architecture and setup overview"
---

# Messaging Gateway

Chat with Quill from Telegram, Discord, Slack, WhatsApp, Signal, SMS, Email, Home Assistant, Mattermost, Matrix, DingTalk, Feishu/Lark, WeCom, Weixin, BlueBubbles (iMessage), QQ, Yuanbao, Microsoft Teams, LINE, or your browser. The gateway is a single background process that connects to all your configured platforms, handles sessions, runs cron jobs, and delivers voice messages.

For the full voice feature set — including CLI microphone mode, spoken replies in messaging, and Discord voice-channel conversations — see [Voice Mode](/docs/user-guide/features/voice-mode) and [Use Voice Mode with Quill](/docs/guides/use-voice-mode-with-quill).
...
```mermaid
flowchart TB
    subgraph Gateway["Quill Gateway"]
...
```bash
quill gateway setup        # Interactive setup for all messaging platforms
```
...
```bash
quill gateway              # Run in foreground
quill gateway setup        # Configure messaging platforms interactively
quill gateway install      # Install as a user service (Linux) / launchd service (macOS)
sudo quill gateway install --system   # Linux only: install a boot-time system service
quill gateway start        # Start the default service
quill gateway stop         # Stop the default service
quill gateway status       # Check default service status
quill gateway status --system         # Linux only: inspect the system service explicitly
```
...
| `/update` | Update Quill Agent to the latest version |
...
Configure per-platform overrides in `~/.quill/gateway.json`:
...
quill pairing approve telegram XKGH5N7P

# Other pairing commands:
quill pairing list          # View pending + approved users
quill pairing revoke telegram 123456789  # Remove access
```
...
The first time you message a busy agent on any platform, Quill appends a one-line reminder to the busy-ack explaining the knob (`"💡 First-time tip — …"`). The reminder fires once per install — a flag under `onboarding.seen.busy_input_prompt` latches it. Delete that key to see the tip again.
...
Control how much tool activity is displayed in `~/.quill/config.yaml`:
...
When the agent running a background session uses `terminal(background=true)` to start long-running processes (servers, builds, etc.), the gateway can push status updates to your chat. Control this with `display.background_process_notifications` in `~/.quill/config.yaml`:
...
```bash
QUILL_BACKGROUND_NOTIFICATIONS=result
```
...
```bash
quill gateway install               # Install as user service
quill gateway start                 # Start the service
quill gateway stop                  # Stop the service
quill gateway status                # Check status
journalctl --user -u quill-gateway -f  # View logs

# Enable lingering (keeps running after logout)
sudo loginctl enable-linger $USER

# Or install a boot-time system service that still runs as your user
sudo quill gateway install --system
sudo quill gateway start --system
sudo quill gateway status --system
journalctl -u quill-gateway -f
```

Use the user service on laptops and dev boxes. Use the system service on VPS or headless hosts that should come back at boot without relying on systemd linger.

Avoid keeping both the user and system gateway units installed at once unless you really mean to. Quill will warn if it detects both because start/stop/status behavior gets ambiguous.

:::info Multiple installations
If you run multiple Quill installations on the same machine (with different `QUILL_HOME` directories), each gets its own systemd service name. The default `~/.quill` uses `quill-gateway`; other installations use `quill-gateway-<hash>`. The `quill gateway` commands automatically target the correct service for your current `QUILL_HOME`.
:::

### macOS (launchd)

```bash
quill gateway install               # Install as launchd agent
quill gateway start                 # Start the service
quill gateway stop                  # Stop the service
quill gateway status                # Check status
tail -f ~/.quill/logs/gateway.log   # View logs
```

The generated plist lives at `~/Library/LaunchAgents/ai.quill.gateway.plist`. It includes three environment variables:
...
- **QUILL_HOME** — scopes the gateway to your Quill installation.

:::tip PATH changes after install
launchd plists are static — if you install new tools (e.g. a new Node.js version via nvm, or ffmpeg via Homebrew) after setting up the gateway, run `quill gateway install` again to capture the updated PATH. The gateway will detect the stale plist and reload automatically.
:::

:::info Multiple installations
Like the Linux systemd service, each `QUILL_HOME` directory gets its own launchd label. The default `~/.quill` uses `ai.quill.gateway`; other installations use `ai.quill.gateway-<suffix>`.
:::
...
|----------|---------|--------------|
| CLI | `quill-cli` | Full access |
| Telegram | `quill-telegram` | Full tools including terminal |
| Discord | `quill-discord` | Full tools including terminal |
| WhatsApp | `quill-whatsapp` | Full tools including terminal |
| Slack | `quill-slack` | Full tools including terminal |
| Google Chat | `quill-google_chat` | Full tools including terminal |
| Signal | `quill-signal` | Full tools including terminal |
| SMS | `quill-sms` | Full tools including terminal |
| Email | `quill-email` | Full tools including terminal |
| Home Assistant | `quill-homeassistant` | Full tools + HA device control (ha_list_entities, ha_get_state, ha_call_service, ha_list_services) |
| Mattermost | `quill-mattermost` | Full tools including terminal |
| Matrix | `quill-matrix` | Full tools including terminal |
| DingTalk | `quill-dingtalk` | Full tools including terminal |
| Feishu/Lark | `quill-feishu` | Full tools including terminal |
| WeCom | `quill-wecom` | Full tools including terminal |
| WeCom Callback | `quill-wecom-callback` | Full tools including terminal |
| Weixin | `quill-weixin` | Full tools including terminal |
| BlueBubbles | `quill-bluebubbles` | Full tools including terminal |
| QQBot | `quill-qqbot` | Full tools including terminal |
| Yuanbao | `quill-yuanbao` | Full tools including terminal |
| Microsoft Teams | `quill-teams` | Full tools including terminal |
| API Server | `quill-api-server` | Full tools (drops `clarify`, `send_message`, `text_to_speech` — programmatic access doesn't have an interactive user) |
| Webhooks | `quill-webhook` | Full tools including terminal |
...
1. **Gateway log** (`~/.quill/logs/gateway.log` or the systemd / launchd unit log). Search for the platform name and `circuit breaker`, `paused`, or `disabled`. The trip event includes the failure count and the last error.
