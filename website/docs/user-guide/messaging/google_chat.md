...
sidebar_position: 12
title: "Google Chat"
description: "Set up Quill Agent as a Google Chat bot using Cloud Pub/Sub"
---

# Google Chat Setup

Connect Quill Agent to Google Chat as a bot. The integration uses Cloud Pub/Sub
pull subscriptions for inbound events and the Chat REST API for outbound messages.
Equivalent ergonomics to Slack Socket Mode or Telegram long-polling: your Quill
...
download the file. Save it somewhere only Quill can read (e.g.,
`~/.quill/google-chat-sa.json`, `chmod 600`).
...
- Topic ID: `quill-chat-events`
- Leave the defaults for everything else.

After creation, the topic's detail page has a **Subscriptions** tab. Create one:

- Subscription ID: `quill-chat-events-sub`
- Delivery type: **Pull**
- Message retention: **7 days** (so backlog survives a quill restart)
...
- Principal: `quill-chat-bot@<your-project>.iam.gserviceaccount.com`
- Role: `Pub/Sub Subscriber`

Also grant `Pub/Sub Viewer` on the same subscription — Quill calls
...
- **App name**: whatever you want users to see ("Quill" is reasonable).
...
- **Connection settings**: select **Cloud Pub/Sub**, enter the topic name
  `projects/<your-project>/topics/quill-chat-events`.
...
`ADDED_TO_SPACE` event that Quill uses to cache the bot's own `users/{id}` for
self-message filtering.

---

## Step 9: Configure Quill

Add the Google Chat section to `~/.quill/.env`:

```bash
# Required
GOOGLE_CHAT_PROJECT_ID=my-chat-bot-123
GOOGLE_CHAT_SUBSCRIPTION_NAME=projects/my-chat-bot-123/subscriptions/quill-chat-events-sub
GOOGLE_CHAT_SERVICE_ACCOUNT_JSON=/home/you/.quill/google-chat-sa.json
...
Install the dependencies the Google Chat adapter needs (no Quill extra is currently published — install them directly):
...
Send "hola" in the test DM. The bot posts a "Quill is thinking…" marker, then
...
Thread support: when a user replies inside a thread, Quill detects the
`thread.name` and posts its reply in the same thread, so each thread gets a
separate Quill session.
...
3. Download the JSON. Move it onto the host that runs Quill.
4. On the host, register the client with Quill:
...
That writes `~/.quill/google_chat_user_client_secret.json`. This is shared
...
The token lands at `~/.quill/google_chat_user_tokens/<sanitized_email>.json`.
...
single-user token at `~/.quill/google_chat_user_token.json` (if present from
...
1. Check the Pub/Sub subscription has undelivered messages in the console.
   If it does, Quill isn't authenticated — verify `GOOGLE_CHAT_SERVICE_ACCOUNT_JSON`
...
3. Check `quill gateway` logs for `[GoogleChat] Connected`. If you see
...
- **Attachment download protection**: Quill will only attach the SA bearer
