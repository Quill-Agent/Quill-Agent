...
title: "Git Worktrees"
description: "Run multiple Quill agents safely on the same repository using git worktrees and isolated checkouts"
---

# Git Worktrees

Quill Agent is often used on large, long‑lived repositories. When you want to:
...
This page shows how to combine worktrees with Quill so each session has a clean, isolated working directory.

## Why Use Worktrees with Quill?

Quill treats the **current working directory** as the project root:

- CLI: the directory where you run `quill` or `quill chat`
...
git worktree add ../repo-feature feature/quill-experiment
```

This creates:

- A new directory: `../repo-feature`
- A new branch: `feature/quill-experiment` checked out in that directory

Now you can `cd` into the new worktree and run Quill there:

```bash
cd ../repo-feature

# Start Quill in the worktree
quill
```
...
git worktree add ../repo-experiment-a feature/quill-a
git worktree add ../repo-experiment-b feature/quill-b
```
...
- Works on its own branch (`feature/quill-a` vs `feature/quill-b`).
...
- Quill checkpoint data under `~/.quill/checkpoints/` is not automatically pruned when you remove a worktree, but it is usually very small.

## Best Practices

- **One worktree per Quill experiment**
...
- **Name branches after the experiment**
  - e.g. `feature/quill-checkpoints-docs`, `feature/quill-refactor-tests`.
- **Commit frequently**
...
- **Avoid running Quill from the bare repo root when using worktrees**
  - Prefer the worktree directories instead, so each agent has a clear scope.

## Using `quill -w` (Automatic Worktree Mode)

Quill has a built‑in `-w` flag that **automatically creates a disposable git worktree** with its own branch. You don't need to set up worktrees manually — just `cd` into your repo and run:
...
- Check out an isolated branch (e.g. `quill/quill-<hash>`).
...
```bash
quill -w -q "Fix issue #123"
```

For parallel agents, open multiple terminals and run `quill -w` in each — every invocation gets its own worktree and branch automatically.

## Putting It All Together

- Use **git worktrees** to give each Quill session its own clean checkout.
