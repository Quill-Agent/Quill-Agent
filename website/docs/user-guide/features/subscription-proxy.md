...
chat completions — use your Quill-managed provider subscription as their
...
| Use case | "Use Quill as a chat backend" | "Use my Portal sub from another app" |
...
This opens your browser for the Nous Portal OAuth flow. Quill stores
the refresh token in `~/.quill/auth.json` — the same place all Quill
...
```
Starting Quill proxy for Nous Portal
...
Model:      Quill-4-70B    # or Quill-4.3-36B, Quill-4-405B
```
...
```
Quill proxy upstream adapters

  [nous    ] Nous Portal — ready (bearer expires 2026-05-15T06:43:21Z)
```

If you see `not logged in`, run `quill login nous`. If you see
...
supports; check `portal.example.com/models`.
...
INFERENCE_TEXT_MODEL=Quill-4-70B
```
...
```bash
quill proxy start --host 0.0.0.0 --port 8645
```
...
`quill_cli/proxy/adapters/<provider>.py` and registering it in
