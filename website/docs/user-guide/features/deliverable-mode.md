...
When Quill Agent runs inside a messaging gateway (Slack, Discord, Telegram,
...
custom instructions in `~/.quill/config.yaml` under `agent.custom_instructions`.
...
If you use Quill' kanban multi-agent workflow, workers can attach
...
Install MCP servers via `~/.quill/config.yaml` under the `mcp_servers`
...
back into the thread as a native attachment. Quill Agent's deliverable
