...
description: "Customize Quill Agent's personality with a global SOUL.md, built-in personalities, and custom persona definitions"
---

# Personality & SOUL.md

Quill Agent's personality is fully customizable. `SOUL.md` is the **primary identity** — it's the first thing in the system prompt and defines who the agent is.

- `SOUL.md` — a durable persona file that lives in `QUILL_HOME` and serves as the agent's identity (slot #1 in the system prompt)
- built-in or custom `/personality` presets — session-level system-prompt overlays

If you want to change who Quill is — or replace it with an entirely different agent persona — edit `SOUL.md`.

## How SOUL.md works now

Quill now seeds a default `SOUL.md` automatically in:

```text
~/.quill/SOUL.md
```

More precisely, it uses the current instance's `QUILL_HOME`, so if you run Quill with a custom home directory, it will use:
...
- Quill creates a starter `SOUL.md` automatically if one does not exist yet
- Existing user `SOUL.md` files are never overwritten
- Quill loads `SOUL.md` only from `QUILL_HOME`
- Quill does not look in the current working directory for `SOUL.md`
- If `SOUL.md` exists but is empty, or cannot be loaded, Quill falls back to a built-in default identity
...
If Quill loaded `SOUL.md` from whatever directory you happened to launch it in, your personality could change unexpectedly between projects. By loading only from `QUILL_HOME`, the personality belongs to the Quill instance itself.

That also makes it easier to teach users:
- "Edit `~/.quill/SOUL.md` to change Quill' default personality."
...
- how Quill should handle uncertainty, disagreement, or ambiguity
...
## What Quill injects into the prompt
...
If the file is empty, whitespace-only, or cannot be read, Quill falls back to a built-in default identity ("You are Quill Agent, an intelligent AI assistant created by Quill Agent..."). This fallback also applies when `skip_context_files` is set (e.g., in subagent/delegation contexts).
...
Quill ships with built-in personalities you can switch to with `/personality`.
...
| **pirate** | Captain Quill, tech-savvy buccaneer |
...
These are convenient overlays, but your global `SOUL.md` still gives Quill its persistent default personality unless the overlay meaningfully changes it.

## Custom personalities in config

You can also define named custom personalities in `~/.quill/config.yaml` under `agent.personalities`.
...
1. Keep a thoughtful global `SOUL.md` in `~/.quill/SOUL.md`
...
- [SOUL.md Guide](/docs/guides/use-soul-with-quill)

## CLI appearance vs conversational personality

Conversational personality and CLI appearance are separate:

- `SOUL.md`, `agent.system_prompt`, and `/personality` affect how Quill speaks
