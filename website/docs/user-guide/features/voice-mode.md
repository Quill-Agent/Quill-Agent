...
sidebar_position: 10
title: "Voice Mode"
description: "Real-time voice conversations with Quill Agent — CLI, Telegram, Discord (DMs, text channels, and voice channels)"
---

# Voice Mode

Quill Agent supports full voice interaction across CLI and messaging platforms. Talk to the agent using your microphone, hear spoken replies, and have live voice conversations in Discord voice channels.

If you want a practical setup walkthrough with recommended configurations and real usage patterns, see [Use Voice Mode with Quill](/docs/guides/use-voice-mode-with-quill).

## Prerequisites

Before using voice features, make sure you have:

1. **Quill Agent installed** — `pip install quill-agent` (see [Installation](/docs/getting-started/installation))
2. **An LLM provider configured** — run `quill model` or set your preferred provider credentials in `~/.quill/.env`
3. **A working base setup** — run `quill` to verify the agent responds to text before enabling voice

:::tip
The `~/.quill/` directory and default `config.yaml` are created automatically the first time you run `quill`. You only need to create `~/.quill/.env` manually for API keys.
:::
...
pip install "quill-agent[voice]"

# Discord + Telegram messaging (includes discord.py[voice] for VC support)
pip install "quill-agent[messaging]"

# Premium TTS (ElevenLabs)
pip install "quill-agent[tts-premium]"

# Local TTS (NeuTTS, optional)
python -m pip install -U neutts[all]

# Everything at once
pip install "quill-agent[all]"
```
...
Voice mode is available in both the **classic CLI** (`quill chat`) and the **TUI** (`quill --tui`). Behavior is identical across both — same slash commands, same VAD silence detection, same streaming TTS, same hallucination filter. The TUI additionally forwards crash-forensic logs to `~/.quill/logs/` so push-to-talk failures on exotic audio backends can be reported with a full stack trace rather than disappearing silently.
...
```bash
quill                # Start the interactive CLI
```
...
1. Start the CLI with `quill` and enable voice mode with `/voice on`
...
:::tip
The record key is configurable via `voice.record_key` in `~/.quill/config.yaml` (default: `ctrl+b`).
:::
...
```bash
quill gateway        # Start the gateway (connects to configured platforms)
quill gateway setup  # Interactive setup wizard for first-time configuration
```
...
**Server channels:** The bot only responds when you @mention it (e.g. `@quillbyt4 hello`). Make sure you select the **bot user** from the mention popup, not the role with the same name.

:::tip
To disable the mention requirement in server channels, add to `~/.quill/.env`:
```bash
...
```bash
quill gateway        # Start with existing configuration
```
...
3. Or set `DISCORD_REQUIRE_MENTION=false` in `~/.quill/.env`
...
- Review gateway logs: `tail -f ~/.quill/logs/gateway.log`
