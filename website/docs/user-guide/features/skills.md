...
All skills live in **`~/.quill/skills/`** — the primary directory and source of truth. On fresh install, bundled skills are copied from the repo. Hub-installed and agent-created skills also go here. The agent can modify or delete any skill.

You can also point Quill at **external skill directories** — additional folders scanned alongside the local one. See [External Skill Directories](#external-skill-directories) below.
...
The bundled `plan` skill is a good example. Running `/plan [request]` loads the skill's instructions, telling Quill to inspect context if needed, write a markdown implementation plan instead of executing the task, and save the result under `.quill/plans/` relative to the active workspace/backend working directory.

You can also interact with skills through natural conversation:

```bash
quill chat --toolsets skills -q "What skills do you have?"
quill chat --toolsets skills -q "Show me the axolotl skill"
```
...
/home/user/.quill/cache/chart-q4-2025.png
...
When a missing value is encountered, Quill asks for it securely only when the skill is actually loaded in the local CLI. You can skip setup and keep using the skill. Messaging surfaces never ask for secrets in chat — they tell you to use `quill setup` or `~/.quill/.env` locally instead.
...
Settings are stored under `skills.config` in your config.yaml. `quill config migrate` prompts for unconfigured settings, and `quill config show` displays them. When a skill loads, its resolved config values are injected into the context so the agent knows the configured values automatically.
...
```text
~/.quill/skills/                  # Single source of truth
...
If you maintain skills outside of Quill — for example, a shared `~/.agents/skills/` directory used by multiple AI tools — you can tell Quill to scan those directories too.

Add `external_dirs` under the `skills` section in `~/.quill/config.yaml`:
...
- **Read-only**: External dirs are only scanned for skill discovery. When the agent creates or edits a skill, it always writes to `~/.quill/skills/`.
- **Local precedence**: If the same skill name exists in both the local dir and an external dir, the local version wins.
- **Full integration**: External skills appear in the system prompt index, `skills_list`, `skill_view`, and as `/skill-name` slash commands — no different from local skills.
- **Non-existent paths are silently skipped**: If a configured directory doesn't exist, Quill ignores it without errors. Useful for optional shared directories that may not be present on every machine.

### Example

```text
~/.quill/skills/               # Local (primary, read-write)
├── devops/deploy-k8s/
│   └── SKILL.md
└── mlops/axolotl/
    └── SKILL.md
...
Bundles live in **`~/.quill/skill-bundles/<slug>.yaml`** and look like this:
...
- `description` (optional) — short text shown in `/bundles` and `quill bundles list`.
...
quill bundles create research

# Overwrite an existing bundle
quill bundles create backend-dev --skill ... --force

# Delete a bundle
quill bundles delete backend-dev

# Re-scan ~/.quill/skill-bundles/ and report changes
quill bundles reload
```
...
- You want to ship a team-wide "task profile" by checking the bundle YAML into a shared dotfiles repo and symlinking it into `~/.quill/skill-bundles/`.

A bundle is just a YAML alias — it doesn't install skills for you. The skills themselves must already be present (in `~/.quill/skills/` or an external skill directory). Otherwise the bundle invocation just skips the missing ones.
...
```bash
quill skills browse                              # Browse all hub skills (official first)
quill skills browse --source official            # Browse only official optional skills
quill skills search kubernetes                   # Search all sources
quill skills search react --source skills-sh     # Search the skills.sh directory
quill skills search https://mintlify.com/docs --source well-known
quill skills inspect openai/skills/k8s           # Preview before installing
quill skills install openai/skills/k8s           # Install with security scan
quill skills install official/security/1password
quill skills install skills-sh/vercel-labs/json-render/json-render-react --force
quill skills install well-known:https://mintlify.com/docs/.well-known/skills/mintlify
...
quill skills list --source hub                   # List hub-installed skills
quill skills check                               # Check installed hub skills for upstream updates
quill skills update                              # Reinstall hub skills with upstream changes when needed
quill skills audit                               # Re-scan all hub skills for security
quill skills uninstall k8s                       # Remove a hub skill
quill skills reset google-workspace              # Un-stick a bundled skill from "user-modified" (see below)
quill skills reset google-workspace --restore    # Also restore the bundled version, deleting your local edits
quill skills publish skills/my-skill --to github --repo owner/repo
quill skills snapshot export setup.json          # Export skill config
quill skills tap add myorg/skills-repo           # Add a custom GitHub source
```
...
|--------|---------|-------|
| `official` | `official/security/1password` | Optional skills shipped with Quill. |
| `skills-sh` | `skills-sh/vercel-labs/agent-skills/vercel-react-best-practices` | Searchable via `quill skills search <query> --source skills-sh`. Quill resolves alias-style skills when the skills.sh slug differs from the repo folder. |
...
Quill currently integrates with these skills ecosystems and discovery sources:

#### 1. Official optional skills (`official`)

These are maintained in the Quill repository itself and install with builtin trust.
...
This is Vercel's public skills directory. Quill can search it directly, inspect skill detail pages, resolve alias-style slugs, and install from the underlying source repo.
...
```bash
quill skills search https://mintlify.com/docs --source well-known
quill skills inspect well-known:https://mintlify.com/docs/.well-known/skills/mintlify
quill skills install well-known:https://mintlify.com/docs/.well-known/skills/mintlify
```

#### 4. Direct GitHub skills (`github`)

Quill can install directly from GitHub repositories and GitHub-based taps. This is useful when you already know the repo/path or want to add your own custom source repo.
...
```bash
quill skills install openai/skills/k8s
quill skills tap add myorg/skills-repo
```
...
- Quill source id: `clawhub`

#### 6. Claude marketplace-style repos (`claude-marketplace`)

Quill supports marketplace repos that publish Claude-compatible plugin/marketplace manifests.

Known integrated sources include:
- [anthropics/skills](https://github.com/anthropics/skills)
- [aiskillstore/marketplace](https://github.com/aiskillstore/marketplace)

Quill source id: `claude-marketplace`

#### 7. LobeHub (`lobehub`)

Quill can search and convert agent entries from LobeHub's public catalog into installable Quill skills.

- Site: [LobeHub](https://lobehub.com/)
- Public agents index: [chat-agents.lobehub.com](https://chat-agents.lobehub.com/)
- Backing repo: [lobehub/lobe-chat-agents](https://github.com/lobehub/lobe-chat-agents)
- Quill source id: `lobehub`

#### 8. Direct URL (`url`)

Install a single-file `SKILL.md` directly from any HTTP(S) URL — useful when an author hosts a skill on their own site (no hub listing, no GitHub path to type). Quill fetches the URL, parses the YAML frontmatter, security-scans it, and installs.
...
```bash
quill skills install https://sharethis.chat/SKILL.md
quill skills install https://example.com/my-skill/SKILL.md --category productivity
```
...
quill skills install https://example.com/SKILL.md --name sharethis-chat

# Or inside a chat session:
/skills install https://example.com/SKILL.md --name sharethis-chat
```

Trust level is always `community` — the same security scan runs as for every other source. The URL is stored as the install identifier, so `quill skills update` re-fetches from the same URL automatically when you want to refresh.
...
`quill skills inspect ...` now also surfaces upstream metadata when available:
- repo URL
...
| Level | Source | Policy |
|-------|--------|--------|
| `builtin` | Ships with Quill | Always trusted |
...
```bash
quill skills check          # Report which installed hub skills changed upstream
quill skills update         # Reinstall only the skills with updates available
quill skills update react   # Update one specific installed hub skill
```
...
If you want to share a curated set of skills — for your team, your org, or publicly — you can publish them as a **tap**: a GitHub repository other Quill users add with `quill skills tap add <owner/repo>`. No server, no registry sign-up, no release pipeline. Just a directory of `SKILL.md` files.
...
- Each skill directory must contain a `SKILL.md` with standard [SKILL.md frontmatter](#skillmd-format) (`name`, `description`, plus optional `metadata.quill.tags`, `version`, `author`, `platforms`, `metadata.quill.config`).
...
Quill discovers skills by listing every subdirectory of the tap path and probing each for `SKILL.md`.
...
After pushing that to GitHub, any Quill user can subscribe and install:

```bash
quill skills tap add my-org/quill-skills
quill skills search deploy
quill skills install my-org/quill-skills/deploy-runbook
```

#### Non-default paths

If your skills don't live under `skills/` (common when you're adding a `skills/` subtree to an existing project), edit the tap entry in `~/.quill/.hub/taps.json`:
...
The `quill skills tap add` CLI defaults new taps to `path: "skills/"`; edit the file directly if you need a different path. `quill skills tap list` shows the effective path per tap.
...
```bash
quill skills install owner/repo/skills/my-workflow
```
...
Taps are stored in `~/.quill/.hub/taps.json` (created on demand).

## Bundled skill updates (`quill skills reset`)

Quill ships with a set of bundled skills in `skills/` inside the repo. On install and on every `quill update`, a sync pass copies those into `~/.quill/skills/` and records a manifest at `~/.quill/skills/.bundled_manifest` mapping each skill name to the content hash at the time it was synced (the **origin hash**).

On each sync, Quill recomputes the hash of your local copy and compares it to the origin hash:
...
quill skills reset google-workspace --restore
...
:::note Profiles
Each profile has its own `.bundled_manifest` under its own `QUILL_HOME`, so `quill -p coder skills reset <name>` only affects that profile.
:::
