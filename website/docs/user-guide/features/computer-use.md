...
Quill Agent can drive your Mac's desktop — clicking, typing, scrolling,
...
```
quill computer-use install
```

This fetches and runs the upstream cua-driver installer:
`curl -fsSL https://raw.githubusercontent.com/trycua/cua/main/libs/cua-driver/scripts/install.sh`.
Use `quill computer-use status` to verify the install.

**Option 2: enable the toolset interactively.**

1. Run `quill tools`, pick `🖱️ Computer Use (macOS)` → `cua-driver (background)`.
...
- **`quill computer-use install --upgrade`** — manual force-refresh.
...
`quill computer-use status` shows the installed version next to the
binary path.
...
Quill applies multi-layer guardrails:
...
Pair with `approvals.mode: manual` in `~/.quill/config.yaml` if you want every action confirmed.

## Token efficiency

Screenshots are expensive. Quill applies four layers of optimisation:
...
- **Private SPI risk.** Apple can change SkyLight's symbol surface in any
  OS update. Pin the driver version with the `QUILL_CUA_DRIVER_VERSION`
...
```
QUILL_CUA_DRIVER_CMD=/opt/homebrew/bin/cua-driver
QUILL_CUA_DRIVER_VERSION=0.5.0    # optional pin
```

Swap the backend entirely (for testing):

```
QUILL_COMPUTER_USE_BACKEND=noop   # records calls, no side effects
```

## Troubleshooting

**`computer_use backend unavailable: cua-driver is not installed`** — Run
`quill computer-use install` to fetch the cua-driver binary, or run
`quill tools` and enable the Computer Use toolset.
