...
Quill runs full language servers — pyright, gopls, rust-analyzer,
...
This is the same architecture top-tier coding agents use. Quill
...
1. Quill captures a baseline of current diagnostics for the file.
...
…). Quill auto-detects the binary on PATH or in
`<QUILL_HOME>/lsp/bin/`.
...
`node_modules` tree — Quill installs both packages together when you
run `quill lsp install typescript` or auto-install fires on first
use.

## CLI

```
quill lsp status          # service state + per-server install status
quill lsp list            # registry, optionally --installed-only
quill lsp install <id>    # eagerly install one server
quill lsp install-all     # try every server with a known recipe
quill lsp restart         # tear down running clients
quill lsp which <id>      # print resolved binary path
```

`quill lsp status` is the best starting point — it shows which
...
lsp:
  ...
  #   auto    — install via npm/pip/go install into <QUILL_HOME>/lsp/bin
...
When `install_strategy: auto`, Quill installs binaries into
`<QUILL_HOME>/lsp/bin/`. NPM packages land in
`<QUILL_HOME>/lsp/node_modules/` with bin symlinks one level up.
Go binaries come from `go install` with `GOBIN` pointed at the
staging dir.

Nothing is ever installed to `/usr/local/`, `~/.local/`, or any other
shared location — the staging dir is fully Quill-owned and is
...
Servers are kept alive for the life of the Quill process. There's
...
**`quill lsp status` shows a server as "missing"**

The binary isn't on PATH and isn't in `<QUILL_HOME>/lsp/bin/`. Run
`quill lsp install <server_id>` to attempt an auto-install, or
install the binary manually through the language's normal toolchain.

**`Backend warnings` section in `quill lsp status`**
...
When `quill lsp status` shows a `Backend warnings` section, install
...
`~/.quill/logs/agent.log`.

**Server starts but never returns diagnostics**

Check `~/.quill/logs/agent.log` for `[agent.lsp.client]` entries —
...
the rest of the session. Run `quill lsp restart` to clear the set;
