...
Type `@` followed by a reference to inject content directly into your message. Quill expands the reference inline and appends the content under an `--- Attached Context ---` section.
...
- Quill env: `$QUILL_HOME/.env`

These directories are fully blocked (any file inside):
- `~/.ssh/`, `~/.aws/`, `~/.gnupg/`, `~/.kube/`, `$QUILL_HOME/skills/.hub/`
