...
[Honcho](https://github.com/plastic-labs/honcho) is an AI-native memory backend that adds dialectic reasoning and deep user modeling on top of Quill's built-in memory system. Instead of simple key-value storage, Honcho maintains a running model of who the user is — their preferences, communication style, goals, and patterns — by reasoning about conversations after they happen.
...
**Multi-agent profiles**: When multiple Quill instances talk to the same user (e.g., a coding assistant and a personal assistant), Honcho maintains separate "peer" profiles. Each peer sees only its own observations and conclusions, preventing cross-contamination of context.

## Setup

```bash
quill memory setup    # select "honcho" from the provider list
```
...
Honcho is configured in `~/.honcho/config.json` (global) or `$QUILL_HOME/honcho.json` (profile-local). The setup wizard handles this for you.
...
- `per-session` — each `quill` run gets a fresh session. Clean starts, memory via tools. Recommended for new users.
...
Server-side toggles set via the [Honcho dashboard](https://app.honcho.dev) win over local defaults — Quill syncs them back at session init.
...
The `quill honcho` subcommand is **only registered when Honcho is the active memory provider** (`memory.provider: honcho` in `config.yaml`). Run `quill memory setup` and pick Honcho first; the subcommand appears on the next invocation.

```bash
quill honcho status          # Connection status, config, and key settings
quill honcho setup           # Redirects to `quill memory setup`
quill honcho strategy        # Show or set session strategy (per-session/per-directory/per-repo/global)
quill honcho peer            # Show or update peer names + dialectic reasoning level
quill honcho mode            # Show or set recall mode (hybrid/context/tools)
quill honcho tokens          # Show or set token budget for context and dialectic
quill honcho identity        # Seed or show the AI peer's Honcho identity
quill honcho sync            # Sync Honcho config to all existing profiles
quill honcho peers           # Show peer identities across all profiles
quill honcho sessions        # List known Honcho session mappings
quill honcho map             # Map current directory to a Honcho session name
quill honcho enable          # Enable Honcho for the active profile
quill honcho disable         # Disable Honcho for the active profile
quill honcho migrate         # Step-by-step migration guide from openclaw-honcho
```

## Migrating from `quill honcho`

If you previously used the standalone `quill honcho setup`:
...
No re-login or re-setup needed. Run `quill memory setup` and select "honcho" — the wizard detects your existing config.
