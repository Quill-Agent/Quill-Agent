#!/usr/bin/env python3
"""Generate llms.txt and llms-full.txt for the Quill docs site.
...
Both publish at:
  https://quill-agent.example.com/docs/llms.txt
  https://quill-agent.example.com/docs/llms-full.txt
...
SITE_BASE = "https://quill-agent.example.com/docs"
...
SECTIONS: list[tuple[str, list[tuple[str, str, str | None]]]] = [
    ...
    ("Guides & Tutorials", [
        ...
        ("guides/python-library", "Use Quill as a Python Library", None),
        ("guides/use-mcp-with-quill", "Use MCP with Quill", None),
        ("guides/use-voice-mode-with-quill", "Use Voice Mode with Quill", None),
        ("guides/use-soul-with-quill", "Use SOUL.md with Quill", None),
        ("guides/build-a-quill-plugin", "Build a Quill Plugin", None),
    ...
    ("Reference", [
        ...
        ("reference/skills-catalog", "Bundled Skills Catalog", "Table of all ~90 skills bundled with Quill"),
...
def emit_llms_index() -> str:
    ...
    lines: list[str] = []
    lines.append("# Quill Agent")
    lines.append("")
    lines.append(
        "> The self-improving AI agent built by Quill Agent. A terminal-native "
    ...
    lines.append("")
    lines.append(
        "Install: `curl -fsSL https://raw.githubusercontent.com/YOUR_USERNAME/"
        "quill-agent/main/scripts/install.sh | bash`  "
        "(Linux, macOS, WSL2, Termux)"
    )
    lines.append("")
    lines.append("Repo: https://github.com/YOUR_USERNAME/quill-agent")
    lines.append("")
...
def emit_llms_full() -> str:
    ...
    seen: set[Path] = set()
    chunks: list[str] = [
        "# Quill Agent — Full Documentation\n",
        (
            "This file is the entire Quill Agent documentation concatenated for LLM "
            "context ingestion. Section order reflects docs-site navigation: Getting "
            "Started, Using Quill, Features, Messaging, Integrations, Guides, "
            "Developer Guide, Reference, then everything else.\n"
        ),
        "Canonical site: https://quill-agent.example.com/docs\n",
        "Short index: https://quill-agent.example.com/docs/llms.txt\n",
        "\n---\n\n",
    ]
