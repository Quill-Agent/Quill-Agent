...
def rewrite_relative_links(body: str, meta: dict[str, Any]) -> str:
    ...
    base = f"https://github.com/YOUR_USERNAME/quill-agent/blob/main/{source_dir}/{meta['rel_path']}"
...
def render_skill_page(
    ...
    quill_meta = (fm.get("metadata") or {}).get("quill") or {}
    tags = quill_meta.get("tags") or []
    related = quill_meta.get("related_skills") or []
    ...
    if meta["source_kind"] == "bundled":
        ...
        info_rows.append(
            (
                "Source",
                "Optional — install with `quill skills install official/"
                + meta["category"]
                + "/"
                + meta["slug"]
                + "`",
...
def build_catalog_md_bundled(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Catalog of bundled skills that ship with Quill Agent"',
        "---",
        "",
        "# Bundled Skills Catalog",
        "",
        "Quill ships with a large built-in skill library copied into `~/.quill/skills/` on install. Each skill below links to a dedicated page with its full definition, setup, and usage.",
        "",
        "Quill also syncs bundled skills on `quill update`, but the sync manifest respects local deletions and user edits. If a skill listed here is missing from your profile's `~/.quill/skills/` tree, it is still shipped with Quill; restore it with `quill skills reset <name> --restore`.",
        "",
