...
The PID file lives at ``{QUILL_HOME}/gateway.pid``.  QUILL_HOME defaults to
``~/.quill`` but can be overridden via the environment variable.  This means
separate QUILL_HOME directories naturally get separate PID files — a property
...
def _get_pid_path() -> Path:
    """Return the path to the gateway PID file, respecting QUILL_HOME."""
    home = get_quill_home()
...
def _get_lock_dir() -> Path:
    ...
    override = os.getenv("QUILL_GATEWAY_LOCK_DIR")
    if override:
        return Path(override)
    state_home = Path(os.getenv("XDG_STATE_HOME", Path.home() / ".local" / "state"))
    return state_home / "quill" / _LOCKS_DIRNAME
...
def _looks_like_gateway_process(pid: int) -> bool:
    """Return True when the live PID still looks like the Quill gateway."""
...
def acquire_scoped_lock(scope: str, identity: str, metadata: Optional[dict[str, Any]] = None) -> tuple[bool, Optional[dict[str, Any]]]:
    ...
    at once (e.g. the same Telegram bot token across different QUILL_HOME dirs).
    """
...
# enabled in the user's systemd (e.g. ``quill.service`` + ``quill-
# gateway.service``).
#
