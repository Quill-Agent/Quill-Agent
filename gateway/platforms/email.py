"""
Email platform adapter for the Quill gateway.

Allows users to interact with Quill by sending emails.
...
def _send_imap_id(imap: "imaplib.IMAP4") -> None:
    ...
    try:
        ...
        imap.xatom(
            "ID",
            f'("name" "quill-agent" "version" "{_quill_version}" '
...
class EmailAdapter(BasePlatformAdapter):
    ...
    def _send_email(
        ...
        subject = ctx.get("subject", "Quill Agent")
        ...
        msg_id = f"<quill-{uuid.uuid4().hex[:12]}@{self._address.split('@')[1]}>"
