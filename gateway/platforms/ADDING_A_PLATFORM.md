...
There are two ways to add a platform to the Quill gateway:

## Plugin Path (Recommended for Community/Third-Party)

Create a plugin directory in `~/.quill/plugins/` (or under `plugins/platforms/`
...
**zero changes to core Quill code**.
...
- `env_enablement_fn: () -> Optional[dict]` — seeds `PlatformConfig.extra`
  ...
  `quill gateway status` or `get_connected_platforms()` until the SDK
  instantiates.
...
- `plugin.yaml` `requires_env` / `optional_env` rich-dict entries —
  auto-populate `OPTIONAL_ENV_VARS` in `quill_cli/config.py` so the setup
...
Checklist for integrating a platform directly into the Quill core.
...
And add it to the `quill-gateway` composite:

```python
"quill-gateway": {
    "includes": [..., "quill-your-platform"]
}
```
...
## 12. Status Display (`quill_cli/status.py`)
...
## 13. Gateway Setup Wizard (`quill_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ quill_cli/ toolsets.py \
  --include="*.py" -l | sort -u
