"""Windows UTF-8 bootstrap for Quill entry points.
...
should be imported at the very top of every Quill entry point
(``quill``, ``quill-agent``, ``quill-acp``, ``python -m gateway.run``,
...
# Apply on import — entry points just need ``import quill_bootstrap``
# (or ``from quill_bootstrap import apply_windows_utf8_bootstrap``) at
