# OpenAI-Compatible API Server for Quill Agent
...
SSE streaming. By exposing this endpoint, quill-agent becomes instantly
...
│  AnythingLLM     │  │    {"messages": [...]}             │  quill-agent   │
│  NextChat        │  │                                    │  gateway        │
...
A user would:
1. Set `API_SERVER_ENABLED=true` in `~/.quill/.env`
2. Run `quill gateway` (API server starts alongside Telegram/Discord/etc.)
3. Point Open WebUI (or any frontend) at `http://localhost:8642/v1`
4. Chat with quill-agent through any OpenAI-compatible UI
...
| GET | `/v1/models` | List available "models" (returns quill-agent as a model) |
...
Authorization: Bearer quill-api-key-here
...
But quill-agent sessions have persistent state (memory, skills, tool context).
...
Frontends send `"model": "quill-agent"` (or whatever). The actual LLM model
used is configured server-side in config.yaml. The API server maps any
requested model name to the configured quill-agent model.
...
1. `gateway/platforms/api_server.py` — new adapter
   ...
   - Frontend system prompt layered on top of quill-agent's core prompt
...
Once implemented, quill-agent works as a drop-in backend for:
