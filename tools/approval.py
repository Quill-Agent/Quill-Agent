...
def get_current_session_key(default: str = "default") -> str:
    ...
    return get_session_env("QUILL_SESSION_KEY", default)
...
def _get_session_platform() -> str:
    ...
    try:
        ...
        return os.getenv("QUILL_SESSION_PLATFORM", "") or ""


def _is_gateway_approval_context() -> bool:
    """True when this call is inside a gateway/API session.

    Legacy gateway integrations set QUILL_GATEWAY_SESSION in process env.
    Newer concurrent gateway paths bind QUILL_SESSION_PLATFORM via
    ...
    from a gateway platform (cron binds QUILL_SESSION_PLATFORM via
    ...
    """
    if env_var_enabled("QUILL_CRON_SESSION"):
        return False
...
# via shell expansions like $HOME or $QUILL_HOME.
_SSH_SENSITIVE_PATH = r'(?:~|\$home|\$\{home\})/\.ssh(?:/|$)'
_QUILL_ENV_PATH = (
    r'(?:~\/\.quill/|'
    r'(?:\$home|\$\{home\})/\.quill/|'
    r'(?:\$quill_home|\$\{quill_home\})/)'
    r'\.env\b'
)
...
DANGEROUS_PATTERNS = [
    ...
    (r'\bquill\s+gateway\s+(stop|restart)\b', "stop/restart quill gateway (kills running agents)"),
    (r'\bquill\s+update\b', "quill update (restarts gateway, kills running agents)"),
    # Gateway protection: never start gateway outside systemd management
    (r'gateway\s+run\b.*(&\s*$|&\s*;|\bdisown\b|\bsetsid\b)', "start gateway outside systemd (use 'systemctl --user restart quill-gateway')"),
    (r'\bnohup\b.*gateway\s+run\b', "start gateway outside systemd (use 'systemctl --user restart quill-gateway')"),
    # Self-termination protection: prevent agent from killing its own process
    (r'\b(pkill|killall)\b.*\b(quill|gateway|cli\.py)\b', "kill quill/gateway process (self-termination)"),
    # Self-termination via kill + command substitution (pgrep/pidof).
    # The name-based pattern above catches `pkill quill` but not
    # `kill -9 $(pgrep -f quill)` because the substitution is opaque
...
def check_dangerous_command(command: str, env_type: str,
    ...
    if is_truthy_value(os.getenv("QUILL_YOLO_MODE")) or is_current_session_yolo_enabled():
        return {"approved": True, "message": None}
    ...
    is_cli = env_var_enabled("QUILL_INTERACTIVE")
    ...
    if is_gateway or env_var_enabled("QUILL_EXEC_ASK"):
...
def check_all_command_guards(command: str, env_type: str,
    ...
    if is_truthy_value(os.getenv("QUILL_YOLO_MODE")) or is_current_session_yolo_enabled() or approval_mode == "off":
        return {"approved": True, "message": None}

    is_cli = env_var_enabled("QUILL_INTERACTIVE")
    is_gateway = _is_gateway_approval_context()
    is_ask = env_var_enabled("QUILL_EXEC_ASK")
