...
it is automatically downloaded from GitHub releases to $QUILL_HOME/bin/tirith.
...
def _get_quill_home() -> str:
    """Return the Quill home directory, respecting QUILL_HOME env var."""
    return str(get_quill_home())


def _failure_marker_path() -> str:
    """Return the path to the install-failure marker file."""
    return os.path.join(_get_quill_home(), ".tirith-install-failed")
...
def _quill_bin_dir() -> str:
    """Return $QUILL_HOME/bin, creating it if needed."""
    d = os.path.join(_get_quill_home(), "bin")
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    """Download and install tirith to $QUILL_HOME/bin/tirith.
    ...
    try:
        ...
        dest = os.path.join(_quill_bin_dir(), "tirith")
        try:
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    2. $QUILL_HOME/bin/tirith (previously auto-installed)
    3. Auto-install from GitHub releases → $QUILL_HOME/bin/tirith
    ...
    quill_bin = os.path.join(_quill_bin_dir(), "tirith")
    if os.path.isfile(quill_bin) and os.access(quill_bin, os.X_OK):
        _resolved_path = quill_bin
        _install_failure_reason = ""
        _clear_install_failed()
        return quill_bin
