...
This module provides the glue:
    - ``QuillTokenStorage``: persists tokens/client-info to disk so they
    ...
    mcp_servers:
      my_server:
        ...
        oauth:                                  # all fields optional
          ...
          client_name: "My Custom Client"       # default: "Quill Agent"
"""
...
def _get_token_dir() -> Path:
    ...
    Uses QUILL_HOME so each profile gets its own OAuth tokens.
    Layout: ``QUILL_HOME/mcp-tokens/``
    """
    try:
        from quill_constants import get_quill_home
        base = Path(get_quill_home())
    except ImportError:
        base = Path(os.environ.get("QUILL_HOME", str(Path.home() / ".quill")))
...
# ---------------------------------------------------------------------------
# QuillTokenStorage -- persistent token/client-info on disk
# ---------------------------------------------------------------------------


class QuillTokenStorage:
    """Persist OAuth tokens and client registration to JSON files.

    File layout::

        QUILL_HOME/mcp-tokens/<server_name>.json         -- tokens
        QUILL_HOME/mcp-tokens/<server_name>.client.json   -- client info
        QUILL_HOME/mcp-tokens/<server_name>.meta.json     -- oauth server metadata
    """
    ...
    async def get_tokens(self) -> "OAuthToken | None":
        ...
        if data is None:
            return None
        # Quill records an absolute wall-clock ``expires_at`` alongside the
    ...
    class _Handler(BaseHTTPRequestHandler):
        def do_GET(self) -> None:  # noqa: N802
            ...
            body = (
                ...
                "<p>You can close this tab and return to Quill.</p></body></html>"
            ) if code else (
...
async def _redirect_handler(authorization_url: str) -> None:
    ...
    if _oauth_port and (os.getenv("SSH_CLIENT") or os.getenv("SSH_TTY")):
        print(
            ...
            f"\n"
            f"  Then open the URL above. See: https://quill-agent.example.com/docs/guides/oauth-over-ssh\n",
            file=sys.stderr,
        )
...
def _build_client_metadata(cfg: dict) -> "OAuthClientMetadata":
    ...
    client_name = cfg.get("client_name", "Quill Agent")
    scope = cfg.get("scope")
