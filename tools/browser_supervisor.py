...
One ``CDPSupervisor`` runs per Quill ``task_id`` that has a reachable CDP
...
DIALOG_BRIDGE_HOST = "quill-dialog-bridge.invalid"
...
(() => {
  if (window.__quillDialogBridgeInstalled) return;
  window.__quillDialogBridgeInstalled = true;
  const ENDPOINT = "http://quill-dialog-bridge.invalid/";
