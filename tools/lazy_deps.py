"""
Lazy dependency installer for opt-in Quill Agent backends.

Many Quill features (Mistral TTS, ElevenLabs TTS, Honcho memory, Bedrock,
...
(``quill-agent[all]``) and install them eagerly at setup time. That has
two problems:
...
remediation hint pointing at ``quill tools`` or the manual pip command.
...
def _allow_lazy_installs() -> bool:
    ...
    """
    if os.environ.get("QUILL_DISABLE_LAZY_INSTALLS") == "1":
        return False
    try:
...
def _is_satisfied(spec: str) -> bool:
    ...
    ``quill update`` propagate pin bumps in :data:`LAZY_DEPS` to already-
...
def _venv_pip_install(specs: tuple[str, ...], *, timeout: int = 300) -> _InstallResult:
    ...
    Mirrors the strategy in ``quill_cli.tools_config._pip_install`` but
...
def active_features() -> list[str]:
    ...
    Used by ``quill update`` to figure out which lazy backends need a
...
def refresh_active_features(*, prompt: bool = False) -> dict[str, str]:
    ...
    Intended for ``quill update``. Never raises; lazy-install failures
