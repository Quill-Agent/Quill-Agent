...
def _is_quill_provider_credential(name: str) -> bool:
    """True if ``name`` is a Quill-managed provider credential (API key,
    token, or similar) per ``_QUILL_PROVIDER_ENV_BLOCKLIST``.
    ...
    Non-Quill API keys (TENOR_API_KEY, NOTION_TOKEN, etc.) are NOT
    in the blocklist and remain legitimately registerable — skills that
    wrap third-party APIs still work.
    """
    try:
        from tools.environments.local import _QUILL_PROVIDER_ENV_BLOCKLIST
    except Exception:
        return False
    return name in _QUILL_PROVIDER_ENV_BLOCKLIST
...
def register_env_passthrough(var_names: Iterable[str]) -> None:
    ...
    Variables that are Quill-managed provider credentials (from
    ``_QUILL_PROVIDER_ENV_BLOCKLIST``) are rejected here to preserve
    the ``execute_code`` sandbox's credential-scrubbing guarantee per
    GHSA-rhgp-j443-p4rf. A skill that needs to talk to a Quill-managed
    ...
    Non-Quill third-party API keys (TENOR_API_KEY, NOTION_TOKEN, etc.)
    ...
    for name in var_names:
        ...
        if not name:
            continue
        if _is_quill_provider_credential(name):
            logger.warning(
                "env passthrough: refusing to register Quill provider "
                "credential %r (blocked by _QUILL_PROVIDER_ENV_BLOCKLIST). "
