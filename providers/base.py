...
def _profile_user_agent() -> str:
    """Return a ``quill-cli/<version>`` UA string, with a stable fallback.
    ...
    """
    try:
        from quill_cli import __version__ as _ver  # lazy: avoid layer cycle at import time
        return f"quill-cli/{_ver}"
    except Exception:
        return "quill-cli"
