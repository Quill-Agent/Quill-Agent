"""Persistent slash-command worker — one QuillCLI per TUI session.
...
def _run(cli: QuillCLI, command: str) -> str:
...
def main():
    ...
    os.environ["QUILL_SESSION_KEY"] = args.session_key
    os.environ["QUILL_INTERACTIVE"] = "1"

    with contextlib.redirect_stdout(io.StringIO()), contextlib.redirect_stderr(io.StringIO()):
        cli = QuillCLI(model=args.model or None, compact=True, resume=args.session_key, verbose=False)
