...
(SQLITE_PROTOCOL — typical on NFS/SMB), Quill must fall back to
``journal_mode=DELETE`` so ``state.db`` / ``kanban.db`` remain usable.

Without this fallback, users on NFS-mounted ``QUILL_HOME`` silently lose
...
def _reset_last_init_error():
    ...
    quill_state._set_last_init_error(None)
    yield
    quill_state._set_last_init_error(None)
...
def _reset_wal_fallback_warned_paths():
    ...
    quill_state._wal_fallback_warned_paths.clear()
    yield
    quill_state._wal_fallback_warned_paths.clear()
...
class TestApplyWalWithFallback:
    ...
    def test_warning_deduplicated_per_db_label(self, tmp_path, caplog):
        ...
        connection on every operation — see quill_cli/kanban_db.py).
...
class TestGetLastInitError:
    ...
    def test_success_does_not_clear_prior_error(self, tmp_path):
        ...
        """
        quill_state._set_last_init_error("OperationalError: locking protocol")
    ...
    def test_captures_cause_on_failed_init(self, tmp_path):
        ...
        with patch("quill_state.sqlite3.connect", side_effect=gated_connect):
...
class TestFormatSessionDbUnavailable:
    ...
    def test_includes_cause(self):
        ...
        quill_state._set_last_init_error("OperationalError: generic SQLite error")
