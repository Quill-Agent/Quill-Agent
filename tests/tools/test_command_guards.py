...
def _clean_state():
    ...
    saved = {}
    for k in ("QUILL_INTERACTIVE", "QUILL_GATEWAY_SESSION", "QUILL_EXEC_ASK", "QUILL_YOLO_MODE"):
        if k in os.environ:
...
class TestTirithWarnSafe:
    ...
    def test_warn_session_approved(self, mock_tirith):
        os.environ["QUILL_INTERACTIVE"] = "1"
        session_key = os.getenv("QUILL_SESSION_KEY", "default")
    ...
    def test_warn_non_interactive_auto_allow(self, mock_tirith):
        # No QUILL_INTERACTIVE or QUILL_GATEWAY_SESSION set
        result = check_all_command_guards("curl https://bit.ly/abc", "local")
        assert result["approved"] is True
