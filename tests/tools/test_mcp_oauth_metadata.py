...
Covers:
- :class:`QuillTokenStorage` ``.meta.json`` roundtrip (save / load / remove)
- The production manager provider
  (:class:`tools.mcp_oauth_manager.QuillMCPOAuthProvider`) restoring metadata
...
from tools.mcp_oauth import QuillTokenStorage
from tools.mcp_oauth_manager import _QUILL_PROVIDER_CLS
...
# ---------------------------------------------------------------------------
# QuillTokenStorage metadata roundtrip
# ---------------------------------------------------------------------------
...
class TestMetadataStorage:
    def test_save_and_load_roundtrip(self, tmp_path, monkeypatch):
        ...
        storage = QuillTokenStorage("example-server")
    ...
    def test_remove_deletes_meta_file(self, tmp_path, monkeypatch):
        monkeypatch.setenv("QUILL_HOME", str(tmp_path))
        storage = QuillTokenStorage("cleanup-server")
...
# ---------------------------------------------------------------------------
# Manager-path provider (QuillMCPOAuthProvider) — production code path
# ---------------------------------------------------------------------------


def _manager_provider_with_context(storage: QuillTokenStorage, **context_attrs):
    ...
    """
    if _QUILL_PROVIDER_CLS is None:
        pytest.skip("MCP SDK auth not available")
    provider = _QUILL_PROVIDER_CLS.__new__(_QUILL_PROVIDER_CLS)
    provider._quill_server_name = context_attrs.get("server_name", "srv")
    context = MagicMock()
...
class TestManagerOAuthProviderMetadata:
    def test_initialize_restores_metadata_from_disk(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("QUILL_HOME", str(tmp_path))
        storage = QuillTokenStorage("mgr-srv")
        ...
        with patch.object(
            _QUILL_PROVIDER_CLS.__bases__[0], "_initialize", new=AsyncMock()
        ):
    ...
    def test_initialize_skips_restore_when_in_memory_present(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("QUILL_HOME", str(tmp_path))
        storage = QuillTokenStorage("mgr-srv2")
    ...
    def test_persist_metadata_if_changed_writes_on_first_discover(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("QUILL_HOME", str(tmp_path))
        storage = QuillTokenStorage("persist-srv")
        assert storage.load_oauth_metadata() is None
    ...
    def test_persist_metadata_noop_when_unchanged(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("QUILL_HOME", str(tmp_path))
        storage = QuillTokenStorage("noop-srv")
        ...
        with patch.object(
            QuillTokenStorage, "save_oauth_metadata"
        ) as save_spy:
            provider._persist_oauth_metadata_if_changed()
    ...
    def test_async_auth_flow_persists_on_completion(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("QUILL_HOME", str(tmp_path))
        storage = QuillTokenStorage("flow-srv")
        ...
        with patch.object(
            _QUILL_PROVIDER_CLS.__bases__[0],
            "async_auth_flow",
