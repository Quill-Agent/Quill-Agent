...
class TestCronjobRequirements:
    def test_requires_no_crontab_binary(self, monkeypatch):
        ...
        # because quill uses an internal scheduler, not system crontab.
    ...
    def test_accepts_gateway_session(self, monkeypatch):
        ...
        monkeypatch.setenv("QUILL_GATEWAY_SESSION", "1")
        monkeypatch.delenv("QUILL_EXEC_ASK", raising=False)
    ...
    def test_accepts_exec_ask(self, monkeypatch):
        monkeypatch.delenv("QUILL_INTERACTIVE", raising=False)
        monkeypatch.delenv("QUILL_GATEWAY_SESSION", raising=False)
        monkeypatch.setenv("QUILL_EXEC_ASK", "1")
    ...
    def test_rejects_false_like_interactive_env(self, monkeypatch, false_like_value):
        monkeypatch.setenv("QUILL_INTERACTIVE", false_like_value)
        monkeypatch.delenv("QUILL_GATEWAY_SESSION", raising=False)
        monkeypatch.delenv("QUILL_EXEC_ASK", raising=False)
        assert check_cronjob_requirements() is False
    ...
    @pytest.mark.parametrize(
        "var_name",
        ["QUILL_INTERACTIVE", "QUILL_GATEWAY_SESSION", "QUILL_EXEC_ASK"],
    )
    @pytest.mark.parametrize("false_like_value", ["0", "false", "no", "off"])
    def test_rejects_false_like_any_session_env(
        ...
        for v in ("QUILL_INTERACTIVE", "QUILL_GATEWAY_SESSION", "QUILL_EXEC_ASK"):
