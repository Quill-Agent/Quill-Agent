...
def test_quill_provider_subclass_exists():
    """QuillMCPOAuthProvider is defined and subclasses OAuthClientProvider."""
    from tools.mcp_oauth_manager import _QUILL_PROVIDER_CLS
    from mcp.client.auth.oauth2 import OAuthClientProvider

    assert _QUILL_PROVIDER_CLS is not None
    assert issubclass(_QUILL_PROVIDER_CLS, OAuthClientProvider)
...
def test_manager_builds_quill_provider_subclass(tmp_path, monkeypatch):
    """get_or_build_provider returns QuillMCPOAuthProvider, not plain OAuthClientProvider."""
    from tools.mcp_oauth_manager import (
        MCPOAuthManager, _QUILL_PROVIDER_CLS, reset_manager_for_tests,
    )
    reset_manager_for_tests()
    monkeypatch.setenv("QUILL_HOME", str(tmp_path))
