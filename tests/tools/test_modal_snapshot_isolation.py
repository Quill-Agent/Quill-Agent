...
def _restore_tool_modules():
    original_quill_home = os.environ.get("QUILL_HOME")
    original_modules = {
        name: module
    ...
    try:
        ...
        if original_quill_home is None:
            os.environ.pop("QUILL_HOME", None)
        else:
            os.environ["QUILL_HOME"] = original_quill_home
        _reset_modules(("tools", "quill_cli", "modal"))
...
def _install_modal_test_modules(
    ...
    *,
    ...
    quill_cli = types.ModuleType("quill_cli")
    quill_cli.__path__ = []  # type: ignore[attr-defined]
    sys.modules["quill_cli"] = quill_cli
    quill_home = tmp_path / "quill-home"
    os.environ["QUILL_HOME"] = str(quill_home)
    sys.modules["quill_cli.config"] = types.SimpleNamespace(
        get_quill_home=lambda: quill_home,
    )
    ...
    async def _lookup_aio(_name: str, create_if_missing: bool = False):
        return types.SimpleNamespace(name="quill-agent", create_if_missing=create_if_missing)
    ...
    return {
        "snapshot_store": quill_home / "modal_snapshots.json",
