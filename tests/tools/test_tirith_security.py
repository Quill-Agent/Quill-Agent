...
class TestEnsureInstalled:
    ...
    def test_not_found_returns_none(self, mock_cfg):
        ...
        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._quill_bin_dir", return_value="/nonexistent"), \
...
class TestDiskFailureMarker:
    ...
    def test_install_failed_recovers_from_quill_bin(self):
        """After _INSTALL_FAILED, manual install in QUILL_HOME/bin is picked up."""
        from tools.tirith_security import _resolve_tirith_path, _INSTALL_FAILED
        import tempfile
        tmpdir = tempfile.mkdtemp()
        quill_bin = os.path.join(tmpdir, "tirith")
        # Create a fake executable
        with open(quill_bin, "w") as f:
            f.write("#!/bin/sh\n")
        os.chmod(quill_bin, 0o755)

        _tirith_mod._resolved_path = _INSTALL_FAILED

        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._quill_bin_dir", return_value=tmpdir), \
             patch("tools.tirith_security._clear_install_failed") as mock_clear:
            result = _resolve_tirith_path("tirith")
            assert result == quill_bin
            assert _tirith_mod._resolved_path == quill_bin
            mock_clear.assert_called_once()
...
# ---------------------------------------------------------------------------
# QUILL_HOME isolation
# ---------------------------------------------------------------------------

class TestQuillHomeIsolation:
    def test_quill_bin_dir_respects_quill_home(self):
        """_quill_bin_dir must use QUILL_HOME, not hardcoded ~/.quill."""
        from tools.tirith_security import _quill_bin_dir
        import tempfile
        tmpdir = tempfile.mkdtemp()
        with patch.dict(os.environ, {"QUILL_HOME": tmpdir}):
            result = _quill_bin_dir()
        assert result == os.path.join(tmpdir, "bin")
        assert os.path.isdir(result)

    def test_failure_marker_respects_quill_home(self):
        """_failure_marker_path must use QUILL_HOME, not hardcoded ~/.quill."""
        from tools.tirith_security import _failure_marker_path
        with patch.dict(os.environ, {"QUILL_HOME": "/custom/quill"}):
            result = _failure_marker_path()
        assert result == "/custom/quill/.tirith-install-failed"

    def test_conftest_isolation_prevents_real_home_writes(self):
        """The conftest autouse fixture sets QUILL_HOME; verify it's active."""
        quill_home = os.getenv("QUILL_HOME")
        assert quill_home is not None, "QUILL_HOME should be set by conftest"
        assert "quill_test" in quill_home, "Should point to test temp dir"

    def test_get_quill_home_fallback(self):
        """Without QUILL_HOME set, falls back to the active OS home."""
        from tools.tirith_security import _get_quill_home
        with patch.dict(os.environ, {}, clear=True):
            # Remove QUILL_HOME entirely. With HOME also absent, expanduser
            ...
            expected = os.path.join(os.path.expanduser("~"), ".quill")
            result = _get_quill_home()
