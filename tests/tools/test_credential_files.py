...
class TestRegisterCredentialFiles:
    def test_dict_with_path_key(self, tmp_path):
        ...
        quill_home.mkdir()
        (quill_home / "token.json").write_text("{}")
        ...
        assert len(mounts) == 1
        assert mounts[0]["host_path"] == str(quill_home / "token.json")
        assert mounts[0]["container_path"] == "/root/.quill/token.json"

    def test_dict_with_name_key_fallback(self, tmp_path):
        """Skills use 'name' instead of 'path' — both should work."""
        quill_home = tmp_path / ".quill"
        quill_home.mkdir()
        (quill_home / "google_token.json").write_text("{}")
    ...
    def test_string_entry(self, tmp_path):
        quill_home = tmp_path / ".quill"
        quill_home.mkdir()
        (quill_home / "secret.key").write_text("key")
    ...
    def test_path_takes_precedence_over_name(self, tmp_path):
        ...
        quill_home = tmp_path / ".quill"
        quill_home.mkdir()
        (quill_home / "real.json").write_text("{}")
...
class TestSkillsDirectoryMount:
    def test_returns_mount_when_skills_dir_exists(self, tmp_path):
        ...
        assert mounts[0]["container_path"] == "/root/.quill/skills"
    ...
    def test_custom_container_base(self, tmp_path):
        quill_home = tmp_path / ".quill"
        (quill_home / "skills").mkdir(parents=True)

        with patch.dict(os.environ, {"QUILL_HOME": str(quill_home)}):
            mounts = get_skills_directory_mount(container_base="/home/user/.quill")

        assert mounts[0]["container_path"] == "/home/user/.quill/skills"
...
class TestIterSkillsFiles:
    def test_returns_files_skipping_symlinks(self, tmp_path):
        ...
        assert "/root/.quill/skills/cat/myskill/SKILL.md" in paths
        assert "/root/.quill/skills/cat/myskill/scripts/run.sh" in paths
...
class TestPathTraversalSecurity:
    ...
    def test_dotdot_traversal_rejected(self, tmp_path, monkeypatch):
        """'../sensitive' must not escape QUILL_HOME."""
        monkeypatch.setenv("QUILL_HOME", str(tmp_path / ".quill"))
        (tmp_path / ".quill").mkdir()

        # Create a sensitive file one level above quill_home
    ...
    def test_deep_traversal_rejected(self, tmp_path, monkeypatch):
        ...
        # Create a fake sensitive file outside quill_home
    ...
    def test_legitimate_file_still_works(self, tmp_path, monkeypatch):
        """Normal files inside QUILL_HOME must still be registered."""
        quill_home = tmp_path / ".quill"
        quill_home.mkdir()
        monkeypatch.setenv("QUILL_HOME", str(quill_home))
        (quill_home / "token.json").write_text('{"token": "abc"}')
    ...
    def test_nested_subdir_inside_quill_home_allowed(self, tmp_path, monkeypatch):
        """Files in subdirectories of QUILL_HOME must be allowed."""
        quill_home = tmp_path / ".quill"
        quill_home.mkdir()
        subdir = quill_home / "creds"
        subdir.mkdir()
    ...
    def test_symlink_traversal_rejected(self, tmp_path, monkeypatch):
        """A symlink inside QUILL_HOME pointing outside must be rejected."""
        quill_home = tmp_path / ".quill"
        quill_home.mkdir()
        monkeypatch.setenv("QUILL_HOME", str(quill_home))

        # Create a sensitive file outside quill_home
        sensitive = tmp_path / "sensitive.json"
        sensitive.write_text('{"secret": "value"}')

        # Create a symlink inside quill_home pointing outside
        symlink = quill_home / "evil_link.json"
        try:
        ...
        # The resolved path escapes QUILL_HOME — must be rejected
        assert result is False
        assert get_credential_file_mounts() == []
...
class TestConfigPathTraversal:
    ...
    def _write_config(self, quill_home: Path, cred_files: list):
        import yaml
    ...
    def test_config_traversal_rejected(self, tmp_path, monkeypatch):
        """'../secret' in config.yaml must not escape QUILL_HOME."""
        quill_home = tmp_path / ".quill"
        quill_home.mkdir()
        monkeypatch.setenv("QUILL_HOME", str(quill_home))
        ...
        sensitive.write_text("{}")
        self._write_config(quill_home, ["../secret.json"])
    ...
    def test_config_absolute_path_rejected(self, tmp_path, monkeypatch):
        ...
        sensitive.write_text("{}")
        self._write_config(quill_home, [str(sensitive)])

        mounts = get_credential_file_mounts()
        assert mounts == []

    def test_config_legitimate_file_works(self, tmp_path, monkeypatch):
        """Normal files inside QUILL_HOME via config must still mount."""
        quill_home = tmp_path / ".quill"
        quill_home.mkdir()
        monkeypatch.setenv("QUILL_HOME", str(quill_home))

        (quill_home / "oauth.json").write_text("{}")
        self._write_config(quill_home, ["oauth.json"])
...
class TestCacheDirectoryMounts:
    ...
    def test_returns_existing_cache_dirs(self, tmp_path, monkeypatch):
        ...
        quill_home = tmp_path / ".quill"
        quill_home.mkdir()
        (quill_home / "cache" / "documents").mkdir(parents=True)
        (quill_home / "cache" / "audio").mkdir(parents=True)
        monkeypatch.setenv("QUILL_HOME", str(quill_home))

        mounts = get_cache_directory_mounts()
        paths = {m["container_path"] for m in mounts}
        assert "/root/.quill/cache/documents" in paths
        assert "/root/.quill/cache/audio" in paths
    ...
    def test_skips_nonexistent_dirs(self, tmp_path, monkeypatch):
        ...
        mounts = get_cache_directory_mounts()
        assert len(mounts) == 1
        assert mounts[0]["container_path"] == "/root/.quill/cache/documents"
    ...
    def test_legacy_dir_names_resolved(self, tmp_path, monkeypatch):
        ...
        quill_home = tmp_path / ".quill"
        quill_home.mkdir()
        # Use legacy dir name — get_quill_dir prefers old if it exists
        (quill_home / "document_cache").mkdir()
        (quill_home / "image_cache").mkdir()
        monkeypatch.setenv("QUILL_HOME", str(quill_home))

        mounts = get_cache_directory_mounts()
        host_paths = {m["host_path"] for m in mounts}
        assert str(quill_home / "document_cache") in host_paths
        assert str(quill_home / "image_cache") in host_paths
        # Container paths always use the new layout
        container_paths = {m["container_path"] for m in mounts}
        assert "/root/.quill/cache/documents" in container_paths
        assert "/root/.quill/cache/images" in container_paths
...
class TestIterCacheFiles:
    ...
    def test_enumerates_files(self, tmp_path, monkeypatch):
        ...
        quill_home = tmp_path / ".quill"
        doc_dir = quill_home / "cache" / "documents"
    ...
    def test_nested_files(self, tmp_path, monkeypatch):
        ...
        quill_home = tmp_path / ".quill"
        ss_dir = quill_home / "cache" / "screenshots"
        ...
        assert entries[0]["container_path"] == "/root/.quill/cache/screenshots/session_abc/screen1.png"
