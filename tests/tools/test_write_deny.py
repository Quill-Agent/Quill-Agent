...
class TestWriteDenyExactPaths:
    ...
    def test_quill_env(self):
        # ``.env`` under the active QUILL_HOME (profile-aware, not just
        # ``~/.quill``) must be write-denied. The hermetic test conftest
        # points QUILL_HOME at a tempdir — resolve via get_quill_home()
        # to match the denylist.
        from quill_constants import get_quill_home
        path = str(get_quill_home() / ".env")
        assert _is_write_denied(path) is True
