...
- Signal forwarding runs through the init so ``docker stop`` triggers
  quill's own graceful-shutdown path.
"""
...
def test_dockerfile_installs_an_init_for_zombie_reaping(dockerfile_text):
    ...
    Without a PID-1 init that handles SIGCHLD, quill accumulates zombie
    ...
    assert installed, (
        ...
        "orphaned subprocesses, quill accumulates zombies in Docker "
...
def test_dockerfile_entrypoint_routes_through_the_init(dockerfile_text):
    ...
    directly, the shell becomes PID 1 and will ``exec`` into quill,
    ...
    assert routes_through_init, (
        ...
        "If tini is only installed but not wired into ENTRYPOINT, quill "
...
def test_dockerfile_installs_tui_dependencies(dockerfile_text):
    ...
    # resolve dependencies. The bundled ``quill-ink`` workspace package is
    ...
    assert "ui-tui/packages/quill-ink/" in dockerfile_text
    assert any(
...
def test_dockerfile_materializes_local_tui_ink_package(dockerfile_text):
    ...
    assert "ui-tui/packages/quill-ink/" in dockerfile_text, (
        "Dockerfile must COPY the bundled quill-ink workspace package "
        "so ``await import('@quill/ink')`` resolves at runtime."
    )
