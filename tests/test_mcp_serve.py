"""
Tests for mcp_serve — Quill MCP server.
...
def _isolate_quill_home(tmp_path, monkeypatch):
    """Redirect QUILL_HOME to a temp directory."""
    monkeypatch.setenv("QUILL_HOME", str(tmp_path))
    try:
        import quill_constants
        monkeypatch.setattr(quill_constants, "get_quill_home", lambda: tmp_path)
...
def _create_test_db(db_path, session_id, messages):
    """Create a minimal SQLite DB mimicking quill_state schema."""
