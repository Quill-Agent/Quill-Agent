...
def test_session_search_lazily_opens_db_when_entrypoint_did_not_pass_one(monkeypatch):
    ...
    quill_state = ModuleType("quill_state")
    quill_state.SessionDB = FakeSessionDB
    monkeypatch.setitem(sys.modules, "quill_state", quill_state)
    ...
    result = json.loads(agent._invoke_tool("session_search", {"query": "Quill"}, "task-id"))
