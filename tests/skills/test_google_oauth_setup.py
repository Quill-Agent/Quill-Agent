...
class TestQuillConstantsFallback:
    """Tests for _quill_home.py fallback when quill_constants is unavailable."""

    HELPER_PATH = (
        Path(__file__).resolve().parents[2]
        / "skills/productivity/google-workspace/scripts/_quill_home.py"
    )

    def _load_helper(self, monkeypatch):
        """Load _quill_home.py with quill_constants blocked."""
        monkeypatch.setitem(sys.modules, "quill_constants", None)
        spec = importlib.util.spec_from_file_location("_quill_home_test", self.HELPER_PATH)
        module = importlib.util.module_from_spec(spec)
        assert spec.loader is not None
        spec.loader.exec_module(module)
        return module

    def test_fallback_uses_quill_home_env_var(self, monkeypatch, tmp_path):
        """When quill_constants is missing, QUILL_HOME comes from env var."""
        monkeypatch.setenv("QUILL_HOME", str(tmp_path / "custom-quill"))
        module = self._load_helper(monkeypatch)
        assert module.get_quill_home() == tmp_path / "custom-quill"

    def test_fallback_defaults_to_dot_quill(self, monkeypatch):
        """When quill_constants is missing and QUILL_HOME unset, default to ~/.quill."""
        monkeypatch.delenv("QUILL_HOME", raising=False)
        module = self._load_helper(monkeypatch)
        assert module.get_quill_home() == Path.home() / ".quill"

    def test_fallback_ignores_empty_quill_home(self, monkeypatch):
        """Empty/whitespace QUILL_HOME is treated as unset."""
        monkeypatch.setenv("QUILL_HOME", "  ")
        module = self._load_helper(monkeypatch)
        assert module.get_quill_home() == Path.home() / ".quill"

    def test_fallback_display_quill_home_shortens_path(self, monkeypatch):
        """Fallback display_quill_home() uses ~/ shorthand like the real one."""
        monkeypatch.delenv("QUILL_HOME", raising=False)
        module = self._load_helper(monkeypatch)
        assert module.display_quill_home() == "~/.quill"

    def test_fallback_display_quill_home_profile_path(self, monkeypatch):
        """Fallback display_quill_home() handles profile paths under ~/."""
        monkeypatch.setenv("QUILL_HOME", str(Path.home() / ".quill/profiles/coder"))
        module = self._load_helper(monkeypatch)
        assert module.display_quill_home() == "~/.quill/profiles/coder"

    def test_fallback_display_quill_home_custom_path(self, monkeypatch):
        """Fallback display_quill_home() returns full path for non-home locations."""
        monkeypatch.setenv("QUILL_HOME", "/opt/quill-custom")
        module = self._load_helper(monkeypatch)
        assert module.display_quill_home() == "/opt/quill-custom"

    def test_delegates_to_quill_constants_when_available(self):
        """When quill_constants IS importable, _quill_home delegates to it."""
        spec = importlib.util.spec_from_file_location(
            "_quill_home_happy", self.HELPER_PATH
        )
        module = importlib.util.module_from_spec(spec)
        assert spec.loader is not None
        spec.loader.exec_module(module)
