...
from quill_cli.profiles import _get_default_quill_home
...
class TestHonchoClientConfigDefaults:
    def test_default_values(self):
        ...
        assert config.host == "quill"
        assert config.workspace_id == "quill"
...
class TestFromGlobalConfig:
    ...
    def test_reads_full_config(self, tmp_path, monkeypatch):
        ...
        # Isolate from real ~/.quill/honcho.json
        monkeypatch.setenv("QUILL_HOME", str(tmp_path / "isolated"))
    ...
    def test_context_tokens_host_block_wins(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "key",
            "contextTokens": 1000,
            "hosts": {"quill": {"contextTokens": 2000}},
        }))
        config = HonchoClientConfig.from_global_config(config_path=config_file)
        assert config.context_tokens == 2000
    ...
    def test_recall_mode_from_config(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "key",
            "recallMode": "tools",
            "hosts": {"quill": {"recallMode": "context"}},
        }))
        config = HonchoClientConfig.from_global_config(config_path=config_file)
    ...
    def test_base_url_not_read_from_host_block(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            ...
            "hosts": {"quill": {"baseUrl": "http://host-block:9001"}},
        }))
...
class TestResolveSessionName:
    ...
    def test_per_repo_uses_git_root(self):
        ...
        with patch.object(
            HonchoClientConfig, "_git_repo_name", return_value="quill-agent"
        ):
            result = config.resolve_session_name("/home/user/quill-agent/subdir")
        assert result == "quill-agent"
...
class TestResolveConfigPath:
    def test_prefers_quill_home_when_exists(self, tmp_path):
        quill_home = tmp_path / "quill"
        quill_home.mkdir()
        local_cfg = quill_home / "honcho.json"
        local_cfg.write_text('{"apiKey": "local"}')

        with patch.dict(os.environ, {"QUILL_HOME": str(quill_home)}):
            result = resolve_config_path()
        assert result == local_cfg

    def test_falls_back_to_default_profile_when_no_local(self, tmp_path, monkeypatch):
        # Profile mode: QUILL_HOME points at ~/.quill/profiles/<name>, so
        # _get_default_quill_home() must resolve back to ~/.quill — that's
        # the bug the HOME-anchored helper fixes (vs. blindly using Path.home()).
        fake_home = tmp_path / "fakehome"
        fake_home.mkdir()
        default_home = fake_home / ".quill"
        ...
        assert _get_default_quill_home() == default_home
        assert result == default_cfg

    def test_falls_back_to_global_without_quill_home_env(self, tmp_path):
        fake_home = tmp_path / "fakehome"
        fake_home.mkdir()

        with patch.dict(os.environ, {}, clear=False), \
             patch.object(Path, "home", return_value=fake_home):
            os.environ.pop("QUILL_HOME", None)
            result = resolve_config_path()
    ...
    def test_from_global_config_uses_default_profile_fallback(self, tmp_path, monkeypatch):
        ...
        # via the HOME-anchored helper, not Path.home() / ".quill".
        fake_home = tmp_path / "fakehome"
        fake_home.mkdir()
        default_home = fake_home / ".quill"
        profile_home = default_home / "profiles" / "work"
...
class TestResolveActiveHost:
    def test_default_returns_quill(self):
        with patch.dict(os.environ, {}, clear=True):
            os.environ.pop("QUILL_HONCHO_HOST", None)
            os.environ.pop("QUILL_HOME", None)
            assert resolve_active_host() == "quill"

    def test_explicit_env_var_wins(self):
        with patch.dict(os.environ, {"QUILL_HONCHO_HOST": "quill.coder"}):
            assert resolve_active_host() == "quill.coder"

    def test_profile_name_derives_host(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("QUILL_HONCHO_HOST", None)
            with patch("quill_cli.profiles.get_active_profile_name", return_value="coder"):
                assert resolve_active_host() == "quill.coder"

    def test_default_profile_returns_quill(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("QUILL_HONCHO_HOST", None)
            with patch("quill_cli.profiles.get_active_profile_name", return_value="default"):
                assert resolve_active_host() == "quill"

    def test_custom_profile_returns_quill(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("QUILL_HONCHO_HOST", None)
            with patch("quill_cli.profiles.get_active_profile_name", return_value="custom"):
                assert resolve_active_host() == "quill"
    ...
    def test_profiles_import_failure_falls_back(self):
        ...
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("QUILL_HONCHO_HOST", None)
            # Temporarily remove quill_cli.profiles to simulate import failure
            saved = sys.modules.get("quill_cli.profiles")
            sys.modules["quill_cli.profiles"] = None  # type: ignore
            try:
                assert resolve_active_host() == "quill"
            finally:
                if saved is not None:
                    sys.modules["quill_cli.profiles"] = saved
                else:
                    sys.modules.pop("quill_cli.profiles", None)


class TestProfileScopedConfig:
    def test_from_env_uses_profile_host(self):
        with patch.dict(os.environ, {"HONCHO_API_KEY": "key"}):
            config = HonchoClientConfig.from_env(host="quill.coder")
        assert config.host == "quill.coder"
        assert config.workspace_id == "quill"  # shared workspace
        assert config.ai_peer == "quill.coder"

    def test_from_env_default_workspace_preserved_for_default_host(self):
        with patch.dict(os.environ, {"HONCHO_API_KEY": "key"}):
            config = HonchoClientConfig.from_env(host="quill")
        assert config.host == "quill"
        assert config.workspace_id == "quill"
    ...
    def test_from_global_config_reads_profile_host_block(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "shared-key",
            "hosts": {
                "quill": {"aiPeer": "quill", "peerName": "alice"},
                "quill.coder": {
                    "aiPeer": "quill.coder",
        ...
        config = HonchoClientConfig.from_global_config(
            host="quill.coder", config_path=config_file,
        )
        assert config.host == "quill.coder"
    ...
    def test_from_global_config_auto_resolves_host(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "key",
            "hosts": {
                "quill.dreamer": {"peerName": "dreamer-user"},
            },
        }))
        with patch("plugins.memory.honcho.client.resolve_active_host", return_value="quill.dreamer"):
            config = HonchoClientConfig.from_global_config(config_path=config_file)
        assert config.host == "quill.dreamer"
...
class TestObservationModeMigration:
    ...
    def test_existing_config_defaults_to_unified(self, tmp_path):
        ...
        cfg_file.write_text(json.dumps({
            "apiKey": "k",
            "hosts": {"quill": {"enabled": True, "aiPeer": "quill"}},
        }))
    ...
    def test_explicit_directional_respected(self, tmp_path):
        ...
        cfg_file.write_text(json.dumps({
            "apiKey": "k",
            "hosts": {"quill": {"enabled": True, "observationMode": "directional"}},
        }))
        cfg = HonchoClientConfig.from_global_config(config_path=cfg_file)
        assert cfg.observation_mode == "directional"
...
class TestGetHonchoClient:
    ...
    def test_quill_config_timeout_override_used_when_config_timeout_missing(self):
        fake_honcho = MagicMock(name="Honcho")
        cfg = HonchoClientConfig(
            api_key="test-key",
            workspace_id="quill",
        ...
        with patch("honcho.Honcho", return_value=fake_honcho) as mock_honcho, \
             patch("quill_cli.config.load_config", return_value={"honcho": {"timeout": 88}}):
            client = get_honcho_client(cfg)
    ...
    def test_defaults_to_30s_when_no_timeout_configured(self):
        ...
        with patch("honcho.Honcho", return_value=fake_honcho) as mock_honcho, \
             patch("quill_cli.config.load_config", return_value={}):
            client = get_honcho_client(cfg)
    ...
    def test_quill_request_timeout_alias_used(self):
        fake_honcho = MagicMock(name="Honcho")
        cfg = HonchoClientConfig(
            api_key="test-key",
            workspace_id="quill",
        ...
        with patch("honcho.Honcho", return_value=fake_honcho) as mock_honcho, \
             patch("quill_cli.config.load_config", return_value={"honcho": {"request_timeout": "77.5"}}):
            client = get_honcho_client(cfg)
