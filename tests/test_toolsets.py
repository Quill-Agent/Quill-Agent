...
class TestToolsetConsistency:
    ...
    def test_quill_platforms_share_core_tools(self):
        """All quill-* platform toolsets share the same core tools.

        Platform-specific additions (e.g. ``discord`` / ``discord_admin``
        on quill-discord, gated on DISCORD_BOT_TOKEN) are allowed on top —
        the invariant is that the core set is identical across platforms.
        """
        platforms = ["quill-cli", "quill-telegram", "quill-discord", "quill-whatsapp", "quill-slack", "quill-signal", "quill-homeassistant"]
...
class TestDefaultPlatformWebSearchCoverage:
    def test_quill_whatsapp_toolset_includes_web_search(self):
        assert "web_search" in resolve_toolset("quill-whatsapp")
