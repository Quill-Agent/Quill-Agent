"""Tests for quill_bootstrap — Windows UTF-8 stdio shim.

The bootstrap module is imported at the top of every Quill entry point
(quill, quill-agent, quill-acp, gateway, batch_runner, cli.py).  It
...
Key invariants covered by these tests:
  ...
  5. Load order: every Quill entry point imports quill_bootstrap as its
...
def _fresh_import():
    """Return a freshly-imported quill_bootstrap module.

    Drops any cached copy from sys.modules first so module-level code
    runs again and the platform check re-evaluates.
    """
    sys.modules.pop("quill_bootstrap", None)
    import quill_bootstrap  # noqa: WPS433
    return quill_bootstrap
...
class TestWindowsBehavior:
    ...
    def test_stdout_reconfigured_to_utf8_on_windows(self):
        # The live process's stdout should now be UTF-8 (the Quill CLI
...
class TestEntryPointsImportBootstrap:
    """Every Quill entry point must import quill_bootstrap as its
    ...
    # Entry points that invoke Quill as a process.  Each one must
    # import quill_bootstrap before doing any file I/O or stdout writes.
    ENTRY_POINTS = [
        "quill_cli/main.py",   # quill CLI (console_script)
        "run_agent.py",          # quill-agent (console_script)
        "acp_adapter/entry.py",  # quill-acp (console_script)
    ...
    def test_entry_point_imports_bootstrap(self, path):
        """The file must contain 'import quill_bootstrap' and that
        ...
        half-finished ``quill update`` (git-reset landed new code but
        ``uv pip install -e .`` didn't finish re-registering
        ``quill_bootstrap`` as a top-level module) leaves quill
        ...
        """
        # Resolve relative to the quill-agent repo root.  Tests live
        # at tests/test_quill_bootstrap.py, so go up one dir.
        import pathlib
        ...
        for node in ast.iter_child_nodes(tree):
            ...
            # quill start even when quill_bootstrap hasn't been
        ...
        assert first_import_name == "quill_bootstrap", (
            f"{path}: first top-level import is {first_import_name!r}, "
            f"but it must be 'quill_bootstrap' so UTF-8 stdio is "
