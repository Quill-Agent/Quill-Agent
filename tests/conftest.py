"""Shared fixtures for the quill-agent test suite.
...
2. **Isolated QUILL_HOME.** QUILL_HOME points to a per-test tempdir so
   code reading ``~/.quill/*`` via ``get_quill_home()`` can't see the
   real one. (We do NOT also redirect HOME — that broke subprocesses in
   CI. Code using ``Path.home() / ".quill"`` instead of the canonical
   ``get_quill_home()`` is a bug to fix at the callsite.)
3. **Deterministic runtime.** TZ=UTC, LANG=C.UTF-8, PYTHONHASHSEED=0.
4. **No QUILL_SESSION_* inheritance** — the agent's current gateway
...
# QUILL_* vars that change test behavior by being set. Unset all of these
# unconditionally — individual tests that need them set do so explicitly.
_QUILL_BEHAVIORAL_VARS = frozenset({
    "QUILL_YOLO_MODE",
    "QUILL_INTERACTIVE",
    "QUILL_QUIET",
    "QUILL_TOOL_PROGRESS",
    ...
    "QUILL_SESSION_CHAT_ID",
    "QUILL_SESSION_CHAT_NAME",
    "QUILL_SESSION_THREAD_ID",
    "QUILL_SESSION_SOURCE",
    "QUILL_SESSION_KEY",
    "QUILL_GATEWAY_SESSION",
    "QUILL_PLATFORM",
    "QUILL_MODEL",
    "QUILL_INFERENCE_MODEL",
    "QUILL_INFERENCE_PROVIDER",
    "QUILL_TUI_PROVIDER",
    "QUILL_MANAGED",
    "QUILL_DEV",
    "QUILL_CONTAINER",
    ...
    "QUILL_TIMEZONE",
    "QUILL_REDACT_SECRETS",
    "QUILL_BACKGROUND_NOTIFICATIONS",
    "QUILL_EXEC_ASK",
    "QUILL_HOME_MODE",
    "QUILL_AGENT_USE_LEGACY_SESSION_KEYS",
    # Kanban path/board pins must never leak from a developer shell or
    # dispatched worker into tests; otherwise tests can write fake tasks to
    # the real ~/.quill/kanban.db instead of the per-test QUILL_HOME.
    "QUILL_KANBAN_DB",
    "QUILL_KANBAN_BOARD",
    "QUILL_KANBAN_HOME",
    "QUILL_KANBAN_WORKSPACES_ROOT",
    "QUILL_KANBAN_LOGS_ROOT",
    "QUILL_KANBAN_TASK",
    "QUILL_KANBAN_WORKSPACE",
    "QUILL_KANBAN_RUN_ID",
    "QUILL_KANBAN_CLAIM_LOCK",
    "QUILL_KANBAN_DISPATCH_IN_GATEWAY",
    "QUILL_TENANT",
    "TERMINAL_CWD",
    "TERMINAL_ENV",
...
def _hermetic_environment(tmp_path, monkeypatch):
    ...
    Also redirects HOME and QUILL_HOME to per-test tempdirs so code that
    reads ``~/.quill/*`` can't touch the real one, and pins TZ/LANG so
    ...
    # 2. Blank behavioral QUILL_* vars that could change test semantics.
    for name in _QUILL_BEHAVIORAL_VARS:
        monkeypatch.delenv(name, raising=False)

    # 3. Redirect QUILL_HOME to a per-test tempdir. Code that reads
    #    ``~/.quill/*`` via ``get_quill_home()`` now gets the tempdir.
    #
    #    NOTE: We do NOT also redirect HOME. Doing so broke CI because
    #    some tests (and their transitive deps) spawn subprocesses that
    #    inherit HOME and expect it to be stable. If a test genuinely
    #    needs HOME isolated, it should set it explicitly in its own
    #    fixture. Any code in the codebase reading ``~/.quill/*`` via
    #    ``Path.home() / ".quill"`` instead of ``get_quill_home()``
    #    is a bug to fix at the callsite.
    fake_quill_home = tmp_path / "quill_test"
    fake_quill_home.mkdir()
    (fake_quill_home / "sessions").mkdir()
    (fake_quill_home / "cron").mkdir()
    (fake_quill_home / "memories").mkdir()
    (fake_quill_home / "skills").mkdir()
    monkeypatch.setenv("QUILL_HOME", str(fake_quill_home))
    ...
    #    ~/.quill/plugins/ (which, per step 3, is now empty — but the
    #    singleton might still be cached from a previous test).
    try:
        import quill_cli.plugins as _plugins_mod
...
@pytest.fixture(autouse=True)
def _isolate_quill_home(_hermetic_environment):
...
def _reset_module_state():
    ...
    for _logger_name in ("tools", "run_agent", "trajectory_compressor", "cron", "quill_cli"):
...
@pytest.fixture()
def mock_config():
    """Return a minimal quill config dict suitable for unit tests."""
    return {
...
# environment and finds the developer's live ``quill-gateway`` process
...
# correlated with a ``tests/quill_cli/`` pytest run starting up.
#
...
#    ``check_output`` reject any ``systemctl ... <verb> quill-gateway``
...
def _live_system_guard(request, monkeypatch):
    ...
    tokens[0]), so ``bash -c "systemctl restart quill-gateway"``,
    ``sudo systemctl ...``, ``env systemctl ...``, ``setsid systemctl ...``
    are all caught. ``pkill``/``killall``/``taskkill`` invocations
    targeting quill/python patterns are also blocked.
    """
    ...
    def _matches_quill_gateway(cmd_str: str) -> bool:
        low = cmd_str.lower()
        return any(tok in low for tok in _QUILL_TOKENS)

    def _is_blocked_systemctl(cmd) -> bool:
        cmd_str = _cmd_to_string(cmd)
        if "systemctl" not in cmd_str:
            return False
        if not _matches_quill_gateway(cmd_str):
            return False
        try:
    ...
    def _is_process_killer(cmd) -> bool:
        ...
        for tok in tokens:
            ...
            if head in _PROCESS_KILLERS:
                low = cmd_str.lower()
                # pkill -f pattern: catch quill-themed patterns + a
                # plain "python" -f which would catch the live gateway
                # whose cmdline contains "python -m quill_cli.main".
                if (
                    "quill" in low
                    or "gateway" in low
    ...
    def _check_subprocess_cmd(name, cmd):
        if _is_blocked_systemctl(cmd):
            raise RuntimeError(
                ...
                "live quill-gateway systemd unit. Mock "
        ...
        if _is_process_killer(cmd):
            raise RuntimeError(
                ...
                "targeting quill/python could hit the live gateway. "
