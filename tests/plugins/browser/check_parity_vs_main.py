...
Run from the PR worktree:
    ...
    cd ~/.quill/quill-agent/.worktrees/browser-providers-plugin
...
# checkout lives two levels up at ``~/.quill/quill-agent``.
MAIN_DIR = REPO_ROOT.parent.parent  # ~/.quill/quill-agent
PR_DIR = REPO_ROOT  # the worktree we're in
assert (MAIN_DIR / "tools" / "browser_tool.py").exists(), (
    f"MAIN_DIR={MAIN_DIR} doesn't look like a quill-agent checkout"
)
assert (PR_DIR / "tools" / "browser_tool.py").exists(), (
    f"PR_DIR={PR_DIR} doesn't look like a quill-agent checkout"
)
...
# Isolated QUILL_HOME for the config write.
