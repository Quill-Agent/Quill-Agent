...
Reports live under ``~/.quill/logs/curator/{YYYYMMDD-HHMMSS}/`` alongside
...
def curator_env(tmp_path, monkeypatch):
    """Isolated QUILL_HOME with a skills/ dir + reset curator module state."""
    home = tmp_path / ".quill"
    home.mkdir()
    (home / "skills").mkdir()
    (home / "logs").mkdir()
...
def curator_env_with_cron(curator_env, monkeypatch):
    ...
    monkeypatch.setattr(jobs_mod, "QUILL_DIR", home)
