...
""",
        ...
        from quill_cli import config as _config_mod
    ...
    def test_missing_plugin_entry_yields_default_deny(self, tmp_path, monkeypatch):
        ...
        quill_home = tmp_path / ".quill"
        quill_home.mkdir()
        (quill_home / "config.yaml").write_text("plugins: {}\n", encoding="utf-8")
        monkeypatch.setenv("QUILL_HOME", str(quill_home))
        from quill_cli import config as _config_mod
        _config_mod._config_cache = None  # type: ignore[attr-defined]
...
class TestPluginContextIntegration:
    def test_ctx_llm_is_lazy_singleton(self):
        from quill_cli.plugins import PluginContext, PluginManifest, PluginManager
