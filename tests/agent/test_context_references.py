...
def sample_repo(tmp_path: Path) -> Path:
    ...
    repo.mkdir()
    _git(repo, "init")
    _git(repo, "config", "user.name", "Quill Tests")
...
@pytest.mark.asyncio
async def test_blocks_sensitive_home_and_quill_paths(tmp_path: Path, monkeypatch):
    from agent.context_references import preprocess_context_references_async

    monkeypatch.setenv("HOME", str(tmp_path))
    monkeypatch.setenv("QUILL_HOME", str(tmp_path / ".quill"))

    quill_env = tmp_path / ".quill" / ".env"
    quill_env.parent.mkdir(parents=True)
    quill_env.write_text("API_KEY=super-secret\n", encoding="utf-8")
    ...
    result = await preprocess_context_references_async(
        "read @file:.quill/.env and @file:.ssh/id_rsa",
        cwd=tmp_path,
