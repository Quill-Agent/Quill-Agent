...
def test_quill_client_tag_includes_current_version():
    """The client tag must reflect quill_cli.__version__ verbatim."""
    from quill_cli import __version__
    from agent.portal_tags import quill_client_tag

    assert quill_client_tag() == f"client=quill-client-v{__version__}"


def test_quill_client_tag_format():
    """The client tag has the exact shape Nous Portal expects."""
    from agent.portal_tags import quill_client_tag

    tag = quill_client_tag()
    assert tag.startswith("client=quill-client-v")
...
def test_nous_portal_tags_contains_product_and_client():
    ...
    from agent.portal_tags import quill_client_tag, nous_portal_tags

    tags = nous_portal_tags()
    assert "product=quill-agent" in tags
    assert quill_client_tag() in tags
    assert len(tags) == 2
