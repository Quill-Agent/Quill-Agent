...
class TestSpawnEnvIsolation:
    ...
    def test_kanban_worker_adds_only_kanban_writable_root(self, monkeypatch):
        ...
        danger-full-access. Quill passes a narrow app-server config override
        ...
        monkeypatch.setattr(subprocess, "Popen", FakePopen)
        monkeypatch.setenv("HOME", "/users/alice")
        monkeypatch.setenv("QUILL_HOME", "/users/alice/.quill/profiles/backend-worker")
        monkeypatch.setenv("QUILL_KANBAN_TASK", "t_smoke")
        monkeypatch.setenv(
            "QUILL_KANBAN_DB",
            "/users/alice/.quill/kanban/boards/smoke/kanban.db",
        )
        ...
        assert (
            'sandbox_workspace_write.writable_roots=["/users/alice/.quill/kanban/boards/smoke"]'
            in cmd
        )
