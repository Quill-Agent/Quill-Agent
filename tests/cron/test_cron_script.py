...
- Path resolution (absolute, relative to QUILL_HOME/scripts/)
"""
...
def cron_env(tmp_path, monkeypatch):
    """Isolated cron environment with temp QUILL_HOME."""
    quill_home = tmp_path / ".quill"
    quill_home.mkdir()
    (quill_home / "cron").mkdir()
    (quill_home / "cron" / "output").mkdir()
    (quill_home / "scripts").mkdir()
    monkeypatch.setenv("QUILL_HOME", str(quill_home))

    # Clear cached module-level paths
    import cron.jobs as jobs_mod
    monkeypatch.setattr(jobs_mod, "QUILL_DIR", quill_home)
    monkeypatch.setattr(jobs_mod, "CRON_DIR", quill_home / "cron")
    monkeypatch.setattr(jobs_mod, "JOBS_FILE", quill_home / "cron" / "jobs.json")
    monkeypatch.setattr(jobs_mod, "OUTPUT_DIR", quill_home / "cron" / "output")
...
class TestScriptPathContainment:
    ...
    def test_absolute_path_outside_scripts_dir_blocked(self, cron_env):
        """Absolute paths outside ~/.quill/scripts/ must be rejected."""
        from cron.scheduler import _run_job_script
