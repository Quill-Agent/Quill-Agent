...
def _isolate(tmp_path, monkeypatch):
    """Isolate QUILL_HOME so tests don't touch real config."""
    home = tmp_path / ".quill"
    home.mkdir()
...
def cli_obj(_isolate):
    """Create a minimal QuillCLI instance for banner testing."""
    with patch("cli.load_cli_config", return_value={
        ...
        obj = QuillCLI.__new__(QuillCLI)
        obj.model = "test-model"
        obj.enabled_toolsets = ["quill-core"]
        obj.compact = False
