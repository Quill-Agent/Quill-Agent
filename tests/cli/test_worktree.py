...
class TestWorktreeCreation:
    ...
    def test_creates_worktree(self, git_repo):
        ...
        assert info["branch"].startswith("quill/quill-")
...
class TestOrphanedBranchPruning:
    """Test cleanup of orphaned quill/* and pr-* branches."""

    def test_prunes_orphaned_quill_branch(self, git_repo):
        """quill/quill-* branches with no worktree should be deleted."""
        # Create a branch that looks like a worktree branch but has no worktree
        subprocess.run(
            ["git", "branch", "quill/quill-deadbeef", "HEAD"],
            cwd=str(git_repo), capture_output=True,
        )

        # Verify it exists
        result = subprocess.run(
            ["git", "branch", "--list", "quill/quill-deadbeef"],
            capture_output=True, text=True, cwd=str(git_repo),
        )
        assert "quill/quill-deadbeef" in result.stdout
        ...
        assert "quill/quill-deadbeef" in orphaned
        ...
        assert "quill/quill-deadbeef" not in result.stdout
