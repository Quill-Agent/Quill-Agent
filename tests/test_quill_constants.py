"""Tests for quill_constants module."""
...
from quill_constants import (
    ...
    get_default_quill_root,
    is_container,
    parse_reasoning_effort,
)


class TestGetDefaultQuillRoot:
    """Tests for get_default_quill_root() — Docker/custom deployment awareness."""

    def test_no_quill_home_returns_native(self, tmp_path, monkeypatch):
        """When QUILL_HOME is not set, returns ~/.quill."""
        monkeypatch.delenv("QUILL_HOME", raising=False)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)

        assert get_default_quill_root() == tmp_path / ".quill"

    def test_quill_home_is_native(self, tmp_path, monkeypatch):
        """When QUILL_HOME = ~/.quill, returns ~/.quill."""
        native = tmp_path / ".quill"
        native.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("QUILL_HOME", str(native))
        assert get_default_quill_root() == native

    def test_quill_home_is_profile(self, tmp_path, monkeypatch):
        """When QUILL_HOME is a profile under ~/.quill, returns ~/.quill."""
        native = tmp_path / ".quill"
    ...
    def test_quill_home_is_docker(self, tmp_path, monkeypatch):
        """When QUILL_HOME points outside ~/.quill (Docker), returns QUILL_HOME."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("QUILL_HOME", str(docker_home))
        assert get_default_quill_root() == docker_home

    def test_quill_home_is_custom_path(self, tmp_path, monkeypatch):
        """Any QUILL_HOME outside ~/.quill is treated as the root."""
        custom = tmp_path / "my-quill-data"
        custom.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("QUILL_HOME", str(custom))
        assert get_default_quill_root() == custom

    def test_docker_profile_active(self, tmp_path, monkeypatch):
        """When a Docker profile is active (QUILL_HOME=<root>/profiles/<name>),
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("QUILL_HOME", str(profile))
        assert get_default_quill_root() == docker_root
...
class TestIsContainer:
    ...
    def _reset_cache(self, monkeypatch):
        ...
        monkeypatch.setattr(quill_constants, "_container_detected", None)
    ...
    def test_caches_result(self, monkeypatch):
        ...
        monkeypatch.setattr(quill_constants, "_container_detected", True)
        assert is_container() is True
