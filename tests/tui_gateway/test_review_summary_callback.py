...
def server():
    with patch.dict(
        "sys.modules",
        {
            "quill_constants": MagicMock(
                get_quill_home=MagicMock(return_value="/tmp/quill_test_review_summary")
            ),
...
def test_init_session_attaches_background_review_callback(server, monkeypatch):
    ...
    cb("💾 Self-improvement review: Skill 'quill-release' patched")
    ...
    assert sid == "sid-abc"
    assert payload == {
        "text": "💾 Self-improvement review: Skill 'quill-release' patched"
    }
