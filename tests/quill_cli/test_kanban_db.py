"""Tests for the Kanban DB layer (quill_cli.kanban_db)."""
...
from quill_cli import kanban_db as kb


@pytest.fixture
def kanban_home(tmp_path, monkeypatch):
    """Isolated QUILL_HOME with an empty kanban DB."""
    home = tmp_path / ".quill"
    home.mkdir()
...
def test_claim_uses_env_default_ttl(kanban_home, monkeypatch):
    monkeypatch.setenv("QUILL_KANBAN_CLAIM_TTL_SECONDS", "3600")
    with kb.connect() as conn:
        t = kb.create_task(conn, title="x", assignee="a")
...
def test_stale_claim_reclaimed(kanban_home, monkeypatch):
    import signal
    import quill_cli.kanban_db as _kb
...
def test_has_spawnable_ready_true_when_real_profile_present(kanban_home, monkeypatch):
    ...
    has an assignee that maps to a real Quill profile — preserves the
    ...
    with kb.connect() as conn:
        ...
        kb.create_task(conn, title="quill-task", assignee="daily")
...
def test_scratch_workspace_created_under_quill_home(kanban_home):
    with kb.connect() as conn:
        t = kb.create_task(conn, title="x")
        task = kb.get_task(conn, t)
...
# spawned with `quill -p <profile>` must read/write the same kanban.db
...
# where `kanban_db_path()` resolved to the active profile's QUILL_HOME.
# ---------------------------------------------------------------------------

class TestSharedBoardPaths:
    """`kanban_home`/`kanban_db_path`/`workspaces_root`/`worker_log_path`
    must anchor at the **shared root**, not the active profile's QUILL_HOME."""

    def _set_home(self, monkeypatch, tmp_path, quill_home):
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("QUILL_HOME", str(quill_home))
        monkeypatch.delenv("QUILL_KANBAN_HOME", raising=False)

    def test_default_install_anchors_at_home_dot_quill(
        self, tmp_path, monkeypatch
    ):
        # Standard install: QUILL_HOME == ~/.quill, no profile active.
        default_home = tmp_path / ".quill"
        default_home.mkdir()
    ...
    def test_profile_worker_resolves_to_shared_root(
        ...
        # Reproduces the bug: dispatcher uses ~/.quill/kanban.db,
        # worker spawned with -p <profile> previously resolved to
        # ~/.quill/profiles/<profile>/kanban.db. After the fix both
        # converge on ~/.quill/kanban.db.
        default_home = tmp_path / ".quill"
        default_home.mkdir()
    ...
    def test_dispatcher_and_profile_worker_converge(
        ...
        # QUILL_HOME and confirm equality. This is the property the
        ...
        # Worker's perspective (profile activated by `quill -p coder`).
        monkeypatch.setenv("QUILL_HOME", str(profile_home))
    ...
    def test_docker_custom_quill_home_uses_env_path_directly(
        self, tmp_path, monkeypatch
    ):
        # Docker / custom deployment: QUILL_HOME points outside ~/.quill.
        # `get_default_quill_root()` returns env_home directly when it
        # is not a `<root>/profiles/<name>` shape and not under
        # `Path.home() / ".quill"`.
        custom_root = tmp_path / "opt" / "quill"
    ...
    def test_docker_profile_layout_uses_grandparent(
        ...
        # Docker profile shape: QUILL_HOME=/opt/quill/profiles/coder;
        # `get_default_quill_root()` walks up to /opt/quill because
    ...
    def test_explicit_override_via_quill_kanban_home(
        self, tmp_path, monkeypatch
    ):
        # Explicit override: QUILL_KANBAN_HOME beats every other
        # resolution rule.
        default_home = tmp_path / ".quill"
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("QUILL_HOME", str(profile_home))
        monkeypatch.setenv("QUILL_KANBAN_HOME", str(override))
    ...
    def test_empty_override_falls_through(self, tmp_path, monkeypatch):
        ...
        default_home.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("QUILL_HOME", str(default_home))
        monkeypatch.setenv("QUILL_KANBAN_HOME", "   ")
    ...
    def test_dispatcher_and_worker_share_a_real_database(
        ...
        # QUILL_HOME perspectives via a real SQLite file. Without the
        ...
        # Worker switches to the profile QUILL_HOME and reads.
        monkeypatch.setenv("QUILL_HOME", str(profile_home))
        with kb.connect() as conn:
    ...
    def test_quill_kanban_db_pin_beats_kanban_home(
        self, tmp_path, monkeypatch
    ):
        # QUILL_KANBAN_DB pins the file path directly and beats both
        # QUILL_KANBAN_HOME and the `get_default_quill_root()` path.
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("QUILL_HOME", str(default_home))
        monkeypatch.setenv("QUILL_KANBAN_HOME", str(umbrella))
        monkeypatch.setenv("QUILL_KANBAN_DB", str(pinned_db))

        assert kb.kanban_db_path() == pinned_db
        # workspaces_root still follows QUILL_KANBAN_HOME -- the pins
        # are independent.
        assert kb.workspaces_root() == umbrella / "kanban" / "workspaces"

    def test_quill_kanban_workspaces_root_pin_beats_kanban_home(
        self, tmp_path, monkeypatch
    ):
        # QUILL_KANBAN_WORKSPACES_ROOT pins the workspaces root directly.
        default_home = tmp_path / ".quill"
        default_home.mkdir()
        umbrella = tmp_path / "umbrella"
        umbrella.mkdir()
        ...
        monkeypatch.setenv("QUILL_HOME", str(default_home))
        monkeypatch.setenv("QUILL_KANBAN_HOME", str(umbrella))
        monkeypatch.setenv("QUILL_KANBAN_WORKSPACES_ROOT", str(pinned_ws))

        assert kb.workspaces_root() == pinned_ws
        # kanban_db_path still follows QUILL_KANBAN_HOME.
    ...
    def test_empty_per_path_overrides_fall_through(
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("QUILL_HOME", str(default_home))
        monkeypatch.setenv("QUILL_KANBAN_DB", "   ")
        monkeypatch.setenv("QUILL_KANBAN_WORKSPACES_ROOT", "")
    ...
    def test_dispatcher_spawn_injects_kanban_db_and_workspaces_root(
        ...
        # The dispatcher's `_default_spawn` must inject QUILL_KANBAN_DB
        # and QUILL_KANBAN_WORKSPACES_ROOT into the worker env so the
        # worker converges on the dispatcher's paths even when the
        # `-p <profile>` flag rewrites QUILL_HOME.
        default_home = tmp_path / ".quill"
        default_home.mkdir()
        self._set_home(monkeypatch, tmp_path, default_home)
        ...
        env = captured["env"]
        assert env["QUILL_KANBAN_DB"] == str(default_home / "kanban.db")
        assert env["QUILL_KANBAN_WORKSPACES_ROOT"] == str(
            default_home / "kanban" / "workspaces"
        )
        assert env["QUILL_KANBAN_TASK"] == "t_dispatch_env"
        assert env["QUILL_KANBAN_BRANCH"] == "wt/t_dispatch_env"
...
# ---------------------------------------------------------------------------
# NFS / network-filesystem fallback (see quill_state.apply_wal_with_fallback)
# ---------------------------------------------------------------------------
...
def test_connect_falls_back_to_delete_on_locking_protocol(kanban_home, caplog):
    ...
    with _patch("quill_cli.kanban_db.sqlite3.connect", side_effect=wal_blocking_connect):
        with caplog.at_level("WARNING", logger="quill_state"):
            conn = kb.connect()
...
def test_unlink_tasks_triggers_recompute_ready(kanban_home):
    ...
    next dispatcher tick or a manual 'quill kanban recompute' would promote it.
    """
    with kb.connect() as conn:
        # A is done.
...
# ---------------------------------------------------------------------------
# Dispatcher spawn invocation — _resolve_quill_argv()
#
# Workers spawned by the dispatcher must use a `quill` invocation that does
...
# $PATH that doesn't include the venv's bin/, so a bare `["quill", ...]`
...
def test_resolve_quill_argv_prefers_path_shim(monkeypatch):
    """When `quill` is on PATH, use the shim — preserves familiar ps output."""
    import shutil
    import quill_cli.kanban_db as kb

    monkeypatch.delenv("QUILL_BIN", raising=False)
    monkeypatch.setattr(shutil, "which", lambda name: "/usr/local/bin/quill")
    argv = kb._resolve_quill_argv()
    assert argv == ["/usr/local/bin/quill"]


def test_resolve_quill_argv_absolutizes_relative_exe_shim(monkeypatch, tmp_path):
    """A relative executable override must not remain workspace-cwd-dependent."""
    import quill_cli.kanban_db as kb

    monkeypatch.chdir(tmp_path)
    monkeypatch.setenv("QUILL_BIN", ".\\quill.exe")
    monkeypatch.setattr(kb, "_IS_WINDOWS", True)

    assert kb._resolve_quill_argv() == [os.path.abspath(".\\quill.exe")]


def test_resolve_quill_argv_avoids_implicit_windows_batch_shim(monkeypatch, tmp_path):
    ...
    bin_dir.mkdir()
    (bin_dir / "quill.CMD").write_text("@echo off\n", encoding="utf-8")
    monkeypatch.delenv("QUILL_BIN", raising=False)
    monkeypatch.setenv("PATH", str(bin_dir))
    monkeypatch.setenv("PATHEXT", ".CMD")
    monkeypatch.setattr(kb, "_IS_WINDOWS", True)

    assert kb._resolve_quill_argv() == [sys.executable, "-m", "quill_cli.main"]


def test_resolve_quill_argv_honors_quill_bin_path_override(monkeypatch, tmp_path):
    """An explicit path-like QUILL_BIN lets service managers pin the executable."""
    import shutil
    import quill_cli.kanban_db as kb

    shim = tmp_path / "bin" / "quill"
    shim.parent.mkdir()
    shim.write_text("#!/bin/sh\n", encoding="utf-8")
    monkeypatch.setenv("QUILL_BIN", str(shim))
    monkeypatch.setattr(shutil, "which", lambda name: None)

    assert kb._resolve_quill_argv() == [str(shim)]


def test_resolve_quill_argv_quill_bin_bare_name_uses_path(monkeypatch, tmp_path):
    """Bare QUILL_BIN values keep PATH semantics instead of cwd shadowing."""
    import stat
    import quill_cli.kanban_db as kb

    cwd_quill = tmp_path / "quill"
    cwd_quill.write_text("wrong\n", encoding="utf-8")
    cwd_quill.chmod(cwd_quill.stat().st_mode | stat.S_IXUSR)
    path_quill = tmp_path / "bin" / "quill"
    path_quill.parent.mkdir()
    path_quill.write_text("right\n", encoding="utf-8")
    path_quill.chmod(path_quill.stat().st_mode | stat.S_IXUSR)
    monkeypatch.chdir(tmp_path)
    monkeypatch.setenv("PATH", str(path_quill.parent))
    monkeypatch.setenv("QUILL_BIN", "quill")

    assert kb._resolve_quill_argv() == [str(path_quill)]


def test_resolve_quill_argv_quill_bin_bare_name_ignores_cwd(monkeypatch, tmp_path):
    """Bare QUILL_BIN does not accept current-directory shadow executables."""
    import sys
    import quill_cli.kanban_db as kb

    (tmp_path / "quill.exe").write_text("wrong\n", encoding="utf-8")
    monkeypatch.chdir(tmp_path)
...
def test_resolve_quill_argv_quill_bin_bare_cmd_uses_module_fallback(monkeypatch, tmp_path):
    """A PATH-resolved QUILL_BIN batch shim is not used as worker argv[0]."""
    import sys
    import quill_cli.kanban_db as kb
...
def test_resolve_quill_argv_quill_bin_unresolved_bare_name_falls_back(monkeypatch):
    """Unresolved QUILL_BIN command names do not delegate cwd search to Popen."""
    import sys
    import quill_cli.kanban_db as kb
...
def test_resolve_quill_argv_falls_back_to_module_form_when_no_path_shim(monkeypatch):
    """When the shim is not on PATH, fall back to `python -m quill_cli.main`.

    Pins the correct module name (NOT `quill` — there is no top-level
    `quill` package). Regression for #23198: the original PR shipped
    `python -m quill` which fails with `No module named quill` on every
    invocation.
    """
    import shutil
    import sys
    ...
    monkeypatch.setattr(shutil, "which", lambda name: None)
    argv = kb._resolve_quill_argv()
    assert argv == [sys.executable, "-m", "quill_cli.main"]


def test_resolve_quill_argv_module_actually_runs():
    """The fallback module name must be importable + runnable.

    A unit test that pins the literal string is necessary but not
    sufficient — if `quill_cli.main` ever loses `if __name__ == "__main__"`
    handling or its argparse setup, `python -m quill_cli.main --version`
    ...
    assert "Quill Agent" in r.stdout, f"unexpected output: {r.stdout[:200]!r}"
