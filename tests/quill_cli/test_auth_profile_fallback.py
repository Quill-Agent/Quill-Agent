...
When ``QUILL_HOME`` points to a named profile, ``read_credential_pool()``
...
def profile_env(tmp_path, monkeypatch):
    """Set up a global root + an active profile under Path.home()/.quill/profiles/coder.

    * Path.home() -> tmp_path
    * Global root -> tmp_path/.quill            (has its own auth.json fixture)
    * Profile     -> tmp_path/.quill/profiles/coder   (active, QUILL_HOME points here)
...
def test_classic_mode_does_not_double_read_same_file(tmp_path, monkeypatch):
    """In classic mode (QUILL_HOME == global root), no fallback path runs.
    ...
    # sees tmp_path/home/.quill as the "real home" — which is NOT equal
    # to the QUILL_HOME we set (tmp_path/classic), so the guard passes.
    fake_home = tmp_path / "home"
    fake_home.mkdir()
    monkeypatch.setattr(Path, "home", lambda: fake_home)
    quill_home = tmp_path / "classic"
    quill_home.mkdir()
    monkeypatch.setenv("QUILL_HOME", str(quill_home))

    _write(quill_home / "auth.json", _make_auth_store(pool={
        "openrouter": [{
            "id": "only",
            "label": "classic",
            "auth_type": "api_key",
    ...
    from quill_cli.auth import read_credential_pool, _global_auth_file_path

    # Classic mode: QUILL_HOME is set to a custom path that is NOT under
    # ~/.quill/profiles/ — get_default_quill_root() returns QUILL_HOME
...
def test_write_credential_pool_targets_profile_not_global(profile_env):
    from quill_cli.auth import read_credential_pool, write_credential_pool
