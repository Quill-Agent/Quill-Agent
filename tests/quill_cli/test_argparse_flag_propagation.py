...
subparser, placing the flag BEFORE the subcommand (e.g. 'quill --yolo chat')
...
def _build_parser():
    """Build the quill argument parser from the real code.
...
class TestYoloEnvVar:
    """Verify --yolo sets QUILL_YOLO_MODE regardless of flag position.
...
class TestAcceptHooksOnAgentSubparsers:
    ...
    parser and `chat`, so `quill gateway run --accept-hooks` failed
    ...
    def test_accepted_at_every_position(self, argv):
        """Invoking `quill <argv>` must exit 0 (help) rather than
