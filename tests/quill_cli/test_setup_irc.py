"""Tests for IRC gateway configuration via `quill setup gateway` UI.

Covers the full plugin-platform discovery → status → configure flow so that
a fresh Quill install (no state, no env vars) can set up IRC through the
...
class TestIRCFreshInstallDiscovery:
    """IRC appears in the setup menu on a brand-new Quill install."""
    ...
    def test_irc_status_configured_when_env_set(self, monkeypatch):
        ...
        try:
            ...
            monkeypatch.setenv("IRC_CHANNEL", "#quill")
            monkeypatch.setenv("IRC_NICKNAME", "quill-bot")
...
class TestIRCGatewaySetupFreshInstall:
    """Simulate the full `quill setup gateway` experience with IRC present."""
