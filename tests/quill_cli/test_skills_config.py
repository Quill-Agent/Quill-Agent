"""Tests for quill_cli/skills_config.py and skills_tool disabled filtering."""
import pytest
...
class TestSaveDisabledSkills:
    @patch("quill_cli.skills_config.save_config")
    def test_saves_global_sorted(self, mock_save):
        from quill_cli.skills_config import save_disabled_skills
        config = {}
...
class TestIsSkillDisabled:
    @patch("quill_cli.config.load_config")
    ...
    @patch("quill_cli.config.load_config")
    @patch.dict("os.environ", {"QUILL_PLATFORM": "discord"})
...
class TestGetDisabledSkillNames:
    ...
    def test_session_platform_env_var(self, tmp_path, monkeypatch):
        """QUILL_SESSION_PLATFORM should be used when QUILL_PLATFORM is unset."""
        config = tmp_path / "config.yaml"
        config.write_text(
            "skills:\n"
            "  disabled:\n"
    ...
    def test_quill_platform_takes_precedence(self, tmp_path, monkeypatch):
        """QUILL_PLATFORM should win over QUILL_SESSION_PLATFORM."""
        config = tmp_path / "config.yaml"
        config.write_text(
            "skills:\n"
            "  platform_disabled:\n"
    ...
    def test_explicit_param_overrides_env_vars(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("QUILL_HOME", str(tmp_path))
        monkeypatch.setenv("QUILL_PLATFORM", "telegram")
        monkeypatch.setenv("QUILL_SESSION_PLATFORM", "telegram")
