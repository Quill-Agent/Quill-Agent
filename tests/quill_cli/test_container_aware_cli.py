...
def container_env(tmp_path, monkeypatch):
    """Set up a fake QUILL_HOME with .container-mode file."""
    quill_home = tmp_path / ".quill"
    quill_home.mkdir()
    monkeypatch.setenv("QUILL_HOME", str(quill_home))
    monkeypatch.delenv("QUILL_DEV", raising=False)

    container_mode = quill_home / ".container-mode"
    container_mode.write_text(
        "# Written by NixOS activation script. Do not edit manually.\n"
        "backend=podman\n"
        "container_name=quill-agent\n"
        "exec_user=quill\n"
        "quill_bin=/data/current-package/bin/quill\n"
    )
    return quill_home
...
def test_get_container_exec_info_returns_metadata(container_env):
    ...
    with patch("quill_constants.is_container", return_value=False):
        info = get_container_exec_info()

    assert info is not None
    assert info["backend"] == "podman"
    assert info["container_name"] == "quill-agent"
    assert info["exec_user"] == "quill"
    assert info["quill_bin"] == "/data/current-package/bin/quill"


def test_get_container_exec_info_none_inside_container(container_env):
    """Returns None when we're already inside a container."""
    with patch("quill_constants.is_container", return_value=True):
        info = get_container_exec_info()
...
def test_get_container_exec_info_skipped_when_quill_dev(container_env, monkeypatch):
    """Returns None when QUILL_DEV=1 is set (dev mode bypass)."""
    monkeypatch.setenv("QUILL_DEV", "1")
...
def test_get_container_exec_info_not_skipped_when_quill_dev_zero(container_env, monkeypatch):
    """QUILL_DEV=0 does NOT trigger bypass — only '1' does."""
    monkeypatch.setenv("QUILL_DEV", "0")
...
def test_get_container_exec_info_defaults():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        quill_home = Path(tmpdir) / ".quill"
        quill_home.mkdir()
        (quill_home / ".container-mode").write_text(
            "# minimal file with no keys\n"
        )

        with patch("quill_constants.is_container", return_value=False), \
             patch.dict(get_container_exec_info.__globals__, {"get_quill_home": lambda: quill_home}), \
             patch.dict(os.environ, {}, clear=False):
            os.environ.pop("QUILL_DEV", None)
            info = get_container_exec_info()
...
def test_get_container_exec_info_docker_backend(container_env):
    ...
    (container_env / ".container-mode").write_text(
        "backend=docker\n"
        "container_name=quill-custom\n"
        "exec_user=myuser\n"
        "quill_bin=/opt/quill/bin/quill\n"
    )
    ...
    assert info["backend"] == "docker"
    assert info["container_name"] == "quill-custom"
    assert info["exec_user"] == "myuser"
    assert info["quill_bin"] == "/opt/quill/bin/quill"
...
def test_exec_in_container_calls_execvp(docker_container_info):
    ...
    from quill_cli.main import _exec_in_container
    ...
    assert "-it" in cmd
    idx_u = cmd.index("-u")
    assert cmd[idx_u + 1] == "quill"
    e_indices = [i for i, v in enumerate(cmd) if v == "-e"]
    e_values = [cmd[i + 1] for i in e_indices]
    assert "TERM=xterm-256color" in e_values
    assert "LANG=en_US.UTF-8" in e_values
    assert "quill-agent" in cmd
    assert "/data/current-package/bin/quill" in cmd
    assert "chat" in cmd
