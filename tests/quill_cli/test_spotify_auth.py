...
def test_auth_spotify_status_command_reports_logged_in(capsys, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    from quill_cli.auth_commands import auth_status_command
...
def test_spotify_interactive_setup_persists_client_id(
    ...
    """The wizard writes QUILL_SPOTIFY_CLIENT_ID to .env and returns the value."""
    monkeypatch.setenv("QUILL_HOME", str(tmp_path))
    ...
    assert "QUILL_SPOTIFY_CLIENT_ID=wizard-client-123" in env_text
    # Default redirect URI should NOT be persisted.
    assert "QUILL_SPOTIFY_REDIRECT_URI" not in env_text
