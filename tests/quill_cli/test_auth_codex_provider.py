"""Tests for Codex auth — tokens stored in Quill auth store (~/.quill/auth.json)."""
...
def _setup_quill_auth(quill_home: Path, *, access_token: str = "access", refresh_token: str = "refresh"):
    """Write Codex tokens into the Quill auth store."""
...
def test_resolve_codex_runtime_credentials_refreshes_expiring_token(tmp_path, monkeypatch):
    ...
    _setup_quill_auth(quill_home, access_token=expiring_token, refresh_token="refresh-old")
    monkeypatch.setenv("QUILL_HOME", str(quill_home))
    ...
    monkeypatch.setattr("quill_cli.auth._refresh_codex_auth_tokens", _fake_refresh)
...
def test_resolve_codex_runtime_credentials_force_refresh(tmp_path, monkeypatch):
    quill_home = tmp_path / "quill"
    _setup_quill_auth(quill_home, access_token="access-current", refresh_token="refresh-old")
    monkeypatch.setenv("QUILL_HOME", str(quill_home))
...
def test_codex_tokens_not_written_to_shared_file(tmp_path, monkeypatch):
    """Verify _save_codex_tokens writes only to Quill auth store, not ~/.codex/."""
    quill_home = tmp_path / "quill"
    codex_home = tmp_path / "codex-cli"
    quill_home.mkdir(parents=True, exist_ok=True)
    codex_home.mkdir(parents=True, exist_ok=True)
    ...
    _save_codex_tokens({"access_token": "quill-at", "refresh_token": "quill-rt"})

    # ~/.codex/auth.json should NOT exist — _save_codex_tokens only touches Quill store
    assert not (codex_home / "auth.json").exists()

    # Quill auth store should have the tokens
    data = _read_codex_tokens()
    assert data["tokens"]["access_token"] == "quill-at"


def test_resolve_returns_quill_auth_store_source(tmp_path, monkeypatch):
    quill_home = tmp_path / "quill"
    _setup_quill_auth(quill_home)
    monkeypatch.setenv("QUILL_HOME", str(quill_home))
...
def test_login_openai_codex_force_new_login_skips_existing_reuse_prompt(monkeypatch):
    ...
    monkeypatch.setattr(
        "quill_cli.auth.resolve_codex_runtime_credentials",
        lambda: {"base_url": DEFAULT_CODEX_BASE_URL},
    )
    monkeypatch.setattr(
        "quill_cli.auth._import_codex_cli_tokens",
    ...
    monkeypatch.setattr("quill_cli.auth._save_codex_tokens", _fake_save)
    monkeypatch.setattr("quill_cli.auth._update_config_for_provider", lambda *args, **kwargs: "/tmp/config.yaml")
    monkeypatch.setattr(
        "builtins.input",
