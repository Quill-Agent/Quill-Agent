"""Tests for the top-level `./quill` launcher script."""
...
def test_launcher_delegates_to_argparse_entrypoint(monkeypatch):
    """`./quill` should use `quill_cli.main`, not the legacy Fire wrapper."""
    launcher_path = Path(__file__).resolve().parents[2] / "quill"
    called = []

    fake_main_module = types.ModuleType("quill_cli.main")

    def fake_main():
        called.append("quill_cli.main")

    fake_main_module.main = fake_main
    monkeypatch.setitem(sys.modules, "quill_cli.main", fake_main_module)
