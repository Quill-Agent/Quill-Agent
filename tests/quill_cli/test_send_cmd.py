"""Tests for the ``quill send`` CLI subcommand.

Covers the argument parsing / stdin / file / list behavior of
``quill_cli.send_cmd``. The underlying ``send_message_tool`` is stubbed so
...
from quill_cli import send_cmd
...
def test_load_quill_env_bridges_config_yaml_scalars(tmp_path, monkeypatch):
    ...
    ``quill send`` from a fresh shell cannot resolve the home channel
    because ``TELEGRAM_HOME_CHANNEL`` (saved by ``quill config set``) lives
    ...
    quill_home.mkdir()
    (quill_home / ".env").write_text("SOME_TOKEN=abc123\n")
    ...
    # Force get_quill_home() to re-resolve under the patched env.
    from importlib import reload

    import quill_cli.config as _hc_config
    reload(_hc_config)
...
def test_load_quill_env_does_not_override_existing(tmp_path, monkeypatch):
    """Existing env vars must not be clobbered by config.yaml values."""
    import os

    quill_home = tmp_path / ".quill"
    quill_home.mkdir()
    (quill_home / "config.yaml").write_text("TELEGRAM_HOME_CHANNEL: yaml_value\n")
...
def test_load_quill_env_handles_missing_files(tmp_path, monkeypatch):
