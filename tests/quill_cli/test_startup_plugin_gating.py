...
``quill_cli.main`` skips eager plugin discovery at argparse-setup time
when the invocation is clearly targeting a known built-in subcommand.
This saves 500-650ms on ``quill --help``, ``quill version``,
``quill logs``, etc., by not importing ``google.cloud.pubsub_v1``,
...
def _live_subcommand_names() -> set[str]:
    """Run ``quill --help`` in-process and parse the subcommand block.
    ...
    built-in-only set.
    """
    from quill_cli import main as _main
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (["quill", "-w", "--tui"], None),
        (["quill", "version"], "version"),
        (["quill", "--tui", "chat"], "chat"),
        (["quill", "-w", "logs"], "logs"),
        (["quill", "chat", "hello world"], "chat"),
        (["quill", "gateway", "run"], "gateway"),
        # Top-level value-taking flags: the value should be skipped.
        (["quill", "-m", "gpt5", "chat"], "chat"),
        (["quill", "--model", "gpt5", "chat", "hi"], "chat"),
        (["quill", "-m", "gpt5", "--provider", "openai", "chat"], "chat"),
        (["quill", "-z", "hello world"], None),
        (["quill", "-z", "hello", "chat"], "chat"),
        (["quill", "--model=gpt5", "chat"], "chat"),     # inline form
        (["quill", "--", "chat"], "chat"),               # -- terminator
        (["quill", "-w", "--"], None),
        # Unknown positional after skipped flags → plugin-cmd candidate.
        (["quill", "some-plugin-cmd"], "some-plugin-cmd"),
        (["quill", "-m", "gpt5", "some-plugin-cmd"], "some-plugin-cmd"),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ["quill"],                          # bare → chat
        ["quill", "--help"],                # top-level help
        ["quill", "-h"],
        ["quill", "version"],               # known built-in
        ["quill", "logs"],
        ["quill", "gateway", "run"],
        ["quill", "--tui"],
        ["quill", "-w", "--tui"],
        ["quill", "chat", "hi"],
        ["quill", "help"],                  # accepted built-in-ish
        ["quill", "-m", "gpt5", "chat"],    # flag-value-skipping
    ],
)
...
@pytest.mark.parametrize(
    "argv",
    [
        ["quill", "meet", "join"],          # potential google_meet plugin
        ["quill", "honcho", "status"],      # potential memory plugin
        ["quill", "unknown-subcmd"],
    ],
)
...
def test_builtin_set_covers_every_registered_subcommand():
    ...
    # in the set so ``quill help <cmd>`` skips discovery; it won't show
    ...
    assert not missing_from_declaration, (
        ...
        f"quill_cli/main.py::_BUILTIN_SUBCOMMANDS so plugin discovery "
