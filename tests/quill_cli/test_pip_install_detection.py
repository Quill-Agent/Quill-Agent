...
def test_pip_install_detected_when_no_git_dir(tmp_path):
    ...
    with patch("quill_cli.config.get_managed_system", return_value=None), \
         patch("quill_cli.config.get_quill_home", return_value=tmp_path):
...
def test_managed_install_takes_precedence(tmp_path):
    """When QUILL_MANAGED is set, that takes precedence over git detection."""
    (tmp_path / ".git").mkdir()
    with patch("quill_cli.config.get_managed_system", return_value="NixOS"), \
         patch("quill_cli.config.get_quill_home", return_value=tmp_path):
        from quill_cli.config import detect_install_method
        method = detect_install_method(project_root=tmp_path)
        assert method == "nixos"
...
def test_docker_detected_via_dockerenv(tmp_path):
    with patch("quill_cli.config.get_managed_system", return_value=None), \
         patch("quill_cli.config.get_quill_home", return_value=tmp_path), \
         patch("quill_constants.is_container", return_value=True):
        from quill_cli.config import detect_install_method
        assert detect_install_method(project_root=tmp_path) == "docker"
