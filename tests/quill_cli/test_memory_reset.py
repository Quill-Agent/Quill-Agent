"""Tests for the `quill memory reset` CLI command.
...
- Profile-scoped reset (uses QUILL_HOME)
"""
...
def memory_env(tmp_path, monkeypatch):
    """Set up a fake QUILL_HOME with memory files."""
    quill_home = tmp_path / ".quill"
    memories = quill_home / "memories"
    memories.mkdir(parents=True)
    monkeypatch.setenv("QUILL_HOME", str(quill_home))

    # Create sample memory files
    (memories / "MEMORY.md").write_text(
        "§\nQuill repo is at ~/.quill/quill-agent\n§\nUser prefers dark themes",
        encoding="utf-8",
    )
...
def _run_memory_reset(target="all", yes=False, monkeypatch=None, confirm_input="no"):
    ...
    Simulates what happens when `quill memory reset` is run.
    """
...
class TestMemoryReset:
    ...
    def test_reset_no_files_exist(self, tmp_path, monkeypatch):
        ...
        quill_home = tmp_path / ".quill"
        (quill_home / "memories").mkdir(parents=True)
        monkeypatch.setenv("QUILL_HOME", str(quill_home))
    ...
    def test_reset_profile_scoped(self, tmp_path, monkeypatch):
        """Reset should work on the active profile's QUILL_HOME."""
