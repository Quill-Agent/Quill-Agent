"""Unit tests for quill_cli.pty_bridge — PTY spawning + byte forwarding.
...
class TestPtyBridgeIO:
    def test_reads_child_stdout(self):
        bridge = PtyBridge.spawn(["/bin/sh", "-c", "printf quill-ok"])
        try:
            output = _read_until(bridge, b"quill-ok")
            assert b"quill-ok" in output
        finally:
            bridge.close()
...
class TestPtyBridgeEnv:
    ...
    def test_env_is_forwarded(self):
        bridge = PtyBridge.spawn(
            ["/bin/sh", "-c", "printf %s \"$QUILL_PTY_TEST\""],
            env={**os.environ, "QUILL_PTY_TEST": "pty-env-works"},
        )
        try:
