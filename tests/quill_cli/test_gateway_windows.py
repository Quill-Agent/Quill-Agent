...
def test_build_gateway_argv_uses_base_pythonw_for_uv_venv_launcher(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(gateway, "_profile_arg", lambda quill_home: "")
    monkeypatch.setattr("quill_cli.config.get_quill_home", lambda: str(tmp_path / "quill-home"))

    argv, cwd, env_overlay = gateway_windows._build_gateway_argv()

    assert argv[:3] == [str(base_pythonw), "-m", "quill_cli.main"]
