...
from quill_cli.plugins import PluginManager, VALID_HOOKS


def _make_enabled_plugin(quill_home: Path, name: str, register_body: str) -> Path:
    """Create a plugin under <quill_home>/plugins/<name> and opt it in."""
    plugin_dir = quill_home / "plugins" / name
...
def test_hook_receives_expected_kwargs(tmp_path, monkeypatch):
    ...
    quill_home.mkdir(exist_ok=True)
    _make_enabled_plugin(
        quill_home, "capture_hook",
        register_body=(
...
def test_no_plugins_returns_empty_results(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("QUILL_HOME", str(tmp_path / "quill_empty"))
