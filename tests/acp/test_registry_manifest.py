"""Tests for ACP Registry metadata shipped with Quill."""
...
def test_agent_json_matches_official_registry_required_fields():
    ...
    assert data["id"] == "quill-agent"
    assert re.fullmatch(r"[a-z][a-z0-9-]*", data["id"])
    assert data["name"] == "Quill Agent"
    assert data["description"]
    assert data["repository"] == "https://github.com/YOUR_USERNAME/quill-agent"
    assert data["website"].startswith("https://quill-agent.example.com/")
    assert data["authors"] == ["Quill Agent"]
...
def test_agent_json_uses_uvx_distribution_without_local_command_fields():
    ...
    assert uvx["package"] == f"quill-agent[acp]=={data['version']}"
    assert uvx["args"] == ["quill-acp"]
...
def test_agent_json_pins_uvx_package_to_pyproject_version():
    ...
    assert _manifest()["distribution"]["uvx"]["package"] == (
        f"quill-agent[acp]=={_pyproject_version()}"
    )
