# Quill Agent Security Policy

This document describes Quill Agent's trust model, names the one
security boundary the project treats as load-bearing, and defines the
scope for vulnerability reports.

## 1. Reporting a Vulnerability

Report privately via [GitHub Security Advisories](https://github.com/YOUR_USERNAME/quill-agent/security/advisories/new)
or **security@example.com**. Do not open public issues for
security vulnerabilities. **Quill Agent does not operate a bug
bounty program.**
...
- Environment details (`quill version`, commit SHA, OS, Python
  version).
...
- **Trust envelope.** The set of resources an operator has implicitly
  granted Quill Agent access to by running it — typically, whatever
  the operator's own user account can reach on the host.
- **Stance.** An explicit statement in Quill Agent's documentation
...
Quill Agent supports two OS-level isolation postures. They address
...
Quill Agent supports this in two ways:

- **Quill Agent's own Docker image and Compose setup.** Lighter-
...
Under a whole-process wrapper, Quill Agent's in-process heuristics
...
Quill Agent filters the environment it passes to its lower-trust
...
A malicious or buggy plugin is not a vulnerability in Quill Agent
itself. Bugs in Quill Agent's plugin-install or plugin-discovery
...
4. **Within the authorized set, all callers are equally trusted.**
   Quill Agent does not model per-caller capabilities inside a
...
- Trust-model documentation violations: code behaving contrary to
  what this policy, Quill Agent's own documentation, or reasonable
  operator expectations would predict — including cases where
  Quill Agent has documented a stance about how its output should
...
- **Documented break-glass settings.** Operator-selected trade-offs
  ...
  quill-home security, and similar. Reports against those
...
- **Community-contributed skills and plugins.** Third-party skills
  ...
  plugins are in the operator's review surface, not Quill Agent's
  trust surface (§2.4, §2.5). A skill or plugin doing something
  malicious is the expected failure mode of one that wasn't
  reviewed, not a vulnerability in Quill Agent. Bugs in Quill
...
- Quill Agent includes supply-chain guards for MCP server
