...
_QUILL_CORE_TOOLS = [
    ...
    # spawned as a kanban worker (QUILL_KANBAN_TASK env set) or the current
...
TOOLSETS = {
    ...
    "x_search": {
        "description": (
            ...
            "enable in `quill tools` → X (Twitter) Search."
        ),
        "tools": ["x_search"],
        "includes": []
    },
    ...
    "kanban": {
        "description": (
            ...
            "is spawned by the kanban dispatcher (QUILL_KANBAN_TASK env "
    ...
    # ==========================================================================
    # Full Quill toolsets (CLI + messaging platforms)
    #
    ...
    "quill-cli": {
        ...
        "tools": _QUILL_CORE_TOOLS,
        "includes": []
    },

    "quill-cron": {
        # Mirrors quill-cli so cron's "default" toolset is the same set of
        # core tools users see interactively — then `quill tools` filters
        # them down per the platform config. _DEFAULT_OFF_TOOLSETS (moa,
        # homeassistant) are excluded by _get_platform_tools() unless
        # the user explicitly enables them.
        "description": "Default cron toolset - same core tools as quill-cli; gated by `quill tools`",
        "tools": _QUILL_CORE_TOOLS,
        "includes": []
    },
    ...
    "quill-email": {
        "description": "Email bot toolset - interact with Quill via email (IMAP/SMTP)",
        "tools": _QUILL_CORE_TOOLS,
        "includes": []
    },
    ...
    "quill-sms": {
        "description": "SMS bot toolset - interact with Quill via SMS (Twilio)",
        "tools": _QUILL_CORE_TOOLS,
        "includes": []
    },
    ...
    "quill-gateway": {
        ...
        "tools": [],
        "includes": ["quill-telegram", "quill-discord", "quill-whatsapp", "quill-slack", "quill-signal", "quill-bluebubbles", "quill-homeassistant", "quill-email", "quill-sms", "quill-mattermost", "quill-matrix", "quill-dingtalk", "quill-feishu", "quill-wecom", "quill-wecom-callback", "quill-weixin", "quill-qqbot", "quill-webhook", "quill-yuanbao"]
    }
}
...
def resolve_toolset(name: str, visited: Set[str] = None) -> List[str]:
    ...
    if not toolset:
        # Auto-generate a toolset for plugin platforms (quill-<name>).
        # Gives them _QUILL_CORE_TOOLS plus any tools the plugin registered
        # into a toolset matching the platform name.
        if name.startswith("quill-"):
            platform_name = name[len("quill-"):]
            try:
                from gateway.platform_registry import platform_registry
                if platform_registry.is_registered(platform_name):
                    plugin_tools = set(_QUILL_CORE_TOOLS)
                    try:
                        from tools.registry import registry
                        plugin_tools.update(
