# Quill Agent Has Had "Routines" Since March
...
Quill equivalent — works today:
```bash
quill webhook subscribe auth-watch \
  --events "pull_request" \
...
Every use case in their blog post — backlog triage, docs drift, deploy verification, alert correlation, library porting, bespoke PR review — has a working Quill implementation. No new features needed. It's been shipping since March 2026.

---

## What's Different

| | Claude Code Routines | Quill Agent |
|---|---|---|
...
## Things Quill Does That Routines Can't
...
Quill has no daily limit. Run 500 automations a day if you want. The only constraint is your API budget, and you choose which models to use for which tasks.
...
Quill Agent is open source and free. The automation infrastructure — cron scheduler, webhook platform, skill system, multi-platform delivery — is built in.
...
```bash
quill gateway setup    # enable webhooks
quill webhook subscribe pr-review \
  --events "pull_request" \
...
Full automation templates gallery: [quill-agent.example.com/docs/guides/automation-templates](https://quill-agent.example.com/docs/guides/automation-templates)

Documentation: [quill-agent.example.com](https://quill-agent.example.com)

GitHub: [github.com/YOUR_USERNAME/quill-agent](https://github.com/YOUR_USERNAME/quill-agent)

---

*Quill Agent is built by [Quill Agent](https://example.com). Open source, model-agnostic, runs on your infrastructure.*
