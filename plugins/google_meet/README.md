...
Let the quill agent join a Google Meet call, transcribe it, optionally speak
...
```
┌─ gateway (Linux box, where quill runs) ────────────────────────────┐
│                                                                      │
...
│                                                                      │
│   NodeServer (from `quill meet node run`)                           │
│     │                                                                │
...
| `__init__.py` | `register(ctx)` — registers 5 tools + `on_session_end` hook + `quill meet` CLI |
...
| `cli.py` | `quill meet setup / auth / join / status / transcript / say / stop / node ...` |
...
| `node/registry.py` | v3: `$QUILL_HOME/workspace/meetings/nodes.json` |
| `node/server.py` | v3: `NodeServer` (runs on host machine) |
| `node/client.py` | v3: `NodeClient` (used by tool handlers + CLI on gateway) |
| `node/cli.py` | v3: `quill meet node {run,list,approve,remove,status,ping}` |
...
quill meet install                                      # pip + Chromium
quill meet setup                                        # preflight
quill meet auth                                         # optional
quill meet join https://meet.google.com/abc-defg-hij    # transcribe
```
...
```bash
quill meet install --realtime                     # installs pulseaudio-utils
echo 'OPENAI_API_KEY=sk-...' >> ~/.quill/.env
quill meet join https://meet.google.com/abc-defg-hij --mode realtime
# then from the agent or CLI:
quill meet say "Good morning everyone, I'm the note-taker bot."
```

macOS:
```bash
quill meet install --realtime     # runs: brew install blackhole-2ch ffmpeg
# then — manually! — open System Settings → Sound → Input → BlackHole 2ch
echo 'OPENAI_API_KEY=sk-...' >> ~/.quill/.env
quill meet join https://meet.google.com/abc-defg-hij --mode realtime
```

On macOS, quill will **not** switch your system audio input automatically — the
...
quill plugins enable google_meet
quill meet node run --display-name my-mac --host 0.0.0.0 --port 18789
