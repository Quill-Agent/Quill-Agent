...
quill meet install                 # pip deps + Chromium (transcribe only)
quill meet install --realtime      # + pulseaudio-utils / brew blackhole+ffmpeg
quill meet auth                    # optional; skips guest-lobby wait
quill meet setup                   # preflight checks
```

`quill meet install --realtime` prompts before running `sudo apt-get` (Linux)
...
#   Then set OPENAI_API_KEY or QUILL_MEET_REALTIME_KEY in ~/.quill/.env
```
...
pip install playwright websockets && python -m playwright install chromium
quill plugins enable google_meet
quill meet node run --display-name my-mac    # persistent server
# copy the printed token

# on the gateway:
quill meet node approve my-mac ws://<mac-ip>:18789 <token>
quill meet node ping my-mac                   # confirm reachable
```

Run `quill meet setup` to preflight local prereqs.

## Flow

1. **Join** — call `meet_join(url=..., mode=..., node=...)`. Returns immediately.
2. **Announce yourself** — no auto-consent. Say (in whatever channel the user is watching): "A Quill agent bot is in this call taking notes."
...
- Guest mode sits in the lobby until a host admits. Warn the user; `quill meet auth` avoids this.
- **Lobby timeout**: if the host doesn't admit the bot within 5 minutes (configurable via `QUILL_MEET_LOBBY_TIMEOUT` env), the bot leaves and `meet_status` reports `leaveReason: "lobby_timeout"`.
...
| `realtimeDevice` | Audio device name the bot is feeding (e.g. `quill_meet_src`). |
...
Local:
```
$QUILL_HOME/workspace/meetings/<meeting-id>/transcript.txt
```
