...
session-scoped state file under ``$QUILL_HOME/workspace/meetings/.active.json``
...
# File + directory layout (under $QUILL_HOME):
#
#   workspace/meetings/
...
def _root() -> Path:
    return Path(get_quill_home()) / "workspace" / "meetings"
...
def start(
    ...
    *,
    ...
    If a bot is already running for this quill install, leave it first —
    ...
    env = os.environ.copy()
    env["QUILL_MEET_URL"] = url
    env["QUILL_MEET_OUT_DIR"] = str(out)
    env["QUILL_MEET_GUEST_NAME"] = guest_name
    if headed:
        env["QUILL_MEET_HEADED"] = "1"
    if auth_state:
        env["QUILL_MEET_AUTH_STATE"] = auth_state
    if duration:
        env["QUILL_MEET_DURATION"] = duration
    # v2: realtime mode + passthroughs. The bot defaults to transcribe
    # mode if QUILL_MEET_MODE isn't set, matching v1 behavior.
    if mode:
        env["QUILL_MEET_MODE"] = mode
    if realtime_model:
        env["QUILL_MEET_REALTIME_MODEL"] = realtime_model
    if realtime_voice:
        env["QUILL_MEET_REALTIME_VOICE"] = realtime_voice
    if realtime_instructions:
        env["QUILL_MEET_REALTIME_INSTRUCTIONS"] = realtime_instructions
    if realtime_api_key:
        env["QUILL_MEET_REALTIME_KEY"] = realtime_api_key
