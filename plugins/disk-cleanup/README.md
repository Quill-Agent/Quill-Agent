...
Auto-tracks and cleans up ephemeral files created during Quill Agent
sessions — test scripts, temp outputs, cron logs, stale chrome profiles.
Scoped strictly to `$QUILL_HOME` and `/tmp/quill-*`.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `QUILL_HOME`, track it silently as `test` / `temp` / `cron-output`. |
...
- `is_safe_path()` rejects anything outside `QUILL_HOME` or `/tmp/quill-*`
- Windows mounts (`/mnt/c` etc.) are rejected
- The state directory `$QUILL_HOME/disk-cleanup/` is itself excluded
- `$QUILL_HOME/logs/`, `memories/`, `sessions/`, `skills/`, `plugins/`,
