"""disk_cleanup — ephemeral file cleanup for Quill Agent.
...
Rules:
  ...
  - empty dirs    → always delete (under QUILL_HOME)
  - research      → keep 10 newest, prompt for older (deep only)
  - chrome-profile→ prompt after 14 days (deep only)
  - >500 MB files → prompt always (deep only)

Scope: strictly QUILL_HOME and /tmp/quill-*
Never touches: ~/.quill/logs/ or any system directory.
"""
...
try:
    ...
    def get_quill_home() -> Path:  # type: ignore[no-redef]
        val = (os.environ.get("QUILL_HOME") or "").strip()
        return Path(val).resolve() if val else (Path.home() / ".quill").resolve()
...
def get_state_dir() -> Path:
    """State dir — separate from ``$QUILL_HOME/logs/``."""
    return get_quill_home() / "disk-cleanup"
...
def get_log_file() -> Path:
    """Audit log — intentionally NOT under ``$QUILL_HOME/logs/``."""
...
def is_safe_path(path: Path) -> bool:
    """Accept only paths under QUILL_HOME or ``/tmp/quill-*``.

    Rejects Windows mounts (``/mnt/c`` etc.) and any system directory.
    """
    quill_home = get_quill_home()
    try:
        path.resolve().relative_to(quill_home)
        return True
    except (ValueError, OSError):
        pass
    # Allow /tmp/quill-* explicitly
    parts = path.parts
    if len(parts) >= 3 and parts[1] == "tmp" and parts[2].startswith("quill-"):
        return True
    return False
...
def track(path_str: str, category: str, silent: bool = False) -> bool:
    ...
    if not is_safe_path(path):
        _log(f"REJECT: {path} (outside QUILL_HOME)")
        return False
...
def quick() -> Dict[str, Any]:
    ...
    # Remove empty dirs under QUILL_HOME (but leave QUILL_HOME itself and
    ...
    _PROTECTED_TOP_LEVEL = {
        ...
        "quill-agent", "backups", "profiles", ".worktrees",
    }
    empty_removed = 0
    try:
        for dirpath in sorted(quill_home.rglob("*"), reverse=True):
            if not dirpath.is_dir() or dirpath == quill_home:
                continue
            try:
                rel_parts = dirpath.relative_to(quill_home).parts
            except ValueError:
                continue
...
def guess_category(path: Path) -> Optional[str]:
    ...
    quill_home = get_quill_home()
    try:
        rel = path.resolve().relative_to(quill_home)
        top = rel.parts[0] if rel.parts else ""
        if top in {
            "disk-cleanup", "logs", "memories", "sessions", "config.yaml",
            "skills", "plugins", ".env", "USER.md", "MEMORY.md", "SOUL.md",
            "auth.json", "quill-agent",
        }:
            return None
        ...
        # Path isn't under QUILL_HOME (e.g. /tmp/quill-*) — fall through.
        pass
