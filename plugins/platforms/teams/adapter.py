"""
Microsoft Teams platform adapter for Quill Agent.
...
class TeamsAdapter(BasePlatformAdapter):
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            self._app = App(
                ...
                client=ClientOptions(headers={"User-Agent": "Quill"}),
            )
    ...
    async def _on_card_action(
        ...
        data = action.data or {}
        quill_action = data.get("quill_action", "")
        session_key = data.get("session_key", "")

        if not quill_action or not session_key:
        ...
        choice = choice_map.get(quill_action)
        if not choice:
            return InvokeResponse(
                status=200,
                body=AdaptiveCardActionMessageResponse(value="Unknown action."),
        ...
        card = (
            ...
            .with_actions([
                ExecuteAction(
                    title="Allow Once",
                    verb="quill_approve",
                    data={**btn_data_base, "quill_action": "approve_once"},
                    style="positive",
                ),
                ExecuteAction(
                    title="Allow Session",
                    verb="quill_approve",
                    data={**btn_data_base, "quill_action": "approve_session"},
                ),
                ExecuteAction(
                    title="Always Allow",
                    verb="quill_approve",
                    data={**btn_data_base, "quill_action": "approve_always"},
                ),
                ExecuteAction(
                    title="Deny",
                    verb="quill_approve",
                    data={**btn_data_base, "quill_action": "deny"},
                    style="destructive",
                ),
            ])
        )
...
def interactive_setup() -> None:
    ...
    print_info("  teams app create --name \"Quill\" --endpoint \"https://<tunnel>/api/messages\"")
    print()
    ...
    print()
    print_success("Teams configuration saved to ~/.quill/.env")
