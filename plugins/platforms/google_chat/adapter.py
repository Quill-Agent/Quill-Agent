...
# ``model_tools`` import time, so ``quill status``, ``quill chat``, etc.
...
# ``quill_plugins.platforms__google_chat.adapter`` once the plugin
...
#     fresh "Quill is thinking…" card during the small window between
...
def _mime_for_message_type(mime: str) -> MessageType:
    """Map a MIME string to a quill MessageType.
...
class GoogleChatAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Persisted to ${QUILL_HOME}/google_chat_thread_counts.json so
        ...
        try:
            ...
            _quill_home = _get_quill_home()
        except (ModuleNotFoundError, ImportError):
            _quill_home = _Path.home() / ".quill"
        self._thread_count_store = _ThreadCountStore(
            _quill_home / "google_chat_thread_counts.json"
        )
        ...
        # they don't sit in the chat forever as "Quill is thinking…".
    ...
    def _bot_id_cache_path(self) -> _Path:
        ...
        base = os.getenv("QUILL_HOME", str(_Path.home() / ".quill"))
    ...
    async def _dispatch_message(self, msg: Dict[str, Any], envelope: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if text.startswith("/setup-files") and event.source is not None:
                ...
                # ${QUILL_HOME}/google_chat_user_tokens/<sanitized>.json
    ...
    async def _build_message_event(
        ...
        """Parse a Chat API message into a quill MessageEvent."""
        space = envelope.get("space") or msg.get("space") or {}
    ...
    async def send_typing(self, chat_id: str, metadata: Any = None) -> None:
        """Post a visible 'Quill is thinking…' marker message.
        ...
        "Quill is thinking…" stuck in chat forever, plus one card that
        ...
        body: Dict[str, Any] = {"text": "Quill is thinking…"}
        if thread_id:
            body["thread"] = {"name": thread_id}
    ...
    async def stop_typing(self, chat_id: str) -> None:
        ...
        Google Chat has no separate typing API: the "Quill is thinking…"
    ...
    async def on_processing_complete(
        ...
        see "Quill is thinking…" stuck forever.
        """
    ...
    async def _consume_typing_card_with_text(
        ...
        creating a fresh "Quill is thinking…" card during any subsequent
...
def interactive_setup() -> None:
    """Walk the user through Google Chat configuration via ``quill setup``.

    The setup wizard at ``quill_cli/gateway.py`` calls this for plugin
    ...
    prompt for env vars, persist them to ``~/.quill/.env`` so the next
    gateway restart picks them up.
    """
    from quill_cli.cli_output import (
        print_info,
        print_success,
        print_warning,
        prompt,
    ...
    print_info("  3. Create a Pub/Sub topic (e.g. quill-chat-events) and a Pull subscription.")
    ...
    print()
    print_success("Google Chat configuration saved to ~/.quill/.env")
    print_info("Restart the gateway: quill gateway restart")
...
async def _standalone_send(
    ...
    *,
    ...
    runner is not in this process (e.g. ``quill cron`` running as a
    separate process from ``quill gateway``).  Without this hook,
...
def register(ctx) -> None:
    """Plugin entry point — called by the Quill plugin system at startup.
    ...
    ctx.register_platform(
        ...
        install_hint="pip install 'quill-agent[google_chat]'",
        ...
        # the "Quill is thinking..." marker patches and edit overhead.
        ...
        platform_hint=(
            ...
            "are generating a response, a 'Quill is thinking…' marker message "
