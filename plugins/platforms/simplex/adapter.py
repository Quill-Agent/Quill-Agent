"""SimpleX Chat platform adapter (Quill plugin).
...
This adapter ships as a Quill platform plugin under
``plugins/platforms/simplex/``. The Quill plugin loader scans the
...
discoverable and `quill setup` can describe it even when websockets is
...
# Imported at module top because they're stdlib-only inside Quill — no
...
async def _standalone_send(
    ...
    *,
    ...
    try:
        ...
        payload = {
            "corrId": f"quill-snd-{int(time.time() * 1000)}",
            "cmd": cmd_str,
        }
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``quill setup gateway`` → SimpleX.

    Prompts for the WebSocket URL and the optional allowlist / home channel.
    Writes to ``~/.quill/.env`` via ``quill_cli.config``.
    """
    print()
    ...
    try:
        ...
        print("quill_cli.config not available; set SIMPLEX_* vars manually in ~/.quill/.env")
        return
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        # env-only setups show up in `quill gateway status` without
