"""
LINE Messaging API platform adapter for Quill Agent.
...
to Quill Agent. It deliberately ports the *strongest* idea from each into
...
class LineAdapter(BasePlatformAdapter):
    ...
    async def _handle_media(self, request) -> Any:
        ...
        ``/private/tmp`` on macOS), and ``QUILL_HOME``. PR #8398.
        """
        from aiohttp import web
        ...
        try:
            ...
            quill_home = Path.home().joinpath(".quill").resolve()
...
def is_connected(config) -> bool:
    """Surface in ``quill status`` even before the adapter is instantiated."""
...
def _env_enablement() -> Optional[Dict[str, Any]]:
    ...
    Lets ``quill status`` reflect a LINE configuration that lives entirely
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``quill setup line``.

    Mirrors the irc/teams style: prompts for the two required vars, plus
    one optional public URL. Writes to ``~/.quill/.env`` via ``quill_cli.config``.
    """
    print()
    ...
    try:
        from quill_cli.config import get_env_var, set_env_var
    except ImportError:
        print("quill_cli.config not available; set LINE_* vars manually in ~/.quill/.env")
        return
