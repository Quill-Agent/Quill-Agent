...
``_check_spotify_available()`` — when the user has not run ``quill auth
spotify``, the tools remain registered (so they appear in ``quill
...
The Spotify auth flow (``quill auth spotify``), CLI plumbing, and docs
