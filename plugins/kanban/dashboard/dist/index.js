/**
 * Quill Kanban — Dashboard Plugin
 *
 * Board view for the multi-agent collaboration board backed by
 * ~/.quill/kanban.db. Calls the plugin's backend at /api/plugins/kanban/
 * and tails task_events over a WebSocket for live updates.
 *
 * Plain IIFE, no build step. Uses window.__QUILL_PLUGIN_SDK__ for React +
...
(function () {
  ...
  const SDK = window.__QUILL_PLUGIN_SDK__;
  if (!SDK) return;
  ...
  const COLUMN_DOT = {
    triage: "quill-kanban-dot-triage",
    todo: "quill-kanban-dot-todo",
    ready: "quill-kanban-dot-ready",
    running: "quill-kanban-dot-running",
    blocked: "quill-kanban-dot-blocked",
    done: "quill-kanban-dot-done",
    archived: "quill-kanban-dot-archived",
  };
  ...
  const MIME_TASK = "text/x-quill-task";
  ...
  const DOCS_URL = "https://quill-agent.example.com/docs/user-guide/features/kanban";
  const DOCS_TUTORIAL_URL = "https://quill-agent.example.com/docs/user-guide/features/kanban-tutorial";
  ...
  const LS_BOARD_KEY = "quill.kanban.selectedBoard";
  ...
  function renderMarkdown(src) {
    ...
    html = html.replace(/\u0000CODE(\d+)\u0000/g, (_m, i) =>
      `<pre class="quill-kanban-md-code"><code>${escapeHtml(blocks[Number(i)])}</code></pre>`,
    );
    return html;
  }
  ...
  function MarkdownBlock(props) {
    ...
    if (!enabled) {
      return h("pre", { className: "quill-kanban-pre" }, props.source || "");
    }
    return h("div", {
      className: "quill-kanban-md",
  ...
  // standard `drop` event and our `quill-kanban:drop` event.
  // -------------------------------------------------------------------------
  ...
  function attachTouchDrag(el, taskId) {
    ...
    function onDown(e) {
      ...
      proxy.classList.add("quill-kanban-touch-proxy");
      ...
      function move(ev) {
        ...
        if (target !== lastTarget) {
          if (lastTarget) lastTarget.classList.remove("quill-kanban-column--drop");
          if (target) target.classList.add("quill-kanban-column--drop");
          lastTarget = target;
        }
      }
      function up() {
        ...
        if (lastTarget) {
          lastTarget.classList.remove("quill-kanban-column--drop");
          const status = lastTarget.getAttribute("data-kanban-column");
          const isTrash = lastTarget.hasAttribute("data-kanban-trash");
          if (isTrash) {
            lastTarget.dispatchEvent(new CustomEvent("quill-kanban:delete", {
              detail: { taskId },
              bubbles: true,
            }));
          } else if (status) {
            lastTarget.dispatchEvent(new CustomEvent("quill-kanban:drop", {
  ...
  function KanbanPage() {
    ...
    useEffect(function () {
      ...
      function openWs() {
        ...
        const token = window.__QUILL_SESSION_TOKEN__ || "";
   ...
   const deleteTask = useCallback(function (taskId) {
    ...
    return h(ErrorBoundary, null,
      h("div", { className: "quill-kanban flex flex-col gap-4" },
        h(BoardSwitcher, {
          board: board,
          boardList: boardList,
  ...
  function AttentionStrip(props) {
    ...
    return h("div", {
      className: cn(
        "quill-kanban-attention",
        "quill-kanban-attention--" + topSev,
      ),
    },
      h("div", { className: "quill-kanban-attention-bar" },
        h("span", { className: "quill-kanban-attention-icon" },
          topSev === "critical" ? "!!!" : topSev === "error" ? "!!" : "⚠"),
        h("span", { className: "quill-kanban-attention-text" },
          diagTasks.length === 1
      ...
      expanded
        ? h("div", { className: "quill-kanban-attention-list" },
            diagTasks.map(function (task) {
              ...
              return h("div", {
                key: task.id,
                className: cn(
                  "quill-kanban-attention-row",
                  "quill-kanban-attention-row--" + sev,
                ),
              },
                h("span", { className: "quill-kanban-attention-row-sev" },
                  sev === "critical" ? "!!!" : sev === "error" ? "!!" : "⚠"),
                h("span", { className: "quill-kanban-attention-row-id" }, task.id),
                h("span", { className: "quill-kanban-attention-row-title" },
                  task.title || tx(t, "untitled", "(untitled)")),
                h("span", { className: "quill-kanban-attention-row-meta" },
                  task.assignee ? "@" + task.assignee : tx(t, "unassigned", "unassigned"),
                  " \u00b7 ",
                  kinds.length > 0 ? kinds.join(", ") : tx(t, "diagnostic", "diagnostic"),
                ),
                h("button", {
                  className: "quill-kanban-attention-row-btn",
  ...
  function DiagnosticActionButton(props) {
    ...
    const cls = cn(
      "quill-kanban-diag-action-btn",
      action.suggested ? "quill-kanban-diag-action-btn--suggested" : "",
    );
    ...
    return h("span", { className: cls + " quill-kanban-diag-action-btn--unknown" },
      label);
  }
  ...
  function DiagnosticCard(props) {
    ...
    const execAction = function (action) {
      ...
      if (action.kind === "comment") {
        ...
        const ta = document.querySelector(".quill-kanban-drawer-comment-row input, .quill-kanban-drawer-comment-row textarea");
        if (ta) {
    ...
    const sevClass = "quill-kanban-diag--" + (diag.severity || "warning");
    return h("div", { className: cn("quill-kanban-diag", sevClass) },
      h("div", { className: "quill-kanban-diag-header" },
        h("span", { className: "quill-kanban-diag-sev" },
          diag.severity === "critical" ? "!!!" :
          diag.severity === "error" ? "!!" : "\u26a0"),
        h("span", { className: "quill-kanban-diag-title" },
          diag.title),
      ),
      h("div", { className: "quill-kanban-diag-detail" },
        diag.detail),
      diag.data && Object.keys(diag.data).length > 0
        ? h("div", { className: "quill-kanban-diag-data" },
            Object.keys(diag.data).map(function (k) {
              const v = diag.data[k];
              if (Array.isArray(v) && v.length > 0 && typeof v[0] === "string" &&
                  v[0].indexOf("t_") === 0) {
                // Task-id list — render as chips.
                return h("div", { key: k, className: "quill-kanban-diag-data-row" },
                  h("span", { className: "quill-kanban-diag-data-key" }, k + ":"),
                  v.map(function (x) {
                    return h("code", {
                      key: x, className: "quill-kanban-event-phantom-chip",
                    }, x);
                  }),
                );
              }
              return h("div", { key: k, className: "quill-kanban-diag-data-row" },
                h("span", { className: "quill-kanban-diag-data-key" }, k + ":"),
                h("span", { className: "quill-kanban-diag-data-val" },
      ...
      reassignAction
        ? h("div", { className: "quill-kanban-diag-reassign-row" },
            h("span", { className: "quill-kanban-diag-reassign-label" },
              tx(t, "reassignTo", "Reassign to:")),
            h("select", {
              className: "quill-kanban-recovery-select",
        ...
        : null,
      h("div", { className: "quill-kanban-diag-actions" },
      ...
      msg
        ? h("div", {
            className: cn(
              "quill-kanban-diag-msg",
              msg.ok ? "quill-kanban-diag-msg--ok" : "quill-kanban-diag-msg--err",
            ),
          }, msg.text)
        : null,
    );
  ...
  function DiagnosticsSection(props) {
    ...
    return h("div", { className: "quill-kanban-section" },
      h("div", { className: "quill-kanban-section-head-row" },
        h("span", { className: "quill-kanban-section-head" },
          hasOpenDiags
            ? h("span", { className: "quill-kanban-section-head-warning" },
                `\u26a0 ${tx(t, "diagnostics", "Diagnostics")} (${diags.length})`)
            : tx(t, "diagnostics", "Diagnostics"),
        ),
        h("button", {
          className: "quill-kanban-section-toggle",
          onClick: function () { setOpen(function (x) { return !x; }); },
          type: "button",
        }, open ? tx(t, "hide", "Hide") : tx(t, "show", "Show")),
      ),
      open
        ? h("div", { className: "quill-kanban-diag-list" },
  ...
  function DocsLink() {
    return h("a", {
      ...
      className: "quill-kanban-docs-link",
      title: "Open Quill Kanban docs in a new tab",
      "aria-label": "Quill Kanban documentation",
    }, "?");
  }
  ...
  function OrchestrationPanel() {
    ...
    // each triage card (or runs `quill kanban decompose <id>`) and tasks
    ...
    return h(Card, { className: "p-3" },
      h(CardContent, { className: "p-2 flex flex-col gap-3" },
        ...
        msg ? h("div", {
          className: msg.ok ? "quill-kanban-msg-ok" : "quill-kanban-msg-err",
        }, msg.text) : null,
  ...
  function BoardSwitcher(props) {
    ...
    if (!shouldShow) {
      return h("div", {
        className: "quill-kanban-boardswitcher-compact",
    ...
    return h("div", { className: "quill-kanban-boardswitcher" },
      h("div", { className: "quill-kanban-boardswitcher-inner" },
  ...
  function NewBoardDialog(props) {
    ...
    return h("div", {
      className: "quill-kanban-dialog-backdrop",
      onClick: function (e) { if (e.target === e.currentTarget) props.onCancel(); },
    },
      h("form", {
        className: "quill-kanban-dialog",
        onSubmit: onSubmit,
      },
        h("div", { className: "quill-kanban-dialog-title" },
        ...
        h("div", { className: "quill-kanban-dialog-actions" },
          h(Button, {
            type: "button",
  ...
  function BoardToolbar(props) {
    ...
    return h("div", { className: "flex flex-wrap items-end gap-3" },
      ...
      h("div", { className: "flex flex-col gap-1",
                 title: "Filter by assigned Quill profile. Profiles are the named agent identities that claim and work on tasks." },
  ...
  function BulkActionBar(props) {
    ...
    return h("div", { className: "quill-kanban-bulk" },
      h("span", { className: "quill-kanban-bulk-count" },
      ...
      h("div", { className: "quill-kanban-bulk-priority",
      ...
      h("div", { className: "quill-kanban-bulk-reassign",
                 title: "Reassign selected tasks to a different Quill profile. Pick a profile (or unassign) and click Apply." },
        h(Select, Object.assign({
          value: assignee,
          className: "h-7 text-xs",
      ...
      h("label", { className: "quill-kanban-bulk-reclaim-first", title: "Reclaim any active claims before reassigning" },
        h("input", {
          type: "checkbox",
  ...
  function TrashDropZone(props) {
    ...
    useEffect(function () {
      ...
      el.addEventListener("quill-kanban:delete", onTouchDelete);
      return function () { el.removeEventListener("quill-kanban:delete", onTouchDelete); };
    }, [props.onDelete]);
    ...
    return h("div", {
      ...
      className: cn(
        "quill-kanban-trash",
        dragOver ? "quill-kanban-trash--drop" : "",
        props.draggingTaskId ? "quill-kanban-trash--active" : "",
      ),
      onDragOver: handleDragOver,
      onDragLeave: handleDragLeave,
      onDrop: handleDrop,
    },
      h("span", { className: "quill-kanban-trash-icon" }, "🗑️"),
      h("span", { className: "quill-kanban-trash-label" },
  ...
  function BoardColumns(props) {
    const handleDragStart = useCallback(function (e) {
      const card = e.target.closest && e.target.closest(".quill-kanban-card");
      if (!card) return;
    ...
    return h("div", { className: "quill-kanban-columns", onDragStart: handleDragStart, onDragEnd: handleDragEnd },
  ...
  function Column(props) {
    ...
    useEffect(function () {
      ...
      el.addEventListener("quill-kanban:drop", onTouchDrop);
      return function () { el.removeEventListener("quill-kanban:drop", onTouchDrop); };
    ...
    return h("div", {
      ...
      className: cn(
        "quill-kanban-column",
        dragOver ? "quill-kanban-column--drop" : "",
      ),
      onDragOver: handleDragOver,
      onDragLeave: handleDragLeave,
      onDrop: handleDrop,
    },
      h("div", { className: "quill-kanban-column-header",
                 title: colHelp || "" },
        h("input", {
          type: "checkbox",
          className: "quill-kanban-col-check",
        ...
        h("span", { className: cn("quill-kanban-dot", COLUMN_DOT[props.column.name]) }),
        h("span", { className: "quill-kanban-column-label" },
          colLabel || props.column.name),
        h("span", { className: "quill-kanban-column-count",
                    title: `${props.column.tasks.length} task${props.column.tasks.length === 1 ? "" : "s"} in this column` },
          props.column.tasks.length),
        h("button", {
          type: "button",
          className: "quill-kanban-column-add",
          title: tx(t, "createTask", "Create task in this column"),
          onClick: function () { setShowCreate(function (v) { return !v; }); },
        }, showCreate ? "×" : "+"),
      ),
      h("div", { className: "quill-kanban-column-sub" },
        colHelp || ""),
      ...
      h("div", { className: "quill-kanban-column-body" },
        props.column.tasks.length === 0
          ? h("div", { className: "quill-kanban-empty" }, tx(t, "noTasks", "— no tasks —"))
          : lanes
            ? lanes.map(function (lane) {
                return h("div", { key: lane.assignee, className: "quill-kanban-lane" },
                  h("div", { className: "quill-kanban-lane-head" },
                    h("span", { className: "quill-kanban-lane-name" }, lane.assignee),
                    h("span", { className: "quill-kanban-lane-count" }, lane.tasks.length),
                  ),
  ...
  function stalenessClass(task) {
    ...
    if (age >= tier.red)   return "quill-kanban-card--stale-red";
    if (age >= tier.amber) return "quill-kanban-card--stale-amber";
    return "";
  }
  ...
  function TaskCard(props) {
    ...
    const handleDragStart = function (e) {
      ...
      const selectedCards = document.querySelectorAll(".quill-kanban-card--selected");
      if (selectedCards.length > 1 && props.selected) {
        const ghost = document.createElement("div");
        ghost.className = "quill-kanban-drag-ghost";
    ...
    return h("div", {
      ...
      className: cn(
        "quill-kanban-card",
        props.selected ? "quill-kanban-card--selected" : "",
        props.failed ? "quill-kanban-card--failed" : "",
        props.draggingSource ? "quill-kanban-card--dragging-source" : "",
        stalenessClass(t),
      ),
      draggable: true,
      tabIndex: 0,
      ...
      h(Card, null,
        h(CardContent, { className: "quill-kanban-card-content" },
          h("div", { className: "quill-kanban-card-row" },
            h("label", {
              className: "quill-kanban-card-check-wrap",
              title: tx(i18n, "selectForBulk", "Select for bulk actions"),
              onClick: function (e) { e.stopPropagation(); },
            },
              h("input", {
                type: "checkbox",
                className: "quill-kanban-card-check",
            ...
            h("span", { className: "quill-kanban-card-id",
                        title: `Task id: ${t.id}. Use this id with kanban_show, /kanban show, or quill kanban show.` }, t.id),
            t.warnings && t.warnings.count > 0
              ? h("span", {
                  className: cn(
                    "quill-kanban-warning-badge",
                    "quill-kanban-warning-badge--" + (t.warnings.highest_severity || "warning"),
                  ),
                  title: (
            ...
            needsAssignee
              ? h(Badge, {
                  variant: "outline",
                  className: "quill-kanban-needs-assignee",
          ...
          h("div", { className: "quill-kanban-card-title" },
            t.title || tx(i18n, "untitled", "(untitled)")),
          h("div", { className: "quill-kanban-card-row quill-kanban-card-meta" },
            t.assignee
              ? h("span", { className: "quill-kanban-assignee",
                            title: `Assigned to Quill profile @${t.assignee}` }, "@", t.assignee)
              : h("span", { className: "quill-kanban-unassigned",
                            title: needsAssignee
            ...
            t.comment_count > 0
              ? h("span", { className: "quill-kanban-count",
            ...
            t.link_counts && (t.link_counts.parents + t.link_counts.children) > 0
              ...
              : null,
            h("span", { className: "quill-kanban-ago",
  ...
  function InlineCreate(props) {
    ...
    return h("div", { className: "quill-kanban-inline-create" },
      h("textarea", {
        value: title,
      ...
      h("div", { className: "flex gap-2" },
        h(Input, {
          ...
          title: props.columnName === "triage"
            ? "Quill profile that will spec this task (default: the dispatcher's configured specifier). Leave blank to let the dispatcher pick."
            : "Quill profile to assign. Leave blank and the dispatcher will pick from available profiles when the task is Ready.",
  ...
  function TaskDrawer(props) {
    ...
    return h("div", { className: "quill-kanban-drawer-shade", onClick: props.onClose },
      h("div", {
        className: "quill-kanban-drawer",
        onClick: function (e) { e.stopPropagation(); },
      },
        h("div", { className: "quill-kanban-drawer-head" },
          h("span", { className: "text-xs text-muted-foreground" }, props.taskId),
          h("button", {
            type: "button",
            onClick: props.onClose,
            className: "quill-kanban-drawer-close",
        ...
        data ? h("div", { className: "quill-kanban-drawer-comment-row" },
          h(Input, {
            value: newComment,
  ...
  function TaskDetail(props) {
    ...
    return h("div", { className: "quill-kanban-drawer-body" },
      h("div", { className: "quill-kanban-drawer-title" },
        h("span", { className: cn("quill-kanban-dot", COLUMN_DOT[t.status]) }),
        props.editing
          ? h(TitleEditor, {
              initial: t.title || "",
          ...
          : h("span", {
              className: "quill-kanban-drawer-title-text",
              title: tx(i18n, "clickToEdit", "Click to edit"),
              onClick: function () { props.setEditing(true); },
            }, t.title || tx(i18n, "untitled", "(untitled)")),
      ),
      h("div", { className: "quill-kanban-drawer-meta" },
      ...
      t.result ? h("div", { className: "quill-kanban-section" },
        h("div", { className: "quill-kanban-section-head" }, tx(i18n, "result", "Result")),
        h(MarkdownBlock, { source: t.result, enabled: props.renderMarkdown }),
      ) : null,
      h("div", { className: "quill-kanban-section" },
        h("div", { className: "quill-kanban-section-head" },
          `${tx(i18n, "comments", "Comments")} (${comments.length})`),
        comments.length === 0
          ? h("div", { className: "text-xs text-muted-foreground" },
              tx(i18n, "noComments", "— no comments —"))
          : comments.map(function (c) {
              return h("div", { key: c.id, className: "quill-kanban-comment" },
                h("div", { className: "quill-kanban-comment-head" },
                  h("span", { className: "quill-kanban-comment-author" }, c.author || "anon"),
                  h("span", { className: "quill-kanban-comment-ago" },
      ...
      h("div", { className: "quill-kanban-section" },
        ...
        events.slice().reverse().slice(0, 20).map(function (e) {
          ...
          return h("div", {
            ...
            className: cn(
              "quill-kanban-event",
              isDiag ? "quill-kanban-event--hallucination" : "",
            ),
          },
            isDiag
              ? h("div", { className: "quill-kanban-event-header" },
                  h("span", { className: "quill-kanban-event-warning-icon" }, "⚠"),
                  h("span", { className: "quill-kanban-event-warning-label" },
                    getDiagnosticEventLabel(i18n, e.kind) || e.kind),
                  h("span", { className: "quill-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                )
              : h("div", { className: "quill-kanban-event-header-plain" },
                  h("span", { className: "quill-kanban-event-kind" }, e.kind),
                  h("span", { className: "quill-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                ),
            isDiag && phantoms.length > 0
              ? h("div", { className: "quill-kanban-event-phantom-row" },
                  h("span", { className: "quill-kanban-event-phantom-label" },
                    tx(i18n, "phantomIds", "Phantom ids:")),
                  phantoms.map(function (pid) {
                    return h("code", {
                      key: pid,
                      className: "quill-kanban-event-phantom-chip",
                    }, pid);
                  }),
                )
              : null,
            e.payload && !isDiag
              ? h("code", { className: "quill-kanban-event-payload" },
    ...
    return h("div", { className: "quill-kanban-section" },
      h("div", { className: "quill-kanban-section-head-row" },
        ...
        !showAll
          ? h("button", {
              type: "button",
              onClick: function () { setExpanded(true); },
              className: "quill-kanban-edit-link",
      ...
      visible.map(function (r) {
        const outcomeClass = r.ended_at
          ? `quill-kanban-run--${r.outcome || r.status || "ended"}`
          : "quill-kanban-run--active";
        return h("div", { key: r.id, className: cn("quill-kanban-run", outcomeClass) },
          h("div", { className: "quill-kanban-run-head" },
            h("span", { className: "quill-kanban-run-outcome" },
              r.ended_at ? (r.outcome || r.status || tx(t, "ended", "ended")) : tx(t, "active", "active")),
            h("span", { className: "quill-kanban-run-profile" },
              r.profile ? `@${r.profile}` : tx(t, "noProfile", "(no profile)")),
            h("span", { className: "quill-kanban-run-elapsed" }, fmtElapsed(r)),
            h("span", { className: "quill-kanban-run-ago" },
              timeAgo ? timeAgo(r.started_at) : ""),
          ),
          r.summary
            ? h("div", { className: "quill-kanban-run-summary" }, r.summary)
            : null,
          r.error
            ? h("div", { className: "quill-kanban-run-error" }, r.error)
            : null,
          (r.metadata && Object.keys(r.metadata).length > 0)
            ? (function () {
                ...
                return h("details", {
                    className: "quill-kanban-run-meta-block",
                    open: !collapsed,
                  },
                  h("summary", { className: "quill-kanban-run-meta-label" }, "Metadata"),
                  h("code", { className: "quill-kanban-run-meta" }, json),
                );
              })()
            : null,
        );
  ...
  function WorkerLogSection(props) {
    ...
    if (state.loading) {
      ...
      body = h("pre", { className: "quill-kanban-pre quill-kanban-log" },
        data.content || "(empty)");
    }
  ...
  function MetaRow(props) {
    return h("div", { className: "quill-kanban-meta-row" },
      h("span", { className: "quill-kanban-meta-label" }, props.label),
      h("span", { className: "quill-kanban-meta-value" }, props.value),
    );
  }
  ...
  function TitleEditor(props) {
    ...
    return h("div", { className: "quill-kanban-edit-row" },
      h(Input, {
  ...
  function AssigneeEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "quill-kanban-meta-row" },
        h("span", { className: "quill-kanban-meta-label" }, tx(t, "assignee", "Assignee")),
        h("span", {
          className: "quill-kanban-meta-value quill-kanban-editable",
  ...
  function PriorityEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "quill-kanban-meta-row" },
        h("span", { className: "quill-kanban-meta-label" }, tx(t, "priority", "Priority")),
        h("span", {
          className: "quill-kanban-meta-value quill-kanban-editable",
          onClick: function () { setEditing(true); },
  ...
  function BodyEditor(props) {
    ...
    return h("div", { className: "quill-kanban-section" },
      h("div", { className: "quill-kanban-section-head-row" },
        h("span", { className: "quill-kanban-section-head" }, tx(t, "description", "Description")),
        editing
      ...
      editing
        ? h("textarea", {
            className: "quill-kanban-textarea",
            value: v,
            rows: 8,
            onChange: function (e) { setV(e.target.value); },
          })
  ...
  function DependencyEditor(props) {
    ...
    return h("div", { className: "quill-kanban-section" },
      h("div", { className: "quill-kanban-section-head" }, tx(t, "dependencies", "Dependencies")),
      h("div", { className: "quill-kanban-deps-row" },
        h("span", { className: "quill-kanban-deps-label" }, tx(t, "parents", "Parents:")),
        h("div", { className: "quill-kanban-deps-chips" },
          (links.parents || []).length === 0
            ? h("span", { className: "quill-kanban-deps-empty" }, tx(t, "none", "none"))
            : (links.parents || []).map(function (id) {
                return h("span", { key: id, className: "quill-kanban-dep-chip" },
                  id,
                  h("button", {
                    type: "button",
                    className: "quill-kanban-dep-chip-x",
      ...
      h("div", { className: "quill-kanban-deps-row" },
        h("span", { className: "quill-kanban-deps-label" }, tx(t, "children", "Children:")),
        h("div", { className: "quill-kanban-deps-chips" },
  ...
  function StatusActions(props) {
    ...
    return h("div", null,
      h("div", { className: "quill-kanban-actions" },
        specifyButton,
        decomposeButton,
  ...
  function HomeSubsSection(props) {
    ...
    return h("div", { className: "quill-kanban-section" },
      ...
      h("div", { className: "quill-kanban-home-subs" },
        channels.map(function (hc) {
          ...
          return h(Button, {
            ...
            className: hc.subscribed
              ? "quill-kanban-home-sub quill-kanban-home-sub--on"
              : "quill-kanban-home-sub",
          }, label);
        })
      )
    );
  ...
  if (window.__QUILL_PLUGINS__ && typeof window.__QUILL_PLUGINS__.register === "function") {
