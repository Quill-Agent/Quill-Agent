...
> **Honcho docs:** <https://docs.honcho.dev/v3/guides/integrations/quill>
...
```bash
quill honcho setup    # full interactive wizard (cloud or local)
quill memory setup    # generic picker, also works
```

Or manually:
```bash
quill config set memory.provider honcho
...
| Priority | Path | Scope |
|----------|------|-------|
| 1 | `$QUILL_HOME/honcho.json` | Profile-local (isolated Quill instances) |
| 2 | `~/.quill/honcho.json` | Default profile (shared host blocks) |
| 3 | `~/.honcho/config.json` | Global (cross-app interop) |

Host key is derived from the active Quill profile: `quill` (default) or `quill.<profile>`.
...
| 4 | `per-session` strategy | Quill session ID (`20260415_a3f2b1`) |
| 5 | `per-repo` strategy | Git root directory name (`quill-agent`) |
| 6 | `per-directory` strategy | Current directory basename (`src`) |
| 7 | `global` strategy | Workspace name (`quill`) |

Gateway platforms always resolve via priority 3 (per-chat isolation) regardless of `sessionStrategy`. The strategy setting only affects CLI sessions.

If `sessionPeerPrefix` is `true`, the peer name is prepended: `eri-quill-agent`.

#### What each strategy produces

- **`per-directory`** — basename of `$PWD`. Opening quill in `~/code/myapp` and `~/code/other` gives two separate sessions. Same directory = same session across runs.
...
- **`per-session`** — Quill session ID (timestamp + hex). Every `quill` invocation starts a fresh Honcho session. Falls back to `per-directory` if no session ID is available.
- **`global`** — workspace name. One session for everything. Memory accumulates across all directories and runs.

### Multi-Profile Pattern

Multiple Quill profiles can share one workspace while maintaining separate AI identities. Config resolution is **host block > root > env var > default** — host blocks inherit from root, so shared settings only need to be declared once:
...
Both profiles see the same user (`yourname`) in the same shared environment (`quill`), but each AI peer builds its own observations, conclusions, and behavior patterns. The coder's memory stays code-oriented; the main agent's stays broad.

Host key is derived from the active Quill profile: `quill` (default) or `quill.<profile>` (e.g. `quill -p coder` → host key `quill.coder`).
...
| `QUILL_HONCHO_HOST` | Host key override |

## CLI Commands

| Command | Description |
|---------|-------------|
| `quill honcho setup` | Full interactive setup wizard |
| `quill honcho status` | Show resolved config for active profile |
| `quill honcho enable` / `disable` | Toggle Honcho for active profile |
| `quill honcho mode <mode>` | Change recall or observation mode |
| `quill honcho peer --user <name>` | Update user peer name |
| `quill honcho peer --ai <name>` | Update AI peer name |
| `quill honcho tokens --context <N>` | Set context token budget |
| `quill honcho tokens --dialectic <N>` | Set dialectic max chars |
| `quill honcho map <name>` | Map current directory to a session name |
| `quill honcho sync` | Create host blocks for all Quill profiles |
