...
Config file: `$QUILL_HOME/supermemory.json`

| Key | Default | Description |
|-----|---------|-------------|
| `container_tag` | `quill` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags (e.g. `quill-{identity}` → `quill-coder`). |
...
Use `{identity}` in the `container_tag` to scope memories per Quill profile:

```json
{
  "container_tag": "quill-{identity}"
}
```

For a profile named `coder`, this resolves to `quill-coder`. The default profile resolves to `quill-default`. Without `{identity}`, all profiles share the same container.
