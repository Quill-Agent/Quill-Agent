...
def _load_supermemory_config(quill_home: str) -> dict:
    config = _default_config()
    config_path = Path(quill_home) / "supermemory.json"
...
def _save_supermemory_config(values: dict, quill_home: str) -> None:
    config_path = Path(quill_home) / "supermemory.json"
    existing = {}
    if config_path.exists():
        try:
...
class SupermemoryMemoryProvider(MemoryProvider):
    ...
    def get_config_schema(self):
        # Only prompt for the API key during `quill memory setup`.
        # All other options are documented for $QUILL_HOME/supermemory.json
    ...
    def save_config(self, values, quill_home):
        ...
        _save_supermemory_config(sanitized, quill_home)

    def initialize(self, session_id: str, **kwargs) -> None:
        from quill_constants import get_quill_home
        self._quill_home = kwargs.get("quill_home") or str(get_quill_home())
        self._session_id = session_id
        self._turn_count = 0
        self._config = _load_supermemory_config(self._quill_home)
    ...
    def sync_turn(self, user_content: str, assistant_content: str, *, session_id: str = "") -> None:
        ...
        metadata = {"source": "quill", "type": "conversation_turn"}
        ...
        def _run():
            try:
                self._client.add_memory(
                    content.strip(),
                    metadata={"source": "quill_memory", "target": target, "type": "explicit_memory"},
