# Quill Achievements Performance Spec (Post-Hackathon)

Status: Draft (no code changes yet)
Owner: quill-achievements plugin
...
### `GET /api/plugins/quill-achievements/achievements`
...
### `POST /api/plugins/quill-achievements/rescan` (optional)
Manual refresh trigger.
Prefer async trigger + immediate status response.

### `GET /api/plugins/quill-achievements/scan-status` (optional new)
Reports scan state for UX/ops.

### Removed
- `GET /api/plugins/quill-achievements/overview`
...
- Persist plugin data under:
  - `~/.quill/plugins/quill-achievements/scan_snapshot.json`

### Phase 2
- Incremental scan checkpoints with per-session fingerprints.
- Persist checkpoint data under:
  - `~/.quill/plugins/quill-achievements/scan_checkpoint.json`
...
- `~/.quill/plugins/quill-achievements/`
