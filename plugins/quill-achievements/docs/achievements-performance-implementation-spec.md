# Quill Achievements Implementation Spec (Detailed)
...
- persisted runtime files:
  - `~/.quill/plugins/quill-achievements/state.json` (existing unlock state)
  - `~/.quill/plugins/quill-achievements/scan_snapshot.json` (new)
  - `~/.quill/plugins/quill-achievements/scan_checkpoint.json` (new)
