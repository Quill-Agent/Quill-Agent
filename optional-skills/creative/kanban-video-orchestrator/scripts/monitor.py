#!/usr/bin/env python3
"""
Monitor a running video-production kanban. Polls `quill kanban list` and
...
def quill_available() -> bool:
    return shutil.which("quill") is not None


def kanban_list(tenant: str) -> list[dict]:
    """Returns parsed task rows. Falls back to plain stdout parsing if JSON
    output isn't supported by the installed quill CLI."""
    try:
        out = subprocess.run(
            ["quill", "kanban", "list", "--tenant", tenant, "--json"],
        ...
        pass
    # Fallback: textual parse of `quill kanban list`
    out = subprocess.run(
        ["quill", "kanban", "list", "--tenant", tenant],
        capture_output=True, text=True, check=False,
    )
    rows = []
...
def kanban_show(task_id: str) -> dict | None:
    out = subprocess.run(
        ["quill", "kanban", "show", task_id, "--json"],
        capture_output=True, text=True, check=False,
    )
    if out.returncode != 0:
        return None
...
def main():
    ...
    if not quill_available():
        print("ERROR: 'quill' CLI not found in PATH", file=sys.stderr)
        sys.exit(1)
