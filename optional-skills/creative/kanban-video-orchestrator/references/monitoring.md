...
quill kanban watch --tenant <project-slug>

# Snapshot of the board
quill kanban list --tenant <project-slug>
quill kanban list --tenant <project-slug> --json     # machine-readable

# Per-status counts + oldest-ready age
quill kanban stats --tenant <project-slug>

# Visual dashboard (browser)
quill dashboard

# Inspect a specific task (includes comments + events)
quill kanban show <task-id>

# Follow a single task's event stream
quill kanban tail <task-id>
```

Verify available subcommands with `quill kanban --help` — the kanban CLI
...
|---------|--------------|--------|
| Task RUNNING but no heartbeat in 2+ min | Worker stuck, infinite loop, blocked on input | `quill kanban show <id>` — read the worker's last events. The dispatcher SIGTERMs tasks that exceed their `max-runtime`; if you need to stop one earlier, `quill kanban block <id>` then `quill kanban archive <id>`, and create a re-run task. |
| Same task retried 2+ times | Reproducible failure (missing key, bad spec, broken tool) | `quill kanban show <id>` to read failure events. Fix root cause before re-running. |
| RUNNING longer than max_runtime | Task is slow but progressing OR genuinely stuck | Check heartbeats with `quill kanban tail <id>`. If progressing, the dispatcher will SIGTERM eventually anyway — raise `max-runtime` on a re-created task. |
...
| Tasks created but never picked up | Profile not running, or tenant mismatch, or dispatcher not running | Check `quill profile list` (profile exists?), `quill status` (gateway/dispatcher up?), and verify tenant. |
...
quill kanban comment <renderer-task-id> "Scene 3 looks too sparse \
— increase visual density. Tighten color palette to brand spec."

# 2. Create a re-render task with the original as parent
quill kanban create "Scene 3 — re-render with feedback" \
...
NEW_TASK_ID=$(quill kanban create "Generate SRT captions from voiceover" \
    --assignee captioner \
    --workspace dir:"$HOME/projects/video-pipeline/<slug>" \
    --tenant <slug> \
...
quill kanban link "$NEW_TASK_ID" <editor-task-id>
```
...
quill kanban block <task-id>
quill kanban archive <task-id>

# Diagnose what happened
quill kanban show <task-id>      # task body, comments, recent events
quill kanban tail <task-id>      # follow the live event stream
quill kanban log <task-id>       # worker process log
```
...
3. Re-fire the initial `quill kanban create` for the director
...
while true; do
    clear
    quill kanban list --tenant <slug>
    echo "---"
    quill kanban stats --tenant <slug>
    sleep 30
done
```

For a live event feed, run `quill kanban watch --tenant <slug>` in a
...
- **Profile process not running.** Tasks queue indefinitely in READY if no
  worker for that profile is online. Check `quill profile list` and start
