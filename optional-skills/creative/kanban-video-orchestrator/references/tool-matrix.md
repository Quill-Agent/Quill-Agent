...
Maps each role archetype to the Quill skills it should `always_load` and the
toolsets it needs. Only references skills that ship in the public quill-agent
...
## Quill skills relevant to video production

### Visual / rendering skills (`quill-agent/skills/creative/`)
...
### Design / pre-production skills (`quill-agent/skills/creative/`)
...
### Audio / media skills (`quill-agent/skills/creative/` + `skills/media/`)
...
### Kanban infrastructure (`quill-agent/skills/devops/`)
...
These are **not** Quill skills but external CLIs / APIs that profiles invoke.
...
## Built-in Quill tools for media review

These are native Quill tools — not invoked via terminal but through their own
...
key is present in `~/.quill/.env` (or macOS Keychain) before firing the kanban.
...
| Anthropic | `ANTHROPIC_API_KEY` | every Quill profile (Claude) |

If a key is missing, prompt the user to add it. Storage methods, in order of
preference: macOS Keychain → `~/.quill/.env` → environment variable.
...
When a new Quill-public video skill ships:
