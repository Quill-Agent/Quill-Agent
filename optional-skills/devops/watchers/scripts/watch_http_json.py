...
Usage (via cron with --no-agent):
    ...
    quill cron create api-events \\
      --schedule "*/1 * * * *" --no-agent \\
      --script "$QUILL_HOME/skills/devops/watchers/scripts/watch_http_json.py" \\
...
def main() -> int:
    ...
    req = urllib.request.Request(args.url, headers={"User-Agent": "Quill-Watcher/1.0"})
