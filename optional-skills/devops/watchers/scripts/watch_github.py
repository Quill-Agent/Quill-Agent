...
Usage (via cron with --no-agent):
    ...
    quill cron create quill-issues \\
      --schedule "*/5 * * * *" --no-agent \\
      --script "$QUILL_HOME/skills/devops/watchers/scripts/watch_github.py" \\
      --script-args "--name quill-issues --repo YOUR_USERNAME/quill-agent --scope issues"

Set GITHUB_TOKEN (or GH_TOKEN) in ~/.quill/.env to avoid the 60 req/hr
