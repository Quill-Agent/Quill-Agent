...
{ inputs, ... }: {
  perSystem = { pkgs, lib, self', ... }:
    let
      quill-agent = self'.packages.default;
      quillVenv = quill-agent.quillVenv;

      configMergeScript = pkgs.callPackage ./configMergeScript.nix { };

      # Auto-generated config key reference — always in sync with Python
      configKeys = pkgs.runCommand "quill-config-keys" {} ''
        set -euo pipefail
        export HOME=$TMPDIR
        ${quillVenv}/bin/python3 -c '
import json, sys
...
' > $out
    ...
    in {
      ...
      checks = {
        ...
        in pkgs.runCommand "quill-cross-eval" { } (
          if failures != [] then
        ...
        package-contents = pkgs.runCommand "quill-package-contents" { } ''
          set -e
          echo "=== Checking binaries ==="
          test -x ${quill-agent}/bin/quill || (echo "FAIL: quill binary missing"; exit 1)
          test -x ${quill-agent}/bin/quill-agent || (echo "FAIL: quill-agent binary missing"; exit 1)
          echo "PASS: All binaries present"

          echo "=== Checking version ==="
          ${quill-agent}/bin/quill version 2>&1 | grep -qi "quill" || (echo "FAIL: version check"; exit 1)
        ...
        entry-points-sync = pkgs.runCommand "quill-entry-points-sync" { } ''
          set -e
          echo "=== Checking entry points match pyproject.toml [project.scripts] ==="
          for bin in quill quill-agent quill-acp; do
            test -x ${quill-agent}/bin/$bin || (echo "FAIL: $bin binary missing from Nix package"; exit 1)
        ...
        cli-commands = pkgs.runCommand "quill-cli-commands" { } ''
          set -e
          export HOME=$(mktemp -d)

          echo "=== Checking quill --help ==="
          ${quill-agent}/bin/quill --help 2>&1 | grep -q "gateway" || (echo "FAIL: gateway subcommand missing"; exit 1)
          ${quill-agent}/bin/quill --help 2>&1 | grep -q "config" || (echo "FAIL: config subcommand missing"; exit 1)
        ...
        bundled-skills = pkgs.runCommand "quill-bundled-skills" { } ''
          set -e
          echo "=== Checking bundled skills ==="
          test -d ${quill-agent}/share/quill-agent/skills || (echo "FAIL: skills directory missing"; exit 1)
          echo "PASS: skills directory exists"

          SKILL_COUNT=$(find ${quill-agent}/share/quill-agent/skills -name "SKILL.md" | wc -l)
          test "$SKILL_COUNT" -gt 0 || (echo "FAIL: no SKILL.md files found in skills directory"; exit 1)
          echo "PASS: $SKILL_COUNT bundled skills found"

          grep -q "QUILL_BUNDLED_SKILLS" ${quill-agent}/bin/quill || \
            (echo "FAIL: QUILL_BUNDLED_SKILLS not in wrapper"; exit 1)
          echo "PASS: QUILL_BUNDLED_SKILLS set in wrapper"
        ...
        bundled-plugins = pkgs.runCommand "quill-bundled-plugins" { } ''
          set -e
          echo "=== Checking bundled plugins ==="
          test -d ${quill-agent}/share/quill-agent/plugins || (echo "FAIL: plugins directory missing"; exit 1)
          echo "PASS: plugins directory exists"

          test -f ${quill-agent}/share/quill-agent/plugins/platforms/irc/plugin.yaml || \
            (echo "FAIL: irc plugin manifest missing"; exit 1)
          echo "PASS: irc plugin manifest present"

          grep -q "QUILL_BUNDLED_PLUGINS" ${quill-agent}/bin/quill || \
            (echo "FAIL: QUILL_BUNDLED_PLUGINS not in wrapper"; exit 1)
          echo "PASS: QUILL_BUNDLED_PLUGINS set in wrapper"
        ...
        bundled-tui = pkgs.runCommand "quill-bundled-tui" { } ''
          set -e
          echo "=== Checking bundled TUI ==="
          test -d ${quill-agent}/ui-tui || (echo "FAIL: ui-tui directory missing"; exit 1)
          echo "PASS: ui-tui directory exists"

          test -f ${quill-agent}/ui-tui/dist/entry.js || (echo "FAIL: compiled entry.js missing"; exit 1)
          echo "PASS: compiled entry.js present"

          # self-contained bundle; no runtime node_modules expected

          grep -q "QUILL_TUI_DIR" ${quill-agent}/bin/quill || \
            (echo "FAIL: QUILL_TUI_DIR not in wrapper"; exit 1)
          echo "PASS: QUILL_TUI_DIR set in wrapper"
        ...
        # Verify QUILL_NODE is set in wrapper and points to Node 20+
        # (string-width uses the /v regex flag which requires Node 20+)
        quill-node = pkgs.runCommand "quill-node-version" { } ''
          set -e
          echo "=== Checking QUILL_NODE in wrapper ==="
          grep -q "QUILL_NODE" ${quill-agent}/bin/quill || \
            (echo "FAIL: QUILL_NODE not set in wrapper"; exit 1)
          echo "PASS: QUILL_NODE present in wrapper"

          QUILL_NODE=$(sed -n "s/^export QUILL_NODE='\(.*\)'/\1/p" ${quill-agent}/bin/quill)
          test -x "$QUILL_NODE" || (echo "FAIL: QUILL_NODE=$QUILL_NODE not executable"; exit 1)
          echo "PASS: QUILL_NODE executable at $QUILL_NODE"

          NODE_MAJOR=$("$QUILL_NODE" --version | sed 's/^v//' | cut -d. -f1)
          test "$NODE_MAJOR" -ge 20 || \
            (echo "FAIL: Node v$NODE_MAJOR < 20, TUI needs /v regex flag support"; exit 1)
          echo "PASS: Node v$NODE_MAJOR >= 20"

          echo "=== All QUILL_NODE checks passed ==="
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify QUILL_MANAGED guard works on all mutation commands
        managed-guard = pkgs.runCommand "quill-managed-guard" { } ''
          set -e
          export HOME=$(mktemp -d)
          ...
          check_blocked() {
            local label="$1"
            shift
            OUTPUT=$(QUILL_MANAGED=true "$@" 2>&1 || true)
          ...
          echo "=== Checking QUILL_MANAGED guards ==="
          check_blocked "config set" ${quill-agent}/bin/quill config set model foo
          check_blocked "config edit" ${quill-agent}/bin/quill config edit
        ...
        extra-python-packages = let
          ...
          quillWithExtra = quill-agent.override {
            extraPythonPackages = [ testPkg ];
          };
        in pkgs.runCommand "quill-extra-python-packages" { } ''
          set -e
          echo "=== Checking extraPythonPackages PYTHONPATH injection ==="

          grep -q "PYTHONPATH" ${quillWithExtra}/bin/quill || \
            (echo "FAIL: PYTHONPATH not in wrapper"; exit 1)
          echo "PASS: PYTHONPATH present in wrapper"

          grep -q "${testPkg}" ${quillWithExtra}/bin/quill || \
          ...
          if grep -q "PYTHONPATH" ${quill-agent}/bin/quill; then
        ...
        extra-dependency-groups = let
          quillWithGroups = quill-agent.override {
            extraDependencyGroups = [ "honcho" ];
          };
        in pkgs.runCommand "quill-extra-dependency-groups" { } ''
          set -e
          ...
          echo "derivation: ${quillWithGroups}"
          echo "venv: ${quillWithGroups.quillVenv}"
        ...
        in pkgs.runCommand "quill-config-roundtrip" {
          ...
          merge_and_load() {
            ...
            ${configMergeScript} ${nixSettings} "$quill_home/config.yaml"
            ${quillVenv}/bin/python3 -c '
import json, sys
