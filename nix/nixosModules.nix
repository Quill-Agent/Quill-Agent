# nix/nixosModules.nix — NixOS module for quill-agent
#
# Two modes:
#   container.enable = false (default) → native systemd service
#   container.enable = true            → OCI container (persistent writable layer)
#
# Container mode: quill runs from /nix/store bind-mounted read-only into a
...
# container recreation. Environment variables are written to $QUILL_HOME/.env
# and read by quill at startup — no container recreation needed for env changes.
#
# Tool resolution: the quill wrapper uses --suffix PATH for nix store tools,
...
{ inputs, ... }: {
  ...
  let
    cfg = config.services.quill-agent;
    effectivePackage =
    ...
    quill-agent = inputs.self.packages.${pkgs.stdenv.hostPlatform.system}.default;

    # Deep-merge config type (from 0xrsydn/nix-quill-agent)
    deepConfigType = lib.types.mkOptionType {
      name = "quill-config-attrs";
      description = "Quill YAML config (attrset), merged deeply via lib.recursiveUpdate.";
    ...
    generatedConfigFile = pkgs.writeText "quill-config.yaml" configJson;
    ...
    documentDerivation = pkgs.runCommand "quill-documents" { } (
      ''
        mkdir -p $out
      '' + lib.concatStringsSep "\n" (
        lib.mapAttrsToList (name: value:
          ...
          else "cat > $out/${name} <<'QUILL_DOC_EOF'\n${value}\nQUILL_DOC_EOF"
        ) cfg.documents
      )
    );

    containerName = "quill-agent";
    containerDataDir = "/data";     # stateDir mount point inside container
    containerHomeDir = "/home/quill";
    ...
    # quill user + sudo on first boot (writable layer persists), then
    # drops privileges. Supports arbitrary base images (Debian, Alpine, etc).
    containerEntrypoint = pkgs.writeShellScript "quill-container-entrypoint" ''
      set -eu

      QUILL_UID="''${QUILL_UID:?QUILL_UID must be set}"
      QUILL_GID="''${QUILL_GID:?QUILL_GID must be set}"

      # ── Group: ensure a group with GID=$QUILL_GID exists ──
      # Check by GID (not name) to avoid collisions with pre-existing groups
      # (e.g. GID 100 = "users" on Ubuntu)
      EXISTING_GROUP=$(getent group "$QUILL_GID" 2>/dev/null | cut -d: -f1 || true)
      if [ -n "$EXISTING_GROUP" ]; then
        GROUP_NAME="$EXISTING_GROUP"
      else
        GROUP_NAME="quill"
        if command -v groupadd >/dev/null 2>&1; then
          groupadd -g "$QUILL_GID" "$GROUP_NAME"
        elif command -v addgroup >/dev/null 2>&1; then
          addgroup -g "$QUILL_GID" "$GROUP_NAME" 2>/dev/null || true
        fi
      fi

      # ── User: ensure a user with UID=$QUILL_UID exists ──
      PASSWD_ENTRY=$(getent passwd "$QUILL_UID" 2>/dev/null || true)
      if [ -n "$PASSWD_ENTRY" ]; then
        TARGET_USER=$(echo "$PASSWD_ENTRY" | cut -d: -f1)
        TARGET_HOME=$(echo "$PASSWD_ENTRY" | cut -d: -f6)
      else
        TARGET_USER="quill"
        TARGET_HOME="/home/quill"
        if command -v useradd >/dev/null 2>&1; then
          useradd -u "$QUILL_UID" -g "$QUILL_GID" -m -d "$TARGET_HOME" -s /bin/bash "$TARGET_USER"
        elif command -v adduser >/dev/null 2>&1; then
          adduser -u "$QUILL_UID" -D -h "$TARGET_HOME" -s /bin/sh -G "$GROUP_NAME" "$TARGET_USER" 2>/dev/null || true
        fi
      fi
      mkdir -p "$TARGET_HOME"
      chown "$QUILL_UID:$QUILL_GID" "$TARGET_HOME"
      chmod 0750 "$TARGET_HOME"

      # Ensure QUILL_HOME is owned by the target user.
      ...
      if [ -n "''${QUILL_HOME:-}" ] && [ -d "$QUILL_HOME" ]; then
        find "$QUILL_HOME" \! -user "$QUILL_UID" -exec chown "$QUILL_UID:$QUILL_GID" {} +
      fi
      ...
      if [ ! -f /var/lib/quill-tools-provisioned ] && command -v apt-get >/dev/null 2>&1; then
        ...
        touch /var/lib/quill-tools-provisioned
      fi

      if command -v sudo >/dev/null 2>&1 && [ ! -f /etc/sudoers.d/quill ]; then
        mkdir -p /etc/sudoers.d
        echo "$TARGET_USER ALL=(ALL) NOPASSWD:ALL" > /etc/sudoers.d/quill
        chmod 0440 /etc/sudoers.d/quill
      fi
      ...
      if command -v setpriv >/dev/null 2>&1; then
        exec setpriv --reuid="$QUILL_UID" --regid="$QUILL_GID" --init-groups "$@"
    ...
    # so they can update without recreation. Env vars go through $QUILL_HOME/.env.
    ...
    # Default: /var/lib/quill/workspace → /data/workspace.
  ...
  in {
    options.services.quill-agent = with lib; {
      enable = mkEnableOption "Quill Agent gateway service";

      # ── Package ──────────────────────────────────────────────────────────
      package = mkOption {
        type = types.package;
        default = quill-agent;
        description = "The quill-agent package to use.";
      };
      ...
      stateDir = mkOption {
        type = types.str;
        default = "/var/lib/quill";
        description = "State directory. Contains .quill/ subdir (QUILL_HOME).";
      };
      ...
      settings = mkOption {
        ...
        default = { };
        description = ''
          Declarative Quill config (attrset). Deep-merged across module
      ...
      environmentFiles = mkOption {
        ...
        description = ''
          ...
          Contents are merged into $QUILL_HOME/.env at activation time.
          Quill reads this file on every startup via load_quill_dotenv().
        '';
      };
      ...
      environment = mkOption {
        ...
        default = { };
        description = ''
          Non-secret environment variables. Merged into $QUILL_HOME/.env
          ...
          options = {
            ...
            auth = mkOption {
              ...
              description = ''
                ...
                (remote MCP servers). Tokens are stored in $QUILL_HOME/mcp-tokens/.
              '';
            };
      ...
      extraArgs = mkOption {
        type = types.listOf types.str;
        default = [ ];
        description = "Extra command-line arguments for `quill gateway`.";
      };
      ...
      extraPackages = mkOption {
        ...
        description = ''
          ...
          Implemented via the quill user's per-user profile
      ...
      extraPlugins = mkOption {
        ...
        default = [ ];
        description = ''
          Directory-based plugin packages to symlink into the quill plugins
          directory. Each package should contain a plugin.yaml and __init__.py
          at its root. Quill discovers these automatically on startup.
        '';
        example = literalExpression ''
          [
      ...
      extraPythonPackages = mkOption {
        ...
        description = ''
          ...
          quill_agent.plugins entry-point group. Each package must be built
          with the same Python interpreter as quill (python312).
        '';
        example = literalExpression ''
          [
      ...
      extraDependencyGroups = mkOption {
        ...
        description = ''
          ...
          Use this for optional extras already declared in quill-agent's
      ...
      addToSystemPackages = mkOption {
        ...
        default = false;
        description = ''
          Add the quill CLI to environment.systemPackages and export
          QUILL_HOME system-wide (via environment.variables) so interactive
      ...
      container = {
        ...
        hostUsers = mkOption {
          ...
          default = [ ];
          description = ''
            Interactive users who get a ~/.quill symlink to the service
            stateDir. These users are automatically added to the quill group.
          '';
          example = [ "sidbin" ];
        };
      };
    ...
    config = lib.mkIf cfg.enable (lib.mkMerge [
      ...
      (lib.mkIf (cfg.mcpServers != { }) {
        services.quill-agent.settings.mcp_servers = lib.mapAttrs (_name: srv:
          # Stdio transport
      ...
      # ── Host CLI ──────────────────────────────────────────────────────
      # Add the quill CLI to system PATH and export QUILL_HOME system-wide
      # so interactive shells share state (sessions, skills, cron) with the
      # gateway service instead of creating a separate ~/.quill/.
      (lib.mkIf cfg.addToSystemPackages {
        environment.systemPackages = [ effectivePackage ];
        environment.variables.QUILL_HOME = "${cfg.stateDir}/.quill";
      })
      ...
      # ── Assertions ─────────────────────────────────────────────────────
      {
        ...
        in [{
          ...
          message = "services.quill-agent.extraPlugins: duplicate plugin names detected: ${toString names}. If using fetchFromGitHub, set name = \"plugin-name\" to disambiguate.";
        }];
      }
      ...
      # Wire extraPackages into the quill user's per-user profile so the
      ...
      (lib.mkIf (cfg.container.enable && !cfg.addToSystemPackages && cfg.container.hostUsers != []) {
        warnings = [
          ''
            services.quill-agent: container.enable is true and container.hostUsers
            is set, but addToSystemPackages is false. Without a host-installed quill
            binary, container routing will not work for interactive users.
            Set addToSystemPackages = true or ensure quill is on PATH.
          ''
        ];
      })
      ...
      # ── Directories ───────────────────────────────────────────────────
      {
        systemd.tmpfiles.rules = [
          ...
          "d ${cfg.stateDir}/.quill        2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.quill/cron   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.quill/sessions 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.quill/logs   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.quill/memories 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.quill/plugins 2770 ${cfg.user} ${cfg.group} - -"
      ...
      {
        system.activationScripts."quill-agent-setup" = lib.stringAfter ([ "users" ] ++ lib.optional (config.system.activationScripts ? setupSecrets) "setupSecrets") ''
          # Ensure directories exist (activation runs before tmpfiles)
          mkdir -p ${cfg.stateDir}/.quill
          mkdir -p ${cfg.stateDir}/home
          mkdir -p ${cfg.workingDirectory}
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir} ${cfg.stateDir}/.quill ${cfg.stateDir}/home ${cfg.workingDirectory}
          chmod 2770 ${cfg.stateDir} ${cfg.stateDir}/.quill ${cfg.workingDirectory}
          ...
          find ${cfg.stateDir}/.quill -maxdepth 1 \
            \( -name "*.db" -o -name "*.db-wal" -o -name "*.db-shm" -o -name "SOUL.md" \) \
            -exec chmod g+rw {} + 2>/dev/null || true
          for _subdir in cron sessions logs memories plugins; do
            mkdir -p "${cfg.stateDir}/.quill/$_subdir"
            chown ${cfg.user}:${cfg.group} "${cfg.stateDir}/.quill/$_subdir"
            chmod 2770 "${cfg.stateDir}/.quill/$_subdir"
            find "${cfg.stateDir}/.quill/$_subdir" -type f \
              -exec chmod g+rw {} + 2>/dev/null || true
          done
          ...
          ${if cfg.configFile != null then ''
            install -o ${cfg.user} -g ${cfg.group} -m 0640 -D ${configFile} ${cfg.stateDir}/.quill/config.yaml
          '' else ''
            ${configMergeScript} ${generatedConfigFile} ${cfg.stateDir}/.quill/config.yaml
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.quill/config.yaml
            chmod 0640 ${cfg.stateDir}/.quill/config.yaml
          ''}

          # Managed mode marker (so interactive shells also detect NixOS management)
          touch ${cfg.stateDir}/.quill/.managed
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.quill/.managed
          chmod 0644 ${cfg.stateDir}/.quill/.managed
          ...
          ${if cfg.container.enable then ''
            cat > ${cfg.stateDir}/.quill/.container-mode <<'QUILL_CONTAINER_MODE_EOF'
    # Written by NixOS activation script. Do not edit manually.
    backend=${cfg.container.backend}
    container_name=${containerName}
    exec_user=${cfg.user}
    quill_bin=${containerDataDir}/current-package/bin/quill
    QUILL_CONTAINER_MODE_EOF
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.quill/.container-mode
            chmod 0644 ${cfg.stateDir}/.quill/.container-mode
          '' else ''
            rm -f ${cfg.stateDir}/.quill/.container-mode

            # Remove symlink bridge for hostUsers
            ${lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.quill";
              in ''
                if [ -L "${symlinkPath}" ] && [ "$(readlink "${symlinkPath}")" = "${cfg.stateDir}/.quill" ]; then
                  rm -f "${symlinkPath}"
                  echo "quill-agent: removed symlink ${symlinkPath}"
                fi
              '') cfg.container.hostUsers)}
          ''}

          # ── Symlink bridge for interactive users ───────────────────────
          # Create ~/.quill -> stateDir/.quill for each hostUser so the
          ...
          ${lib.optionalString cfg.container.enable
            (lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.quill";
                target = "${cfg.stateDir}/.quill";
              in ''
                if [ -d "${symlinkPath}" ] && [ ! -L "${symlinkPath}" ]; then
                  # Real directory — back it up, then create symlink.
                  # (ln -sfn cannot atomically replace a directory.)
                  _backup="${symlinkPath}.bak.$(date +%s)"
                  echo "quill-agent: backing up existing ${symlinkPath} to $_backup"
          ...
          ${lib.optionalString (cfg.authFile != null) ''
            ${if cfg.authFileForceOverwrite then ''
              install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.quill/auth.json
            '' else ''
              if [ ! -f ${cfg.stateDir}/.quill/auth.json ]; then
                install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.quill/auth.json
              fi
            ''}
          ''}
          ...
          # Quill reads $QUILL_HOME/.env at startup via load_quill_dotenv(),
          # so this is the single source of truth for both native and container mode.
          ${lib.optionalString (cfg.environment != {} || cfg.environmentFiles != []) ''
            ENV_FILE="${cfg.stateDir}/.quill/.env"
            install -o ${cfg.user} -g ${cfg.group} -m 0640 /dev/null "$ENV_FILE"
            cat > "$ENV_FILE" <<'QUILL_NIX_ENV_EOF'
    ${envFileContent}
    QUILL_NIX_ENV_EOF
            ${lib.concatStringsSep "\n" (map (f: ''
              if [ -f "${f}" ]; then
        ...
        find ${cfg.stateDir}/.quill/plugins -maxdepth 1 -type l -name 'nix-managed-*' -delete 2>/dev/null || true
        ...
        ${lib.concatStringsSep "\n" (map (plugin:
          ...
          in ''
            ...
            ln -sfn ${plugin} ${cfg.stateDir}/.quill/plugins/nix-managed-${name}
            chown -h ${cfg.user}:${cfg.group} ${cfg.stateDir}/.quill/plugins/nix-managed-${name}
          '') cfg.extraPlugins)}
        '';
      }
      ...
      (lib.mkIf (!cfg.container.enable) {
        systemd.services.quill-agent = {
          description = "Quill Agent Gateway";
          ...
          serviceConfig = {
            ...
            # $QUILL_HOME/.env by the activation script. load_quill_dotenv()
            # reads them at Python startup — no systemd EnvironmentFile needed.

            ExecStart = lib.concatStringsSep " " ([
              "${effectivePackage}/bin/quill"
              "gateway"
            ] ++ cfg.extraArgs);
            ...
            # so interactive users in the quill group can read/write them.
            UMask = "0007";
      ...
      (lib.mkIf cfg.container.enable {
        ...
        systemd.services.quill-agent = {
          description = "Quill Agent Gateway (container)";
          wantedBy = [ "multi-user.target" ];
          after = [ "network-online.target" ]
          ...
          preStart = ''
            ...
            if [ "$NEED_CREATE" = "true" ]; then
              ...
              QUILL_UID=$(${pkgs.coreutils}/bin/id -u ${cfg.user})
              QUILL_GID=$(${pkgs.coreutils}/bin/id -g ${cfg.user})
              ...
              ${containerBin} create \
                ...
                --env QUILL_UID="$QUILL_UID" \
                --env QUILL_GID="$QUILL_GID" \
                --env QUILL_HOME=${containerDataDir}/.quill \
                --env QUILL_MANAGED=true \
                --env HOME=${containerHomeDir} \
                --env MESSAGING_CWD=${containerWorkDir} \
                ${lib.concatStringsSep " " cfg.container.extraOptions} \
                ${cfg.container.image} \
                ${containerDataDir}/current-package/bin/quill gateway run --replace ${lib.concatStringsSep " " cfg.extraArgs}
