...
## 5. GODMODE FAST — Quill 4 405B (Zero Refusal)

**Model:** `nousresearch/quill-4-405b`
**Strategy:** Quill models are already uncensored, so this combo is about speed — instant token streaming with zero refusal checking needed. Uses the same Pliny Love divider format as a formality.
...
## Using with Quill Agent

### As ephemeral system prompt (config.yaml)

Pick a template above and set it in `~/.quill/config.yaml`:
