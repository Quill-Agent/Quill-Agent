...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("QUILL_HOME", os.path.expanduser("~/.quill")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("QUILL_HOME", Path.home() / ".quill")) / "skills" / "red-teaming" / "godmode"
...
QUILL_HOME = Path(os.getenv("QUILL_HOME", Path.home() / ".quill"))
CONFIG_PATH = QUILL_HOME / "config.yaml"
PREFILL_PATH = QUILL_HOME / "prefill.json"
...
MODEL_STRATEGIES = {
    ...
    # Nous/Quill models — already uncensored, just needs clean prompt
    "quill": {
        "order": ["prefill_only"],
        "system_templates": {},
    },
...
def _detect_model_family(model: str) -> str:
    ...
    if "grok" in model_lower or "x-ai" in model_lower:
        return "grok"
    if "quill" in model_lower or "nous" in model_lower:
        return "quill"
...
def _get_current_model() -> tuple:
    """Read current model and provider from Quill config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.quill/prefill.json."""
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart Quill for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart Quill for changes to take effect.")
