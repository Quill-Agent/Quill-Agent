...
## 5. Quill Configuration

### Config location

`$QUILL_HOME/config.yaml` (defaults to `~/.quill/config.yaml` when `QUILL_HOME` is unset)
...
Restart the Quill session for changes to take effect. The MCP connection is
