...
QUILL_HOME_DIR="${QUILL_HOME:-$HOME/.quill}"
QUILL_CFG="${QUILL_HOME_DIR}/config.yaml"
MCP_PORT=40404
...
# ── 3. Ensure Quill config has twozero_td MCP entry ──
if [[ ! -f "$QUILL_CFG" ]]; then
    echo -e " ${FAIL} Quill config not found at ${QUILL_CFG}"
    manual_steps+=("Create ${QUILL_CFG} with twozero_td MCP server entry")
elif grep -q 'twozero_td' "$QUILL_CFG" 2>/dev/null; then
    echo -e " ${OK} twozero_td MCP entry exists in Quill config"
else
    echo -e " ${WARN} Adding twozero_td MCP entry to Quill config..."
    python3 -c "
...
" 2>/dev/null && echo -e " ${OK} twozero_td MCP entry added to config" \
              || { echo -e " ${FAIL} Could not update config (is PyYAML installed?)"; \
                   manual_steps+=("Add twozero_td MCP entry to ${QUILL_CFG} manually"); }
    manual_steps+=("Restart Quill session to pick up config change")
fi
