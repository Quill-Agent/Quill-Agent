#!/usr/bin/env python3
"""Google Workspace API CLI for Quill Agent.

Uses the Google Workspace CLI (`gws`) when available, but preserves the
existing Quill-facing JSON contract and falls back to the Python client
...
from _quill_home import get_quill_home
...
def _gws_binary() -> str | None:
    override = os.getenv("QUILL_GWS_BIN")
    if override:
        return override
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace API for Quill Agent")
