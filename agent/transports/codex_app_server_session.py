...
Owns one Codex thread per Quill session. Drives `turn/start`, consumes
...
# Quill' tools.terminal.security_mode → Codex's permissions profile id.
# Defaults if config is missing → workspace-write (matches Codex's own default).
_QUILL_TO_CODEX_PERMISSION_PROFILE = {
...
class CodexAppServerSession:
    """One Codex thread per Quill session, lifetime owned by AIAgent.
    ...
    def __init__(
        ...
        *,
        ...
        self._permission_profile = (
            permission_profile or _QUILL_TO_CODEX_PERMISSION_PROFILE.get(
                os.environ.get("QUILL_TERMINAL_SECURITY_MODE", "auto"),
                "workspace-write",
            )
        )
    ...
    def ensure_started(self) -> str:
        ...
        self._client.initialize(
            client_name="quill",
            client_title="Quill Agent",
            client_version=_get_quill_version(),
        )
    ...
    def run_turn(
        ...
        *,
        ...
        # supports rich content but Quill' text path is the common case).
        try:
    ...
    def _handle_server_request(self, req: dict) -> None:
        """Translate a codex server request (approval) into Quill' approval
        ...
        if method == "item/commandExecution/requestApproval":
            ...
            # OAuth, form data). For our own quill-tools callback we
            # auto-accept — the user already approved Quill' tools
...
def _approval_choice_to_codex_decision(choice: str) -> str:
    """Map Quill approval choices onto codex's CommandExecutionApprovalDecision
    / FileChangeApprovalDecision wire values.

    Quill returns 'once', 'session', 'always', or 'deny'.
...
def _get_quill_version() -> str:
    """Best-effort Quill version string for codex's userAgent line."""
    try:
