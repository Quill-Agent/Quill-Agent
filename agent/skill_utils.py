...
from quill_constants import get_config_path, get_skills_dir
...
def get_disabled_skill_names(platform: str | None = None) -> Set[str]:
    ...
    Args:
        platform: Explicit platform name (e.g. ``"telegram"``).  When
            *None*, resolves from ``QUILL_PLATFORM`` or
            ``QUILL_SESSION_PLATFORM`` env vars.  Falls back to the
...
# which becomes the dominant cost of ``quill`` startup when ~120 skills
...
def get_external_skills_dirs() -> List[Path]:
    ...
    paths that resolve to the local ``~/.quill/skills/`` are silently skipped.

    Cached in-process, keyed on ``config.yaml`` mtime — the function is
    called once per skill during banner / tool-registry scans, and YAML
    parsing a non-trivial config dominates ``quill`` cold-start time
    ...
    for entry in raw_dirs:
        ...
        p = Path(expanded)
        # Resolve relative paths against QUILL_HOME, not cwd
        if not p.is_absolute():
            p = (quill_home / p).resolve()
        else:
            p = p.resolve()
        if p == local_skills:
            continue
...
def get_all_skills_dirs() -> List[Path]:
    """Return all skill directories: local ``~/.quill/skills/`` first, then external.
...
def extract_skill_conditions(frontmatter: Dict[str, Any]) -> Dict[str, List]:
    ...
    if not isinstance(metadata, dict):
        metadata = {}
    quill = metadata.get("quill") or {}
    if not isinstance(quill, dict):
        quill = {}
    return {
        "fallback_for_toolsets": quill.get("fallback_for_toolsets", []),
        "requires_toolsets": quill.get("requires_toolsets", []),
        "fallback_for_tools": quill.get("fallback_for_tools", []),
        "requires_tools": quill.get("requires_tools", []),
    }
...
def extract_skill_config_vars(frontmatter: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    if not isinstance(metadata, dict):
        return []
    quill = metadata.get("quill")
    if not isinstance(quill, dict):
        return []
    raw = quill.get("config")
    if not raw:
        return []
