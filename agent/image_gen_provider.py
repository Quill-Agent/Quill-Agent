...
as ``kind: backend``) or ``~/.quill/plugins/image_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).
...
def _images_cache_dir() -> Path:
    """Return ``$QUILL_HOME/cache/images/``, creating parents as needed."""
    from quill_constants import get_quill_home

    path = get_quill_home() / "cache" / "images"
...
def save_b64_image(
    ...
    *,
    ...
    """Decode base64 image data and write it under ``$QUILL_HOME/cache/images/``.
