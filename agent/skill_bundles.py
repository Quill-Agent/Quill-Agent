...
Storage
-------
Bundles live in ``~/.quill/skill-bundles/*.yaml`` (and the equivalent
profile-aware directory under ``QUILL_HOME``). Each file looks like::
...
- :func:`list_bundles` — return rich info for display (``quill bundles``)
...
def _bundles_dir() -> Path:
    """Return the canonical bundles directory under QUILL_HOME.

    Honors ``QUILL_BUNDLES_DIR`` for tests; falls back to
    ``<QUILL_HOME>/skill-bundles``.
    """
    override = os.environ.get("QUILL_BUNDLES_DIR")
    if override:
        return Path(override).expanduser()
    return get_quill_home() / "skill-bundles"
...
# ---------------------------------------------------------------------------
# File-level CRUD helpers — used by `quill bundles` CLI subcommand.
# ---------------------------------------------------------------------------
