...
def openclaw_residue_hint_cli() -> str:
    """Banner shown the first time Quill starts and finds ``~/.openclaw/``.

    Points users at ``quill claw migrate`` (non-destructive port of config,
    memory, and skills) first. ``quill claw cleanup`` is mentioned as the
    ...
    return (
        ...
        "To port your config, memory, and skills over to Quill, run "
        "`quill claw migrate`.\n"
        "If you've already migrated and want to archive the old directory, "
        "run `quill claw cleanup` (renames it to ~/.openclaw.pre-migration — "
