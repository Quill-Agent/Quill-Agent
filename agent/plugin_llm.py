...
Plugins built on Quill Agent often need to make their own LLM calls
...
Today the only stable plugin surfaces extend an existing Quill
...
:class:`~quill_cli.plugins.PluginContext`:
...
embedded slugs, no shorthands. This mirrors Quill' main config
...
handles every provider, fallback chain, and per-task override Quill
supports.
"""
...
class PluginLlm:
    ...
    Instances are constructed by :class:`quill_cli.plugins.PluginContext`
