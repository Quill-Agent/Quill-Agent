...
class MemoryProvider(ABC):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs always include:
          - quill_home (str): The active QUILL_HOME directory path. Use this
            for profile-scoped storage instead of hardcoding ``~/.quill``.
        ...
        kwargs may also include:
          ...
          - agent_workspace (str): Shared workspace name (e.g. "quill").
    ...
    def get_config_schema(self) -> List[Dict[str, Any]]:
        ...
        Used by 'quill memory setup' to walk the user through configuration.
    ...
    def save_config(self, values: Dict[str, Any], quill_home: str) -> None:
        """Write non-secret config to the provider's native location.

        Called by 'quill memory setup' after collecting user inputs.
        ``values`` contains only non-secret fields (secrets go to .env).
        ``quill_home`` is the active QUILL_HOME directory path.
