"""Anthropic Messages API adapter for Quill Agent.

Translates between Quill's internal OpenAI-style message format and
...
# Quill effort → Anthropic adaptive-thinking effort (output_config.effort).
...
def _prefer_refreshable_claude_code_token(env_token: str, creds: Optional[Dict[str, Any]]) -> Optional[str]:
    ...
    Quill historically persisted setup tokens into ANTHROPIC_TOKEN. That makes
...
def resolve_anthropic_token() -> Optional[str]:
    ...
    Priority:
      1. ANTHROPIC_TOKEN env var (OAuth/setup token saved by Quill)
    ...
    # 1. Quill-managed OAuth/setup token env var
    ...
    # This remains as a compatibility fallback for pre-migration Quill configs.
...
# ── Quill-native PKCE OAuth flow ────────────────────────────────────────
# Mirrors the flow used by Claude Code, pi-ai, and OpenCode.
# Stores credentials in ~/.quill/.anthropic_oauth.json (our own file).

_OAUTH_CLIENT_ID = "9d1c250a-e61b-44d9-88ed-5944d1962f5e"
_OAUTH_TOKEN_URL = "https://console.anthropic.com/v1/oauth/token"
_OAUTH_REDIRECT_URI = "https://console.anthropic.com/oauth/code/callback"
_OAUTH_SCOPES = "org:create_api_key user:profile user:inference"
_QUILL_OAUTH_FILE = get_quill_home() / ".anthropic_oauth.json"
...
def run_quill_oauth_login_pure() -> Optional[Dict[str, Any]]:
    """Run Quill-native OAuth PKCE flow and return credential state."""
    import secrets
    import time
    import webbrowser
    ...
    print()
    print("Authorize Quill with your Claude Pro/Max subscription.")
    print()
...
def read_quill_oauth_credentials() -> Optional[Dict[str, Any]]:
    """Read Quill-managed OAuth credentials from ~/.quill/.anthropic_oauth.json."""
    if _QUILL_OAUTH_FILE.exists():
        try:
            data = json.loads(_QUILL_OAUTH_FILE.read_text(encoding="utf-8"))
            if data.get("accessToken"):
                return data
        except (json.JSONDecodeError, OSError, IOError) as e:
            logger.debug("Failed to read Quill OAuth credentials: %s", e)
    return None
    ...
    *base_url* is a Kimi / Moonshot host), unsigned thinking blocks
    ...
    for m in messages:
        ...
        if role == "assistant":
            ...
            # can validate the message history.  See quill-agent#13848.
            #
    ...
    # are preserved.  See quill-agent#13848 (Kimi) and #16748 (DeepSeek).
...
def build_anthropic_kwargs(
    ...
    if is_oauth:
        ...
        for block in system:
            if isinstance(block, dict) and block.get("type") == "text":
                text = block.get("text", "")
                text = text.replace("Quill Agent", "Claude Code")
                text = text.replace("Quill agent", "Claude Code")
                text = text.replace("quill-agent", "claude-code")
                text = text.replace("Quill Agent", "Anthropic")
                block["text"] = text
    ...
    # silently hides reasoning text that Quill surfaces in its CLI. We
