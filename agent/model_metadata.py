...
def _resolve_requests_verify() -> bool | str:
    ...
    by default. Quill also honours QUILL_CA_BUNDLE (its own convention)
    ...
    """
    for env_var in ("QUILL_CA_BUNDLE", "REQUESTS_CA_BUNDLE", "SSL_CERT_FILE"):
...
# Minimum context length required to run Quill Agent.  Models with fewer
...
DEFAULT_CONTEXT_LENGTHS = {
    ...
    "claude-fable-5-thinking-high": 1_000_000,
    ...
    # so these hardcoded fallbacks prevent Quill from probing-down to
    # the default 128k when the user points at https://api.x.ai/v1
    # via a custom provider. Values sourced from models.dev (2026-04).
    # Keys use substring matching (longest-first), so e.g. "grok-4.20"
    # matches "grok-4.20-0309-reasoning" / "-non-reasoning" / "-multi-agent-0309".
    "grok-build-0.1": 256000,   # Grok Build 0.1 — agentic coding (docs.x.ai)
...
_URL_TO_PROVIDER: Dict[str, str] = {
    ...
    "inference-api.example.com": "nous",
    ...
    # (often 8K) makes it unusable for Quill' system prompt, but mapping
...
def _get_context_cache_path() -> Path:
    ...
    return get_quill_home() / "context_length_cache.yaml"
        ...
        with httpx.Client(timeout=3.0, headers=headers) as client:
            ...
            if server_type == "ollama":
                ...
                if resp.status_code == 200:
                    ...
                    # Quill grow conversations past the runtime limit and Ollama
...
def get_model_context_length(
    ...
    if effective_provider in {"copilot", "copilot-acp", "github-copilot"}:
        try:
            from quill_cli.models import get_copilot_model_context
...
def estimate_request_tokens_rough(
    ...
    *,
    ...
    Includes the major payload buckets Quill sends to providers:
