...
from quill_constants import get_quill_home, get_skills_dir, is_wsl
...
_QUILL_MD_NAMES = (".quill.md", "QUILL.md")


def _find_quill_md(cwd: Path) -> Optional[Path]:
    """Discover the nearest ``.quill.md`` or ``QUILL.md``.
...
DEFAULT_AGENT_IDENTITY = (
    "You are Quill Agent, an intelligent AI assistant created by Quill Agent. "
...
QUILL_AGENT_HELP_GUIDANCE = (
    "If the user asks about configuring, setting up, or using Quill Agent "
    "itself, load the `quill-agent` skill with skill_view(name='quill-agent') "
    "before answering. Docs: https://quill-agent.example.com/docs"
)
...
KANBAN_GUIDANCE = (
    ...
    "the shared board at `~/.quill/kanban.db`. Your task id is in "
    "`$QUILL_KANBAN_TASK`; your workspace is `$QUILL_KANBAN_WORKSPACE`. "
    ...
    "ground truth.\n"
    "2. **Work inside the workspace.** `cd $QUILL_KANBAN_WORKSPACE` before "
    ...
    "eyes on it.\n"
    "6. **Collaborate on the board.** Other agents may be running in parallel on "
    "sibling tasks — use `kanban_list`, `kanban_comment`, and parent links to "
    "coordinate; never assume you are the only worker.\n"
    ...
    "## Do NOT\n"
    "\n"
    "- Do not shell out to `quill kanban <verb>` for board operations. Use "
...
PLATFORM_HINTS = {
    ...
    "webui": (
        "You are in the Quill WebUI, a browser-based chat interface. "
...
# container / remote host rather than on the machine where Quill itself
...
def _probe_remote_backend(env_type: str) -> str | None:
    ...
    operate on a different machine than the host Quill runs on.
    """
    ...
    if not is_remote_backend:
        ...
        if probe:
            hints.append(
                ...
                f"where Quill itself is running. The host OS, home, and cwd "
                f"of the Quill process are irrelevant; only the following "
            ...
            hints.append(
                f"Terminal backend: {backend}. Your `terminal`, `read_file`, "
                f"`write_file`, `patch`, and `search_files` tools all operate "
                f"inside {description} — NOT on the machine where Quill "
...
def _skills_prompt_snapshot_path() -> Path:
    return get_quill_home() / ".skills_prompt_snapshot.json"
...
def build_skills_system_prompt(
    ...
    scanned alongside the local ``~/.quill/skills/`` directory.  External dirs
    ...
    _platform_hint = (
        os.environ.get("QUILL_PLATFORM")
        or get_session_env("QUILL_SESSION_PLATFORM")
        or ""
    )
    ...
    if not skills_by_category:
        ...
        result = (
            ...
            "or troubleshoot Quill Agent itself — its CLI, config, models, providers, tools, "
            "skills, voice, gateway, plugins, or any feature — load the `quill-agent` skill "
            "first. It has the actual commands (e.g. `quill config set …`, `quill tools`, "
            "`quill setup`) so you don't have to guess or invent workarounds.\n"
...
def build_nous_subscription_prompt(valid_tool_names: "set[str] | None" = None) -> str:
    ...
    lines.extend(
        [
            ...
            "Useful commands: quill setup, quill setup tools, quill setup terminal, quill status.",
        ]
    )
    return "\n".join(lines)
...
def load_soul_md() -> Optional[str]:
    """Load SOUL.md from QUILL_HOME and return its content, or None.
    ...
    """
    try:
        from quill_cli.config import ensure_quill_home
        ensure_quill_home()
    except Exception as e:
        logger.debug("Could not ensure QUILL_HOME before loading SOUL.md: %s", e)

    soul_path = get_quill_home() / "SOUL.md"
...
def _load_quill_md(cwd_path: Path) -> str:
    """.quill.md / QUILL.md — walk to git root."""
    quill_md_path = _find_quill_md(cwd_path)
    if not quill_md_path:
        return ""
    try:
        content = quill_md_path.read_text(encoding="utf-8").strip()
        if not content:
            return ""
        content = _strip_yaml_frontmatter(content)
        rel = quill_md_path.name
        try:
            rel = str(quill_md_path.relative_to(cwd_path))
        except ValueError:
            pass
        content = _scan_context_content(content, rel)
        result = f"## {rel}\n\n{content}"
        return _truncate_content(result, ".quill.md")
    except Exception as e:
        logger.debug("Could not read %s: %s", quill_md_path, e)
        return ""
...
def build_context_files_prompt(cwd: Optional[str] = None, skip_soul: bool = False) -> str:
    ...
    Priority (first found wins — only ONE project context type is loaded):
      1. .quill.md / QUILL.md  (walk to git root)
      2. AGENTS.md / agents.md   (cwd only)
      3. CLAUDE.md / claude.md   (cwd only)
      4. .cursorrules / .cursor/rules/*.mdc  (cwd only)

    SOUL.md from QUILL_HOME is independent and always included when present.
    ...
    # SOUL.md from QUILL_HOME only — skip when already loaded as identity
    if not skip_soul:
