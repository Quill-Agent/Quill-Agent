...
# 5. Symlinks the 'quill' CLI command into a user-facing bin dir
...
echo ""
echo -e "${CYAN}⚕ Quill Agent Setup${NC}"
echo ""
...
if is_termux; then
    ...
    if [ -n "$UV_CMD" ]; then
        ...
        _uv_log="$(mktemp 2>/dev/null || echo "/tmp/quill-uv-install.$$.log")"
...
# ============================================================================
# PATH setup — symlink quill into a user-facing bin dir
# ============================================================================

echo -e "${CYAN}→${NC} Setting up quill command..."

QUILL_BIN="$SCRIPT_DIR/venv/bin/quill"
COMMAND_LINK_DIR="$(get_command_link_dir)"
COMMAND_LINK_DISPLAY_DIR="$(get_command_link_display_dir)"
mkdir -p "$COMMAND_LINK_DIR"
ln -sf "$QUILL_BIN" "$COMMAND_LINK_DIR/quill"
echo -e "${GREEN}✓${NC} Symlinked quill → $COMMAND_LINK_DISPLAY_DIR/quill"
...
# ============================================================================
# Seed bundled skills into ~/.quill/skills/
# ============================================================================

QUILL_SKILLS_DIR="${QUILL_HOME:-$HOME/.quill}/skills"
mkdir -p "$QUILL_SKILLS_DIR"

echo ""
echo "Syncing bundled skills to ~/.quill/skills/ ..."
if "$SCRIPT_DIR/venv/bin/python" "$SCRIPT_DIR/tools/skills_sync.py" 2>/dev/null; then
    echo -e "${GREEN}✓${NC} Skills synced"
else
    # Fallback: copy if sync script fails (missing deps, etc.)
    if [ -d "$SCRIPT_DIR/skills" ]; then
        cp -rn "$SCRIPT_DIR/skills/"* "$QUILL_SKILLS_DIR/" 2>/dev/null || true
...
echo "Other commands:"
echo "  quill status        # Check configuration"
if is_termux; then
    echo "  quill gateway       # Run gateway in foreground"
else
    echo "  quill gateway install # Install gateway service (messaging + cron)"
fi
echo "  quill cron list     # View scheduled jobs"
echo "  quill doctor        # Diagnose issues"
echo ""
...
if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
    ...
    "$SCRIPT_DIR/venv/bin/python" -m quill_cli.main setup
fi
