"""ACP auth helpers — detect and advertise Quill authentication methods."""
...
TERMINAL_SETUP_AUTH_METHOD_ID = "quill-setup"


def detect_provider() -> Optional[str]:
    """Resolve the active Quill runtime provider, or None if unavailable.
...
def has_provider() -> bool:
    """Return True if Quill can resolve any runtime provider credentials."""
    return detect_provider() is not None


def build_auth_methods() -> list[Any]:
    """Return registry-compatible ACP auth methods for Quill.
    ...
    not have Quill provider credentials configured yet, so Quill always
    ...
    if provider:
        methods.append(
            AuthMethodAgent(
                ...
                description=(
                    "Authenticate Quill using the currently configured "
