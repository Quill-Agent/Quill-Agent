"""CLI entry point for the quill-agent ACP adapter.

Loads environment variables from ``~/.quill/.env``, configures logging
...
def _load_env() -> None:
    """Load .env from QUILL_HOME (default ``~/.quill``)."""
    from quill_cli.env_loader import load_quill_dotenv

    quill_home = get_quill_home()
    loaded = load_quill_dotenv(quill_home=quill_home)
    if loaded:
        for env_file in loaded:
            logging.getLogger(__name__).info("Loaded env from %s", env_file)
    else:
        logging.getLogger(__name__).info(
            "No .env found at %s, using system env", quill_home / ".env"
        )
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        prog="quill-acp",
        description="Run Quill Agent as an ACP stdio server.",
    )
    parser.add_argument("--version", action="store_true", help="Print Quill version and exit")
    parser.add_argument(
        "--check",
        action="store_true",
...
def _run_check() -> None:
    ...
    from acp_adapter.server import QuillACPAgent  # noqa: F401

    print("Quill ACP check OK")


def _run_setup() -> None:
    from quill_cli.main import main as quill_main

    old_argv = sys.argv[:]
    try:
        sys.argv = [old_argv[0] if old_argv else "quill", "model"]
        quill_main()
    finally:
        sys.argv = old_argv
...
def _run_setup_browser(assume_yes: bool = False) -> int:
    ...
    with ``quill postinstall`` and the runtime lazy installer.
...
def main(argv: list[str] | None = None) -> None:
    ...
    logger.info("Starting quill-agent ACP adapter")
    ...
    import acp
    from .server import QuillACPAgent
