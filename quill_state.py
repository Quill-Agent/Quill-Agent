#!/usr/bin/env python3
"""
SQLite State Store for Quill Agent.
...
DEFAULT_DB_PATH = get_quill_home() / "state.db"
...
# quill_cli/kanban_db.py for ~30 call sites) would re-log the same
...
def apply_wal_with_fallback(
    ...
    *,
    ...
    Shared by :class:`SessionDB` and ``quill_cli.kanban_db.connect`` so
...
def _log_wal_fallback_once(db_label: str, exc: Exception) -> None:
    ...
    connection on every operation — see quill_cli/kanban_db.py) would
...
class SessionDB:
    ...
    # With multiple quill processes (gateway + CLI sessions + worktree agents)
    ...
    def __init__(self, db_path: Path = None):
        ...
        try:
            ...
            # ``quill_state._set_last_init_error(None)`` explicitly.
    ...
    def apply_telegram_topic_migration(self) -> None:
        ...
        reconciliation. Operators must be able to upgrade Quill, keep the old
    ...
    def bind_telegram_topic(
        ...
        *,
        ...
        """Bind one Telegram DM topic thread to one Quill session.

        A Quill session may only be linked to one Telegram topic in MVP.
    ...
    def is_telegram_session_linked_to_topic(self, *, session_id: str) -> bool:
        """Return True if a Quill session is already bound to any Telegram DM topic.
